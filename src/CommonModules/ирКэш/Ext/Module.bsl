...
Функция Получить() Экспорт
	...
	Если ирПлатформа = Неопределено Тогда
		...
		#КонецЕсли
		ирПлатформа.ПрочитатьОбщиеНастройки();
	КонецЕсли;
	Возврат ирПлатформа;
КонецФункции
...
Функция ПостроительТаблицыМетаданныхЛкс(Знач ПолноеИмяМД) Экспорт
	Если ирКэш.ЛиПортативныйРежимЛкс() Тогда
		КэшФункции = ирПортативный.КэшФункцииЛкс("ПостроительТаблицыМетаданныхЛкс");
		КлючНаборПараметров = "" + ПолноеИмяМД;
		Результат = КэшФункции[КлючНаборПараметров];
		Если Результат <> Неопределено Тогда
			Возврат Результат;
		КонецЕсли;
	...
	ПолноеИмяТаблицыБД = ирКэш.ИмяТаблицыИзМетаданныхЛкс(ПолноеИмяМД);
	Если ПолноеИмяТаблицыБД <> Неопределено Тогда
		Результат = Новый ПостроительЗапроса("ВЫБРАТЬ * ИЗ " + ПолноеИмяТаблицыБД + " КАК Т");
...
Функция ФорматБуфераОбмена1СЛкс() Экспорт
	//ФорматБуфераОбмена1С = ирКэш.ВКОбщаяЛкс().RegisterClipboardFormat("V8Value");
...
Функция ВКОбщаяЛкс() Экспорт
	...
	// Все способы помещения ВК в кэш приводят к ошибке платформы https://github.com/tormozit/RDT1C/issues/29
...
Функция РегВыражениеЛкс(Ключ = "") Экспорт
	Если ирКэш.ЛиПортативныйРежимЛкс() Тогда
		КэшФункции = ирПортативный.КэшФункцииЛкс("РегВыражениеЛкс");
...
Функция ИмяТаблицыИзМетаданныхЛкс(Знач ПолноеИмяМД, Знач ЛиТаблицаИзменений = Ложь, Знач ЛиДвиженияССубконтоДляРегистраБухгалтерии = Истина, Знач ТолькоРазрешенные = Ложь) Экспорт
	...
	Если ЛиТаблицаИзменений Тогда
		...
		Если Истина
			И ДублиТаблицыБД.Количество() > 0
			И ДублиТаблицыБД[0].Тип = "ТабличнаяЧасть"
		Тогда
...
// 0.4с для 100 подсистем и 5000 пар
// Лучше стараться использовать ирОбщий.ОбъектыПодсистемЛкс
Функция ПодсистемыОбъектовМДЛкс(Знач ИмяВерхнейПодсистемы = "") Экспорт
	Если ирКэш.ЛиПортативныйРежимЛкс() Тогда
		КэшФункции = ирПортативный.КэшФункцииЛкс("ПодсистемыОбъектовМДЛкс");
		КлючНаборПараметров = "";
		Результат = КэшФункции[КлючНаборПараметров];
		Если Результат <> Неопределено Тогда
			Возврат Результат;
	...
	Если ЗначениеЗаполнено(ИмяВерхнейПодсистемы) Тогда
		ВерхняяПодсистема = Метаданные.Подсистемы[ИмяВерхнейПодсистемы];
	КонецЕсли;
	Для Каждого ЭлементСписка Из ВсеПодсистемыЛкс(ВерхняяПодсистема) Цикл
		Подсистема = ЭлементСписка.Значение;
		Для Каждого ЭлементСостава Из Подсистема.Состав Цикл // ОбъектМетаданных
			Если ЭлементСостава = Неопределено Тогда
				// Битая ссылка
				Продолжить;
			КонецЕсли;
			СтрокаТЗ = Результат.Добавить();
			СтрокаТЗ.ИмяОбъектаМД = ЭлементСостава.ПолноеИмя();
			СтрокаТЗ.Подсистема = Подсистема;
		КонецЦикла;
	КонецЦикла;
	Результат.Индексы.Добавить("ИмяОбъектаМД");
	Если ирКэш.ЛиПортативныйРежимЛкс() Тогда
...
Функция ЭтоУчебнаяПлатформаЛкс() Экспорт
	...
	Исключение
		...
		Если Ложь
			Или Найти(ОписаниеОшибки, "синхронных") > 0  // Использование синхронных методов на клиенте запрещено!
			Или Найти(ОписаниеОшибки, "synchronous") > 0  // Cannot call synchronous methods on the client!
		Тогда
			Возврат Ложь;
		КонецЕсли;
		Возврат Истина;
...
Функция ДоступнаПодсистемаПоискаСсылокБСПЛкс() Экспорт
	Результат = ирКэш.НомерВерсииБСПЛкс() >= 203;
	Возврат Результат;
КонецФункции
...
Функция СобытияПоИмениОбщегоТипаЛкс(Знач ИмяОбщегоТипа, Знач ИмяТипаРасширения = Неопределено, Знач СРасширениями = Ложь) Экспорт
	мПлатформа = ирКэш.Получить();
	мПлатформа.ИнициацияОписанияМетодовИСвойств();
	Если ИмяТипаРасширения = Неопределено И СРасширениями Тогда
		СтруктураТипа = мПлатформа.НоваяСтруктураТипа(ИмяОбщегоТипа);
		МассивОбщихТипов = мПлатформа.РазвернутаяТаблицаТиповИзСтруктурыТипа(СтруктураТипа, Ложь,, Истина).ВыгрузитьКолонку("ИмяОбщегоТипа");
	Иначе
		МассивОбщихТипов = Новый Массив;
		МассивОбщихТипов.Добавить(ИмяОбщегоТипа);
	КонецЕсли;
	ВсеСобытия = Новый Массив;
	Для Каждого ИмяОбщегоТипа Из МассивОбщихТипов Цикл
		...
		//! ВсеСобытия.Добавить(СобытияЭлементаФормы[0]);
		ирОбщий.ДополнитьМассивЛкс(ВсеСобытия, СобытияЭлементаФормы);
		Если ИмяТипаРасширения <> Неопределено Тогда
			СобытияРасширения = ирКэш.СобытияПоИмениОбщегоТипаЛкс(ИмяТипаРасширения);
			ирОбщий.ДополнитьМассивЛкс(ВсеСобытия, СобытияРасширения);
		КонецЕсли;
	КонецЦикла;
...
// Выполнение может достигать 700мс на файловой базе. Поэтому перенес в кэшируемый модуль
Функция ПустаяТаблицаЖурналРегистрацииЛкс() Экспорт
...
Функция ИменаПредопределенныхЭлементовЛкс(Знач ПолноеИмяМД, Знач ДобавлятьКод = Ложь) Экспорт
	СхемаКоллекции = Новый Соответствие;
	Если НомерВерсииПлатформыЛкс() < 803001 Тогда
		Возврат СхемаКоллекции;
	КонецЕсли;
	ТекстВыбора = "";
	Если Истина
		И ДобавлятьКод
		И ирКэш.НомерРежимаСовместимостиЛкс() >= 803001
		И ирКэш.ПоляТаблицыБДЛкс(ПолноеИмяМД).Найти("Код", "Имя") <> Неопределено
	Тогда
		ТекстВыбора = ", Т.Код";
	КонецЕсли;
	ОписаниеТипов = Новый ОписаниеТипов(ирОбщий.ИмяТипаИзПолногоИмениМДЛкс(ПолноеИмяМД));
	Если Истина
		И ПустаяСтрока(ТекстВыбора)
		И ирКэш.НомерВерсииПлатформыЛкс() >= 803005
	Тогда
		ОбъектМД = Метаданные.НайтиПоПолномуИмени(ПолноеИмяМД);
		Для Каждого РезультатЭлемент Из ОбъектМД.ПолучитьИменаПредопределенных() Цикл
			СхемаКоллекции.Вставить(РезультатЭлемент, ОписаниеТипов);
		КонецЦикла;
	Иначе
		Запрос = Новый Запрос;
		Запрос.Текст = "ВЫБРАТЬ Т.ИмяПредопределенныхДанных" + ТекстВыбора + " ИЗ " + ПолноеИмяМД + " КАК Т ГДЕ Т.Предопределенный";
		Результат = Запрос.Выполнить().Выгрузить();
		Для Каждого СтрокаРезультата Из Результат Цикл
			СхемаКоллекции.Вставить(СтрокаРезультата.ИмяПредопределенныхДанных + " _" + СтрЗаменить(СтрокаРезультата.Код, ".", "_"), ОписаниеТипов);
		КонецЦикла;
	КонецЕсли;
	Возврат СхемаКоллекции;
КонецФункции

Функция ДоступнаБлокировкаРаботыСВнешнимиРесурсамиБСП() Экспорт
	Возврат Истина
		И ирКэш.НомерВерсииБСПЛкс() >= 300
		И Метаданные.ОбщиеМодули.Найти("БлокировкаРаботыСВнешнимиРесурсами") <> Неопределено;
КонецФункции
...
Функция ИдентификаторыОбъектовМДЛкс() Экспорт
	Если ирКэш.ЛиПортативныйРежимЛкс() Тогда
		КэшФункции = ирПортативный.КэшФункцииЛкс("ИдентификаторыОбъектовМДЛкс");
		КлючНаборПараметров = ""; 
		Результат = КэшФункции[КлючНаборПараметров]; 
		Если Результат <> Неопределено Тогда
			Возврат Результат;
		КонецЕсли;
	КонецЕсли;
	ТаблицаИмен = ирКлиент.ИдентификаторыОбъектовМДЛкс();
	Если ирКэш.ЛиПортативныйРежимЛкс() Тогда
		КэшФункции[КлючНаборПараметров] = Результат;
	КонецЕсли;
	Возврат ТаблицаИмен;
...
Функция ЛиДоступноВыполнениеКомандныхФайловЛкс(Знач ВыброситьИсключение = Ложь) Экспорт
	...
	Иначе
		...
		Результат = ирОбщий.ВыполнитьКомандуОСЛкс("""" + ИмяФайла + """"); 
...
Функция ПолеТекстаМодуляБезСтруктурыТипаЛкс(ИмяМодуля) Экспорт
	...
	КонецЕсли;
	ПолеТекстаМодуля.мФлагиКомпиляции = Получить().НовыеФлагиКомпиляции();
	Если ирКэш.ЛиПортативныйРежимЛкс() Тогда
		КэшФункции[КлючНаборПараметров] = Результат;
	КонецЕсли;
	Возврат ПолеТекстаМодуля;
...
Функция ПолеТекстаПрограммы(Знач ЯзыкПрограммы = 0) Экспорт
	...
	Результат.ОбновитьМодульМетаданных(); // Чтобы мМодульМетаданных установился
	Возврат Результат;
КонецФункции

Функция ТаблицаИменМетаТиповЛкс(Знач ЛиСсылочные = Истина, Знач ЛиОпределяемые = Истина) Экспорт
	Анализатор = ирОбщий.НовыйАнализаторКодаЛкс();
	Если ЛиСсылочные Тогда
		Для Каждого ТипыЭлемент Из ирОбщий.ОписаниеТиповВсеСсылкиЛкс().Типы() Цикл
			Строка = Анализатор.ТаблицаСлов.Добавить();
			Строка.ТипЗначения = ирОбщий.ПредставлениеТипаЛкс(ТипыЭлемент,, Истина);
			Строка.Слово = ирОбщий.ПоследнийФрагментЛкс(Строка.ТипЗначения);
			Строка.НСлово = НРег(Строка.Слово);
		КонецЦикла;
	КонецЕсли;
	Если ЛиОпределяемые Тогда
		Для Каждого Объект Из Метаданные.ОпределяемыеТипы Цикл
			Строка = Анализатор.ТаблицаСлов.Добавить();
			Строка.ТипЗначения = Объект.ПолноеИмя();
			Строка.Слово = Объект.Имя;
			Строка.НСлово = НРег(Строка.Слово);
		КонецЦикла;
	КонецЕсли;
	Анализатор.ТаблицаСлов.Сортировать("НСлово");
	Возврат Анализатор.ТаблицаСлов;
КонецФункции
...
Функция ДоступноРасширенияКонфигурацииЛкс() Экспорт
	Результат = ирКэш.НомерВерсииПлатформыЛкс() >= 803008;
	Возврат Результат;
КонецФункции
