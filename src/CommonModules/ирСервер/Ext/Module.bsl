...
//    - 
Функция ПодключитьВнешнийОбъектЛкс(Знач АдресХранилища, Знач Имя = "") Экспорт
	Если Найти(АдресХранилища, ":") > 0 Тогда
		Файл = Новый Файл(АдресХранилища);
		Если Файл.Существует() Тогда
			Строка = ирОбщий.МенеджерВнешнегоОбъектаЛкс(Файл.ПолноеИмя).Метаданные().Имя;
		КонецЕсли;
	Иначе
...
Функция ПриНачалеРаботыСистемыРасширениеЛкс(ПараметрЗапуска) Экспорт 
	...
	Тогда   
		...
		ИначеЕсли Не РольДоступна("ирПользователь") Тогда  
			...
			Если Истина
				И ЭтотРасширение <> Неопределено
				И ЭтотРасширение.БезопасныйРежим 
			Тогда
				ВызватьИсключение "Расширение """ + ЭтотРасширение.Имя + """ не поддерживает работу в безопасном режиме. 
				|Используйте установщик расширения https://devtool1c.ucoz.ru/load/osnovnye/ustanovshhik_varianta_rasshirenie/1-1-0-21";
			КонецЕсли;
		...
		Для Каждого ОписаниеСоединения Из СоединенныеРегистры Цикл
			...
			Если ирОбщий.СтрНачинаетсяСЛкс(ИмяКолонки, ПрефиксИмениПолей) Тогда
				...
				Если ирОбщий.СтрКончаетсяНаЛкс(ИмяПоляТаблицы, СуффиксПоляТипаЗначения) Тогда
					ИмяПоляТаблицы = ирОбщий.ТекстБезПоследнегоФрагментаЛкс(ИмяПоляТаблицы, СуффиксПоляТипаЗначения);
					МодификаторПоля = "тип"; 
				КонецЕсли;
...
Функция ЗаголовокПоляТаблицыБДДляФормыЛкс(Знач ОсновнойЭУ, Знач ПолеТаблицы, Знач РежимИмяСиноним, Подсказка = "", Знач РазрешитьОбрезку = Истина, Знач МодификаторПоля = "",
	...
	Тогда
		...
		Иначе
			Заголовок = ирОбщий.ПредставлениеИзИдентификатораЛкс(ирОбщий.ТекстБезПоследнегоФрагментаЛкс(ПолеТаблицы.Имя, МаркерТипаЗначения)) + " (тип)";
		КонецЕсли;
...
Процедура УправляемаяФорма_ОбновитьСлужебныеДанныеЛкс(Знач ЭтаФорма, Знач ПоляДляЗапоминанияТипов = Неопределено, Знач ПоляСИсториейВыбора = Неопределено, Знач ДинамическиеСписки = Неопределено) Экспорт 
	...
	МассивПолей = Новый Массив(); // Массив из ПолеФормы
...
//.
// Параметры:
//   КоманднаяПанельИсточник - РасширениеГруппыФормыДляГруппыКнопок - 
//   КоманднаяПанельПриемник -  - 
//   ПрефиксИмени -  - 
...
Функция ЗаполнитьТаблицуСловИзТекста(Знач ТекстДляАнализа, Знач ВременныйТекст, Знач Шаблон, Знач КоличествоГрупп, Знач ТолькоСвойства, Знач ПроверятьКомпиляцией, Знач ТаблицаСлов) Экспорт
	АнализаторКода = ирОбщий.НовыйАнализаторКодаЛкс(0);
	АнализаторКода.ТаблицаСлов.Загрузить(ТаблицаСлов);
	АнализаторКода.ЗаполнитьТаблицуСловИзТекста(ТекстДляАнализа, ВременныйТекст, Шаблон, КоличествоГрупп, ТолькоСвойства, ПроверятьКомпиляцией);
	Возврат АнализаторКода.ТаблицаСлов.Выгрузить();
КонецФункции
