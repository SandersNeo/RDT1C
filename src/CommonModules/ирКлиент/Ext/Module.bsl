...
// Авторское право (с) 2007-2026, Старых С.А.
// Лицензия MIT
...
Функция ПерейтиКОбъектуМетаданныхВКонфигуратореЛкс(Знач ПолноеИмяОбъекта, Знач ПутьВОбъекте = "") Экспорт 
	
	мПлатформа = ирКэш.Получить();
	#Если Сервер И Не Сервер Тогда
		мПлатформа = Обработки.ирПлатформа.Создать();
	#КонецЕсли 
	ОбъектМД = Метаданные.НайтиПоПолномуИмени(ПолноеИмяОбъекта);
	Если ОбъектМД = Неопределено Тогда
		ирОбщий.СообщитьЛкс("Объект метаданных """ + ПолноеИмяОбъекта + """ не найден.");
		Возврат Ложь;
	КонецЕсли;
	СсылкаМД = ирОбщий.ПолноеИмяМДСРасширениемЛкс(ОбъектМД) + ПутьВОбъекте;
	Если мПлатформа.мСвязанныйКонфигуратор <> Неопределено Тогда
		ОткрытьСсылкуВКонфигуратореЛкс(СсылкаМД);
		Возврат Истина;
	КонецЕсли;
	ТекстМодуля = "// " + СсылкаМД;
...
//   - 
//
Функция ОткрытьСсылкуСтрокиМодуляЛкс(Знач Ссылка, Знач ДляИзменения = Ложь, Знач Автооткрытие = Истина) Экспорт
...
Процедура ОткрытьСсылкуВКонфигуратореЛкс(Знач Ссылка) Экспорт
	...
	Если мПлатформа.мСвязанныйКонфигуратор <> Неопределено Тогда
		...
		ирОбщий.ПаузаМиллисекундЛкс(150); // 100 не хватает для первого вызова в рабочей среде
...
//.  
// Параметры:
//    СтрокаСлова - см. ирКэш.Получить().НоваяТаблицаМетодовМодуля()[0], см. ирКэш.Получить().НоваяТаблицаПеременныхМодуля()[0] - 
// Возвращаемое значение:
//    Строка - 
...
Процедура ОткрытьЗамерПроизводительностиЛкс(Знач ИмяФайла, Знач СтрокаДляПоиска = "") Экспорт
	...
	Если ЗначениеЗаполнено(СтрокаДляПоиска) Тогда
		//ПолеТабличногоДокумента = Форма.РезультатОтчета; // Почему то Неопределено
		ПолеТабличногоДокумента = Форма.ЭлементыФормы.ТабличныйДокумент;
		Форма.ТекущийЭлемент = ПолеТабличногоДокумента;
		НайтииПоказатьЯчейкуПоляТабличногоДокументаЛкс(ПолеТабличногоДокумента, СтрокаДляПоиска);
	КонецЕсли;
КонецПроцедуры     
...
//   ЯчейкаЦеликом - Булево - 
Функция НайтииПоказатьЯчейкуПоляТабличногоДокументаЛкс(Знач ПолеТабличногоДокумента, Знач СтрокаДляПоиска, Знач ЯчейкаЦеликом = Истина) Экспорт
	Область = ПолеТабличногоДокумента.НайтиТекст(СтрокаДляПоиска,,,, ЯчейкаЦеликом);
	Успех = Область <> Неопределено;
	Если Успех Тогда
		ПолеТабличногоДокумента.ТекущаяОбласть = Область;
	КонецЕсли;
	Возврат Успех;
КонецФункции     

// Вызывается из адаптера
Функция ВыбратьЭлементДереваЖСОНЛкс(Знач Текст, Знач Заголовок = "Выберите строку") Экспорт
	ОбъектЖСОН = ирОбщий.ОбъектИзТекстаЖСОНЛкс(Текст, Истина);
	Дерево = ирОбщий.ДеревоЗначенийИзМассиваСтруктурЛкс(ОбъектЖСОН);
	Дерево.Колонки.Свойство.Ширина = 50;
	Дерево.Колонки.Удалить("Значение");
	СтрокаДерева = ОткрытьТаблицуЗначенийЛкс(Дерево,,, Заголовок, Истина,,, Заголовок,,,, Истина); // СтрокаДереваЗначений: см. Дерево
	Результат = ирОбщий.Дерево_ПутьСтрокойЛкс(СтрокаДерева, "Свойство", , , Дерево);
	Возврат Результат;
КонецФункции
...
Функция ОткрытьТаблицуИзМассиваИзСтрокиВнутрЛкс(Знач ТекстЯчеек, Знач ТекстТекущейЯчейки = "", Знач ЛиТаблица = Ложь, Знач ЛиМодально = Ложь, Знач ЧислоКолонок = Неопределено, Знач Заголовок = "",
		Знач КлючХраненияВыбора = Неопределено, Знач НомерТекущейСтроки = Неопределено, Знач ИмяФайлаТабличногоДокумента = "", Знач МожноФокусВФильтр = Истина) Экспорт 
	мПлатформа = ирКэш.Получить();
	Массив = ирОбщий.ОбъектИзТекстаЖСОНЛкс(ТекстЯчеек); // Массив
	...
	Если Не ЛиТаблица Тогда
		...
		КонецЕсли;
		ФормаСписка.МожноФокусВФильтр = МожноФокусВФильтр;
	...
	Иначе
		Если ЗначениеЗаполнено(ИмяФайлаТабличногоДокумента) Тогда
			...
			ТекущаяСтрока = Таблица.Найти(ирОбщий.ТекстБезПоследнегоФрагментаЛкс(ТекстТекущейЯчейки, " "), Таблица.Колонки[ИндексТекущейКолонки].Имя);
			Если Таблица.Колонки[0].Имя = "Индекс" Тогда
				//! Таблица.Колонки.Добавить("Индекс");
		...
		Иначе
			...
			Тогда
				...
				Для Каждого Строка Из Таблица Цикл     
					Строка.Метод = мПлатформа.НайтиИмяМетодаСтрокиМодуля(Строка.ИмяМодуля, ирОбщий.СтрокаВЧислоЛкс(Строка.Строка),, ТекстыМодулейКэш,,, Строка.Текст);
				КонецЦикла;
			ИначеЕсли Заголовок = "Результаты поиска" Тогда
				...
				Для Каждого Строка Из Таблица Цикл     
					...
					Если СтруктураСсылки <> Неопределено Тогда
						Строка.Метод = мПлатформа.НайтиИмяМетодаСтрокиМодуля(СтруктураСсылки.МодульСРасширением, СтруктураСсылки.НомерСтроки,, ТекстыМодулейКэш);
					КонецЕсли;
				КонецЦикла;
			Иначе
				ПодготовитьТаблицуИзКонфигуратора(Таблица);
...
Функция ОткрытьВыборПодсистемЛкс(Знач ФильтрПодсистем = Неопределено) Экспорт
...
Функция ЧистоеОписаниеОшибкиЛкс(Знач ИнформацияОбОшибке, Знач ПолеТекста = Неопределено, Знач СтартоваяСтрока = 0, Знач СтартоваяКолонка = 0, Знач СтартоваяСтрокаДоп = 0, Знач СтартоваяСтрокаДопПорог = 0,
	...
	Вхождения = ирОбщий.НайтиРегВыражениеЛкс(ОписаниеОшибки, "\{(.*?)\}");
	Если Вхождения.Количество() > 0 Тогда
		...
		ФигурноеОписаниеОшибки = ПоследнееВхождение.Группа0;
		//ОписаниеОшибки = Сред(ИнформацияОбОшибке.Описание, ПоследнееВхождение.ПозицияВхождения + 1); // - вредно для ВызватьИсключение "Куку {Кир}" https://www.hostedredmine.com/issues/1006203
	КонецЕсли;
	Если НомерСтроки = 0 Тогда
...
// Параметры:
//  ОбъектКомпоненты - ОбработкаОбъект.ирКлсПолеТекстаПрограммы - компонента;
//  КнопкиМакета  - КнопкиКоманднойПанели - источник;
//  КнопкаПриемника - КнопкаКоманднойПанели - приемник;
...
Процедура ДобавитьКнопкиКоманднойПанелиКомпонентыЛкс(ОбъектКомпоненты, КнопкиМакета, КнопкаПриемника,
	...
	СчетчикРазделителей = 10;
	Для Каждого КнопкаМакета Из КнопкиМакета Цикл
		Если КнопкаМакета.ТипКнопки = ТипКнопкиКоманднойПанели.Разделитель Тогда
			Кнопка = Неопределено;
		Иначе
			Кнопка = КнопкиПриемника.Найти(КнопкаМакета.Имя);
		КонецЕсли;
		Если Ложь
			Или Не НеДобавлятьЕслиСуществует
		...
		Тогда
			...
			Если КнопкаМакета.ТипКнопки = ТипКнопкиКоманднойПанели.Действие Тогда 
				...
				Иначе
					...
					ВызватьИсключение ирОбщий.СтрШаблонЛкс("Недопустимо назначать кнопке (%1) макета обработчик", КнопкаМакета.Имя);
				КонецЕсли;
			//ИначеЕсли КнопкаМакета.ТипКнопки = ТипКнопкиКоманднойПанели.Разделитель Тогда
			//	ИмяКнопкиСПрефиксом = ИмяКнопкиСПрефиксом + СчетчикРазделителей;
			//	СчетчикРазделителей = СчетчикРазделителей + 1;
			КонецЕсли;
			Если Кнопка = Неопределено Тогда
				Если КнопкаМакета.ТипКнопки = ТипКнопкиКоманднойПанели.Подменю Тогда
					Кнопка = КнопкиПриемника.Найти(ИмяКнопкиСПрефиксом);
				ИначеЕсли КнопкаМакета.ТипКнопки = ТипКнопкиКоманднойПанели.Разделитель Тогда
					ИмяКнопкиСПрефиксом = "";
				ИначеЕсли Истина
					И НеДобавлятьЕслиСуществует = Истина 
					И КнопкиПриемника.Найти(ИмяКнопкиСПрефиксом) <> Неопределено 
				Тогда
					Продолжить;
				КонецЕсли;
				Если Кнопка = Неопределено Тогда
					...
					Исключение
						ВызватьИсключение ирОбщий.СтрШаблонЛкс("Невозможно добавить кнопку компоненты в приемник, т.к. там уже есть одноименная (%1)", ИмяКнопкиСПрефиксом);
					КонецПопытки;
					ЗаполнитьЗначенияСвойств(Кнопка, КнопкаМакета, , "Действие, Имя");
				КонецЕсли;
			КонецЕсли;
		...
		КонецЕсли;
		Если КнопкаМакета.ТипКнопки = ТипКнопкиКоманднойПанели.Подменю Тогда 
			ДобавитьКнопкиКоманднойПанелиКомпонентыЛкс(ОбъектКомпоненты, КнопкаМакета.Кнопки, Кнопка, ДействияКнопокКомпонент, ОбщийПриемник, НеДобавлятьЕслиСуществует);
		КонецЕсли; 
	КонецЦикла;
...
//  ОформленияСтрок				 - Массив из ОформлениеСтроки
...
Процедура ПриПолученииДанныхТабличногоПоляКомпоновкиЛкс(ЭтаФорма, Элемент, ОформленияСтрок, ТабличноеПолеВыбранныхПолей = Неопределено, ОтображатьВВидеИмен = Ложь) Экспорт
	...
	Для каждого ОформлениеСтроки Из ОформленияСтрок Цикл
		...
		ЯчейкаПоля = Ячейки[КолонкаКартинки.Имя];
...
// Подсказка к колонке "Заголовок" задается программно в ПодключитьОбработчикиСобытийДоступныхПолейКомпоновкиЛкс
// Вызывающие модули:
//  Обработка.ирКлсПолеТекстаПрограммы.Форма.КонструкторЗапроса
Процедура ОформитьСтрокуДоступногоПоляЛкс(ОформлениеСтроки, СтрокаПоля, Знач ТипТаблицы = Неопределено, ИмяКолонки = "Заголовок", Знач ЭтоИндекс = Ложь, Использовано = Ложь, Знач Измерения = "") Экспорт 
	...
	//	И (Ложь
	//		Или ТипТаблицы = Неопределено
	//		Или (Истина
	//			И ТипТаблицы <> "ДвиженияССубконто"
	//			И ТипТаблицы <> "ВиртуальнаяТаблица"
	//			И ТипТаблицы <> "ВременнаяТаблица"
	//			И ТипТаблицы <> "Параметр"
	...
	ЭтоИзмерение = Истина
		И СтрокаПоля <> Неопределено
		И (Ложь
			Или Найти(ирОбщий.ПолноеИмяМДПоляТаблицыЛкс(СтрокаПоля), ".Измерение.")
			Или Найти(Измерения, "," + СтрокаПоля.Имя + ",")
			Или (Истина
				И ТипЗнч(СтрокаПоля.Метаданные) = Тип("ОписаниеСтандартногоРеквизита") 
				И (Ложь
					Или СтрокаПоля.Имя = ирОбщий.ПеревестиСтроку("Период") И ТипТаблицы <> "ВиртуальнаяТаблица"
					Или СтрокаПоля.Имя = ИмяСсылка И ТипТаблицы <> "ТабличнаяЧасть")));
	Если Истина
		И СтрокаПоля <> Неопределено 
	...
	Если Ложь
		...
		Или (Истина
			И СтрокаПоля <> Неопределено
			И СтрокаПоля.Имя = ИмяСсылка 
			И ТипТаблицы <> Неопределено
			И ирОбщий.НуженСдвигПоляСсылкаВНачалоСпискаЛкс(ТипТаблицы))
	Тогда
...
Процедура ОформитьСтрокуДопРеквизитаБСПЛкс(Знач ОформлениеСтроки) Экспорт
	ОформлениеСтроки.ЦветТекста = Новый Цвет(0, 0, 128);
КонецПроцедуры

// Вызывающие модули:
//  Обработка.ирКонсольКомпоновокДанных.Форма.Форма
Процедура УсловноеОформлениеПриВыводеСтрокиЛкс(Знач ЭтаФорма, Знач Элемент, Знач ОформлениеСтроки, Знач ДанныеСтроки, Знач КомпоновщикУсловий = Неопределено, Знач КомпоновщикПолей = Неопределено) Экспорт 
	...
	#Если Сервер И Не Сервер Тогда
		...
		КомпоновщикПолей = Новый КомпоновщикНастроекКомпоновкиДанных;
	#КонецЕсли
	ТабличноеПолеПриВыводеСтрокиЛкс(ЭтаФорма, Элемент, ОформлениеСтроки, ДанныеСтроки);
	...
	Если КомпоновщикУсловий <> Неопределено Тогда
		Если ирОбщий.ЛиЕстьНеактуальныеПоляВГруппеОтбораЛкс(КомпоновщикУсловий, ДанныеСтроки.Отбор) Тогда 
			ЯчейкаОтбор.УстановитьКартинку(ирКэш.КартинкаПоИмениЛкс("ирНедоступноеПоле"));
		КонецЕсли;
	КонецЕсли;
	Если КомпоновщикПолей = Неопределено Тогда
		КомпоновщикПолей = КомпоновщикУсловий
	КонецЕсли;
	Если КомпоновщикПолей <> Неопределено Тогда
		Для Каждого Поле Из ДанныеСтроки.Поля.Элементы Цикл
			Если Истина
				И ЗначениеЗаполнено("" + Поле.Поле) 
				И КомпоновщикПолей.Настройки.ДоступныеПоляВыбора.НайтиПоле(Поле.Поле) = Неопределено 
			Тогда
...
// Вызывающие модули:
//  Обработка.ирСравнениеТаблиц.Форма.Форма
...
// Также устанавливает картинку строк и подсказку шапки первой колонки.
// Параметры:
...
Процедура ПодключитьОбработчикиСобытийДоступныхПолейКомпоновкиЛкс(ТабличноеПоле) Экспорт 
	...
	Для Каждого КлючИЗначение Из ИменаСобытий Цикл
		...
		Тогда 
			...
			Попытка
				ТабличноеПоле.УстановитьДействие(КлючИЗначение.Ключ, Обработчик);
				ЛиПодключенСуществующийОбработчик = Истина;
			Исключение
				ЛиПодключенСуществующийОбработчик = Ложь;
			КонецПопытки;
			Если ЛиПодключенСуществующийОбработчик Тогда
				Продолжить;
			КонецЕсли;
		КонецЕсли; 
		Попытка
	...
	КолонкаКартинки.КартинкиСтрок = ирКэш.КартинкаПоИмениЛкс("ирТипыДоступныхПолейКомпоновки");
	КолонкаКартинки.ПодсказкаВШапке = 
	"В скобках после имени поля может быть указано:
	|- <Число>т - число возможных типов 
	|- Индекс - признак наличия индекса с этим полем в первой позиции 
	|- Измер - признак вхождения поля в уникальный набор, т.е. каждому сочетанию полей соответствует только одна строка таблицы";
...
// В модуле формы требуется блоки обработчиков ОбщиеОбработчикиТабличныхПолейКомпоновки
// Параметры:
//  ТабличноеПоле - ТабличноеПоле - отбора компоновки
...
Функция ЛиНедоступныйПутьКлиентСервераЛкс(Знач Путь = "", Знач ТолькоПортативный = Ложь) Экспорт
	Если ПустаяСтрока(Путь) И ирКэш.ЛиПортативныйРежимЛкс() Тогда    
		ВнешняяОбработкаОбъект = ирКлиент; // ВнешняяОбработкаОбъект  
		Путь = ВнешняяОбработкаОбъект.ИспользуемоеИмяФайла;  
	КонецЕсли;
	Результат = Истина
		И (Ложь
			Или Не ТолькоПортативный
...
Процедура ОтключитьОбработчикОжиданияСПараметрамиЛкс(ИмяМетодаОбщегоМодуля = "") Экспорт 
...
Процедура АктивироватьЭлементФормыОтложеноЛкс(Параметры) Экспорт
...
//  ОбщееНазвание								 - 																 - 
//
Функция Сравнить2ЗначенияВФормеЛкс(Знач Значение1, Знач Значение2, Знач Модально = Ложь, Знач Название1 = Неопределено, Знач Название2 = Неопределено, 
	...
	Знач ОбщееНазвание = "", Знач КлючевыеКолонки = "", Знач СравниваемыеКолонки = "", Знач НомерСтроки1 = 0, Знач НомерСтроки2 = 0, Знач РежимВыбора = Ложь) Экспорт 
	...
	Если Ложь
		...
		Или ТипЗнч(Значение1) = Тип("COMОбъект")
	Тогда
		Возврат СравнитьЗначенияВФормеЧерезXMLЛкс(Значение1, Значение2, Модально, Название1, Название2, ПолучатьXMLПредставлениеДляНеизвестныхТипов, ВариантСинтаксиса, ОбщееНазвание,, КлючУникальностиФормы, НомерСтроки1, НомерСтроки2, РежимВыбора);
	КонецЕсли; 
	...
	Тогда
		//! Значение2.Найти(, КлючевыеКолонки);
		//! Значение2.Найти(, СравниваемыеКолонки);
		Возврат СравнитьТаблицыИнтерактивноЛкс(Значение1, Значение2, Модально,,  КлючевыеКолонки, СравниваемыеКолонки);
	КонецЕсли; 
	Объекты = Новый СписокЗначений;
	Объекты.Добавить(Значение1, Название1);
	Объекты.Добавить(Значение2, Название2);
	Возврат СравнитьОбъектыВФормеЛкс(Объекты, Модально, ТекущееСвойство,, КлючУникальностиФормы);
	
КонецФункции

Функция СравнитьТаблицыИзФайловТабличныхДокументовЛкс(Знач ИмяФайла1, Знач ИмяФайла2, Знач ЛиПерваяСтрокаСодержитИменаКолонок = Истина, Знач ИсключитьКолонки = "") Экспорт
	Документ1 = Новый ТабличныйДокумент;
	Документ1.Прочитать(ИмяФайла1);
	Значение1 = ирОбщий.ТаблицаЗначенийИзТабличногоДокументаЛкс(Документ1, ЛиПерваяСтрокаСодержитИменаКолонок);
	Документ2 = новый ТабличныйДокумент;
	Документ2.Прочитать(ИмяФайла2);
	Значение2 = ирОбщий.ТаблицаЗначенийИзТабличногоДокументаЛкс(Документ2, ЛиПерваяСтрокаСодержитИменаКолонок);
	Возврат СравнитьТаблицыИнтерактивноЛкс(Значение1, Значение2,,,,, ИсключитьКолонки);
КонецФункции

Функция СравнитьТаблицыИнтерактивноЛкс(Знач Значение1, Знач Значение2, Знач Модально = Ложь, Знач БезКлючевыхКолонок = Ложь, Знач КлючевыеКолонки = "", Знач СравниваемыеКолонки = "", Знач ИсключитьКолонки = "") Экспорт 
	...
	ФормаСравнителя.ПараметрКлючевыеКолонки = КлючевыеКолонки;
	ФормаСравнителя.ПараметрСравниваемыеКолонки = СравниваемыеКолонки;
	ФормаСравнителя.ПараметрИсключитьКолонки = ИсключитьКолонки;
	Если Модально Тогда
		Результат = ФормаСравнителя.ОткрытьМодально();
	Иначе
		ФормаСравнителя.Открыть();
		ФормаСравнителя.СравнитьТаблицыВФорме();
		Результат = ФормаСравнителя;
	КонецЕсли;
	Возврат Результат;
КонецФункции
...
//
Функция СравнитьОбъектыВФормеЛкс(Знач Объекты, Знач Модально = Ложь, ТекущееСвойство = "", ТекущийОбъект = Неопределено, КлючУникальностиФормы = Неопределено, Знач НомерЭталона = Неопределено,
	Знач ЭтаФорма = Неопределено) Экспорт 
	
	Если Объекты.Количество() = 0 Тогда
		Возврат Неопределено;
	ИначеЕсли Объекты.Количество() = 1 Тогда
		Возврат ДобавитьОбъектВБуферИПредложитьСравнениеЛкс(ТекущийОбъект, ЭтаФорма);
	КонецЕсли;
	Если ЭтаФорма <> Неопределено Тогда
...
//   Название1 -  - 
//   Название2 -  - 
//   XMLПредставлениеДляНеизвестныхТипов -  - 
//   ВариантСинтаксиса -  - 
//   ОбщееНазвание -  - 
//   РазрешитьСобственныйСравнитель -  - 
//   КлючУникальностиФормы -  - 
//   НомерСтроки1 -  - 
//   НомерСтроки2 -  - 
//   РежимВыбора -  - 
// Возвращаемое значение:
//   Строка, Форма, Неопределено - если тип Строка, то содержит новый текст
Функция СравнитьЗначенияВФормеЧерезXMLЛкс(Знач Значение1, Знач Значение2, Знач Модально = Ложь, Знач Название1 = "", Знач Название2 = "", Знач XMLПредставлениеДляНеизвестныхТипов = Истина, ВариантСинтаксиса = "",
	Знач ОбщееНазвание = "", Знач РазрешитьСобственныйСравнитель = Истина, Знач КлючУникальностиФормы = "", Знач НомерСтроки1 = 0, Знач НомерСтроки2 = 0, Знач РежимВыбора = Ложь) Экспорт 
	...
	Если Истина
		...
		И (Ложь
			...
			Или ТипЗнч(Значение2) = Тип("COMОбъект"))
	Тогда
		Если ТипЗнч(Значение1) <> Тип("Строка") Тогда
			Значение1 = Значение1.ПолучитьТекст();
		КонецЕсли; 
		Если ТипЗнч(Значение2) <> Тип("Строка") Тогда
		...
		ФормаСравнителя.ПараметрНомерСтроки1 = НомерСтроки1;
		ФормаСравнителя.ПараметрНомерСтроки2 = НомерСтроки2;
		ФормаСравнителя.РежимВыбора = РежимВыбора;
		Если Модально Тогда
			Результат = ФормаСравнителя.ОткрытьМодально();
		Иначе
			ФормаСравнителя.Открыть();
...
Функция НоваяФормаСравнителяТекстовЛкс(Знач КлючУникальности = "") Экспорт 
	...
	Если ПустаяСтрока(КлючУникальности) тогда
		КлючУникальности = Новый УникальныйИдентификатор;
	КонецЕсли;
	ФормаСравнителя = ПолучитьФормуЛкс("Обработка.ирСравнениеТекстов.Форма",,, КлючУникальности);
...
//
Функция ЗапомнитьСодержимоеЭлементаФормыДляСравненияЛкс(ЭтаФорма, ЭлементУправления, ВариантСинтаксиса = "", Знач РазрешитьВопросы = Истина, Знач Текст = "", Знач ЗаголовокЭлементаФормы = "",
	МассивСравнения = Неопределено) Экспорт
	...
	Тогда
		...
		КонецЕсли;
		ДопСвойства = ирКлиент.ДопСвойстваЭлементаФормыЛкс(ЭтаФорма, ЭлементУправления);
		Если ДопСвойства <> Неопределено Тогда
			КлючевыеКолонки = ДопСвойства.КлючевыеКолонки;
			СравниваемыеКолонки = ДопСвойства.СравниваемыеКолонки;
		КонецЕсли;
	ИначеЕсли Ложь
	...
	Иначе
		...
		Возврат Сообщение;
	КонецЕсли;
	Возврат ДобавитьОбъектВБуферИПредложитьСравнениеЛкс(СравниваемыйДокумент, ЭтаФорма,, ЗаголовокЭлементаФормы, ВариантСинтаксиса, РазрешитьВопросы, МассивСравнения, КлючевыеКолонки, СравниваемыеКолонки);
...
// Вызывается и с передачей КОМ объектов из адаптера.
//
// Параметры:
...
Функция ДобавитьОбъектВБуферИПредложитьСравнениеЛкс(Знач ЗначениеДляСравнения, Знач ЭтаФорма = Неопределено, Знач ТекущееСвойство = "", Знач ЗаголовокЭлементаФормы = "", ВариантСинтаксиса = "",
	Знач РазрешитьВопросы = Истина, МассивСравнения = Неопределено, Знач КлючевыеКолонки = "", Знач СравниваемыеКолонки = "") Экспорт 
	
	//! ЗначениеДляСравнения.Найти(, КлючевыеКолонки);
	//! ЗначениеДляСравнения.Найти(, СравниваемыеКолонки);
	МассивСравнения = ДобавитьОбъектВБуферСравненияЛкс(ЗначениеДляСравнения, ЗаголовокЭлементаФормы, МассивСравнения);
	Если МассивСравнения.Количество() = 2 Тогда
		...
		Иначе
			Если ЭтаФорма <> Неопределено Тогда
				ПредложитьЗакрытьМодальнуюФормуЛкс(ЭтаФорма);
			КонецЕсли; 
			СравниваемыйДокумент1 = МассивСравнения.Получить(0).Значение; // - Без [] для КОМ
			СравниваемыйДокумент2 = МассивСравнения.Получить(1).Значение; 
			Заголовок1 = МассивСравнения.Получить(0).Заголовок;
			Заголовок2 = МассивСравнения.Получить(1).Заголовок;
			Результат = Сравнить2ЗначенияВФормеЛкс(СравниваемыйДокумент1, СравниваемыйДокумент2,, Заголовок1, Заголовок2,, ТекущееСвойство,,, ВариантСинтаксиса,, КлючевыеКолонки, СравниваемыеКолонки);
			МассивСравнения.Очистить();
			Возврат Результат;
		КонецЕсли;
	КонецЕсли;
~ВыходЛожь:
	Сообщение = ирОбщий.СтрШаблонЛкс("Значение для сравнения запомнено%1. Теперь передайте второе значение.", 
...
// 
Функция ДобавитьОбъектВБуферСравненияЛкс(Знач ЗначениеДляСравнения, Знач ЗаголовокЗначения = "", МассивСравнения = Неопределено) Экспорт 
...
Функция ВывестиСтрокиТабличногоПоляЛкс(Знач ЭтаФорма, Знач ТабличноеПоле, Знач ПараметрыВывода, Отладка = Ложь, выхТекущаяСтрока = Неопределено, Знач НастройкиСписка = Неопределено) Экспорт
	...
	Если Ложь
		Или ЗначениеТабличногоПоля = Неопределено 
	Тогда
		КоллекцияСтрок = ТаблицаИлиДеревоЗначенийИзТаблицыФормыСКоллекциейЛкс(ТабличноеПоле,,, Ложь);
	ИначеЕсли Ложь
		Или ТипЗнч(ЗначениеТабличногоПоля) = Тип("ДанныеФормыКоллекция") 
		Или ТипЗнч(ЗначениеТабличногоПоля) = Тип("ДанныеФормыДерево")
	Тогда
		ЗначениеТабличногоПоля = ТаблицаИлиДеревоЗначенийИзТаблицыФормыСКоллекциейЛкс(ТабличноеПоле,,, Ложь,,, ЭтаФорма);
	КонецЕсли; 
		...
		Тогда
			...
			Иначе
				...
				Иначе
					...
					//! КопияСтрок.Колонки.Добавить("НомерСтроки");
	...
	Если ПараметрыВывода.БезОформления Тогда
		...
		Если ТипИсточника = "Список" И ЗначениеЗаполнено(ПолноеИмяТаблицыБД) Тогда
			...
			Если ирОбщий.ЛиКорневойТипСсылкиЛкс(ирОбщий.ТипТаблицыБДЛкс(ПолноеИмяТаблицыБД)) И ПараметрыВывода.КолонкиИдентификаторов Тогда
				...
				//! КоллекцияСтрок.Колонки.Добавить("ИдентификаторСсылкиЛкс")
	...
	Иначе
		...
		Если ТипЗнч(ТабличноеПоле) = Тип("ТаблицаФормы") Тогда
			ВызватьИсключение НСтр("ru='Вывод с оформлением недоступен для управляемой формы'");
		КонецЕсли;
	...
	Иначе
		...
		Тогда
			...
			#Если Сервер И Не Сервер Тогда
				Запрос = Новый Запрос;
			#КонецЕсли
			Результат = ирОбщий.ПустаяТаблицаИзТекстаЗапросаЛкс(Запрос.Текст);
		Иначе
			КлючТекущейСтроки = Неопределено;
...
// Параметры:
//   ТабличноеПоле - ТабличноеПоле, ТаблицаФормы - 
//   НуженВидимыйПорядок - Булево - для динамического списка вычисление видимого порядка долго. Если Ложь, то используется порядок выделения строк.
...
Процедура ВывестиСтрокиТабличногоПоляСОформлениемЛкс(Знач ЭтаФорма, Знач КоллекцияСтрок, НомерСтроки = Неопределено, Знач Результат, Знач ТабличноеПоле, Смещение = "", ВстроитьЗначенияВРасшифровки = Истина,
	...
	Для Каждого СтрокаИсточника Из КоллекцияСтрок Цикл
		...
		Тогда
			...
			КонецЕсли;
			ВывестиСтрокиТабличногоПоляСОформлениемЛкс(ЭтаФорма, СтрокаИсточника.Строки, НомерСтроки, Результат, ТабличноеПоле, Смещение + "    ", ВстроитьЗначенияВРасшифровки, ВыбранныеКолонки);
			Если ТипЗнч(Результат) = Тип("ТабличныйДокумент") Тогда
...
//   ФормаИнициатор - Форма, УправляемаяФорма - которая будет использована в качестве инициатора события
//              если не указана, то будет создана временная форма-пустышка;
//   КонтролироватьУспех - Булево - проверять успешность установки значения, долго и не надежно для управляемой формы
// Возвращаемое значение:
//   Булево - Если КонтролироватьУспех, то успешно ли значение установлено, иначе Неопределено
...
//   Значение - Произвольный - присваиваемое значение;
//   ФормаИнициатор - Форма, УправляемаяФормы - которая будет использована в качестве инициатора события;
//              если не указана, то будет создана временная форма-пустышка;
//   ВосстанавитьТекущуюКолонку - Булево - 
//   ВключитьРежимРедактирования - Булево - 
//   КонтролироватьТекущиеДанные - Булево - если значение не установилось интерактивно, то установить его программно
//   ВыключитьРежимРедактирования -  - 
...
Функция ИнтерактивноЗаписатьВКолонкуТабличногоПоляЛкс(ТабличноеПоле, Знач Колонка, Знач Значение, Знач ФормаИнициатор = Неопределено, Знач ВосстанавитьТекущуюКолонку = Истина,
	...
	Иначе //! Если ТипЗнч(ТабличноеПоле) = Тип("ТабличноеПоле") Тогда
		...
		КонецЕсли;
	КонецЕсли;
	Если ирКэш.НомерРежимаСовместимостиЛкс() < 802001 Тогда
		// Аварийное завершение https://devtool1c.ucoz.ru/forum/2-2939-1
		КонтролироватьТекущиеДанные = истина;
	ИначеЕсли ЭлементУправления <> Неопределено Тогда
...
Процедура НастроитьФормуПоМакетуЛкс(пФорма, пМакетФормы) Экспорт
	...
	Для Каждого ЭлементМакета Из МакетФормы.ЭлементыФормы Цикл
		...
		Если ТипЗнч(ЭлементМакета) = Тип("КоманднаяПанель") Тогда
			//! ЭлементФормы = 0 // КоманднаяПанель
...
Процедура ИзменитьСвернутостьЛкс(ЭтаФорма, Видимость, ГлавныйЭлемент, Разделитель, Панель, Направление, ПодчиненныйЭлемент = Неопределено,
	...
	КонецЕсли;
	Если Видимость И ГлавныйЭлемент.Свертка = РежимСверткиЭлементаУправления.Нет Тогда
		Возврат;
	КонецЕсли;
...
Функция ОткрытьКонстантуВСпискеЛкс(ИмяКонстанты) Экспорт
...
Функция ОткрытьИсториюГитаЛкс(Знач ЧтоИскать = "", Знач ИмяМодуля = "", Знач Коммит = "", Знач НомерСтроки = 0) Экспорт
	Форма = ПолучитьФормуЛкс("Обработка.ирИсторияГита.Форма");
	Форма.ПараметрЧтоИскать = ЧтоИскать;
	Форма.ПараметрИмяМодуля = ИмяМодуля;
	Форма.ПараметрКоммит = Коммит;
	Форма.ПараметрНомерСтроки = НомерСтроки;
	Форма.Открыть();
	Возврат Форма;
КонецФункции

Функция ГитРепозиторий(Знач ДобавитьSrc = Ложь) Экспорт
	Результат = ирОбщий.ВосстановитьЗначениеЛкс("ГитРепозиторий");
	Если ДобавитьSrc И ЗначениеЗаполнено(Результат) Тогда
		Результат = Результат + ирОбщий.РазделительПутиКФайлуЛкс() + "src";
	КонецЕсли;
	Возврат Результат;
КонецФункции

Функция ИмяФайлаМодуляВПапкеГитаЛкс(Знач ИмяМодуля, ПапкаГитаКэш = "", Знач ДобавитьSrc = Истина) Экспорт
	Если ПустаяСтрока(ПапкаГитаКэш) Тогда
		ПапкаГитаКэш = ГитРепозиторий();  
	КонецЕсли;
	Если Не ЗначениеЗаполнено(ПапкаГитаКэш) Тогда
		ирКлиент.ОткрытьНастройкиПоляПрограммыЛкс("Укажите ""Гит репозиторий"" в открывшейся форме настроек",, "ГитРепозиторий");
		Возврат "";
	КонецЕсли;
	ЛиМножественное = Истина;
	Разделитель = ирОбщий.РазделительПутиКФайлуЛкс();
	Фрагменты = ирОбщий.СтрРазделитьЛкс(ИмяМодуля);
	ИмяФайлаМодуляВГите = Разделитель;
	Если ДобавитьSrc Тогда
		ИмяФайлаМодуляВГите = ИмяФайлаМодуляВГите + "src" + Разделитель;
	КонецЕсли;
	ПроверочныйФайл = Новый Файл(ПапкаГитаКэш + ИмяФайлаМодуляВГите + "Configuration.xml");
	ЭтоЕДТ = Не ПроверочныйФайл.Существует();
	ЭтоМодульФормы = Ложь;
	ЭтоМакет = Ложь;
	Если Фрагменты.Количество() = 1 Тогда
		ИмяФайлаМодуляВГите = Разделитель + "Ext" + Разделитель + Фрагменты[0];
	Иначе
		Для Индекс = 0 По Фрагменты.Количество() / 2 - 1 Цикл
			ИмяРус = Фрагменты[Индекс * 2];
			Если ИмяРус = "Конфигурация" Тогда
				ИмяФайлаМодуляВГите = ИмяФайлаМодуляВГите + "Configuration" + Разделитель;
				Фрагменты.Удалить(0);
				Прервать;
			КонецЕсли;
			ИмяАнг = ирОбщий.ПеревестиИзРусскогоВАнглийскийЛкс(ИмяРус);
			Если ЛиМножественное Тогда
				Если ИмяРус = "БизнесПроцесс" Тогда
					ИмяАнг = "BusinessProcesses"
				ИначеЕсли ИмяРус = "ПланВидовХарактеристик" Тогда
					ИмяАнг = "ChartsOfCharacteristicTypes";
				Иначе
					ИмяАнг = ИмяАнг + "s";
				КонецЕсли;
			КонецЕсли;
			Если ИмяАнг = Неопределено Тогда
				ИмяАнг = ИмяРус;
			КонецЕсли;
			ИмяФайлаМодуляВГите = ИмяФайлаМодуляВГите + ИмяАнг + Разделитель + Фрагменты[Индекс * 2 + 1] + Разделитель;
			Если Истина
				И Фрагменты.ВГраница() >= (Индекс+1)*2
				И Фрагменты[Индекс * 2 + 2] = "Форма"
				И Фрагменты[Индекс * 2 + 3] = "Модуль"
			Тогда
				ЭтоМодульФормы = Истина;
				Прервать;
			КонецЕсли;
		КонецЦикла;
		Если Истина
			И Фрагменты.ВГраница() = 3
			И Фрагменты[2] = "Макет"
		Тогда
			Фрагменты.Добавить("Макет");
			ЭтоМакет = Истина;
		КонецЕсли;
	КонецЕсли;
	Если Фрагменты.Количество() % 2 = 1 Или ЭтоМодульФормы Тогда
		...
		Если Не ЭтоЕДТ Тогда
			ИмяФайлаМодуляВГите = ИмяФайлаМодуляВГите + "Ext" + Разделитель;
			Если ЭтоМодульФормы Тогда
				ИмяФайлаМодуляВГите = ИмяФайлаМодуляВГите + "Form" + Разделитель;
			КонецЕсли;
		КонецЕсли;
		Если ЭтоМакет Тогда
			РасширениеФайла = "txt";
		Иначе
			РасширениеФайла = "bsl";
		КонецЕсли;
		ИмяФайлаМодуляВГите = ПапкаГитаКэш + ИмяФайлаМодуляВГите + ИмяСвойстваАнг + "." + РасширениеФайла;
	Иначе
		ИмяФайлаМодуляВГите = ПапкаГитаКэш + ирОбщий.СтрокаБезКонцаЛкс(ИмяФайлаМодуляВГите); /// папка
	КонецЕсли;
	Возврат ИмяФайлаМодуляВГите;
КонецФункции

//.
// Параметры:
//   ВнутреннееИмяФайлаГита - Строка - например "src/Catalogs/Валюты.xml"
// Возвращаемое значение:
//    - 
Функция ИмяМодуляИзИмениФайлаГитаЛкс(Знач ВнутреннееИмяФайлаГита) Экспорт
	РасширениеФайла = ирОбщий.ПоследнийФрагментЛкс(ВнутреннееИмяФайлаГита);
	Фрагменты = ирОбщий.СтрРазделитьЛкс(ирОбщий.ТекстБезПоследнегоФрагментаЛкс(ВнутреннееИмяФайлаГита), "/");
	Если Фрагменты.Найти("src") <> Неопределено Тогда
	...
	Для Индекс = 0 По Фрагменты.Количество() / 2 - 1 Цикл
		...
		Если ИмяАнг = "Ext" Тогда
			...
			КонецЕсли;
		Иначе
			Если ирОбщий.СтрНачинаетсяСЛкс(ИмяАнг, "Charts") Тогда
				ИмяАнг = СтрЗаменить(ИмяАнг, "Charts", "Chart");
			Иначе
			...
			Если Ложь
				Или ИмяАнг = "Form" 
				Или ИмяАнг = "CommonForm"
			Тогда
				ЭтоФорма = Истина;
			КонецЕсли;
			ИмяРус = ирОбщий.ПеревестиВРусский(ИмяАнг);
			Если ИмяРус = ИмяАнг Тогда
				КонецМножественного = "e";
				Если ирОбщий.СтрКончаетсяНаЛкс(ИмяАнг, КонецМножественного) Тогда
					ИмяАнг = ирОбщий.СтрокаБезКонцаЛкс(ИмяАнг, СтрДлина(КонецМножественного));
				КонецЕсли;
				ИмяРус = ирОбщий.ПеревестиВРусский(ИмяАнг);
			...
			Если ИмяФайлаМодуляВГите = "Конфигурация" Тогда
				Фрагменты.Удалить(0);
				Прервать;
			КонецЕсли;
	...
	Если ирОбщий.СтрокиРавныЛкс(РасширениеФайла, "mdo") Тогда
		//
	ИначеЕсли ЭтоФорма Тогда
	...
	ИначеЕсли (Фрагменты.Количество() - ЧислоСлужебныхФрагментов) % 2 = 1 Тогда
		...
		КонецЕсли;
		Результат = Результат + ИмяРус;
	КонецЕсли;
	Возврат Результат;
КонецФункции

Функция ФайлКэшаМодуляИзФайлаГитаЛкс(Знач ФайлГита, Знач ПапкаГита = "", Модуль = Неопределено) Экспорт
	Если ПустаяСтрока(ПапкаГита) Тогда
		ПапкаГита = ГитРепозиторий();
	КонецЕсли;
	ВнутреннееИмяФайла = СтрЗаменить(Сред(ФайлГита.ПолноеИмя, СтрДлина(ПапкаГита) + 2), ирОбщий.РазделительПутиКФайлуЛкс(), "/");
	Модуль = ИмяМодуляИзИмениФайлаГитаЛкс(ВнутреннееИмяФайла);
	ФайлКэша = ирКэш.Получить().ФайлМодуляИзИмениМодуля(Модуль);
	Возврат ФайлКэша;
КонецФункции

Функция ПолноеИмяМДИзИмениМодуляЛкс(Знач ИмяМодуля) Экспорт
...
Функция ПрочитатьЗначениеИзФайлаСКонтролемПотерьЛкс(ПолноеИмяФайла) Экспорт
	...
	Если Не СравнениеФайлов.Сравнить() Тогда 
		...
		ирОбщий.СообщитьЛкс("При чтении из файла вероятно была потеряна часть данных обычно связанная с метаданными", СтатусСообщения.Внимание);
	КонецЕсли; 
...
Процедура ОтладитьОбъектПоСтруктуреЛкс(Знач СтруктураПараметров, Знач Модально = Ложь) Экспорт 
	...
	Если ТипОперации = "Отладить" Тогда
		...
		Если СтруктураПараметров.Свойство("ТипОбъекта") И СтруктураПараметров.ТипОбъекта = "HttpСоединение" Тогда
			//! Объект = Новый Структура("Сервер, Порт, Пользователь, Пароль, Защищенное, ИспользоватьАутентификациюОС, Таймаут");
			Объект = ирОбщий.СоединениеHTTPЛкс(Объект.Сервер, Объект.Порт, Объект.Пользователь, Объект.Пароль,, Объект.Таймаут, Объект.Защищенное, Объект.ИспользоватьАутентификациюОС);
			ЗаполнитьЗначенияСвойств(Объект, СтруктураПараметров.Объект);
			Объект2 = ирОбщий.ЗапросHTTPЛкс();
			ЗаполнитьЗначенияСвойств(Объект2, СтруктураПараметров.НастройкаКомпоновки);
			ТелоДвоичныеДанные = СтруктураПараметров.НастройкаКомпоновки.ТелоДвоичныеДанные; //@skip-check unknown-method-property
			Если ТелоДвоичныеДанные <> Неопределено Тогда
				Объект2.УстановитьТелоИзДвоичныхДанных(ТелоДвоичныеДанные);
			КонецЕсли; 
	...
	ИначеЕсли ТипОперации = "Исследовать" Тогда
		//! СтруктураПараметров.Вставить("КакКоллекцию");
		Если СтруктураПараметров.Свойство("СериализацияФабрикой") Тогда 
			...
			КонецЕсли; 
		КонецЕсли;      
		ирОбщий.ИсследоватьЛкс(Объект, Модально, СтруктураПараметров.КакКоллекцию,,,,,,, Истина);
	КонецЕсли;
...
Процедура ПолеВводаСИсториейВыбора_ПриИзмененииЛкс(Знач ЭлементФормыИлиЭлементОтбора, Знач КлючИстории, Знач ЗапоминатьПоследние = 20, Знач _НеЗапоминатьПустыеТипизированные = Истина,
	ДополнительныйКлючИстории = "", Знач НовоеЗначение = Неопределено, Знач МаксДлинаТекста = 0, Знач КлючевойЭлементФормы = Неопределено) Экспорт
	...
	Если НовоеЗначение = Неопределено Тогда
		...
		КонецЕсли;
	КонецЕсли; 
	Если КлючевойЭлементФормы = Неопределено Тогда
		КлючевойЭлементФормы = ЭлементФормыИлиЭлементОтбора;
	КонецЕсли;
	Если ЗначениеЗаполнено(НовоеЗначение) Тогда
		КлючИстории = ирОбщий.КлючИсторииВыбораПоляВводаЛкс(КлючевойЭлементФормы, КлючИстории, ДополнительныйКлючИстории);
...
Функция ОткрытьЗначениеЛкс(РасширенноеЗначение, РедактированиеРазрешено = Истина, СтандартнаяОбработка = Истина, ЗаголовокФормы = "", РедактироватьМодально = Истина,
	...
	выхФорма = Неопределено, Знач ОтображаемыйТекст = "") Экспорт 
	...
	Если Ложь
		...
		Или (Истина
			...
			И (Ложь
				...
				Или ирОбщий.ЛиСодержитОпасныеНепечатныеСимволыЛкс(РедактируемоеЗначение)
	...
	Тогда 
		...
		Иначе
			...
			Если ТипРасширенногоЗначения = Тип("Строка") Тогда
				ВариантПросмотра = "";
				Если Найти(ОтображаемыйТекст, ИндикаторНепечатныеСимволыЛкс()) Тогда
					ВариантПросмотра = "НепечатныеСимволы";
				КонецЕсли;
				выхФорма = ПолучитьФормуТекстаЛкс(,, ВариантПросмотра,, КлючУникальности);
	...
	ИначеЕсли ТипРасширенногоЗначения = Тип("Цвет") Тогда
		...
		Если Ложь
			Или РедактированиеРазрешено 
			Или ирКэш.НомерВерсииПлатформыЛкс() < 803003 
		Тогда
...
Процедура ОткрытьЦветИлиШрифтПоПолномуИмениЛкс(Знач ПолноеИмя) Экспорт
	Если Найти(ПолноеИмя, "(") Тогда
		Возврат;
	КонецЕсли;
	ирКлиент.ОткрытьЗначениеЛкс(Вычислить(ПолноеИмя), Ложь);
КонецПроцедуры
...
Процедура ЗапуститьПриложение1СЛкс(РежимКонфигуратора, СтрокаСоединения, ИмяВСпискеБазПользователя = "", ОткрытьПортативныеИнструменты = Истина, Знач СообщитьСтрокуПараметров = Истина,
		Знач Пользователь = "", Знач РежимЗапуска = "", Знач Пароль = "") Экспорт 
	ПараметрыЗапуска = ирОбщий.ПараметрыЗапускаПриложения1СЛкс(Пользователь, Пароль,, РежимКонфигуратора, РежимЗапуска, Ложь,,, СообщитьСтрокуПараметров, СтрокаСоединения,
...
Функция ОткрытьТаблицуЗначенийЛкс(Знач ТаблицаЗначений, Знач ТекущаяСтрока = Неопределено, Модально = Истина, Заголовок = "", РежимВыбора = Ложь, ТолькоПросмотр = Истина,
	ОтключитьОформление = Ложь, КлючУникальности = Неопределено, Знач ТекущаяКолонка = Неопределено, Знач ИзменятьПорядокСтрок = Истина, АтрибутыКолонок = Неопределено, Знач АктивироватьПоиск = Ложь) Экспорт 
	...
	ФормаРедактирования.ПараметрАктивироватьПоиск = АктивироватьПоиск;
	Если Модально Тогда
		Результат = ФормаРедактирования.ОткрытьМодально(); // см. ТаблицаЗначений, см. ТаблицаЗначений[0]
...
Функция ЯчейкаТабличногоПоляРасширенногоЗначения_ВыборЛкс(ЭтаФорма, ТабличноеПоле, СтандартнаяОбработка = Ложь, РасширенноеЗначение = Null,
	...
	Если ТабличноеПоле.ТекущаяСтрока <> Неопределено Тогда 
		...
		Если ТипЗнч(ТабличноеПоле) = Тип("ТабличноеПоле") Тогда
			...
			ОтображаемыйТекст = Ячейка.Текст;
	...
	Если Результат = Неопределено Тогда
		...
		КонецЕсли;
		Результат = ОткрытьЗначениеЛкс(РасширенноеЗначение, РедактированиеРазрешено, СтандартнаяОбработка, Заголовок,, ПринудительноВОтдельнойФорме,, ТабличноеПоле, СтрокаПоиска, КлючУникальности,, ОтображаемыйТекст);
	КонецЕсли; 
	Если Результат Тогда
...
Процедура ФильтрПодсистемОбработкаВыбораЛкс(ЭтаФорма, Элемент, ВыбранноеЗначение, СтандартнаяОбработка) Экспорт
...
Процедура ТабличноеПолеПриВыводеСтрокиЛкс(Знач ЭтаФорма, Знач Элемент, Знач ОформлениеСтроки, Знач ДанныеСтроки, Знач КнопкаРежимаОтображения = Неопределено, Знач КолонкиСПиктограммамиТипов = "",
	...
	Если ДопСвойства.ЗапретРазметкиВхождений <> Истина Тогда
		...
		Если МенеджерПоиска <> Неопределено Тогда
			...
			КонецЕсли; 
		КонецЕсли;
		СтрокаПоиска = ТабличноеПолеСтрокаПоискаЛкс(ЭтаФорма, Элемент, ПрефиксСтрокиПоиска,, ДопСвойства);
		Если МенеджерПоиска <> Неопределено Тогда
		...
		Если ЗначениеЗаполнено(СтрокаПоиска) Тогда
			...
			КонецЕсли; 
			РегВыражениеПоиска = ирКэш.РегВыражениеЛкс("ПоискСлов");
			#Если Сервер И Не Сервер Тогда
		...
		ИначеЕсли ТипЗнч(Элемент.Значение) <> Тип("НастройкиКомпоновкиДанных") Тогда 
			...
			Если ДопСвойства.Отбор <> Неопределено Тогда
				...
				Если КолонкиПоиска.Количество() > 0 Тогда
					...
					РегВыражениеПоиска = ирКэш.РегВыражениеЛкс();
			...
			КонецЕсли;
		КонецЕсли;
		РегВыражениеНачальныхПустыхСтрок = ирКэш.РегВыражениеЛкс("НачальныеПустыеСтроки");
		#Если Сервер И Не Сервер Тогда
	...
	Если ОтображатьИдентификаторы Тогда
		РегВыражениеНепечатное = ирОбщий.НовоеРегВыражениеЛкс();
		ирОбщий.ЛиСодержитОпасныеНепечатныеСимволыЛкс("", Истина, РегВыражениеНепечатное);
	КонецЕсли;
		...
		Для Каждого Колонка Из КолонкиТаблицы Цикл
			...
			Если Истина
				...
				И (Ложь
					...
					Или ТипЗначенияЯчейки = Тип("ДвоичныеДанные")
					Или РасширенноеПредставлениеДат И ТипЗначенияЯчейки = Тип("Дата")
					Или ТипЗначенияЯчейкиXML = Неопределено)
			Тогда
				ПредставлениеЗначения = ирОбщий.РасширенноеПредставлениеЛкс(ЗначениеЯчейки, Колонка,, РасширенноеПредставлениеХранилищЗначений,, ОтбражатьПустые,,, РасширенноеПредставлениеХранилищЗначений);
			КонецЕсли; 
			...
			Если ОтбражатьПустые И ПрименятьРежимОтображения Тогда
				Если ТипЗначенияЯчейки = Тип("Строка") Тогда
					...
					ИначеЕсли ОтображатьИдентификаторы Тогда
						...
						Если РегВыражениеНепечатное.Проверить(ЗначениеЯчейки) Тогда 
							ПредставлениеЗначения = ПредставлениеЗначения + ИндикаторНепечатныеСимволыЛкс();
						КонецЕсли;
					КонецЕсли; 
...
Функция ИндикаторНепечатныеСимволыЛкс() Экспорт
	Возврат "<Непечатные символы>";
КонецФункции
		...
		Для Каждого ЭлементОтбора Из Отбор Цикл
			...
			Тогда
				...
				Если ирОбщий.СтрКончаетсяНаЛкс(ЭлементОтбора.Имя, "Представление") Тогда
					...
					ЭлементОтбораРодитель = Отбор.Найти(ирОбщий.ТекстБезПоследнегоФрагментаЛкс(ЭлементОтбора.Имя, "Представление"));
					Если Истина
...
//   ЭтаФорма -  - 
//   ЭлементФормы -  - 
//   КлючевыеКолонки - Строка - применяется только при первом обращении (создании структуры свойств)
//   СравниваемыеКолонки - Строка - применяется только при первом обращении (создании структуры свойств)
// Возвращаемое значение:
//    - 
Функция ДопСвойстваЭлементаФормыЛкс(Знач ЭтаФорма, Знач ЭлементФормы, Знач КлючевыеКолонки = "", Знач СравниваемыеКолонки = "") Экспорт 
	СлужебныеДанныеФормы = ирОбщий.СлужебныеДанныеФормыЛкс(ЭтаФорма);
	Если Ложь
	...
	Если Не СвойстваЭлементов.Свойство(ЭлементФормы.Имя, СвойстваЭлемента) Тогда
		//! ЭлементФормы.Значение.Найти(,КлючевыеКолонки);
		//! ЭлементФормы.Значение.Найти(,СравниваемыеКолонки);
		СвойстваЭлемента = Новый Структура;
		СвойстваЭлемента.Вставить("ВременноОформленныеСтроки");
		СвойстваЭлемента.Вставить("ЗапретОтображения");
		СвойстваЭлемента.Вставить("ЗапретПодвала");
		СвойстваЭлемента.Вставить("ЗапретРазметкиВхождений");
		СвойстваЭлемента.Вставить("ЗапросИтоговПоТаблицеКлючей");
		СвойстваЭлемента.Вставить("ЗначениеТекущейЯчейки");
		СвойстваЭлемента.Вставить("КнопкаОтображенияПодвала");
		СвойстваЭлемента.Вставить("КнопкаОформленияТекущихСтрок");
		СвойстваЭлемента.Вставить("КнопкаРежимаОтображения");
		СвойстваЭлемента.Вставить("МенеджерПоиска");
		СвойстваЭлемента.Вставить("НеинтерактивноеИзменение");
		СвойстваЭлемента.Вставить("Отбор", ОтборТабличногоПоляЛкс(ЭлементФормы));
		СвойстваЭлемента.Вставить("КлючевыеКолонки", КлючевыеКолонки);
		СвойстваЭлемента.Вставить("СравниваемыеКолонки", СравниваемыеКолонки);
...
// Параметры:
//   ЭтаФорма -  - 
//   ПредставлениеОбъекта -  - если пустое, то оно берется как остаток от заголовка после ":"
//   МоноРежим -  - 
//   АвтоСжатие -  - 
Процедура ФормаОбъекта_ОбновитьЗаголовокЛкс(Знач ЭтаФорма, ПредставлениеОбъекта = "", Знач МоноРежим = Ложь, Знач АвтоСжатие = Истина) Экспорт 
	...
	Если ТипЗнч(ЭтаФорма) = Тип("Форма") Тогда
		...
		КонецЕсли;
	КонецЕсли;
	//Если ЭтаФорма.ЗакрыватьПриЗакрытииВладельца И ТипЗнч(ЭтаФорма.ВладелецФормы) = Тип("Форма") Тогда
	//	НовыйЗаголовок = НовыйЗаголовок + " (подч.)";
	//КонецЕсли;
...
//
Процедура ТабличноеПолеПриАктивизацииСтрокиЛкс(Знач ЭтаФорма, Знач Элемент, Знач ОбновлятьПодвал = Истина) Экспорт 
	...
	ВсеСтрокиОбновлены = ТабличноеПолеПриАктивацииКолонкиЛкс(ЭтаФорма, Элемент);
	Если Не ВсеСтрокиОбновлены Тогда
		ДопСвойства = ДопСвойстваЭлементаФормыЛкс(ЭтаФорма, Элемент);
		#Если Сервер И Не Сервер Тогда
			Элемент = Новый ТабличноеПоле;
		#КонецЕсли
		НовыеВыделенныеСтроки = ВыделенныеСтрокиТабличногоПоляЛкс(Элемент, Ложь);
		// ТекущаяСтрока должна всегда быть последней 
		Если Элемент.ТекущаяСтрока <> Неопределено Тогда
			ИндексТекущейСтроки = НовыеВыделенныеСтроки.Найти(Элемент.ТекущаяСтрока);
			Если ИндексТекущейСтроки <> Неопределено Тогда
				НовыеВыделенныеСтроки.Удалить(ИндексТекущейСтроки);
			КонецЕсли;
			НовыеВыделенныеСтроки.Добавить(Элемент.ТекущаяСтрока);
		КонецЕсли;
		...
		Иначе
			...
			КонецЕсли;
		КонецЕсли;
		Элемент.ОбновитьСтроки(СтрокиОбновленияОформления1);
	...
	Если ОбновлятьПодвал Тогда
		...
		ПараметрыОбработчика.Вставить("ТабличноеПоле", Элемент);
		Если Элемент.ВыделенныеСтроки.Количество() < 50 Тогда 
			Задержка = 0;
		Иначе
			Задержка = 0.1;
		КонецЕсли;
	...
	КонецЕсли; 
	ТабличноеПолеОповеститьОбАктивацииСтрокиЛкс(ЭтаФорма, Элемент);
...
// Лечит в частности это https://www.hostedredmine.com/issues/1007843
...
// .
// Параметры:
//   ТабличноеПолеОтбора - РасширениеТабличногоПоляОтбораКомпоновкиДанных - 
//   Тип - Число - 0 - Элемент
//              1 - Выражение
//              2 - Группа-И
//              3 - Группа-Или
//              4 - Группа-Не
Процедура ДобавитьСтрокуВТабличноеПолеОтбораЛкс(Знач ТабличноеПолеОтбора, Знач Тип = 0) Экспорт

	ТекущаяСтрокаОтбора = ТабличноеПолеОтбора.ТекущаяСтрока; // ГруппаЭлементовОтбораКомпоновкиДанных, ЭлементОтбораКомпоновкиДанных
	Если Тип > 1 Тогда
	...
	Если ТекущаяСтрокаОтбора = Неопределено Тогда
		ТекущаяСтрокаОтбора = ТабличноеПолеОтбора.Значение;
	КонецЕсли; 
	Если ТипЗнч(ТекущаяСтрокаОтбора) = Тип("ОтборКомпоновкиДанных") Тогда
		Родитель = ТабличноеПолеОтбора.Значение;
	Иначе
		Родитель = ирОбщий.РодительСтрокиДереваЛкс(ТекущаяСтрокаОтбора, ТабличноеПолеОтбора.Значение); // ОтборКомпоновкиДанных
	КонецЕсли;
	Если Ложь
	...
	Если ТипСтроки = Тип("ГруппаЭлементовОтбораКомпоновкиДанных") Тогда 
		...
		КонецЕсли; 
	КонецЕсли;
	ВыделенныеСтроки = ирКлиент.ВыделенныеСтрокиТабличногоПоляЛкс(ТабличноеПолеОтбора);
	Если ВыделенныеСтроки.Количество() > 1 Тогда
		Для Каждого ВыделенныеЭлемент Из ВыделенныеСтроки Цикл // ЭлементОтбораКомпоновкиДанных, ГруппаЭлементовОтбораКомпоновкиДанных
			Приемник = НоваяСтрока.Элементы.Добавить(ТипЗнч(ВыделенныеЭлемент));
			ЗаполнитьЗначенияСвойств(Приемник, ВыделенныеЭлемент);
			Если ТипЗнч(ВыделенныеЭлемент) = Тип("ГруппаЭлементовОтбораКомпоновкиДанных") Тогда
				ирОбщий.СкопироватьЭлементыКомпоновкиЛкс(Приемник, ВыделенныеЭлемент);
			КонецЕсли;
			Родитель.Элементы.Удалить(ВыделенныеЭлемент);
		КонецЦикла;
		ТабличноеПолеОтбора.Развернуть(НоваяСтрока);
	КонецЕсли;
	Если Тип = 1 Тогда
...
Функция НайтиВозможныеСтрокиОписанияСловаВСинтаксПомощникеЛкс(Знач Слово, ЯзыкПрограммы = 0, ПоискСУчетомТипаСлова = Истина, Знач ТипКонтекста = "") Экспорт
	...
	Тогда
		...
		МассивВозможныхТиповСлова.Добавить("Параметр");
	...
	ТипыКонтекста = Новый Массив;
	ТипыКонтекста.Добавить(ТипКонтекста);
	СтруктураТипа = мПлатформа.ОписаниеРасширенногоТипа(ТипКонтекста);
	Если СтруктураТипа <> Неопределено Тогда
		ТипыКонтекста.Добавить(СтруктураТипа.ОсновнойТип);
	КонецЕсли;
	Для Каждого ТипКонтекста Из ТипыКонтекста Цикл
...
Функция ТаблицаИлиДеревоЗначенийИзТаблицыФормыСКоллекциейЛкс(Знач ТабличноеПоле, Знач МассивСтрок = Неопределено, Знач СУчетомОтбора = Ложь, Знач ПреобразоватьДеревоВТаблицу = Истина,
	...
	Знач ВернутьКопию = Ложь, ДанныеПоля = Неопределено, СерверныеМетаданныеФормы = Истина) Экспорт 
	...
	КонецЕсли;
	ПолноеИмяТаблицыБД = "";
	ТипИсточника = ирОбщий.ОбщийТипДанныхТабличногоПоляЛкс(ТабличноеПоле,,, ПолноеИмяТаблицыБД, ДанныеПоля, СерверныеМетаданныеФормы);
	Если ТипИсточника = "Список" И ЗначениеЗаполнено(ПолноеИмяТаблицыБД) Тогда
		Возврат Неопределено;
	КонецЕсли; 
			...
			Иначе
				...
				Иначе
					...
					Иначе
						...
						ВызватьИсключение ирОбщий.СтрШаблонЛкс("Неподдерживаемый тип данных табличного поля (%1)", ТипЗнч(ДанныеПоля));
					КонецЕсли; 
				КонецЕсли; 
			КонецЕсли; 
		КонецЕсли; 
	...
	Иначе // ТипЗнч(ИсточникДействий) = Тип("ТаблицаФормы") 
		...
		Если ДанныеПоля = Неопределено Тогда
			ДанныеПоля = ирОбщий.ДанныеЭлементаФормыЛкс(ТаблицаФормы,,,, СерверныеМетаданныеФормы);
		КонецЕсли;
		ОтборСтрок = Неопределено;
		Если ТипЗнч(ДанныеПоля) = Тип("ДинамическийСписок") Тогда
			ДанныеПоля = Неопределено; 
		Иначе
			Если ТипЗнч(ДанныеПоля) = Тип("ДанныеФормыКоллекция") Тогда
				Попытка
			...
			КонецЕсли;
		КонецЕсли;
		Если МассивСтрок = Неопределено И ТипЗнч(ДанныеПоля) = Тип("ДанныеФормыДерево") Тогда
			Результат = ДанныеФормыВЗначение(ДанныеПоля, Тип("ДеревоЗначений")); // Для табличных частей не работает
		Иначе
			Если ДанныеПоля = Неопределено Тогда 
				Если МассивСтрок = Неопределено Тогда
					МассивСтрок = ВыделенныеСтрокиТабличногоПоляЛкс(ТаблицаФормы);
					ирОбщий.СообщитьЛкс("Эта таблица позволяет выводить только выделенные строки, т.к. ее имя не совпадает с имененем ее данных",,, Истина, Ложь);
				КонецЕсли; 
			Иначе
				...
				Если МассивСтрок = Неопределено Тогда
					Попытка
						МассивСтрок = ДанныеПоля.Элементы; // Настройка компоновки
					Исключение
						МассивСтрок = ДанныеПоля;
					КонецПопытки;
				КонецЕсли; 
			КонецЕсли; 
			#Если Сервер И Не Сервер Тогда
			...
			ИначеЕсли ДанныеПоля <> Неопределено Тогда 
				ДобавитьКолонкиГруппыФормыВТаблицуЗначенийЛкс(ТаблицаФормы.ПодчиненныеЭлементы, Результат, ДанныеПоля, ИменаКолонокРезультата);
			КонецЕсли; 
...
Функция ВыбранныйЭлементПоследнихЗначенийЛкс(Знач ЭтаФорма, Знач ТабличноеПолеИлиКлюч, Знач Кнопка, Знач ТипКлюча = "Выбранные", ДопКлючХранения = "") Экспорт 
...
Процедура ПоследниеВыбранныеЗаполнитьПодменюЛкс(Знач ЭтаФорма, Знач КнопкаПодменю, Знач ТабличноеПолеИлиКлюч = Неопределено, Знач ОбработчикКнопки = Неопределено,
	...
	Иначе
		Если ПустаяСтрока(ОбработчикКнопки) И КнопкаПодменю.Кнопки.Количество() Тогда
...
Процедура НайденныеСтандартноСсылкиПриВыводеСтрокиЛкс(Знач ОформлениеСтроки, Знач ПолучатьПредставленияСсылок = Ложь, Знач ПолноеИмяМД = "") Экспорт 
	...
	ДанныеСтроки = ОформлениеСтроки.ДанныеСтроки; // ОбработкаТабличнаяЧастьСтрока.ирУдалениеОбъектовСКонтролемСсылок.СсылкиНаКандидата, ОбработкаТабличнаяЧастьСтрока.ирРедакторОбъектаБД.ВерсииОбъекта, ОбработкаТабличнаяЧастьСтрока.ирИсторияДанных.Версии
	Ячейки = ОформлениеСтроки.Ячейки;
	...
	КлючОбъекта = КлючОбъектаНайденнойСсылкиЛкс(ДанныеСтроки, ПолноеИмяМД);
...
Функция ПолучитьФормуТекстаЛкс(Знач Текст, Знач Заголовок = "", ВариантПросмотра = "Компактный", ТолькоПросмотр = Ложь, Знач КлючУникальности = Неопределено, ВладелецФормы = Неопределено,
...
// .
// Параметры:
//  ВариантПросмотра - Строка - если пусто, то автоматически, иначе "Обычный", "Компактный", "ЯзыкЗапросов", "ВстроенныйЯзык", "XML", "JSON", "Дерево", "РезультатыПоиска", "Markdown"...
Функция ОткрытьТекстЛкс(Текст, Знач Заголовок = "", ВариантПросмотра = "Компактный", ТолькоПросмотр = Ложь, Знач КлючУникальности = Неопределено, ВладелецФормы = Неопределено, ВыделитьВсе = Ложь,
	Знач Модально = Ложь, ВыделениеДвумерное = Неопределено, Знач ИскомаяСтрока = "") Экспорт
	...
	Если ТипЗнч(Текст) = Тип("Строка") Тогда
		...
		ФормаТекста.ПараметрСтрокаПоиска = ИскомаяСтрока;
		Если Модально Тогда
...
Процедура ПолеВводаРегулярногоВыраженияНачалоВыбораЛкс(Знач Элемент, Знач ПроверочныйТекст) Экспорт
	...
	Конструктор.ПараметрПроверочныйТекст = ПроверочныйТекст;
	Конструктор.ПараметрВыражение = Элемент.Значение;
...
Функция ТабличноеПолеОтборДляЗначенияВТекущейКолонкеЛкс(Знач ТабличноеПоле, Знач ОтборПоЗначению = Ложь, Знач Отбор = Неопределено, Знач СообщитьОДобавлении = Ложь,
	...
	Если ТипЗнч(ЭлементОтбора) = Тип("ЭлементОтбораКомпоновкиДанных") И НазначитьПользовательскийИдентификатор Тогда
		ирОбщий.ПроверитьВключитьЭлементВПользовательскиеНастройкиКомпоновкиЛкс(ЭлементОтбора);
	КонецЕсли;
...
Процедура ТабличноеПолеСОтборомПросмотраОтобратьЛкс(Знач ЭтаФорма, Знач ТабличноеПоле, Знач ИмяКолонкиНомерСтроки = "", Знач РежимОтладки = Ложь, Знач НовоеИспользованиеОтбора = Неопределено,
	...
	Если ЗначениеЗаполнено(СтрокаПоиска) Тогда
		Если ТаблицаБезДанныхСУзкимиТипами = Неопределено И ТаблицаЗначений.Количество() < 1000 Тогда
		...
		Для Каждого ДоступноеПоле Из Компоновщик.Настройки.ДоступныеПоляОтбора.Элементы Цикл
			...
			КонецЕсли;
		КонецЦикла;
		Если ГруппаИли.Элементы.Количество() <= 1 Тогда
			ирОбщий.СообщитьЛкс(НСтр("ru='Не найдено типизированных колонок для отбора'"));
		КонецЕсли;
	...
	ИначеЕсли Не ирКэш.ДоступноКомпоновкаТаблицыЗначенийКлс() Тогда
		...
		ирОбщий.СообщитьЛкс("Корректная работа отбора просмотра недоступна в режиме совместимости 8.3.5- https://github.com/tormozit/RDT1C/issues/434");
		ЭтаФорма[ИмяТП + "ИспользоватьОтбор"] = Ложь;
	КонецЕсли; 
...
//   НоваяСтрока -  - 
//   Копирование -  - 
Процедура ТабличноеПолеСОтборомПросмотраПриНачалеРедактированияЛкс(Знач ЭтаФорма, Знач ТабличноеПоле, Знач НоваяСтрока, Знач Копирование) Экспорт 
	Если НоваяСтрока И Копирование Тогда
		ИмяКолонкиНомерСтроки = ирОбщий.ИмяКолонкиВнутреннегоИДСтрокиЛкс();
		Попытка
			ТабличноеПоле.ТекущиеДанные[ИмяКолонкиНомерСтроки] = 0;
		Исключение
			// Эта колонка может отсутствовать до первого включения отбора
		КонецПопытки;
	КонецЕсли;
КонецПроцедуры
...
Процедура ТабличноеПолеСОтборомПросмотраПриОкончанииРедактированияЛкс(Знач ЭтаФорма, Знач ТабличноеПоле) Экспорт 
	ИмяТП = ПрефиксРеквизитовКомпоновкиТабличногоПоляЛкс(ЭтаФорма, ТабличноеПоле);
...
Функция ТабличноеПолеСтрокаПоискаЛкс(Знач ЭтаФорма, Знач ТабличноеПоле, ПрефиксСтрокиПоиска = "", выхПутьКДанным = "", Знач ДопСвойстваЭлемента = Неопределено) Экспорт 
	...
	Если Не ЗначениеЗаполнено(ПрефиксСтрокиПоиска) Тогда
		...
		Если ДопСвойстваЭлемента = Неопределено Тогда
			ДопСвойстваЭлемента = ДопСвойстваЭлементаФормыЛкс(ЭтаФорма, ТабличноеПоле);
		КонецЕсли;
		Если ирОбщий.СвойствоСтруктурыЛкс(ДопСвойстваЭлемента, "ЛиСтрокаПоискаОтсутствует", Ложь) Тогда
			Возврат "";
		КонецЕсли;
	КонецЕсли;
	Попытка
	...
	Если Истина
		...
		И ДопСвойстваЭлемента <> Неопределено 
	Тогда
		ДопСвойстваЭлемента.Вставить("ЛиСтрокаПоискаОтсутствует", Истина);
	КонецЕсли;
...
Процедура ТабличноеПолеСОтборомПросмотраУстановитьДанныеЛкс(Знач ЭтаФорма, Знач ТабличноеПоле, Знач ОтключитьОтбор = Истина, Знач ОбновитьКомпоновщик = Ложь) Экспорт 
	...
	Если ОбновитьКомпоновщик Тогда
		ТабличноеПолеСОтборомПросмотраОбновитьКомпоновщикЛкс(ЭтаФорма, ТабличноеПоле, Ложь);
	КонецЕсли;
...
Процедура ТабличноеПолеСОтборомПросмотраОбновитьКомпоновщикЛкс(Знач ЭтаФорма, Знач ТабличноеПоле, Знач УстановитьПодсказкуФлажка = Истина, Знач ТаблицаБезДанныхСУзкимиТипами = Неопределено) Экспорт 
	...
	КонецЕсли;
	СхемаКомпоновки = ирОбщий.СоздатьСхемуКомпоновкиПоТаблицеЗначенийЛкс(ТаблицаБезДанныхСУзкимиТипами);
	Компоновщик = КомпоновкаТП.Компоновщик;
	#Если Сервер И Не Сервер Тогда
		Компоновщик = Новый КомпоновщикНастроекКомпоновкиДанных;
	#КонецЕсли
...
Функция ФормаВыбораОбъектаМетаданныхЛкс(Знач ВладелецФормы, Знач КлючУникальности, Знач НачальноеЗначениеВыбора, МножественныйВыбор = Ложь, ОтображатьСсылочныеОбъекты = Ложь,
	...
	ОтображатьСетевыеСервисы = Ложь, ОтображатьРоли = Ложь, ОтображатьОбщиеКоманды = Ложь, ФильтрКорневыхТипов = Неопределено) Экспорт 
	...
	лСтруктураПараметров = ПараметрыВыбораОбъектаМетаданныхЛкс(ОтображатьСсылочныеОбъекты, ОтображатьВыборочныеТаблицы, ОтображатьРегистры, ОтображатьПоследовательности,
		...
		ОтображатьПеречисления, ОтображатьОбработки, ОтображатьОтчеты, ОтображатьПерерасчеты, МножественныйВыбор, ОтображатьРегламентныеЗадания, ОтображатьСетевыеСервисы, ОтображатьРоли,,, ОтображатьОбщиеКоманды,
		ФильтрКорневыхТипов);
...
//
Функция ВыбратьОбъектМетаданныхЛкс(Знач ДоступныеОбъекты = Неопределено, Знач НачальноеЗначениеВыбора = "") Экспорт 
	...
	ФормаВыбора = ирКлиент.ПолучитьФормуВыбораОбъектаМетаданныхСтруктуройЛкс(,, НачальноеЗначениеВыбора, ПараметрыВыбора);
	Результат = ФормаВыбора.ОткрытьМодально();
	Если Результат <> Неопределено Тогда
...
Функция ПараметрыВыбораОбъектаМетаданныхЛкс(ОтображатьСсылочныеОбъекты = Ложь, ОтображатьВыборочныеТаблицы = Ложь, ОтображатьРегистры = Ложь, ОтображатьПоследовательности = Ложь,
	...
	ОтображатьРегламентныеЗадания = Ложь, ОтображатьСетевыеСервисы = Ложь, ОтображатьРоли = Ложь, ОтображатьПодсистемы = Ложь, ОтображатьВиртуальныеТаблицы = Ложь, ОтображатьОбщиеКоманды = Ложь,
	ФильтрКорневыхТипов = Неопределено) Экспорт 
	...
	Параметры.Вставить("ФильтрКорневыхТипов", ФильтрКорневыхТипов);
	Возврат Параметры;
...
//.
// Параметры:
//   СтрокаСобытия - СтрокаТаблицыЗначений - 
//   НаСервере -  - 
//   СтандартнаяОбработка -  - 
// Возвращаемое значение:
//    - 
...
// Для ВариантПросмотра = "ВстроенныйЯзык" (по умолчанию) не помещает текст в буфер обмена, а только открывает окно с текстом
// Параметры:
//   Текст - Строка - 
//   ВариантПросмотра - Строка - см. ОткрытьТекстЛкс()
// Возвращаемое Текст:
//    - 
Функция ТекстВБуферОбменаОСЛкс(Знач Текст, Знач ВариантПросмотра = "ВстроенныйЯзык") Экспорт
	Текст = "" + Текст;
	Если Ложь
	...
	Иначе
		ВКОбщая = ирКэш.ВКОбщаяЛкс();
		Попытка
			ВКОбщая.УстановитьБуферОбмена(Текст);
		Исключение
			ОписаниеОшибки = ОписаниеОшибки(); // Для отладки
		КонецПопытки;
		Если ТекстИзБуфераОбменаОСЛкс() <> Текст Тогда
			ОткрытьТекстЛкс(Текст, "Скопируй текст в буфер обмена", ВариантПросмотра,,,, Истина); 
			Возврат Ложь;
		КонецЕсли; 
	КонецЕсли;
	Возврат Истина;
...
Функция ТекстИзБуфераОбменаОСЛкс(выхЗначение = Неопределено) Экспорт
	...
	ВКОбщая = ирКэш.ВКОбщаяЛкс();
	Попытка
		Результат = ВКОбщая.ПолучитьБуферОбмена("text");
	Исключение
		ОписаниеОшибки = ОписаниеОшибки(); // Для отладки
		Результат = "";
	КонецПопытки;
...
Функция ПолучитьФормуСпискаЛкс(Знач ИмяТаблицыИлиМДИлиТип, Знач ФиксОтбор = Неопределено, Знач ИспользоватьДинамическийСписокИР = Неопределено, Знач ВладелецФормы = Неопределено, Знач РежимВыбора = Ложь,
	...
	Если ПользовательскийОтбор <> Неопределено Тогда
		...
		ирОбщий.ПроверитьВключитьЭлементВПользовательскиеНастройкиКомпоновкиЛкс(ПользовательскийОтборКомпоновки, "dfcece9d-5077-440b-b6b3-45a5cb4538eb");
	КонецЕсли; 
	...
	Тогда
		...
		Попытка
			...
			Если ТипЗнч(ФормаСписка) = Тип("Форма") Тогда 
				...
				ПодготовитьЭлементыФормыЛкс(ФормаСписка);
...
Функция СоздатьМенеджерСохраненияНастроекФормыЛкс(Знач ЭтаФорма, Знач КлючНазначенияИспользования = "", Знач ЗагружатьСохранятьНастройкуПоУмолчанию = Истина, Знач ПараметрыЗагрузкиНастройкиПоУмолчанию = Неопределено,
	Знач РасширениеФайла = "", Знач СоставНастройкиФормы = Неопределено, Знач СохранятьВидимостьКолонок = Ложь) Экспорт 
	...
	МенеджерСохраненияНастроек.СохранятьВидимостьКолонок = СохранятьВидимостьКолонок;
	...
	Попытка
		...
		//Тогда
		//	// Предотвращаем чтение настроек из БД
		//	// Антибаг платформы https://www.hostedredmine.com/issues/1006941
		//Иначе
...
Функция НовыйМенеджерСохраненияНастроекФормыЛкс() Экспорт
	Возврат Новый Структура("СохраненныеНастройки, ПоследняяНастройка, КлючНастроек, ЗагружатьСохранятьНастройкуПоУмолчанию, РасширениеФайла, СоставНастройкиФормы, СохранятьВидимостьКолонок");
КонецФункции
...
Процедура ЗагрузитьНастройкуФормыПоУмолчаниюЛкс(Знач ЭтаФорма, Знач ЗагрузитьПустуюНастройку = Ложь, Знач ПараметрыЗагрузкиНастройкиПоУмолчанию = Неопределено) Экспорт 
	...
	Иначе
		...
		Если НастройкаПоУмолчанию.Значение = Неопределено Тогда
			СохранитьНастройкуФормыЛкс(ЭтаФорма, Ложь);
			НастройкаПоУмолчанию = ПолучитьНастройкуПоУмолчаниюИзСпискаСохраненныхНастроекЛкс(МенеджерСохраненияНастроек.СохраненныеНастройки);
		Иначе
			//ХранимыеВБазеСвойстваФормыЛкс(ЭтаФорма);
		КонецЕсли; 
...
Процедура ЗагрузитьНастройкуФормыЧерезОбработчикЛкс(Знач ЭтаФорма, Знач НастройкаФормы, Знач ПараметрыЗагрузкиНастройкиПоУмолчанию = Неопределено) Экспорт 
	...
	Если ирОбщий.МетодРеализованЛкс(ЭтаФорма, "ЗагрузитьНастройкуВФорме") Тогда
		//! ЭтаФорма = 0; // см. Обработка.ирПлатформа.Форма.СохранениеНастроекКнт
...
Процедура ЗагрузитьНастройкуФормыЛкс(Знач ЭтаФорма, Знач НастройкаФормы, Знач БелыеСвойства = Неопределено, Знач ЧерныеСвойства = Неопределено, Знач ЗагружатьОстальные = Истина) Экспорт 
	...
	мПлатформа = ирКэш.Получить();
	Если Истина
		И мПлатформа.ЛиГлобальныеНастройкиВсехБаз 
		И ирОбщий.МетодРеализованЛкс(ЭтаФорма, "СвойстваГлобализуемые") 
	Тогда
		СвойстваГлобализуемые = ирОбщий.ВосстановитьЗначениеЛкс(ирОбщий.СлужебныеДанныеФормыЛкс(ЭтаФорма).МенеджерСохраненияНастроек.КлючНастроек + ".Глобально");
		Если СвойстваГлобализуемые <> Неопределено Тогда
			ЗагрузитьСтруктуруНастройкиФормыЛкс(ЭтаФорма, СвойстваГлобализуемые, БелыеСвойства, ЧерныеСвойства, ЗагружатьОстальные);
		КонецЕсли;
	КонецЕсли;
	Если НастройкаФормы = Неопределено Тогда
		Возврат;
	КонецЕсли; 
	ЗагрузитьСтруктуруНастройкиФормыЛкс(ЭтаФорма, НастройкаФормы, БелыеСвойства, ЧерныеСвойства, ЗагружатьОстальные);

КонецПроцедуры

Процедура ЗагрузитьСтруктуруНастройкиФормыЛкс(Знач ЭтаФорма, Знач НастройкаФормы, Знач БелыеСвойства = Неопределено, Знач ЧерныеСвойства = Неопределено, Знач ЗагружатьОстальные = Истина) Экспорт
	ПрефиксХраненияВидимостиКолонок = ПрефиксХраненияВидимостиКолонок();
	Для Каждого КлючИЗначение Из НастройкаФормы Цикл
		Если ирОбщий.СтрНачинаетсяСЛкс(КлючИЗначение.Ключ, ПрефиксХраненияВидимостиКолонок) Тогда
			ТабличноеПоле = ЭтаФорма.ЭлементыФормы.Найти(ирОбщий.ПоследнийФрагментЛкс(КлючИЗначение.Ключ, ПрефиксХраненияВидимостиКолонок));
			Если ТабличноеПоле <> Неопределено Тогда
				ВидимостьКолонок = КлючИЗначение.Значение; // Структура
				Для Каждого ВидимостьКолонки Из ВидимостьКолонок Цикл
					Колонка = ТабличноеПоле.Колонки.Найти(ВидимостьКолонки.Ключ);
					Если Колонка <> Неопределено Тогда
						Колонка.Видимость = ВидимостьКолонки.Значение;
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
		КонецЕсли;
...
Процедура СохранитьНастройкуФормыЛкс(Знач ЭтаФорма, Знач СохранитьГлобальные = Истина) Экспорт 
	
	МенеджерСохраненияНастроек = ирОбщий.СлужебныеДанныеФормыЛкс(ЭтаФорма).МенеджерСохраненияНастроек;
	Если МенеджерСохраненияНастроек <> Неопределено И МенеджерСохраненияНастроек.ЗагружатьСохранятьНастройкуПоУмолчанию Тогда
		мПлатформа = ирКэш.Получить();
		Если Истина
			И СохранитьГлобальные
			И мПлатформа.ЛиГлобальныеНастройкиВсехБаз 
			И ирОбщий.МетодРеализованЛкс(ЭтаФорма, "СвойстваГлобализуемые") 
		Тогда
			//! ЭтаФорма = 0; // см. Обработка.ирПлатформа.Форма.СохранениеНастроекКнт
			СохраняемаяНастройкаФормы = Новый Структура;
			ДополнитьСохраняемуюНастройкуФормыЛкс(ЭтаФорма, СохраняемаяНастройкаФормы, ЭтаФорма.СвойстваГлобализуемые());
			ирОбщий.СохранитьЗначениеЛкс(МенеджерСохраненияНастроек.КлючНастроек + ".Глобально", СохраняемаяНастройкаФормы);
		КонецЕсли;
		//Если Истина
		//	И ТранзакцияАктивна()
		//	И ирКэш.ЛиФайловаяБазаЛкс()
		...
		//Иначе
			НастройкаПоУмолчанию = ПолучитьНастройкуПоУмолчаниюИзСпискаСохраненныхНастроекЛкс(МенеджерСохраненияНастроек.СохраненныеНастройки);
			СохраняемаяНастройка = СохраняемаяНастройкаФормыЛкс(ЭтаФорма);
			Если МенеджерСохраненияНастроек.СохранятьВидимостьКолонок Тогда
				ПрефиксХраненияВидимостиКолонок = ПрефиксХраненияВидимостиКолонок();
				Для Каждого ЭлементФормы Из ЭтаФорма.ЭлементыФормы Цикл
					Если ТипЗнч(ЭлементФормы) = Тип("ТабличноеПоле") Тогда
						КолонкиТабличногоПоля = Новый Структура;
						Для Каждого Колонка Из ЭлементФормы.Колонки Цикл
							Если Колонка.ИзменятьВидимость Тогда
								КолонкиТабличногоПоля.Вставить(Колонка.Имя, Колонка.Видимость);
							КонецЕсли;
						КонецЦикла;
						СохраняемаяНастройка.Вставить(ПрефиксХраненияВидимостиКолонок + ЭлементФормы.Имя, КолонкиТабличногоПоля);
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
			НастройкаПоУмолчанию.Значение = СохраняемаяНастройка;
...
Функция ПрефиксХраненияВидимостиКолонок()
	Возврат "ВидимостьКолонок_";
КонецФункции
...
Функция СохраняемаяНастройкаФормыЛкс(Знач ЭтаФорма, выхНовоеНаименование = "", ПорогТаблицыЗначений = 100000) Экспорт 
    ...
    СохраняемаяНастройкаФормы = Неопределено;
    ИменаСвойств = ХранимыеВБазеСвойстваФормыЛкс(ЭтаФорма, СохраняемаяНастройкаФормы);
	Если ЗначениеЗаполнено(ИменаСвойств) Тогда
		Если СохраняемаяНастройкаФормы = Неопределено Тогда
			СохраняемаяНастройкаФормы = Новый Структура;
		КонецЕсли;
		ДополнитьСохраняемуюНастройкуФормыЛкс(ЭтаФорма, СохраняемаяНастройкаФормы, ИменаСвойств, ПорогТаблицыЗначений);
	КонецЕсли;
	мПлатформа = ирКэш.Получить();
	Если Истина
		И Не мПлатформа.ЛиГлобальныеНастройкиВсехБаз 
		И ирОбщий.МетодРеализованЛкс(ЭтаФорма, "СвойстваГлобализуемые") 
	Тогда
		СвойстваГлобализуемые = ЭтаФорма.СвойстваГлобализуемые();
		ДополнитьСохраняемуюНастройкуФормыЛкс(ЭтаФорма, СохраняемаяНастройкаФормы, ЭтаФорма.СвойстваГлобализуемые(), ПорогТаблицыЗначений);
		ирОбщий.СкопироватьКоллекциюЛкс(СвойстваГлобализуемые, СохраняемаяНастройкаФормы);
	КонецЕсли;
...
// Параметры:
//   ЭтаФорма -  - 
//   СохраняемаяНастройкаФормы -  - 
//   ИменаСвойствФормы - Структура - 
//   ПорогТаблицыЗначений -  - 
Процедура ДополнитьСохраняемуюНастройкуФормыЛкс(Знач ЭтаФорма, Знач СохраняемаяНастройкаФормы, Знач ИменаСвойствФормы, Знач ПорогТаблицыЗначений = 100000) Экспорт
	ЗначенияСвойств = Новый Структура();
	Для Каждого КлючИЗначение Из ИменаСвойствФормы Цикл
...
Функция ХранимыеВБазеСвойстваФормыЛкс(Знач ЭтаФорма, СохраняемаяНастройкаФормы = Неопределено) Экспорт
	...
	КонецЕсли;
	НастройкиФормы = новый Структура(ИменаСвойств);
	мПлатформа = ирКэш.Получить();
	Если Истина
		И Не мПлатформа.ЛиГлобальныеНастройкиВсехБаз 
		И ирОбщий.МетодРеализованЛкс(ЭтаФорма, "СвойстваГлобализуемые") 
	Тогда
		//! ЭтаФорма = 0; // см. Обработка.ирПлатформа.Форма.СохранениеНастроекКнт
		ирОбщий.СкопироватьКоллекциюЛкс(ЭтаФорма.СвойстваГлобализуемые(), НастройкиФормы);
	КонецЕсли;
...
//   ЭтаФорма -  - 
//   Кнопка -  - 
//   ОбработчикЗавершения - Строка -  имя экспортного метода формы с параметрами (СостояниеЗадания, РезультатЗадания)
// Возвращаемое значение:
//   Строка - порядок
Функция ПрочитатьДопПоляСсылающихсяОбъектовЛкс(Знач ТабличноеПоле, Знач КомпоновщикДопПолей, Знач ТаблицаДанных = Неопределено, Знач ТолькоТекущаяСтрока = Ложь, 
	Знач ЭтаФорма = Неопределено, Знач Кнопка = Неопределено, Знач ОбработчикЗавершения = Неопределено, выхЗапущеноЗадание = Ложь) Экспорт 
	
	СтрокаПорядка = ПрочитатьДопПоляОбъектовЛкс(ТабличноеПоле, КомпоновщикДопПолей, ТаблицаДанных, "Данные", Истина, ТолькоТекущаяСтрока, ЭтаФорма, Кнопка, ОбработчикЗавершения, выхЗапущеноЗадание);
...
Функция ПрочитатьДопПоляОбъектовЛкс(Знач ТабличноеПоле, Знач КомпоновщикДопПолей, Знач ТаблицаДанных = Неопределено, Знач ИмяПоляСсылки = "Данные", Знач ЧитатьСтандартныеРеквизиты = Ложь, 
	Знач ТолькоТекущаяСтрока = Ложь, Знач ЭтаФорма = Неопределено, Знач Кнопка = Неопределено, Знач ОбработчикЗавершения = Неопределено, выхЗапущеноЗадание = Ложь) Экспорт
	...
	Если ЧитатьСтандартныеРеквизиты Тогда
		...
		Для Каждого КлючИЗначение Из СтандартныеРеквизиты Цикл
			...
			Если ДоступныеПоляПорядка.НайтиПоле(Новый ПолеКомпоновкиДанных("Объект." + ИмяРеквизита)) = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			МассивДопПолей.Вставить(ИмяРеквизита, ИмяРеквизита);
			СтрокаВыбора = СтрокаВыбора + ",
	...
	Если ТипЗнч(ТаблицаДанных) <> Тип("ТаблицаЗначений") Или ТолькоТекущаяСтрока Тогда
		...
		КонецЕсли;
		ТаблицаДанных = ирКлиент.ТаблицаИлиДеревоЗначенийИзТаблицыФормыСКоллекциейЛкс(ТабличноеПоле, МассивСтрок);
		БлокируемыеЭлементыФормы.Добавить(ТабличноеПоле);
	Иначе
		Если ТаблицаДанных.Колонки.Найти("ДанныеСтрока") <> Неопределено Тогда
			Фильтр = Новый Структура("ДанныеСтрока", "");
		КонецЕсли;
	КонецЕсли;
	ДоступноеПолеОбъект = ДоступныеПоляПорядка.НайтиПоле(Новый ПолеКомпоновкиДанных("Объект"));
	Если ДоступноеПолеОбъект <> Неопределено Тогда
		ОписаниеТиповОбъекта = ДоступноеПолеОбъект.ТипЗначения;
	КонецЕсли;
	ТаблицаДанных = ТаблицаДанных.Скопировать(Фильтр, ИмяПоляСсылки);
	РазрешитьАсинхронно = ТаблицаДанных.Количество() > 1000;
	РезультатЗапуска = ирОбщий.ПрочитатьДопПоляОбъектовЛкс(ИмяПоляСсылки, ЧитатьСтандартныеРеквизиты, ТаблицаДанных, МассивДопПолей, ОписаниеТиповОбъекта, СтрокаВыбора,,, 
		ЭтаФорма, ?(РазрешитьАсинхронно, Кнопка, Неопределено), ОбработчикЗавершения, БлокируемыеЭлементыФормы);
	выхЗапущеноЗадание = РезультатЗапуска = Неопределено;
	Возврат СтрокаПорядка;
КонецФункции
...
Функция ОткрытьТекущуюСтрокуТабличногоПоляТаблицыБДВРедактореОбъектаБДЛкс(ТабличноеПоле, ПолноеИмяМД = Неопределено, ДоступныеПоляВыбора = Неопределено, Связанный = Ложь,
	...
	Если ЭтоТаблицаИзменений Тогда
		ПолноеИмяТаблицы = ирОбщий.ТекстБезПоследнегоФрагментаЛкс(ПолноеИмяТаблицы);
	КонецЕсли;
	...
	Иначе
		...
		Если ЗначениеЗаполнено(ПолноеИмяМД) Тогда
			Если ТипЗнч(ТабличноеПоле) = Тип("ТаблицаФормы") Тогда
				...
				КонецЕсли;
				Для Каждого ЭлементОтбора Из ирОбщий.ВсеЭлементыИерархииНастроекКомпоновкиЛкс(НастройкиСписка.Отбор,, Истина) Цикл // ЭлементОтбораКомпоновкиДанных
					Если Истина
		...
		ОткрытьРедакторОбъектаБДЛкс(ПолноеИмяТаблицы, ИмяКолонки, Связанный, КлючОбъекта, ОбъектыНаСервере, ТабличноеПоле.ТекущиеДанные, ТабличноеПоле.ТекущаяСтрока, ФормаРедактора,
			НайденныеСсылки, СтрокиПоиска,, АктивироватьУзел, ВладелецФормы,, Истина);
	КонецЕсли; 
...
Функция ОткрытьРедакторОбъектаБДЛкс(ПолноеИмяТаблицы, Знач ИмяРеквизита = "", Знач Связанный = Ложь, Знач КлючОбъекта = Неопределено, Знач ОбъектыНаСервере = Неопределено,
	...
	Знач ПоказатьСвязанныеКолонкиБД = Ложь, Знач АктивироватьУзел = Неопределено, Знач ВладелецФормы = Неопределено, Знач ИскомоеЗначение = Неопределено, Знач ВключитьОстатки = Ложь) Экспорт 
	...
	КонецЕсли;
	Если ВключитьОстатки Тогда
		ФормаРедактора.КоманднаяПанельНабораЗаписейОтображатьОстатки();
	КонецЕсли;
	Возврат ФормаРедактора;
...
Функция ОткрытьКолонкуБДЛкс(Знач ИмяТаблицыИлиПоля, Знач ИмяПоля = "") Экспорт 
	Если Не ЗначениеЗаполнено(ИмяПоля) Тогда
		...
		ИмяТаблицы = ирОбщий.ТекстБезПоследнегоФрагментаЛкс(ИмяТаблицыИлиПоля);
	Иначе
...
Процедура ОбновитьСтатистикуПоТаблицеОбъектаМДВРезультатеПакетаЛкс(РезультатПакета, ПолноеИмяМД, ИмяКлючевойКолонки = "ПолноеИмяОбъекта", ИмяКолонкиКоличества = "КоличествоСтрок", ЛиТаблицыИзменений = Ложь,
	...
	//! СтатистикаПоТаблице.Колонки.Добавить("ИмяТаблицы");
	//! СтатистикаПоТаблице.Колонки.Добавить(ИмяКлючевойКолонки);
	//! СтатистикаПоТаблице.Колонки.Добавить(ИмяКолонкиКоличества);
...
Функция ДобавитьПоляВКоллециюНастроекКомпоновкиПеретаскиваниемЛкс(Знач ДоступныеПоля, Знач Приемник, Знач ПроверятьУникальность = Ложь, Знач Строка = Неопределено) Экспорт 
	...
	Для Каждого ДоступноеПоле Из ДоступныеПоля Цикл
		НовыйЭлемент = ДобавитьПолеВКоллекциюНастроекКомпоновкиПеретаскиваниемЛкс(ДоступноеПоле, Приемник, ПроверятьУникальность, Строка);
	КонецЦикла;
	Возврат НовыйЭлемент;

КонецФункции

Функция ДобавитьПолеВКоллекциюНастроекКомпоновкиПеретаскиваниемЛкс(Знач ДоступноеПоле, Знач Приемник, Знач ПроверятьУникальность = Ложь, Знач Строка = Неопределено) Экспорт 
...
// Возвращаемое значение:
//   - 
Функция ПрименитьСтрокуПоискаКТабличномуПолюЛкс(Знач ЭтаФорма, Знач ТабличноеПоле, Знач СтрокаПоиска, Знач ИменаКолонокДанныхДляПоиска = "", Знач АктивироватьНайденнуюСтроку = Истина,
	...
	//! ТабличноеПоле.Значение.Колонки[ИменаКолонокДанныхДляПоиска]
...
Процедура ДобавитьСсылкуВИсториюРаботыЛкс(Ссылка, ДобавлятьВИсториюРаботыПлатформы = Истина, ДобавлятьВИсториюИнтерфейснойПанели = Истина) Экспорт
	...
	КонецЕсли; 
	Если Истина
		И ДобавлятьВИсториюИнтерфейснойПанели 
		И Не ирКэш.ЛиМодальностьЗапрещенаЛкс() // Иначе будет часто выводится сообщение об ошибке модальности при инициации формы 
	Тогда
		Параметр = Новый Структура("Форма");
		ОповеститьФормыПодсистемыЛкс("ПолучитьФорму.ирИнтерфейснаяПанель", Параметр);
		Форма = Параметр.Форма;
		Если Форма = Неопределено Тогда
			Форма = ПолучитьФормуЛкс("Обработка.ирИнтерфейснаяПанель.Форма"); // - В модальном пространстве получим новую форму, поэтому перед этим сначала ищем существующую форму через оповещение
		КонецЕсли;
		СтруктураЭлемента = Новый Структура();
		СтруктураЭлемента.Вставить("Вид", Ссылка.Метаданные().ПолноеИмя());
		СтруктураЭлемента.Вставить("Имя", Ссылка);
		Форма.ДобавитьСтрокуВСтатическуюВетку(СтруктураЭлемента, Ложь);
	КонецЕсли;
...
Процедура ДобавитьИнструментВИсториюРаботыЛкс(Форма, ДобавлятьВИсториюРаботыПлатформы = Истина, ДобавлятьВИсториюИнтерфейснойПанели = Истина) Экспорт
	...
	Если СтрокаИнструмента.Количество() > 0 Тогда
		Если ДобавлятьВИсториюИнтерфейснойПанели Тогда
			...
			СтруктураЭлемента = Новый Структура();
			СтруктураЭлемента.Вставить("Вид", ирОбщий.ИмяТипаМДМножественноеИзЕдинЛкс(Фрагменты[0]));
...
// .
// Параметры:
//   МассивИлиТаблицаСсылок -  - 
//   ВыбранныеПоля - Массив, ТаблицаЗначений - 
//   ЭтаФорма -  - 
//   ТекущееПолеТаблицы -  - 
//   КлючТекущейСтроки -  - 
//   КлючУникальности -  - 
//   ПараметрОбработка - Строка - имя обработки из Обработка.ирПодборИОбработкаОбъектов.Макет.Обработки
// Возвращаемое значение:
//    - 
Функция ОткрытьМассивОбъектовВПодбореИОбработкеОбъектовЛкс(Знач МассивИлиТаблицаСсылок, Знач ВыбранныеПоля = Неопределено, Знач ЭтаФорма = Неопределено, Знач ТекущееПолеТаблицы = Неопределено,
	Знач КлючТекущейСтроки = Неопределено, Знач КлючУникальности = "", Знач ПараметрОбработка = "") Экспорт 

	Если МассивИлиТаблицаСсылок.Количество() = 0 Тогда
		Возврат Неопределено;
	КонецЕсли; 
	Если ЭтаФорма <> Неопределено Тогда
		ПредложитьЗакрытьМодальнуюФормуЛкс(ЭтаФорма);
	...
	Форма.ПараметрМассивСсылок = МассивИлиТаблицаСсылок;
	Форма.ПараметрВыбранныеПоля = ВыбранныеПоля;
	Форма.ПараметрТекущееПоле = ТекущееПолеТаблицы;
	Форма.ПараметрКлючТекущейСтроки = КлючТекущейСтроки;
	Форма.ПараметрОбработка = ПараметрОбработка;
	Форма.Открыть();
	//Форма.ЗагрузитьОбъектыДляОбработки(РазличныеЗначенияМассиваЛкс(МассивИлиТаблицаСсылок),, ВыбранныеПоля);
	Возврат Форма;
...
Функция СохранитьФайлВКонсолиСВосстановлениемЛкс(Знач ДиалогВыбораФайла, Знач ИмяСохраняемогоФайла, ИмяОткрытогоФайла = "", Знач ДанныеДляФайла, Знач СтруктураВосстановления,
	...
	Если СохранитьФайл Тогда
		...
		Если ДляВосстановления Тогда 
			МаксЧислоТочек = 10;
			МинРасстояниеМеждуТочкамиСекунд = 100; // - секунд
			...
			ирОбщий.СократитьГруппуФайловОткатаЛкс(ВсеФайлыВосстановления, ОставитьТочекОтката, МинРасстояниеМеждуТочкамиСекунд);
...
Процедура СчитатьПорциюВыборкиВТаблицуЛкс(Выборка, ТаблицаПриемник, Знач РазмерПорции = 9999, СчитыватьЧерезКопиюТаблицы = Истина, СсылкаНаБуфернуюТаблицу = Неопределено) Экспорт
	...
	#КонецЕсли
	РезультатЗапроса = Выборка.Владелец();
	...
	Если ТаблицаПриемник.Колонки.Количество() = 0 Тогда
		Для Каждого Колонка Из Колонки Цикл
			...
			КонецЕсли;
			ТаблицаПриемник.Колонки.Добавить(Колонка.Имя, ТипЗначения, Колонка.Имя, Колонка.Ширина);
		КонецЦикла;
	КонецЕсли;
...
// Параметры:
//  Форма - Форма - 
Функция НовыйФоновыйЗапросХттпЛкс(Знач Форма) Экспорт  
	ИмяМетода = "СлужебноеПолеХТМЛonclick";
	Если Не ирОбщий.МетодРеализованЛкс(Форма, ИмяМетода) Тогда
		ВызватьИсключение "В форме отсутствует метод " + ИмяМетода;
	КонецЕсли;
	ЭлементыФормы = Форма.ЭлементыФормы;
	ЭлементыФормы.СлужебноеПолеХТМЛ.ЦветРамки = ЦветаСтиля.ЦветФонаФормы;
	ФоновыйЗапросХттп = ирОбщий.СоздатьОбъектПоИмениМетаданныхЛкс("Обработка.ирФоновыйЗапросХттп");
	Если ирКэш.ДоступноБраузерWebKitЛкс() Тогда
		ФоновыйЗапросХттп.мБраузер = ЭлементыФормы.СлужебноеПолеХТМЛ.Документ.defaultView;
		ЭлементыФормы.СлужебноеПолеХТМЛ.УстановитьТекст(ФоновыйЗапросХттп.ПолучитьМакет("ФоновыйHTTPЗапрос").ПолучитьТекст());
	КонецЕсли;
	Возврат ФоновыйЗапросХттп;
КонецФункции
...
//
// Параметры:
//  htmlElement		 - ОбработкаОбъект.ирТипHTMLElement - 
//  МодификаторCTRL	 - 	 - 
// 
// Возвращаемое значение:
...
Функция ОткрытьГиперссылкуИзПоляHTMLЛкс(Знач htmlElement, Знач МодификаторCTRL = Истина) Экспорт 
	...
	Если htmlElement.hasAttribute("data-href") Тогда
		Если Не МодификаторCTRL Тогда
			Возврат Ложь;
		КонецЕсли;
		НовыйАдрес = htmlElement.innerText;  
	Иначе
...
Процедура ДобавитьДействияРасшифровкиПоПолюЛкс(ЛиПрямоеПоле = Истина, Знач СписокДействийОткрыть, Знач ЗначениеПоля, Знач СписокДействийОтборать, Знач СписокДействийОформить,
	...
	ПредставлениеЗначения = ирОбщий.ПредставлениеСОбрезкойДлиныЛкс(ирОбщий.РасширенноеПредставлениеЛкс(ЗначениеПоля.Значение,,,,, Истина), ПорогДлиныПредставления);
	Если Ложь
...
Функция ПроверитьПодпискиЛкс(Знач ВсеПроверки = Ложь) Экспорт 
	...
	СписокМодулей = ПроблемныеОбщиеМодулиЛкс(СписокМодулейВызоваСервера, ИмяКлиента, ВсеПроверки);
...
Функция ПроблемныеОбщиеМодулиЛкс(СписокМодулейВызоваСервера = Неопределено, ИмяКлиента = "", Знач ВсеПроверки = Ложь) Экспорт 
	...
	Если ВсеПроверки Или ирКэш.НомерВерсииПлатформыЛкс() < 803023 Тогда
		// В 8.3.23 подключение этих подписок в большинстве случаев исправили https://www.hostedredmine.com/issues/882845
...
Процедура ИнициироватьФормуЛкс(ЭтаФорма, ПолноеИмяФормы, РазрешитьОткрытиеДополнительныхФорм = Истина) Экспорт
	...
	КонецЕсли;
	Если Истина
		И ирКэш.НомерВерсииПлатформыЛкс(Истина) > 8030240001
		И ирКэш.НомерВерсииПлатформыЛкс(Истина) < 8030241500 
	Тогда
		ирОбщий.СообщитьЛкс(НСтр("ru='Версии платформы 8.3.24.1-1500 содержат критические ошибки поля HTML документа, используемого во многих инструментах. Крайне рекомендую сменить платформу.'"),,, Истина, Ложь);
	КонецЕсли;
	...
	СлужебныеДанные = ирОбщий.СлужебныеДанныеФормыЛкс(ЭтаФорма);
	СлужебныеДанные.ИмяФормы = ПолноеИмяФормы;
	...
	Если мПлатформа.мПроверкаСовместимостиКонфигурацииВыполнялась <> Истина Тогда
		...
		Если Метаданные.ХранилищеОбщихНастроек <> Неопределено Тогда 
			...
			Если ирОбщий.ВосстановитьЗначениеЛкс(ИмяПроверочнойНастройки) = Неопределено Тогда 
				ирОбщий.СообщитьЛкс("В конфигурации переопределено хранилище общих настроек и оно не восстанавливает сохраненные значения. Корректная работа подсистемы """ + ирОбщий.НазваниеПодсистемыЛкс() + """ невозможна", СтатусСообщения.Важное,, Истина, Ложь);
			КонецЕсли; 
		КонецЕсли; 
		Если Не ирОбщий.ЛиСовместимыйЯзыкСистемыЛкс() Тогда 
			ирОбщий.СообщитьЛкс("Язык системы сеанса 1С (параметр /L) не является русским или английским. Корректная работа подсистемы """ + ирОбщий.НазваниеПодсистемыЛкс() + """ не гарантируется", СтатусСообщения.Важное,, Истина, Ложь);
		КонецЕсли; 

		мПлатформа.мПроверкаСовместимостиКонфигурацииВыполнялась = Истина;
		#Если Сервер И Не Сервер Тогда
			ВыполнитьПроверкиПриНачалеСеансаЛкс();
		#КонецЕсли
		ПодключитьГлобальныйОбработчикОжиданияЛкс("ВыполнитьПроверкиПриНачалеСеансаЛкс", 1, Истина);
...
Процедура ПодготовитьЭлементыФормыЛкс(ЭтаФорма) Экспорт
	...
	мПлатформа = ирКэш.Получить();
	СлужебныеДанныеФормы = ирОбщий.СлужебныеДанныеФормыЛкс(ЭтаФорма);
	Если СлужебныеДанныеФормы.Свойство("КомандныеПанелиКнопок") Тогда
		// Повторный вызов. Например в Обработка.ирДинамическийСписок.Форма.Форма
		Возврат;
	КонецЕсли;
	...
	Если ирОбщий.МетодРеализованЛкс(ЭтаФорма, "СохраняемаяНастройкаФормы") Тогда
		//! ЭтаФорма = 0; // см. Обработка.ирПлатформа.Форма.СохранениеНастроекКнт
		ХранимыеСвойстваФормы = ХранимыеВБазеСвойстваФормыЛкс(ЭтаФорма);
		Если ирОбщий.МетодРеализованЛкс(ЭтаФорма, "СвойстваГлобализуемые") Тогда
			СвойстваГлобализуемые = ЭтаФорма.СвойстваГлобализуемые();
			ирОбщий.СкопироватьКоллекциюЛкс(СвойстваГлобализуемые, ХранимыеСвойстваФормы);
		КонецЕсли;
	ИначеЕсли ирОбщий.СтрНачинаетсяСЛкс(ирОбщий.ПоследнийФрагментЛкс(СлужебныеДанныеФормы.ИмяФормы), "Настройки") Тогда
		Если Истина
			И ЭтаФорма.ВладелецФормы <> Неопределено 
			И ирОбщий.МетодРеализованЛкс(ЭтаФорма.ВладелецФормы, "СохраняемаяНастройкаФормы")
		Тогда
			//! ЭтаФорма.ВладелецФормы = 0; // см. Обработка.ирПлатформа.Форма.СохранениеНастроекКнт
			ХранимыеСвойстваФормы = ХранимыеВБазеСвойстваФормыЛкс(ЭтаФорма.ВладелецФормы);
			Если ирОбщий.МетодРеализованЛкс(ЭтаФорма.ВладелецФормы, "СвойстваГлобализуемые") Тогда
				СвойстваГлобализуемые = ЭтаФорма.ВладелецФормы.СвойстваГлобализуемые();
				ирОбщий.СкопироватьКоллекциюЛкс(СвойстваГлобализуемые, ХранимыеСвойстваФормы);
			КонецЕсли;
		Иначе
			СвойстваГлобализуемые = мПлатформа.НовыйОбщиеНастройкиГлобализуемые();
			ХранимыеСвойстваФормы = СвойстваГлобализуемые;
		КонецЕсли;
	КонецЕсли;
	...
	Для Каждого ИмяОсновногоРеквизита Из ВозможныеИменаРеквизита Цикл
		...
		КонецЕсли; 
	КонецЦикла;
	ОписаниеРеквизитов = Новый Структура("НастройкиХранимые, СвойстваГлобализуемые, ОсновнойРеквизит", ХранимыеСвойстваФормы, СвойстваГлобализуемые, ОсновнойРеквизит);
	Для Каждого ЭлементФормы Из ЭтаФорма.ЭлементыФормы Цикл
		Попытка
		...
		КонецПопытки; 
		Если Истина
			И ЗначениеЗаполнено(Данные)
			И Найти(Данные, ".") = 0
		Тогда
			ДополнитьПодсказкуЭлементаФормы(ЭлементФормы, Данные, ОписаниеРеквизитов);
		КонецЕсли;
		...
		Если ТипЗнч(ЭлементФормы) = Тип("КоманднаяПанель") Тогда
			...
			ВсеКнопки = ПодготовитьОбщиеКнопкиКоманднойПанелиЛкс(КоманднаяПанель, ОписаниеРеквизитов);
	...
	Если ТипЗнч(ДействияФормы) = Тип("КоманднаяПанель") Тогда
		НадписьПараметрыЗаписи = ДействияФормы.Кнопки.Найти("ПараметрыЗаписи");
	Иначе
		ПанельФормы = ЭтаФорма.Панель;
		Если ПанельФормы.КонтекстноеМеню = Неопределено Тогда
			ДействияФормы = ЭтаФорма.ЭлементыФормы.Добавить(Тип("КоманднаяПанель"), "ДействияФормыАвто", Ложь);
	...
	КонецЕсли;
	Если НадписьПараметрыЗаписи = Неопределено Тогда
		НадписьПараметрыЗаписи = ЭтаФорма.ЭлементыФормы.Найти("НадписьОбщиеПараметрыЗаписи");
	КонецЕсли;
	Если НадписьПараметрыЗаписи <> Неопределено Тогда
		ОбновитьКнопкуПараметрыЗаписиЛкс(НадписьПараметрыЗаписи);
	КонецЕсли;
...
Процедура ОбновитьКнопкуПараметрыЗаписиЛкс(Кнопка) Экспорт
	СписокРежимов = Новый СписокЗначений;
	ПараметрыЗаписи = ирКэш.ПараметрыЗаписиОбъектовЛкс();
	СписокРежимов.Добавить("!КЗ", "Отключить контроль записи", ПараметрыЗаписи.ОтключатьКонтрольЗаписи = Истина);
	СписокРежимов.Добавить("!АР", "Без авторегистрации изменений", ПараметрыЗаписи.БезАвторегистрацииИзменений = Истина);
	СписокРежимов.Добавить("ПР", "Привилегированный режим", ПараметрыЗаписи.ПривилегированныйРежим = Истина);
	СписокРежимов.Добавить("ДС", "Дополнительные свойства", ПараметрыЗаписи.ДополнительныеСвойства.Найти(Истина, "Пометка") <> Неопределено);
	Включенные = ирОбщий.ОтобратьКоллекциюЛкс(СписокРежимов, "Э.Пометка", "Значение");
	Заголовок = "Параметры записи: " + ирОбщий.СтрСоединитьЛкс(Включенные, ",");
	Кнопка.Текст = Заголовок;
	Обозначения = Новый Массив;
	Для Каждого Режим Из СписокРежимов Цикл
		Обозначения.Добавить(Режим.Значение + " - " + Режим.Представление);
	КонецЦикла;
	Кнопка.Подсказка = "Параметры записи для всех инструментов.
		|Обозначения активных: " + ирОбщий.СтрСоединитьЛкс(Обозначения);
КонецПроцедуры

Процедура ДополнитьПодсказкуЭлементаФормы(Знач ЭлементФормы, Знач ИмяРеквизита, Знач ОписаниеРеквизитов) Экспорт
	Если Лев(ИмяРеквизита, 1) = "ф" Тогда
		ИмяРеквизита = Сред(ИмяРеквизита, 2);
	КонецЕсли;
	Подсказка = ЭлементФормы.Подсказка;
	Если Истина
		И ОписаниеРеквизитов.ОсновнойРеквизит <> Неопределено
		И Не ЗначениеЗаполнено(Подсказка)
	Тогда
		РеквизитОбъекта = ОписаниеРеквизитов.ОсновнойРеквизит.Метаданные().Реквизиты.Найти(ИмяРеквизита);
		Если РеквизитОбъекта <> Неопределено Тогда
			Подсказка = РеквизитОбъекта.Подсказка;
		КонецЕсли;
	КонецЕсли;
	Если Истина
		И ОписаниеРеквизитов.НастройкиХранимые <> Неопределено
		И ОписаниеРеквизитов.НастройкиХранимые.Свойство(ИмяРеквизита)
	Тогда
		...
		ТекстГлобализации = "";
		Если Истина
			И ОписаниеРеквизитов.СвойстваГлобализуемые <> Неопределено
			И ОписаниеРеквизитов.СвойстваГлобализуемые.Свойство(ИмяРеквизита)
		Тогда
			ТекстГлобализации = " с глобализацией";
		КонецЕсли;
		Подсказка = Подсказка + "Сохраняется" + ТекстГлобализации + " при закрытии.";
	КонецЕсли;
	Если ЭлементФормы.Подсказка <> Подсказка Тогда
		ЭлементФормы.Подсказка = Подсказка;
	КонецЕсли;
КонецПроцедуры

Функция ПодготовитьОбщиеКнопкиКоманднойПанелиЛкс(Знач КоманднаяПанель, Знач ОписаниеРеквизитов, РезультатРекурсия = Неопределено)  Экспорт 
	...
	Для Каждого Кнопка Из КоманднаяПанель.Кнопки Цикл
		Если Кнопка.ТипКнопки = ТипКнопкиКоманднойПанели.Подменю Тогда
			...
			КонецЕсли; 
			ПодготовитьОбщиеКнопкиКоманднойПанелиЛкс(Кнопка, ОписаниеРеквизитов, РезультатРекурсия);
		ИначеЕсли Кнопка.ТипКнопки = ТипКнопкиКоманднойПанели.Действие Тогда
			...
			Тогда
				...
				Если ОбщаяКоманда = Неопределено Тогда
					Если Ложь
						Или Кнопка.Имя = "БуферОбмена_Вставить" 
						Или Кнопка.Имя = "БуферОбмена_Копировать"
					Тогда
						// https://www.hostedredmine.com/issues/1007711
						Продолжить;
					КонецЕсли;
			...
			КонецЕсли; 
			ДополнитьПодсказкуЭлементаФормы(Кнопка, Кнопка.Имя, ОписаниеРеквизитов);
		КонецЕсли; 
	КонецЦикла;
...
//   ИмяЭлемента -  - 
//   ПутьКРеквизиту -  - 
Процедура ОткрытьСтруктуруФормыЛкс(Знач ЭтаФорма, Знач КлючУникальности = Неопределено, Знач ИмяЭлемента = "", Знач ПутьКРеквизиту = "") Экспорт
	...
	Если ЗначениеЗаполнено(ИмяЭлемента) Тогда
		...
		Иначе
			ЭлементыФормы = ЭтаФорма.Элементы;
		КонецЕсли;
		ФормаСтруктуры.ПараметрЭлементФормы = ЭлементыФормы.Найти(ИмяЭлемента);
	КонецЕсли;
	Если ЗначениеЗаполнено(ПутьКРеквизиту) Тогда
		ФормаСтруктуры.ПараметрПутьКРеквизиту = ПутьКРеквизиту;
	КонецЕсли;
	ФормаСтруктуры.ОткрытьМодально();
...
Функция ОткрытьОбщиеПараметрыЗаписиЛкс(Знач ЭлементФормы, Знач ТолькоОбъектыНаСервере = Ложь) Экспорт 
	мПлатформа = ирКэш.Получить();
	#Если Сервер И Не Сервер Тогда
		мПлатформа = Обработки.ирПлатформа.Создать();
	#КонецЕсли
	...
	Если Форма.ОткрытьМодально() <> Неопределено Тогда
		ОбновитьКнопкуПараметрыЗаписиЛкс(ЭлементФормы);
	КонецЕсли;
КонецФункции

Процедура ОткрытьПараметрыИИЛкс() Экспорт
	ОткрытьНастройкиПоляПрограммыЛкс("", Истина, "МодельИИ");
КонецПроцедуры
...
//  Произвольный - 
//
Функция ДанныеСтрокиТабличногоПоляЛкс(ТабличноеПоле, Знач Строка = Неопределено, ДанныеПоля = Неопределено, СерверныеМетаданныеФормы = Неопределено) Экспорт 
	...
	Если ТипЗнч(ТабличноеПоле) = Тип("ТаблицаФормы") Тогда
		...
		Если ТипЗнч(Строка) = Тип("ИдентификаторКомпоновкиДанных") Тогда
			Если ДанныеПоля = Неопределено Тогда
				ДанныеПоля = ирОбщий.ДанныеЭлементаФормыЛкс(ТабличноеПоле,,,, СерверныеМетаданныеФормы);
			КонецЕсли;
			Если ДанныеПоля <> Неопределено Тогда
...
Функция НоваяФормаРезультатаФоновогоЗаданияЛкс(Знач ДолгийРежим = Ложь) Экспорт 
	...
	Если ирКэш.НомерВерсииПлатформыЛкс() >= 803021 И Не ДолгийРежим Тогда
...
//.
// Параметры:
//   ЭтаФорма - см. Обработка.ирПлатформа.Форма.ФоновыеЗаданияКнт - 
//   ОписаниеЗадания -  - 
//   Перезапустить -  - 
//   выхОшибкаСериализации -  - 
// Возвращаемое значение:
//    - 
Функция ЗапуститьИлиОтменитьФоновоеЗаданиеФормыЛкс(Знач ЭтаФорма, Знач ОписаниеЗадания, Знач Перезапустить = Ложь, выхОшибкаСериализации = Ложь) Экспорт 
	...
	Если ЗаданияФормы.Свойство(ОписаниеЗадания.Имя) Тогда
		...
		Если Ложь
			Или ОписаниеЗаданияСтарое.УникальныйИдентификатор <> Неопределено 
			Или ПустаяСтрока(ОписаниеЗадания.Метод)
		Тогда
			Если Ложь
	...
	Если ПустаяСтрока(ОписаниеЗадания.Метод) Тогда
		ОписаниеЗадания.Состояние = СостояниеФоновогоЗадания.Активно;
	Иначе
		Попытка
	...
	ЖдатьЗавершения = ОписаниеЗадания.ЖдатьЗавершения;
...
Функция ОписаниеФоновогоЗаданияФормыЛкс(Имя = "", Метод = "", Параметры = Неопределено, Знач Представление = "", Кнопка = Неопределено, ОбработчикЗавершения = "", АдресРезультата = "",
	ОповещатьПользователяОбУспехе = Истина, БлокируемыеЭлементыФормы = Неопределено, Многопоточное = Ложь, ПрефиксыОповещений = Неопределено, Знач ПоддерживаетПрерывание = Ложь, Знач ЖдатьЗавершения = Ложь,
	Знач КонтролироватьОтмену = Истина) Экспорт 
	...
	|ОповещатьПользователяОбУспехе, БлокируемыеЭлементыФормы, ФормаЗадания, Многопоточное, ПрефиксыОповещений, ПоддерживаетПрерывание, ЖдатьЗавершения, КонтролироватьОтмену");
	...
	ОписаниеЗадания.ЖдатьЗавершения = ЖдатьЗавершения; 
	ОписаниеЗадания.КонтролироватьОтмену = КонтролироватьОтмену;
...
Функция ПроверитьЗавершениеФоновыхЗаданийФормыЛкс(Знач ЭтаФорма, Знач ОписаниеЗаданияОтбора = Неопределено, Результат = Неопределено) Экспорт 
	...
	Для Каждого КлючИЗначение Из СлужебныеДанные.Задания Цикл
		ОписаниеЗадания = КлючИЗначение.Значение; // см. ОписаниеФоновогоЗаданияФормыЛкс
		Если Истина
		...
		Если ПустаяСтрока(ОписаниеЗадания.Метод) Тогда
			СостояниеЗадания = ОписаниеЗадания.Состояние;
		Иначе
		...
		Иначе
			...
			Если ФоновоеЗадание = Неопределено Тогда
				ДлительностьЗадания = ТекущаяДата() - ОписаниеЗадания.НачалоВыполнения;
				Наименование = ОписаниеЗадания.Имя;
			Иначе
			...
			Если СостояниеЗадания = СостояниеФоновогоЗадания.ЗавершеноАварийно Тогда 
				Если ФоновоеЗадание <> Неопределено Тогда
					ирОбщий.СообщитьЛкс(ирОбщий.СтрШаблонИменЛкс("Ошибка через %1 фонового задания %2: " + ирОбщий.ПодробноеПредставлениеОшибкиЛкс(Результат),
						1, ДлительностьСтрока, 2, Наименование), СтатусСообщения.Внимание);
				КонецЕсли;
			ИначеЕсли Не ЭтаИлиФормаЗаданияАктивна И ОписаниеЗадания.ОповещатьПользователяОбУспехе Тогда 
				ирОбщий.СообщитьЛкс(ирОбщий.СтрШаблонИменЛкс("Завершено через %1 фоновое задание %2",, ДлительностьСтрока, 2, Наименование));
			КонецЕсли;
			Если ФормаЗадания <> Неопределено Тогда
			...
			//Завершенные.Добавить(КлючИЗначение.Ключ); // перенес в ОбработатьЗавершениеЗаданияФормыЛкс
		КонецЕсли;
	КонецЦикла;
...
Процедура ОбработатьЗавершениеЗаданияФормыЛкс(Знач ОписаниеЗадания, Знач ЭтаФорма, Знач ФоновоеЗадание = Неопределено, Знач ПоЗапросуПользователя = Ложь, Знач ЗакрытьФормуЗадания = Ложь,
	Знач Результат = Неопределено) Экспорт
	
	Если Истина
		И ФоновоеЗадание = Неопределено 
		И ЗначениеЗаполнено(ОписаниеЗадания.Метод) 
	Тогда
		ФоновоеЗадание = ФоновыеЗадания.НайтиПоУникальномуИдентификатору(ОписаниеЗадания.УникальныйИдентификатор);
	КонецЕсли; 
	Если ОписаниеЗадания.Кнопка <> Неопределено Тогда
	...
	Если ПустаяСтрока(ОписаниеЗадания.Метод) Тогда
		...
		Иначе
			Состояние = ОписаниеЗадания.Состояние;
		КонецЕсли; 
		ОписаниеЗадания.Состояние = Состояние;
	Иначе
		ФормаЗадания = ФормаЗаданияФормыЛкс(ОписаниеЗадания, ЭтаФорма);
		...
		КонецЕсли; 
	КонецЕсли;
	ирОбщий.СлужебныеДанныеФормыЛкс(ЭтаФорма).Задания.Удалить(ОписаниеЗадания.Имя);
...
// Параметры:
//   Браузер - Произвольный - см. Обработка.ирФоновыйЗапросХттп.Макет.ФоновыйHTTPЗапрос
//   Адрес - Строка - 
//   ТелоЗапросаСтрока - Строка - 
//   Заголовки - Соответствие - 
Процедура ОтправитьЗапросHTTPФоновоЛкс(Знач Браузер, Знач Адрес, Знач ТелоЗапросаСтрока, Знач Заголовки, Знач ПотоковыйРежим = Ложь) Экспорт
...
Процедура ОтменитьФоновоеЗаданиеЛкс(Знач ИдентификаторИлиФоновоеЗадание, Знач КонтролироватьОтмену = Истина) Экспорт 
	...
	Тогда
		...
		Если КонтролироватьОтмену Тогда
			мПлатформа = ирКэш.Получить();
			#Если Сервер И Не Сервер Тогда
				мПлатформа = Обработки.ирПлатформа.Создать();
...
Функция ПолучитьФормуЛкс(ПолноеИмяФормы, Параметры = Неопределено, Владелец = Неопределено, Уникальность = Неопределено, Окно = Неопределено, ТолькоВнешниеФормы = Ложь, Знач НачатьОтслеживание = Истина) Экспорт
	...
	Если Фрагменты.Количество() > 2 Тогда
		...
		МенеджерОбъекта = ирОбщий.МенеджерВнешнегоОбъектаЛкс(Фрагменты[1]);
		...
		Если Не ЗначениеЗаполнено(ИмяФормы) Тогда
			...
			ОсновнаяФорма = ОбъектМД.ОсновнаяФорма;
			Если Истина
				И ОсновнаяФорма <> Неопределено 
				И ОбъектМД.Формы.Индекс(ОсновнаяФорма) >= 0 // Иначе общая форма
			Тогда
				ИмяФормы = ОсновнаяФорма.Имя;
			КонецЕсли;
		КонецЕсли;
		Если ирКэш.ЛиСеансТолстогоКлиентаУПЛкс() Тогда
			ИмяОбъекта = ПодключитьВнешнийОбъектЛкс(МенеджерОбъекта.ИспользуемоеИмяФайла);
			//ирПортативный #Если Сервер И Не Сервер Тогда
				...
				ПолноеИмяФормыОбъекта = ирОбщий.ПервыйФрагментЛкс(МенеджерОбъекта, "Объект.") + "." + ИмяОбъекта + ".Форма." + ИмяФормы;
				Результат = ПолучитьФорму(ПолноеИмяФормыОбъекта, Параметры, Владелец, Уникальность);
			//ирПортативный #КонецЕсли
		Иначе
			Результат = МенеджерОбъекта.ПолучитьФорму(ИмяФормы, Владелец, Уникальность);
		КонецЕсли;
	Иначе
		Фрагменты = ирОбщий.СтрРазделитьЛкс(ПолноеИмяФормы);
		Если Истина
			...
			//ирПортативный #Если Сервер И Не Сервер Тогда
				...
				Тогда
					...
					Если ОбъектМД = Метаданные.Обработки.ирПлатформа Тогда 
						МенеджерОбъекта = ирКэш.Получить(); // Так управляемые формы нельзя получать
					Иначе
						МенеджерОбъекта = Новый (ирОбщий.ПеревестиСтроку(Фрагменты[0]) + ирОбщий.ПеревестиСтроку("Менеджер") + "." + Фрагменты[1]);
					КонецЕсли;
				...
				Если Истина 
					...
					И ТипЗнч(Результат) = Тип("Форма") 
					И Фрагменты.Количество() = 3
					И (Ложь
						Или ирОбщий.СтрокиРавныЛкс(Фрагменты[0], "Обработка")
						Или ирОбщий.СтрокиРавныЛкс(Фрагменты[0], "Отчет"))
				Тогда
					ОбъектМД = ирКэш.ОбъектМДПоПолномуИмениЛкс(Фрагменты[0] + "." + Фрагменты[1]);
					Если ОбъектМД.ОсновнаяФорма <> Неопределено Тогда 
						ПолноеИмяФормы = ПолноеИмяФормы + "." + ОбъектМД.ОсновнаяФорма.Имя;
					КонецЕсли;
				КонецЕсли;
		...
		Иначе
			...
			Если ОбъектМД = Неопределено Тогда
				...
				Иначе
					...
					МенеджерОбъекта = ирПортативный.ПолучитьМенеджерТипаМетаданныхЛкс(ТипМетаданных); // ВнешниеОбработкиМенеджер
					ПолноеИмяФайла = ирПортативный.ПолноеИмяФайлаПортативногоОбъектаМетаданныхЛкс(Фрагменты[1], ТипМетаданных);
					Результат = МенеджерОбъекта.ПолучитьФорму(ПолноеИмяФайла, ИмяФормы, Владелец, Уникальность);
				КонецЕсли; 
			Иначе
...
Функция ПодключитьВнешнийОбъектЛкс(ИмяФайла) Экспорт
	// https://partners.v8.1c.ru/forum/t/870763/m/870763
	ИмяОбъекта = ирСервер.ПодключитьВнешнийОбъектЛкс(ИмяФайла); // - 20мс+
	Если ПустаяСтрока(ИмяОбъекта) Тогда
		ИмяОбъекта = ирСервер.ПодключитьВнешнийОбъектЛкс(ПоместитьВоВременноеХранилище(Новый ДвоичныеДанные(ИмяФайла))); // - 20мс+
	КонецЕсли;
	Возврат ИмяОбъекта;
КонецФункции
...
Процедура ВставитьЗначениеВФормуЛкс(Знач ЭтаФорма, Знач Значение, Знач ЛиОсновнаяВставка = Истина) Экспорт 
	...
	Если Истина
		...
		И (Ложь
			Или ТипЗнч(Значение) = Тип("СписокЗначений") И ЭтаФорма.ValueList.ТипЗначения.СодержитТип(ТипЗнч(Значение[0].Значение))
			Или ТипЗнч(Значение) = Тип("Строка") И ЭтаФорма.ValueList.ТипЗначения.СодержитТип(ТипЗначения))
	Тогда
		...
		Если Истина
			...
			И (Ложь
				...
				Или (Истина
					...
					И (Ложь
						Или ТипЗнч(Значение) = Тип("СписокЗначений") И ЛиОписаниеТиповСодержитТипЛкс(ЭтаФорма.ОписаниеТипов, ТипЗнч(Значение[0].Значение))
						Или ТипЗнч(Значение) = Тип("Строка") И ЛиОписаниеТиповСодержитТипЛкс(ЭтаФорма.ОписаниеТипов, Тип("Строка")))))
		Тогда
			Если ТипЗнч(Значение) = Тип("Строка") Тогда
				НовоеЗначение = Новый СписокЗначений;
				НовоеЗначение.ЗагрузитьЗначения(ирОбщий.СтрРазделитьЛкс(Значение, Символы.ПС, Истина));
...
Функция БуферОбмена_КопироватьЛкс(Знач ЭтаФорма = Неопределено, Знач УстановитьПредставление = Истина) Экспорт
	...
	Если Значение <> Неопределено Тогда
		Если Истина
			И ТипЗнч(Значение) = Тип("СписокЗначений")
			И Значение.Количество() > 1
			И (Ложь
				Или ТипЗнч(ЭтаФорма.ТекущийЭлемент) = Тип("ТабличноеПоле")
				Или ТипЗнч(ЭтаФорма.ТекущийЭлемент) = Тип("ТаблицаФормы"))
		Тогда 
			// Исправляем порядок элементов https://www.hostedredmine.com/issues/881648
			УстановитьПредставление = Истина;
			ТаблицаФормы = ЭтаФорма.ТекущийЭлемент;
			ПараметрыВывода = ирКэш.Получить().ПолучитьФорму("ПараметрыВыводаСтрокТаблицы");
			ПараметрыВывода.БезОформления = ТипЗнч(ТаблицаФормы) = Тип("ТаблицаФормы");
			ПараметрыВывода.ВыводВТаблицуЗначений = Истина;
			ПараметрыВывода.ТолькоВыделенныеСтроки = Истина;
			ПараметрыВывода.ТабличноеПоле = ТаблицаФормы;
			ТекущаяКолонка = ТабличноеПолеТекущаяКолонкаЛкс(ТаблицаФормы);
			КолонкаДанных = ирОбщий.ПутьКДаннымКолонкиТабличногоПоляЛкс(ТаблицаФормы);
			ПараметрыВывода.ДобавитьКолонки(ирОбщий.ЗначенияВМассивЛкс(ТекущаяКолонка));
			ТекстыЯчеек = ВывестиСтрокиТабличногоПоляЛкс(ЭтаФорма, ТаблицаФормы, ПараметрыВывода);
			Представление = ирОбщий.СтрСоединитьЛкс(ТекстыЯчеек.ВыгрузитьКолонку(КолонкаДанных), Символы.ПС);
		КонецЕсли;
		БуферОбменаПриложения_УстановитьЗначениеЛкс(Значение, УстановитьПредставление, Представление);
	Иначе
...
Функция Форма_ЗначениеТекущегоПоляЛкс(Знач ЭтаФорма, Знач ДляКопирования = Ложь, выхПолеТекста = Неопределено, СерверныеМетаданныеФормы = Неопределено) Экспорт 
	...
	Если Не ЕстьСсылкиВТекущемПоле И ТипЗнч(Значение) <> Тип("СписокЗначений") Тогда
		...
		Если ЛиПолеФормыИмеетТипЛкс(ТекущийЭлементФормы, Тип("ПолеВвода")) Тогда
			Значение = ирОбщий.ДанныеЭлементаФормыЛкс(ТекущийЭлементФормы,,,, СерверныеМетаданныеФормы);
		...
		Тогда
			Если ДляКопирования Тогда
				...
				Тогда
					выхПолеТекста = ОболочкаПоляТекстаЛкс(ТекущийЭлементФормы);
					// Антибаг платформы. Копирование в буфер обмена не выполняется платформой
					ВыделенныйТекст = выхПолеТекста.ВыделенныйТекст();
			...
			Иначе
				выхПолеТекста = ОболочкаПоляТекстаЛкс(ТекущийЭлементФормы);
				Значение = выхПолеТекста.ПолучитьТекст();
			КонецЕсли;
		ИначеЕсли ТипЗнч(ТекущийЭлементФормы) = Тип("ПолеФормы") И ТекущийЭлементФормы.Вид = ВидПоляФормы.ПолеФорматированногоДокумента Тогда
			ДанныеЭлемента = ирОбщий.ДанныеЭлементаФормыЛкс(ТекущийЭлементФормы,,,, СерверныеМетаданныеФормы);
		...
		Тогда
			...
			Если ТекущийЭлементФормы.ТекущаяСтрока <> Неопределено Тогда 
				...
				ДанныеПоля = ирОбщий.ДанныеЭлементаФормыЛкс(ТекущийЭлементФормы,,,, СерверныеМетаданныеФормы);
				ИмяКолонкиДанных = ирОбщий.ПутьКДаннымКолонкиТабличногоПоляЛкс(ТекущийЭлементФормы,, СерверныеМетаданныеФормы);
				Если Ложь
							...
							Если ТипЗнч(Значение) = Тип("Строка") Тогда
								Если ПустаяСтрока(Прав(Значение, 1)) Тогда
									...
									Если Истина
										...
										И ЗначениеЗаполнено(ПолноеИмяТаблицыБД) 
									Тогда
										ТаблицаЗначений = ирОбщий.ПустаяТаблицаИзТаблицыБДЛкс(ПолноеИмяТаблицыБД);
									Иначе
									...
									Если ТаблицаЗначений <> Неопределено Тогда
										КолонкаТаблицы = ТаблицаЗначений.Колонки.Найти(ирОбщий.ПутьКДаннымКолонкиТабличногоПоляЛкс(ТекущийЭлементФормы,, СерверныеМетаданныеФормы));
										Если Ложь
...
//   ИмяФайлаМодуля -  - 
//   ПолеТекстаФрагмента -  - 
//   РасширениеФайлаМодуля -  - 
//   ТекстФайла - Строка - кэш текста файла на время сеанса! Надо передавать если нужен свежий!
//   ЧислоПредСтрок -  - 
//   ИмяМетода -  - 
//   ЗаполнитьТекстСтроки -  - 
//   ИмяПоляМетода -  - 
//   БыстроГрубо -  - 
Процедура РазобратьПозициюМодуляВСтрокеТаблицыЛкс(Знач ТекущаяСтрока, Знач ИмяФайлаМодуля, Знач ПолеТекстаФрагмента = Неопределено, Знач РасширениеФайлаМодуля = "txt", Знач ТекстФайла = Неопределено,
	Знач ЧислоПредСтрок = 2, Знач ИмяМетода = "", Знач ЗаполнитьТекстСтроки = Ложь, Знач ИмяПоляМетода = "", Знач БыстроГрубо = Истина) Экспорт 
	...
	//Если Не ЗначениеЗаполнено(ТекущаяСтрока.НомерСтрокиМодуля) Тогда
		...
		Если ПолеТекстаМодуля = Неопределено Тогда 
			ТекущаяСтрока.Ссылка = ПустаяСсылкаСтрокиМодуляЛкс(); 
			Возврат;
		КонецЕсли;
		#Если Сервер И Не Сервер Тогда
		...
		Если Не ЗначениеЗаполнено(ТекущаяСтрока.Ссылка) Тогда
			...
			Если ИмяМетода = мПлатформа.ИмяМетодаИнициация() Тогда
				// Тогда делали точный разбор, но для него нужна полноценная загрузка модуля с инициализацией метаданных. Поэтому заменил на абсолютную ссылку.
				//ПолеТекстаМодуля.РазобратьТекущийКонтекст();
				//ИмяМетода = ПолеТекстаМодуля.ИмяТекущегоМетода();
				ИмяМетодаДляСсылка = "";
				СмещениеСтрокиМетода = 0;
			Иначе
				ИмяМетодаДляСсылка = ИмяМетода;
			КонецЕсли;
			...
			ТекущаяСтрока.Ссылка = ирОбщий.СсылкаСтрокиМодуляЛкс(ИмяМодуля, ПолеТекстаМодуля.мНачальнаяСтрока, ИмяМетодаДляСсылка, СмещениеСтрокиМетода, ТекстСтроки, ПолеТекстаМодуля.мНачальнаяКолонка);
			Попытка
...
Функция ПустаяСсылкаСтрокиМодуляЛкс() Экспорт
	Возврат "<Файл модуля не найден в кэше модулей>";
КонецФункции

//.
// Параметры:
//    ИмяФайлаИлиМодуля - Строка - если полное имя файла, то чтение текста кэшируется через ТекстФайла
//    Позиция - Число - позиция в тексте модуля, начиная с 1 
//    ТекстФайла - Строка - кэш текста файла на время сеанса! Надо передавать если нужен свежий!
...
Функция ПолеТекстаМодуляБезСтруктурыТипаЛкс(Знач ИмяФайлаИлиМодуля, ТекстФайла = Неопределено, выхФайлМодуля = Неопределено) Экспорт
	...
	Иначе
		...
		Если ТекстФайла <> Неопределено Тогда  
			ПолеТекстаМодуля.УстановитьТекстБезМетаданных(ТекстФайла);
		Иначе
			Модуль = мПлатформа.МодульМетаданныхИзКэша(ИмяФайлаИлиМодуля); 
			Если Модуль <> Неопределено Тогда
				ПолеТекстаМодуля.УстановитьТекстБезМетаданных(Модуль.Текст);
			КонецЕсли;
			ТекстФайла = ПолеТекстаМодуля.мОригинальныйТекст;
		КонецЕсли;
		Если СтрДлина(ТекстФайла) = 0 Тогда
...
Функция СсылкаНаМодульКонфигурацииЛкс(Знач ПолноеИмяМодуля) Экспорт 
	...
	Возврат "{" + ПолноеИмяМодуля + "(0)}"; // 0 - позволяет активировать модуль через Турбоконф без смены текущей строки
...
//   ДокументДОМ - ДокументDOM, Неопределено - 
Функция НайтиСоздатьЭлементДОМИзСтруктурыСсылкиМодуляЛкс(Знач ИмяФайла, Знач СтруктураСсылки, ДокументДОМ) Экспорт
	...
	Иначе
		ИмяМодуляРус = СтруктураСсылки.Модуль;
	КонецЕсли;
	Фрагменты = ирОбщий.СтрРазделитьЛкс(ИмяМодуляРус);
	ИмяМодуляАнг = "";
	Если Фрагменты.Количество() = 1 Тогда
		Если Не ЗначениеЗаполнено(СтруктураСсылки.Расширение) Тогда
	...
	Иначе
		Для Индекс = 0 По Фрагменты.Количество() / 2 - 1 Цикл
			...
			Если ИмяАнг = Неопределено Тогда
				ИмяАнг = ИмяРус;
			КонецЕсли;
			Если ИмяАнг = "Configuration" И ЗначениеЗаполнено(СтруктураСсылки.Расширение) Тогда
				Продолжить;
			КонецЕсли;
	...
	Если ДокументДОМ = Неопределено Тогда
		ДокументДОМ = ирОбщий.ФайлВДокументDOMЛкс(ИмяФайла);
	КонецЕсли;
	ЭлементМодуля = null;
	Корень = ДокументДОМ.ПервыйДочерний; // Было 1 
	...
	Если ЭлементМодуля = null Тогда
		ЭлементМодуля = ДокументДОМ.СоздатьЭлемент("moduleBPInfo");
		ИДТочки = ДокументДОМ.СоздатьЭлемент("id");
		ЭлементМодуля.ДобавитьДочерний(ИДТочки);
		Корень.ДобавитьДочерний(ЭлементМодуля);
		ЭлементТипаМодуля = ДокументДОМ.СоздатьЭлемент("debugBaseData:type");
		...
		Если Не ПустаяСтрока(СтруктураСсылки.Расширение) Тогда
			...
			ЭлементРасширения = ДокументДОМ.СоздатьЭлемент("debugBaseData:extensionName");
		...
		КонецЕсли;
		ЭлементОбъектаМД = ДокументДОМ.СоздатьЭлемент("debugBaseData:MDObject");
		ИДТочки.ДобавитьДочерний(ЭлементОбъектаМД);
		ЭлементОбъектаМД.ТекстовоеСодержимое = ИмяМодуляАнг;
		ЭлементСвойстваМД = ДокументДОМ.СоздатьЭлемент("debugBaseData:MDProperty");
		ИДТочки.ДобавитьДочерний(ЭлементСвойстваМД);
		ЭлементСвойстваМД.ТекстовоеСодержимое = ИмяСвойстваАнг;
		Если ЗначениеЗаполнено(СтруктураСсылки.Файл) Тогда
			ЭлементФайла = ДокументДОМ.СоздатьЭлемент("debugBaseData:URL");
...
// Параметры:
//   ИмяМодуляСжатое - Строка - такое имя в частности используется в замере производительности, например "МодульУправляемогоПриложения", "ИнструментыРазработчика Обработка.ирПлатформа.Форма.ВыборПодсистемы.Форма"
// Возвращаемое значение:
//   Строка - например "Конфигурация.МодульУправляемогоПриложения", "ИнструментыРазработчика Обработка.ирПлатформа.Форма.ВыборПодсистемы.Форма.Модуль"
Функция ИмяМодуляИзСжатогоИмениЛкс(Знач ИмяМодуляСжатое) Экспорт 
	Если Найти(ИмяМодуляСжатое, ".") = 0 Тогда
		Фрагменты = ирОбщий.СтрРазделитьЛкс(ИмяМодуляСжатое, " ");
		ИмяМодуля = "Конфигурация" + "." + Фрагменты[Фрагменты.ВГраница()];
		Если Фрагменты.Количество() > 1 Тогда
			ИмяМодуля = Фрагменты[0] + " " + ИмяМодуля;
		КонецЕсли;
	Иначе
		ИмяМодуля = ИмяМодуляСжатое;
		Если ирОбщий.СтрКончаетсяНаЛкс(ИмяМодуляСжатое, ".Форма") Тогда
...
//.
// Параметры:
//  ИмяМодуляСжатое	 - Строка - например "МодульУправляемогоПриложения", "ИнструментыРазработчика Обработка.ирПлатформа.Форма.ВыборПодсистемы.Форма"
Процедура ОткрытьОбъектМетаданныхПоИмениМодуляЛкс(Знач ИмяМодуляСжатое) Экспорт 
	ИмяОбъектаМодуля = ирОбщий.ТекстБезПоследнегоФрагментаЛкс(ИмяМодуляСжатое);
	Если ЗначениеЗаполнено(ИмяОбъектаМодуля) Тогда
		ОбъектМД = ирКэш.ОбъектМДПоПолномуИмениЛкс(ирОбщий.ПоследнийФрагментЛкс(ИмяОбъектаМодуля, " "));
		Если ОбъектМД <> Неопределено Тогда
...
//.
// Параметры:
//   ЭтаФорма - см. Обработка.ирПлатформа.Форма.СохранениеНастроекКнт - 
//   КлючНазначенияИспользования -  - 
//   ЗагружатьСохранятьНастройкуПоУмолчанию -  - 
//   ПараметрыЗагрузкиНастройкиПоУмолчанию -  - 
//   РасширениеФайла -  - 
//   СоставНастройкиФормы -  - 
//   СохранятьВидимостьКолонок -  - 
Процедура Форма_ПриОткрытииЛкс(Знач ЭтаФорма, Знач КлючНазначенияИспользования = "", Знач ЗагружатьСохранятьНастройкуПоУмолчанию = Истина, Знач ПараметрыЗагрузкиНастройкиПоУмолчанию = Неопределено,
		Знач РасширениеФайла = "", Знач СоставНастройкиФормы = Неопределено, Знач СохранятьВидимостьКолонок = Ложь) Экспорт
	ирКэш.СостояниеПодготовкиКэшМДСеансаЛкс();
	мПлатформа = ирКэш.Получить();
	#Если Сервер И Не Сервер Тогда
	...
	КонецЕсли;
	Если ирОбщий.МетодРеализованЛкс(ЭтаФорма, "СохраняемаяНастройкаФормы") Тогда
		СоздатьМенеджерСохраненияНастроекФормыЛкс(ЭтаФорма, КлючНазначенияИспользования, ЗагружатьСохранятьНастройкуПоУмолчанию, ПараметрыЗагрузкиНастройкиПоУмолчанию, РасширениеФайла,
			СоставНастройкиФормы, СохранятьВидимостьКолонок);
	КонецЕсли;
...
Процедура Форма_ПриЗакрытииЛкс(ЭтаФорма, СохранитьНастройкуПоУмолчанию = Истина) Экспорт 
	...
	Если ТипЗнч(ЭтаФорма) = Тип("Форма") Тогда
		Если СохранитьНастройкуПоУмолчанию Тогда
			СохранитьНастройкуФормыЛкс(ЭтаФорма);
		КонецЕсли;
		Если ирОбщий.МетодРеализованЛкс(ЭтаФорма, "ОбработкаОповещения") Тогда
			ЭтаФорма.УстановитьДействие("ОбработкаОповещения", Новый Действие("ОбработкаОповещения")); // для диагностики неуничтожения формы
		КонецЕсли;
	КонецЕсли;
	ЗаданияФормы = Неопределено;
	СлужебныеДанные = ирОбщий.СлужебныеДанныеФормыЛкс(ЭтаФорма);
	Если СлужебныеДанные.Задания.Количество() > 0 Тогда
		//! ЭтаФорма = 0 // см. Обработка.ирПлатформа.Форма.ФоновыеЗаданияКнт
	...
	Если МодальныеГруппы.Количество() > 0 Тогда
		ИндексГруппы = МодальныеГруппы.Количество() - 1;
		Если ЭтаФорма.МодальныйРежим Тогда 
			// Выполняется никогда, т.к. свойство МодальныйРежим платформа сбрасывает перед ПриЗакрытии
...
// Параметры:
//   ЭтаФорма - см. Обработка.ирПлатформа.Форма.ФоновыеЗаданияКнт - 
//   ОписаниеЗадания - см. ОписаниеФоновогоЗаданияФормыЛкс - 
//   ЗакрытьФормуЗадания -  - 
// Возвращаемое значение:
//    - 
Функция ОтменитьЗаданиеФормыЛкс(Знач ЭтаФорма, Знач ОписаниеЗадания, Знач ЗакрытьФормуЗадания = Ложь) Экспорт
	
	Если Истина
		И ЗначениеЗаполнено(ОписаниеЗадания.Метод)
		И Не ЗначениеЗаполнено(ОписаниеЗадания.УникальныйИдентификатор) 
	Тогда
		Возврат Ложь;
	КонецЕсли; 
	ЗаданияФормы = ирОбщий.СлужебныеДанныеФормыЛкс(ЭтаФорма).Задания;
	ОтменитьФоновоеЗаданиеЛкс(ОписаниеЗадания.УникальныйИдентификатор, ОписаниеЗадания.КонтролироватьОтмену);
	Если ОписаниеЗадания.Многопоточное Тогда
	...
	КонецЕсли; 
	Если ПустаяСтрока(ОписаниеЗадания.Метод) Тогда
		ОписаниеЗадания.Состояние = СостояниеФоновогоЗадания.Отменено; 
	КонецЕсли;
...
Функция ПериодОчисткиМенеджераВременныхТаблицЛкс(выхПоУмолчаниюОчищать = Ложь) Экспорт 
	...
	ПараметрыОчистки = ирКэш.Получить().ПредлагатьОчиститьМенеджерВременныхТаблицЧерезМинут;
...
Функция УстановитьВременныеСвойстваПользователюИБЛкс(ПользовательИБ, ПодменитьПарольНаВремяЗапуска = Истина, ВременноПредоставитьПравоРазработчикИР = Истина,
	...
	Если ВременноПредоставитьПравоРазработчикИР И Не ирКэш.ЛиПортативныйРежимЛкс() Тогда
		...
		КонецЕсли;
		РольЗапускТолстогоКлиента = Метаданные.Роли.Найти("ЗапускТолстогоКлиента");
		Если Истина
			И РольЗапускТолстогоКлиента <> Неопределено
			И Не ПользовательИБ.Роли.Содержит(РольЗапускТолстогоКлиента)
		Тогда
			УдалитьРоли.Добавить(РольЗапускТолстогоКлиента);
			ПользовательИБ.Роли.Добавить(РольЗапускТолстогоКлиента);
		КонецЕсли;
...
Процедура ВернутьПостоянныеСвойстваПользователюИБЛкс(НаборПараметров = Неопределено) Экспорт;
	...
	Если УдалитьРоли.Количество() > 0 Тогда 
		...
		КонецЦикла;
		ПользовательИБ.ПолноеИмя = ирОбщий.ТекстБезПоследнегоФрагментаЛкс(ПользовательИБ.ПолноеИмя, ирОбщий.МаркерВременныхРолейЛкс());
	КонецЕсли; 
...
Процедура ОткрытьСвязанныйСеансТонкогоКлиентаЛкс() Экспорт 
	...
	Для Каждого Окно Из Окна Цикл
		...
		Для Каждого Форма Из Содержимое Цикл
			Попытка
				СсылкаCOM = Форма.Параметры.Ключ;
			Исключение
				СсылкаCOM = Неопределено;
			КонецПопытки; 
			Если СсылкаCOM <> Неопределено Тогда
				ФрагментыИмениТипа = ирОбщий.СтрРазделитьЛкс(Форма.ИмяФормы);
...
Функция ОткрытьОбъектМетаданныхЛкс(ОбъектИлиПолноеИмяМД, Знач ПоказатьВДеревеМетаданных = Ложь, Знач ПараметрКоманда = "") Экспорт 
	...
	Если ирОбщий.ПеревестиСтроку("Роль") = Фрагменты[0] Тогда
		...
		Возврат Форма;
	КонецЕсли;
	Если Не ПоказатьВДеревеМетаданных Тогда
		Если Ложь
			...
			Или ирОбщий.ПеревестиСтроку("ОбщийМодуль") = Фрагменты[0]
			Или ирОбщий.ПеревестиСтроку("ОбщаяКоманда") = Фрагменты[0]
			Или ирОбщий.ПеревестиСтроку("Команда") = Фрагменты[0]
			Или (Истина
				И Фрагменты.Количество() >= 4
				И Не ирОбщий.ЛиКорневойТипВнешнегоИсточникаДанныхЛкс(Фрагменты[0]))
		Тогда
			//Если Фрагменты.Количество() < 4 Тогда 
				Форма = ПолучитьФормуЛкс("Обработка.ирИнтерфейснаяПанель.Форма.ОбъектМетаданных",,, ПолноеИмяМД);
				Форма.Открыть();
				Возврат Форма;
			//КонецЕсли;
	...
	Если Форма = Неопределено Тогда
		...
		Форма.ПараметрКоманда = ПараметрКоманда;
...
Функция КлючиСтрокБДИзТаблицыФормыЛкс(Форма = Неопределено, выхКлючТекущейСтроки = Неопределено, выхТаблицаФормыДинамическогоСписка = Неопределено, Знач НуженВидимыйПорядок = Истина,
	ИсключаяСсылкиМетаданных = Ложь, СерверныеМетаданныеФормы = Неопределено) Экспорт 
	...
	Тогда
		...
		ПолноеИмяТаблицыБД = "";
		ОбщийТипДанныхТаблицы = ирОбщий.ОбщийТипДанныхТабличногоПоляЛкс(ТаблицаФормы,,, ПолноеИмяТаблицыБД,, СерверныеМетаданныеФормы);
		...
		ДанныеПоля = Неопределено;
		Для Каждого ВыделеннаяСтрока Из ВыделенныеСтроки Цикл
			ДанныеСтроки = ДанныеСтрокиТабличногоПоляЛкс(ТаблицаФормы, ВыделеннаяСтрока, ДанныеПоля, СерверныеМетаданныеФормы);
			Если ОбщийТипДанныхТаблицы = "Список" И ЗначениеЗаполнено(ПолноеИмяТаблицыБД) Тогда
				КлючСтроки = ирОбщий.КлючСтрокиТаблицыБДИзСтрокиТаблицыЗначенийЛкс(ПолноеИмяТаблицыБД, ДанныеСтроки);
...
Функция ТекущийЭлементАктивнойФормыЛкс(АктивнаяФорма = Неопределено) Экспорт 
	...
	Если ТипЗнч(АктивнаяФорма) = ирОбщий.ТипУправляемаяФормаЛкс() Тогда
		Если ТипЗнч(ТекущийЭлемент) = Тип("ОсновнойЭлементФормы") Тогда //@skip-check unknown-method-property
...
Процедура ОтладитьКомпоновкуДанныхАктивнойУправляемойФормыЛкс(Форма = Неопределено) Экспорт 
	...
	КонецЕсли; 
	Если ТипЗнч(Форма) = ирОбщий.ТипУправляемаяФормаЛкс() Тогда
		//! Форма = 0 // РасширениеФормыКлиентскогоПриложенияДляОтчета
		Попытка
...
Процедура ЗаполнитьИзмененныеПоляВСтрокеВерсииДанныхЛкс(Знач ДанныеСтроки, Знач КлючОбъекта = Неопределено, Знач ПолноеИмяМД = "") Экспорт
	#Если Сервер И Не Сервер Тогда
...
//.
// Параметры:
//   ДанныеСтроки - ОбработкаТабличнаяЧастьСтрока.ирУдалениеОбъектовСКонтролемСсылок.СсылкиНаКандидата, ОбработкаТабличнаяЧастьСтрока.ирРедакторОбъектаБД.ВерсииОбъекта, ОбработкаТабличнаяЧастьСтрока.ирИсторияДанных.Версии -
//   ПолноеИмяМД -  -
// Возвращаемое значение:
//    -
Функция КлючОбъектаНайденнойСсылкиЛкс(Знач ДанныеСтроки, Знач ПолноеИмяМД = "") Экспорт
...
Функция ДобавитьОтборВИсториюТабличногоПоляЛкс(Знач ЭтаФорма, Знач ТабличноеПолеИлиКлюч, Знач Отбор, СтарыйОтбор, Знач ИгнорироватьЭлементы = "") Экспорт 
	...
	Если Строка(Отбор) <> Строка(СтарыйОтбор) Тогда 
		Если "" + Отбор <> "" Тогда 
			...
			Иначе
				#Если Клиент Тогда
				Схема = ирОбщий.СоздатьСхемуКомпоновкиПоТаблицамЗначенийЛкс(Новый Структура("Т", ТабличноеПолеИлиКлюч.Значение.ВыгрузитьКолонки()));
			...
			ПредставлениеОтбора = ирОбщий.ПредставлениеСОбрезкойДлиныЛкс(НастройкаКомпоновки.Отбор, 150);
		...
		ПостроительЗапроса = Новый ПостроительЗапроса;
		Если ТипЗнч(ТабличноеПолеИлиКлюч) = Тип("Строка") Тогда
			ПустаяТаблица = ирОбщий.ПустаяТаблицаИзТаблицыБДЛкс(ТабличноеПолеИлиКлюч);
		Иначе
...
Функция ДобавитьОтборКомпоновкиВИсториюТаблицыБДЛкс(Знач ЭтаФорма, Знач ПолноеИмяТаблицы, Знач АктивнаяНастройка, Знач СтараяНастройка, выхОтборИзменен = Ложь) Экспорт 
	...
	Если выхОтборИзменен Тогда 
		Если ПредставлениеОтбора <> "" Тогда 
			...
			ПредставлениеЗначения = ирОбщий.ПредставлениеСОбрезкойДлиныЛкс(НастройкаКомпоновки.Отбор, 150);
			ДобавленВСписок = ПоследниеВыбранныеДобавитьЛкс(ЭтаФорма, НастройкаКомпоновки, ПредставлениеЗначения, ПолноеИмяТаблицы, "Отборы", Истина);
		КонецЕсли;
...
Функция ОткрытьНастройкиПоляПрограммыЛкс(Знач Причина = "Для анализа содержимого модулей нужно обновить кэш модулей через открывшуюся форму настроек.", Знач ВзятьФокус = Ложь,
	Знач АктивироватьЭлемент = "") Экспорт 
	...
	Если ЗначениеЗаполнено(АктивироватьЭлемент) Тогда
		Форма.ТекущийЭлемент = Форма.ЭлементыФормы[АктивироватьЭлемент];
	КонецЕсли;
	Если ВзятьФокус Тогда
		Форма.Открыть();
	Иначе
...
Функция ДобавитьКнопкиКомандБСПЛкс(Знач КоманднаяПанельДляБСП, Знач ИмяТаблицы) Экспорт 
	...
	Если ПодменюБСП <> Неопределено Тогда
		...
		Тогда 
			Попытка
				КомандыБСПСтрока = ирСервер.КомандыБСППоИмениТаблицыЛкс(ИмяТаблицы);
			Исключение
				// Подавляем нарушения контрактов подсистемы БСП https://www.hostedredmine.com/issues/1003532
				Описание = ОписаниеОшибки();
				КомандыБСПСтрока = Неопределено; 
			КонецПопытки;
			Если КомандыБСПСтрока <> Неопределено Тогда
				КомандыБСП = ирОбщий.ОбъектИзСтрокиXMLЛкс(КомандыБСПСтрока);
...
Функция ОткрытьПолеТекстаМодуляКонфигурацииЛкс(Знач ИмяМодуля, выхФормаТекста = Неопределено) Экспорт
	...
	Если Не ФайлМодуля.Существует() Тогда
		ОткрытьНастройкиПоляПрограммыЛкс();
		Возврат Неопределено;
	КонецЕсли;
...
Функция ОбновитьКэшМодулейЛкс(Знач КаталогКэша = "", Знач Асинхронно = Ложь, Знач РазрешитьДиалоги = Истина) Экспорт 
	...
	Если Асинхронно Тогда
		ФормаНастроек = ОткрытьНастройкиПоляПрограммыЛкс("");
	КонецЕсли;
	Если РазрешитьДиалоги Тогда
		ОтключенныеРасширения = ирОбщий.ВосстановитьЗначениеЛкс("ОтключенныеРасширенияДляКэшаМодулей");
	КонецЕсли;
	Если ОтключенныеРасширения = Неопределено Тогда
		ОтключенныеРасширения = Новый Структура;
	КонецЕсли;
	ВсеГруппыМетаданных = Новый СписокЗначений;
	ПрефиксИмениРасширенияПатча = "EF_";
	СписокПатчей = Новый Массив;
	Для Каждого РасширенияЭлемент Из ирКэш.РасширенияКонфигурацииЛкс() Цикл
		РасширениеОбъект = РасширенияЭлемент.Ключ;
		ИмяГруппы = РасширенияЭлемент.Ключ.Имя;
		Если ирОбщий.СтрНачинаетсяСЛкс(ИмяГруппы, ПрефиксИмениРасширенияПатча) Тогда
			СписокПатчей.Добавить(РасширениеОбъект);
		Иначе
			ВсеГруппыМетаданных.Добавить(РасширениеОбъект, ИмяГруппы, Не ОтключенныеРасширения.Свойство(ИмяГруппы));
		КонецЕсли;
	КонецЦикла;
	Если СписокПатчей.Количество() > 0 Тогда
		ИмяГруппыПатчей = "Патчи" + ПрефиксИмениРасширенияПатча;
		ВсеГруппыМетаданных.Добавить(СписокПатчей, ИмяГруппыПатчей, Не ОтключенныеРасширения.Свойство(ИмяГруппыПатчей));
	КонецЕсли;
	ВсеГруппыМетаданных.Добавить(, "<Конфигурация> (до нескольких минут)", Истина);
	Если РазрешитьДиалоги Тогда
		...
		Если Не Асинхронно Тогда
			ВыбранныеГруппыМетаданных = ирКлиент.ВыбратьЭлементСпискаЗначенийЛкс(ВсеГруппыМетаданных,,, "Выгрузка " + ТекстВопроса + " конфигуратора", Истина);
			Если ВыбранныеГруппыМетаданных = Неопределено Тогда
				Возврат Ложь;
			КонецЕсли;
			ВыбранныеГруппыМетаданных = ирОбщий.ВыгрузитьСвойствоЛкс(ВыбранныеГруппыМетаданных, "Значение");
		...
		КонецЕсли;
	Иначе
		ВыбранныеГруппыМетаданных = ВсеГруппыМетаданных.ВыгрузитьЗначения();
	КонецЕсли;
	КаталогЗеркалаБазы = "";
	СтрокаСоединенияБазы = ирОбщий.СтрокаСоединенияБазыДляВыгрузкиЛкс(СтрокаСоединенияБазы, КаталогЗеркалаБазы);
	Если Асинхронно Тогда
		ирОбщий.СообщитьЛкс(ирОбщий.СтрШаблонЛкс("Будут выгружены модули расширений, кроме патчей (%1), и затем основной конфигурации", ПрефиксИмениРасширенияПатча));
	...
	ТекстЛога = ""; 
	ОтключенныеРасширения = Новый Структура;
	Для Каждого ЭлементСписка Из ВсеГруппыМетаданных Цикл 
		РасширенияГруппы = ЭлементСписка.Значение;
		Если Истина
			И ВыбранныеГруппыМетаданных <> Неопределено
			И ВыбранныеГруппыМетаданных.Найти(РасширенияГруппы) = Неопределено 
		Тогда
			Если РасширенияГруппы <> Неопределено Тогда
				ОтключенныеРасширения.Вставить(ЭлементСписка.Представление);
			КонецЕсли;
			Продолжить;
		КонецЕсли;
		Если ТипЗнч(РасширенияГруппы) <> Тип("Массив") Тогда
			РасширенияГруппы = ирОбщий.ЗначенияВМассивЛкс(РасширенияГруппы);
		КонецЕсли;
		Для Каждого РасширениеКонфигурации Из РасширенияГруппы Цикл
			КаталогГруппы = КаталогКэша;
			Если РасширениеКонфигурации <> Неопределено Тогда
				КаталогГруппы = КаталогГруппы + "\" + РасширениеКонфигурации.Имя;
			КонецЕсли;
			УдалитьФайлы(КаталогГруппы, "*.txt");
			УдалитьФайлы(КаталогГруппы, "*.ixd");
			УдалитьФайлы(КаталогГруппы, "*.icl");
			УдалитьФайлы(КаталогГруппы, "*.ipr");
			Если РасширениеКонфигурации <> Неопределено Тогда
				#Если Сервер И Не Сервер Тогда
					РасширениеКонфигурации = РасширенияКонфигурации.Создать();
				#КонецЕсли
				Если Асинхронно И ирОбщий.СтрНачинаетсяСЛкс(РасширениеКонфигурации.Имя, ПрефиксИмениРасширенияПатча) Тогда
				...
				КонецЕсли;
				Успех = ирОбщий.ВыполнитьКомандуКонфигуратораЛкс("/DumpConfigFiles """ + КаталогГруппы + """ -Extension """ + РасширениеКонфигурации.Имя + """ -Module",
			...
			Иначе
				...
				//	Успех = ирОбщий.ВыполнитьКомандуАвтоСервераБазыЛкс("infobase config export """ + КаталогГруппы + """", ТекстЛога, "Выгрузка модулей конфигурации");
				//Иначе
					Если ЧерезАвтоСервер Тогда
						...
						УдалитьФайлы(ИмяВременногоФайла);
					КонецЕсли;
					Успех = ирОбщий.ВыполнитьКомандуКонфигуратораЛкс("/DumpConfigFiles """ + КаталогГруппы + """ -Module", СтрокаСоединенияБазы, ТекстЛога, Истина, "Выгрузка модулей конфигурации");
				//КонецЕсли; 
				Если Не Успех Тогда 
					ирОбщий.СообщитьЛкс(ТекстЛога);
					Если ЛиТексСодержитБлокировкуКонфигуратораЛкс(ТекстЛога) Тогда
				...
				//КонецЕсли;
				ирОбщий.СохранитьЗначениеВФайлЛкс(1, КаталогГруппы + "\" + мПлатформа.ИмяКонтрольногоФайлаКэшаМодулей());
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
	Если РазрешитьДиалоги Тогда
		ирОбщий.СохранитьЗначениеЛкс("ОтключенныеРасширенияДляКэшаМодулей", ОтключенныеРасширения);
	КонецЕсли;
	...
	Если Метаданные.ВариантВстроенногоЯзыка = Метаданные.СвойстваОбъектов.ВариантВстроенногоЯзыка.Английский Тогда
		Для Каждого Файл Из НайтиФайлы(КаталогКэша, "*") Цикл
			Если Файл.ЭтоКаталог() Тогда
				Продолжить;
			КонецЕсли;
			Фрагменты = ирОбщий.СтрРазделитьЛкс(Файл.ИмяБезРасширения);
			Для Индекс = 0 По Фрагменты.ВГраница() Цикл
				Если Ложь
					Или Индекс % 2 = 0
					Или Фрагменты[Индекс] = "Module"
				Тогда
					Фрагменты[Индекс] = ирОбщий.ПеревестиВРусский(Фрагменты[Индекс]);
				КонецЕсли;
			КонецЦикла;
			НовоеИмя = ирОбщий.СтрСоединитьЛкс(Фрагменты, ".");
			ПереместитьФайл(Файл.ПолноеИмя, Файл.Путь + ирОбщий.РазделительПутиКФайлуЛкс() + НовоеИмя + Файл.Расширение);
		КонецЦикла;
	КонецЕсли;
...
// Возвращаемое значение:
//    Структура - 
Функция СтруктураПервыхОбновленийТабличногоПоляЛкс() Экспорт
	СтруктураОбновленияТабличногПоля = Новый Структура;
	СтруктураОбновленияТабличногПоля.Вставить("ПерваяПорцияОтображена", Ложь);
	СтруктураОбновленияТабличногПоля.Вставить("ПоследнееОбновление", Дата(1,1,1));
...
// Параметры:
//   ТабличноеПоле - ТабличноеПоле - 
//   СтруктураОбновленийТабличногПоля - Неопределено, см. СтруктураПервыхОбновленийТабличногоПоляЛкс - состояние
Процедура ПроверитьПервоеОбновлениеТабличногоПоляЛкс(Знач ТабличноеПоле, СтруктураОбновленийТабличногПоля, Знач МинимальноеЧислоСтрок = 50, знач ИнтервалОбновления = 2) Экспорт
	Если СтруктураОбновленийТабличногПоля = Неопределено Тогда
		СтруктураОбновленийТабличногПоля = СтруктураПервыхОбновленийТабличногоПоляЛкс();
	КонецЕсли;
	Если Истина
		И ТекущаяДата() - СтруктураОбновленийТабличногПоля.ПоследнееОбновление > ИнтервалОбновления
		И Не СтруктураОбновленийТабличногПоля.ПерваяПорцияОтображена 
		И ТабличноеПоле.Значение.Количество() > СтруктураОбновленийТабличногПоля.ПоследнееКоличество 
	Тогда
		ТабличноеПоле.ОбновитьСтроки();
		СтруктураОбновленийТабличногПоля.ПоследнееКоличество = ТабличноеПоле.Значение.Количество();
		СтруктураОбновленийТабличногПоля.ПоследнееОбновление = ТекущаяДата();
		Если ТабличноеПоле.Значение.Количество() >= МинимальноеЧислоСтрок Тогда
			СтруктураОбновленийТабличногПоля.ПерваяПорцияОтображена = Истина;
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Функция ТехническаяИнформацияЛкс(Знач НазваниеИнструмента = "Прочее") Экспорт
	...
	|Инструменты разработчика. Глобальные настройки всех баз: " + БулевоВСтрокуЛкс(мПлатформа.ЛиГлобальныеНастройкиВсехБаз) + "
	...
	Если ирКэш.ЛиПлатформаWindowsЛкс() Тогда
		...
		|Клиент. ОС " + ирОбщий.ВыполнитьКомандуОСЛкс("Chcp",,,,, КодировкаТекста.OEM) + "
		|";
	КонецЕсли;
	Если Не ирКэш.ЛиФайловаяБазаЛкс() Тогда
		ПингТекст = "";
		ЗамеритьПингДоСервераЛкс(ПингТекст);
		Текст = Текст + 
		"Клиент-Сервер. Пинг: " + ПингТекст + "
		|";
...
Функция ЗамеритьПингДоСервераЛкс(выхТекст = "") Экспорт
	...
	ЧислоМС = 0; 
	Если Не ирКэш.ЛиФайловаяБазаЛкс() Тогда
		Начало = ирОбщий.ТекущееВремяВМиллисекундахЛкс();
		ирСервер.ТекущаяДатаЛкс();
		ЧислоМС = ирОбщий.ТекущееВремяВМиллисекундахЛкс() - Начало;
		#Если ТолстыйКлиентОбычноеПриложение Тогда
			Поправка = 1;
		#Иначе
			Поправка = 10;
		#КонецЕсли
		ЧислоМС = Макс(0, ЧислоМС - Поправка); // https://www.hostedredmine.com/issues/1007791
	КонецЕсли;
	//ЧислоМС = 30; // Для теста
	Если ЧислоМС > 20 Тогда
		Оценка = "Плохо";
	ИначеЕсли ЧислоМС > 10 Тогда
		Оценка = "Терпимо";
	ИначеЕсли ЧислоМС > 5 Тогда
		Оценка = "хорошо";
	Иначе
		Оценка = "отлично";
	КонецЕсли;
	выхТекст = "" + ЧислоМС + "мс (" + Оценка + ")";
	Если ирКэш.ЛиСквознаяФайловаяСистемаКлиентаИСервера() Тогда 
		выхТекст = "локально " + выхТекст;
	КонецЕсли;
	Возврат ЧислоМС;
КонецФункции
...
// Параметры:
//   Код -  - фрагмент кода 
//   Запрос -  - на естественном яызке
//   ПолеПрограммы - ОбработкаОбъект.ирКлсПолеТекстаПрограммы - 
//   ЛиМодально -  - 
// Возвращаемое значение:
//    - 
Функция ОткрытьФормуЧатаИИЛкс(Знач Код = "", Знач Запрос = "", Знач ПолеПрограммы = Неопределено, Знач ЛиМодально = Ложь) Экспорт
	Форма = ПолучитьФормуЛкс("Обработка.ирПлатформа.Форма.ЧатИИ",,, Новый УникальныйИдентификатор);
	Форма.мПолеТекстаПрограммы = ПолеПрограммы;
	Форма.ТекстЗапроса = Запрос;
	Форма.КодЗапроса = Код;
	Если ЛиМодально Тогда 
		Результат = Форма.ОткрытьМодально();
	Иначе
		Форма.Открыть();
...
Функция ИдентификаторыОбъектовМДЛкс(Знач ОбновитьФайл = Ложь) Экспорт
	КаталогВыгрузки = ирКэш.Получить().КаталогФайловогоКэша;
	Файл = Новый Файл(КаталогВыгрузки + ирОбщий.РазделительПутиКФайлуЛкс() + "ConfigDumpInfo.xml");
	Если Файл.Существует() Тогда
		ТекстВопроса = ирОбщий.СтрШаблонЛкс("Обнаружен кэш идентификаторов метаданных от %1. Обновить (через конфигуратор)?", Файл.ПолучитьВремяИзменения());
		Ответ = Вопрос(ТекстВопроса, РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Нет);
		Если Ответ = КодВозвратаДиалога.Да Тогда
			ОбновитьФайл = Истина;
		КонецЕсли;
	Иначе
		ирОбщий.СообщитьЛкс("Не найден кэш идентификаторов метаданных. Выполняется обновление кэша через конфигуратор.");
		ОбновитьФайл = Истина;
	КонецЕсли;
	СтрокаСоединения = СтрокаСоединенияИнформационнойБазы();
	Если ОбновитьФайл Тогда
		Если Не ирОбщий.ВыполнитьКомандуКонфигуратораЛкс("/DumpConfigToFiles """+Файл.Путь+""" -configDumpInfoOnly", СтрокаСоединения,,,,,,,,,,, Истина,,, Истина) Тогда
			ВызватьИсключение "Не удалось обновить кэш идентификаторов метаданных (ConfigDumpInfo).";
		КонецЕсли;
	КонецЕсли;
	ТекстХМЛ = ирОбщий.ПрочитатьТекстИзФайлаЛкс(Файл.ПолноеИмя);
	ДокументДОМ = ирОбщий.ТекстВДокументDOMЛкс(ТекстХМЛ);
	Разыменователь = Новый РазыменовательПространствИменDOM(ДокументДОМ);
	Выражение = ДокументДОМ.СоздатьВыражениеXPath("/xmlns:ConfigDumpInfo/xmlns:ConfigVersions/xmlns:Metadata", Разыменователь);
	РезультатПоиска = Выражение.Вычислить(ДокументДОМ);
	ТаблицаИмен = Новый ТаблицаЗначений;
	ТаблицаИмен.Колонки.Добавить("ПолноеИмя", Новый ОписаниеТипов("Строка"));
	ТаблицаИмен.Колонки.Добавить("Идентификатор", Новый ОписаниеТипов("Строка"));
	...
	Пока Истина Цикл
		...
		Если СтрЧислоВхождений(ИмяМета, ".") > 1 Тогда
			Продолжить;
		КонецЕсли;
		ЧастиИмени = ирОбщий.СтрРазделитьЛкс(ИмяМета);
		ЧастиИмени[0] = ирОбщий.ПеревестиВРусский(ЧастиИмени[0]);
		НоваяСтрока = ТаблицаИмен.Добавить();
		НоваяСтрока.ПолноеИмя = ЧастиИмени[0] + "." + ЧастиИмени[1];
		НоваяСтрока.Идентификатор = Узел.Атрибуты[1].Значение;
	КонецЦикла;
	ТаблицаИмен.Индексы.Добавить("ПолноеИмя");
	Возврат ТаблицаИмен;
КонецФункции

// Вызывается из адаптера
Функция ЛиВФайловойБазеЕстьВключенныеРегламентныеЗаданияЛкс() Экспорт
	Результат = Истина
		И ирКэш.ЛиФайловаяБазаЛкс() 
		//И Найти(ирКэш.КоманднаяСтрокаТекущегоПроцессаОСЛкс(), "/AllowExecuteScheduledJobs -Off")>0
		;
	Если Результат Тогда
		Отбор = Новый Структура;
		Отбор.Вставить("Использование", Истина);
		Результат = РегламентныеЗадания.ПолучитьРегламентныеЗадания(Отбор).Количество()>0;
	КонецЕсли;
	Возврат Результат;
КонецФункции

// Вызывается из адаптера
Функция ОткрытьКонсольЗаданийЛкс(Знач ОтключитьВсеЗадания = Ложь, Знач АктивироватьСписокМетодов = Ложь) Экспорт
	Форма = ПолучитьФормуЛкс("Обработка.ирКонсольЗаданий.Форма");
	Форма.ПараметрОтключитьВсеЗадания = ОтключитьВсеЗадания;
	Форма.ПараметрАктивироватьСписокМетодов = АктивироватьСписокМетодов;
	Форма.Открыть();
	Возврат Форма;
КонецФункции
...
Процедура ПерезапуститьСеансПослеДобавленияРолиЛкс() Экспорт 
	Заголовок = ирОбщий.НазваниеПодсистемыЛкс(); 
	Попытка
		Ответ = Вопрос("Перезапустить сеанс для подключения ролей ИР?", РежимДиалогаВопрос.ОКОтмена,,, Заголовок);
	Исключение 
		// Запрещена модальность
		ирОбщий.СообщитьЛкс(Заголовок + ". Перезапустите сеанс.");
	КонецПопытки;
	ПерезапуститьСеансПослеДобавленияРолиДействиеЛкс(Ответ);
КонецПроцедуры 

Процедура ПерезапуститьСеансПослеДобавленияРолиДействиеЛкс(Ответ, Параметры = Неопределено) Экспорт
	Если Ответ = КодВозвратаДиалога.Отмена Тогда
		Возврат;
	КонецЕсли;
...
Процедура ОткрытьПанельИнструментовЛкс(Знач ТолькоОткрытьФормуСпискаИнструментов = Ложь, Знач ОткрытьСтраницуНастроек = Ложь, Знач ИмяЭлементаАктивировать = "") Экспорт
	...
	#Если ТолстыйКлиентОбычноеПриложение Или ТолстыйКлиентУправляемоеПриложение Тогда
		...
		Иначе
			...
			ФормаНастроек = ОткрытьФормуЛкс("Обработка.ирПортативный.Форма.Настройки"); 
			Если ОткрытьСтраницуНастроек Тогда
				...
				Если ЗначениеЗаполнено(ИмяЭлементаАктивировать) Тогда
					Элемент = ФормаНастроек.ЭлементыФормы.Найти(ИмяЭлементаАктивировать);
...
Процедура ОткрытьБазыПользователяОСЛкс(Знач РежимРабочегоСтола = Ложь) Экспорт
	Если Не ирОбщий.ПроверитьЧтоСеансТолстогоКлиентаЛкс() Тогда
		Возврат;
	КонецЕсли; 
	#Если ТолстыйКлиентОбычноеПриложение Или ТолстыйКлиентУправляемоеПриложение Тогда
		...
		Если Не ФормаВыбора.Открыта() Тогда
			...
			ФормаВыбора.РежимРабочегоСтола = РежимРабочегоСтола = Истина;
		КонецЕсли;
		ФормаВыбора.Открыть();
	#КонецЕсли
КонецПроцедуры
          
Процедура ОткрытьУправлениеПрофайлами1СЛкс(Знач ПометитьНастройкиКонфигуратора = Ложь) Экспорт
	Если Не ирОбщий.ПроверитьЧтоСеансТолстогоКлиентаЛкс() Тогда
		Возврат;
	КонецЕсли; 
	#Если ТолстыйКлиентОбычноеПриложение Или ТолстыйКлиентУправляемоеПриложение Тогда
		ФормаВыбора = ПолучитьФормуЛкс("Обработка.ирУправлениеПрофайлами1С.Форма");
		ФормаВыбора.ПараметрПометитьНастройкиКонфигуратора = ПометитьНастройкиКонфигуратора;
		ФормаВыбора.Открыть();
	#КонецЕсли
КонецПроцедуры
...
Функция Форма_ВводДоступенЛкс(Знач ЭтаФорма, Знач ПроверятьПолеHTML = Ложь) Экспорт 
	...
	Результат = Ложь
		...
		Или (Истина
			...
			// Этот антибаг работает некорректно - для неактивной формы возвращает Истина. Поэтому эту проверку имеет смысл включать только убедившись что надежным способом активной формы не нашлось
			И (Ложь
				Или (Истина
...
Процедура Форма_ВнешнееСобытиеЛкс(ЭтаФорма = Неопределено, Источник, Событие, Данные, ВводДоступен = Неопределено) Экспорт
	Если Источник = "KeyboardHook" Тогда
		...
		#Если Не ТонкийКлиент И Не ВебКлиент И Не МобильныйКлиент Тогда
			мПлатформа = ирКэш.Получить();
			мПлатформа.мДатаАктивностиПользователя = ТекущаяДата();
			Если ирОбщий.ПроверитьНажатиеКлавишЛкс(Данные, "CTRL+ALT+G", КодыКлавиш) Тогда
				Форма = ПолучитьФормуЛкс("Обработка.ирИнтерфейснаяПанель.Форма");
		...
		КонецЕсли;
		Если Ложь
			Или ирОбщий.ПроверитьНажатиеКлавишЛкс(Данные, "CTRL+~", КодыКлавиш)
		Тогда
			ОткрытьГлобальноеМенюЛкс(ЭтаФорма);
			...
			Тогда
				Если ирОбщий.ПроверитьНажатиеКлавишЛкс(Данные, "CTRL+C", КодыКлавиш) Тогда
					Попытка
					...
					КонецПопытки;
				ИначеЕсли ирОбщий.ПроверитьНажатиеКлавишЛкс(Данные, "ALT+SHIFT+V", КодыКлавиш) Тогда
					БуферОбмена_ВставитьЛкс(ЭтаФорма);
				ИначеЕсли ирОбщий.ПроверитьНажатиеКлавишЛкс(Данные, "CTRL+V", КодыКлавиш) Тогда
					БуферОбмена_ВставитьЛкс(ЭтаФорма, Ложь);
				ИначеЕсли ирОбщий.ПроверитьНажатиеКлавишЛкс(Данные, "CTRL+A", КодыКлавиш) Тогда
					...
					КонецЕсли;
				ИначеЕсли ирОбщий.ПроверитьНажатиеКлавишЛкс(Данные, "Space", КодыКлавиш) Тогда
