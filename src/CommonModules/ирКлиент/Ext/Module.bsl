...
Функция Сравнить2ЗначенияВФормеЛкс(Знач Значение1, Знач Значение2, Знач Модально = Ложь, Знач Название1 = Неопределено, Знач Название2 = Неопределено, 
	...
	Знач ОбщееНазвание = "", Знач КлючевыеКолонки = "", Знач СравниваемыеКолонки = "", Знач НомерСтроки1 = 0, Знач НомерСтроки2 = 0) Экспорт 
	...
	Тогда
		Возврат СравнитьЗначенияВФормеЧерезXMLЛкс(Значение1, Значение2, Модально, Название1, Название2, ПолучатьXMLПредставлениеДляНеизвестныхТипов, ВариантСинтаксиса, ОбщееНазвание,, КлючУникальностиФормы, НомерСтроки1, НомерСтроки2);
	КонецЕсли; 
...
//
Функция ТекстВБуферОбменаОСЛкс(Знач Значение, ВариантПросмотра = "ВстроенныйЯзык") Экспорт
	Значение = "" + Значение;
	Если Ложь
	...
	Иначе
		...
		Попытка
			ВКОбщая.УстановитьБуферОбмена(Значение);
		Исключение
			ОписаниеОшибки = ОписаниеОшибки(); // Для отладки
		КонецПопытки;
