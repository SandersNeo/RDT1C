...
Процедура ДействияФормыСформировать(Кнопка = Неопределено) Экспорт 
	...
	Если ИспользоватьНаборПолей Тогда
		...
		Для Каждого ЭлементСписка Из НаборПолей Цикл
			...
			СтрокаПоля.ОбъектМДПолноеИмя = ирОбщий.ТекстБезПоследнегоФрагментаЛкс(СтрокаПоля.ПолеПолноеИмя);
			...
			Если ирОбщий.ЛиТипВложеннойТаблицыБДЛкс(ТипТаблицы) Тогда
				...
				СтрокаПоля.ОбъектМДПолноеИмя = ирОбщий.ТекстБезПоследнегоФрагментаЛкс(СтрокаПоля.ОбъектМДПолноеИмя);
			КонецЕсли; 
