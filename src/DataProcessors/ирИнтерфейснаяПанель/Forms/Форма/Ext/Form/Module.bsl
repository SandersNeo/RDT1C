Перем мНастройкиФормы; // см. ПрочитатьНастройки
Перем КомандыПоиска;
...
Функция ОткрытьНастройкуОтчета(ИмяОтчета, ИмяНастройки)
	Отчет = Отчеты[ИмяОтчета].Создать(); // Произвольный   
...
Функция ОткрытьНастройкуКонсолиЗапросовПостроителейКомпоновок(ИмяФайлаНастроек)
	...
	Если Расширение = "RCF" Тогда
		ИмяОбъекта = "ирКонсольПостроителейОтчетов";
	ИначеЕсли Расширение = "SEL" Тогда
		ИмяОбъекта = "ирКонсольЗапросов";
	ИначеЕсли Расширение = "DCF" Тогда
		ИмяОбъекта = "ирКонсольКомпоновокДанных";
	Иначе
		Возврат Неопределено;
	КонецЕсли;
...
Функция ПолучитьФормуИзСправочникаВнешнихОбработок(Ключ)
	...
	Иначе 
		...
		Если Ложь
			Или Ссылка.Вид = Перечисления.ВидыДополнительныхОтчетовИОбработок.Отчет //@skip-check unknown-method-property
			Или Ссылка.Вид = Перечисления.ВидыДополнительныхОтчетовИОбработок.ДополнительныйОтчет //@skip-check unknown-method-property
		Тогда
...
Функция НайтиЭлементСправочникаВнешнихОбработок(Знач Ключ, ДвоичныеДанные = Неопределено) Экспорт
	...
	Иначе 
		Ссылка = Справочники.ДополнительныеОтчетыИОбработки.НайтиПоРеквизиту("ИмяОбъекта", Ключ); //@skip-check unknown-method-property
...
Процедура СгенерироватьКомандыКонтекстногоМенюПоСтроке(Знач ТабличноеПоле, Знач ТекущаяСтрока)
	...
	Тогда
		Возврат;
	КонецЕсли;
	КоманднаяПанель = ТабличноеПоле.КонтекстноеМеню;
	...
	ИначеЕсли ЗначениеЗаполнено(ВидОбъекта) И ЗначениеЗаполнено(ИмяОбъекта) Тогда
		...
		Если ирОбщий.ЛиКорневойТипТаблицыБДЛкс(КорневойТип) Тогда
			ДобавитьПодменюТаблицыБД(ДействиеОткрытиеФормы, КнопкиКоманднойПанели, ОбъектМД);
		КонецЕсли; 
...
Процедура ДобавитьПодменюФорм(ДействиеОткрытиеФормы, Знач КнопкиКоманднойПанели, Знач ОбъектМД)
	...
	КонецЦикла;
	ДобавитьКнопку(ПодменюФормы.Кнопки, "ПоискПоФормамОбъекта", "Поиск по формам объекта", , ДействиеОткрытиеФормы, ирПолучитьКартинкуЕслиЕсть("Найти"));

КонецПроцедуры

Процедура ДобавитьПодменюТаблицыБД(ДействиеОткрытиеФормы, Знач КнопкиКоманднойПанели, Знач ОбъектМД)
...
Функция ОбъектМДСтрокиСписка(Знач ТекущаяСтрока = Неопределено)
	...
	Иначе
		Если Найти(ВидОбъекта, ".") > 0 Тогда
			...
			КонецЕсли;
		Иначе
			ИмяМножественное = ирОбщий.ИмяТипаМДМножественноеИзЕдинЛкс(ВидОбъекта);
...
Процедура ПриОткрытии()
	...
	Если ПрикреплятьОкноПослеОткрытия И ПараметрКоманда = "" Тогда
...
Процедура ПриОткрытииОтложенно()
	ПоказатьСвернутьЧастое(Ложь); // Если выполнить в ПриОткрытии, то будут ломаться привязки положения при следующем открытии
	Если ПараметрКоманда = "ЗагрузитьМетаданные" Тогда
		КомандыПанелиЗагрузитьМетаданные(, ирКэш.ЛиФайловаяБазаЛкс());
	ИначеЕсли ПараметрКоманда = "ВыгрузитьМетаданные" Тогда
		КомандыПанелиВыгрузитьМетаданные();
	КонецЕсли;
	ЭтаФорма.ПараметрКоманда = "";
КонецПроцедуры
...
Функция НайтиАктивироватьСтрокуОбъектаИзПараметраФормы()
	...
	Если ЛиНашли Тогда
		...
		ЭтаФорма.ТекущийЭлемент = ЭлементыФормы.Дерево;
	КонецЕсли;
	Возврат ЛиНашли;
...
Процедура ФильтрПодсистем_НачалоВыбора(Элемент, СтандартнаяОбработка)
	...
	Если ЭтоФильтрПодсистемВыбранногоОбъекта Тогда
		...
		КонецЕсли; 
	КонецЕсли;
	ОткрытьВыборПодсистем(Элемент, КлючУникальностиОткрытия, ЭтоФильтрПодсистемВыбранногоОбъекта);
		...
		Если ТипЗнч(Команда) = Тип("КнопкаКоманднойПанели") Тогда
			...
			Попытка
				...
				Тогда
					...
					ИначеЕсли Кнопка.Имя = "ПоискПоФормамОбъекта" Тогда
						ИменаОбъектовМД = ирОбщий.ЗначенияВМассивЛкс(ОбъектМД.ПолноеИмя());
						ОткрытьПоискПоФормам(ИменаОбъектовМД);
					...
					ИначеЕсли Кнопка.Имя = "РедакторОбъектаБД" Тогда
						...
						Иначе
							НоваяФорма = ирКлиент.ОткрытьРедакторОбъектаБДЛкс(СтрокаПолногоИмени,,,,,, СтрокаПолногоИмени);
						КонецЕсли; 
...
Функция ПрочитатьНастройки() Экспорт 
	...
	НастройкиФормы = ирОбщий.ВосстановитьЗначениеЛкс("ирИнтерфейснаяПанель.Настройки");
	Если ТипЗнч(НастройкиФормы) <> Тип("Структура") тогда
		НастройкиФормы = Новый Структура;
	КонецЕсли;
	Если НЕ НастройкиФормы.Свойство("ФильтрПодсистем") тогда
		НастройкиФормы.Вставить("ФильтрПодсистем");
	КонецЕсли;
	Если НЕ НастройкиФормы.Свойство("ДеревоСтрокаПоиска") тогда
		НастройкиФормы.Вставить("ДеревоСтрокаПоиска");
	КонецЕсли;
	Если НЕ НастройкиФормы.Свойство("РазвернутыеСтроки") тогда
		НастройкиФормы.Вставить("РазвернутыеСтроки");
		НастройкиФормы.РазвернутыеСтроки = Новый Массив;
	КонецЕсли;
	Если НЕ НастройкиФормы.Свойство("ВыбраннаяСтрока") тогда
		НастройкиФормы.Вставить("ВыбраннаяСтрока");
	КонецЕсли;
	Если НЕ НастройкиФормы.Свойство("ТекущееИзбранное") тогда
		НастройкиФормы.Вставить("ТекущееИзбранное");
	КонецЕсли;
	Если НЕ НастройкиФормы.Свойство("СтатистикаИспользования") тогда
		НастройкиФормы.Вставить("СтатистикаИспользования", НоваяТаблицаСтатистики());
	КонецЕсли;	
	// Для произвольных статических веток
	Для каждого КлючЗначение Из МассивСтатическихВеток Цикл
		ИмяКолонки = КлючЗначение.Имя;
		Если НастройкиФормы.СтатистикаИспользования.Колонки.Найти(ИмяКолонки) = Неопределено Тогда
			// Эта колонка была добавлена позже, ее может не быть в сохраненных настройках
			Попытка
				НастройкиФормы.СтатистикаИспользования.Колонки.Добавить(ИмяКолонки); 
			Исключение
			КонецПопытки;
		КонецЕсли;
	КонецЦикла;          
	//! НастройкиФормы.СтатистикаИспользования.Колонки.Добавить("Избранное", Новый ОписаниеТипов("Число"));
	Если НЕ НастройкиФормы.Свойство("Каталоги") тогда
		НастройкиФормы.Вставить("Каталоги", Новый Соответствие);
	КонецЕсли;
	Если НЕ НастройкиФормы.Свойство("ПоискЧерезРегулярныеВыражения") Тогда
		НастройкиФормы.Вставить("ПоискЧерезРегулярныеВыражения", ПоискЧерезРегулярныеВыражения);
	КонецЕсли;
	Если Не ирКэш.ДоступноРегВыраженияЛкс() Тогда
		НастройкиФормы.ПоискЧерезРегулярныеВыражения = Ложь;
	КонецЕсли; 
	Если НастройкиФормы.ПоискЧерезРегулярныеВыражения Тогда
	...
	КонецЕсли;
	Если НЕ НастройкиФормы.Свойство("ИспользоватьДинамическийСписокИР") Тогда
		НастройкиФормы.Вставить("ИспользоватьДинамическийСписокИР", Истина);
	КонецЕсли;
	Если НЕ НастройкиФормы.Свойство("флИмяСиноним") Тогда
		НастройкиФормы.Вставить("флИмяСиноним", флИмяСиноним);
	КонецЕсли;
	Если НЕ НастройкиФормы.Свойство("флИскатьВПодкаталогах") Тогда
		НастройкиФормы.Вставить("флИскатьВПодкаталогах", Истина);
	КонецЕсли;
	Если НЕ НастройкиФормы.Свойство("флВложенныеПодсистемы") Тогда
		НастройкиФормы.Вставить("флВложенныеПодсистемы", Истина);
	КонецЕсли;
	Если НЕ НастройкиФормы.Свойство("СостояниеОкна") Тогда
		НастройкиФормы.Вставить("СостояниеОкна", СостояниеОкна);
	КонецЕсли;
...
Функция НоваяТаблицаСтатистики() Экспорт
	...
	Результат.Колонки.Добавить("ПоследнееИспользование", Новый ОписаниеТипов("Дата"));
	Возврат Результат;
КонецФункции
...
Процедура ЗаполнитьТабличныеПоляСтатистики(РежимЗаполненияИзбранного = Неопределено, Знач ОбновленнаяСтрока = Неопределено, Знач УстановитьТекущуюСтроку = Неопределено)
	...
	Если ОбновленнаяСтрока = Неопределено Тогда
		...
		Если СтатистикаИспользования.Найти("Справочники", "Вид") <> Неопределено Тогда
			...
			Для Каждого СтрокаСтатистики Из СтатистикаИспользования Цикл
				ИмяЕдинственное = ирОбщий.ИмяТипаМДЕдинственноеИзМножЛкс(СтрокаСтатистики.Вид);
	...
	Для каждого СтатическаяВетка Из МассивСтатическихВеток Цикл
		...
		Если ОбновленнаяСтрока <> Неопределено Тогда
			...
			Иначе
				СтрокаТабличногоПоля = ДобавитьСтрокуВТабличноеПоле(СловаФильтра, стПоиск, ОбновленнаяСтрока, ТабличноеПоле, ФильтроватьСтатистику И РежимЗаполненияИзбранного = Ложь);
		...
		Иначе
			...
			Для каждого стрСтатистика из ВыборкаСтатистики цикл
				...
				Тогда
					Прервать;
				КонецЕсли;
				СтрокаТабличногоПоля = ДобавитьСтрокуВТабличноеПоле(СловаФильтра, стПоиск, стрСтатистика, ТабличноеПоле, ФильтроватьСтатистику И РежимЗаполненияИзбранного = Ложь);
				Если СтрокаТабличногоПоля = Неопределено Тогда
					Продолжить;
				КонецЕсли;
				Если СтатическаяВетка.Имя = "Последнее" Тогда
...
Процедура ОчиститьКэшПоискаПоСтатистике(ОчищатьКэшСвязейСДеревом = Ложь)
	...
	СтатистикаИспользования = мНастройкиФормы.СтатистикаИспользования;
	КолонкаТаблицы = СтатистикаИспользования.Колонки.Найти("ПредставлениеСсылки");
	Если КолонкаТаблицы <> Неопределено Тогда
		СтатистикаИспользования.Колонки.Удалить(КолонкаТаблицы);
	КонецЕсли;
	КолонкаТаблицы = СтатистикаИспользования.Колонки.Найти("СтрокаДерева");
	Если Истина
		И ОчищатьКэшСвязейСДеревом 
		И КолонкаТаблицы <> Неопределено  
	Тогда
		СтатистикаИспользования.Колонки.Удалить(КолонкаТаблицы);
	КонецЕсли;
...
Функция ДобавитьСтрокуВТабличноеПоле(СловаФильтра, Знач стПоиск, Знач стрСтатистика, Знач ТабличноеПоле, Знач Фильтровать = Ложь)
...
Функция ПрефиксПолногоИмениМД(Знач Единственное)
	...
	Если ирОбщий.ИмяТипаМДМножественноеИзЕдинЛкс(Единственное) = Неопределено Тогда
		ПрефиксПолногоИмени = "";
...
Процедура ДобавитьВетвьМетаданных(Знач ИмяЕдинственное, Знач КартинкаВетви = 0, Знач КартинкаОбъекта = 0, Знач ИспользоватьПолноеИмя = Ложь, Знач ЕстьДоступ = Истина, 
	...
	ИмяМножественное = ирОбщий.ИмяТипаМДМножественноеИзЕдинЛкс(ИмяЕдинственное);
...
Процедура ПоискПоМетаданным()
	...
	Для каждого стрКорень из дзВсеОбъекты.Строки цикл
		...
		Для каждого стрОбъект из стрКорень.Строки цикл
			...
			ПремияФильтра = 0;
			Если Истина
				И СловаФильтра <> Неопределено
				И ЗначениеЗаполнено(СловаФильтра.Слова) 
			Тогда
				ФильтрПредставления = ирОбщий.СтрСоединитьЛкс(СловаФильтра.Слова, " ");
				Если СтрДлина(ФильтрПредставления) > 1 Тогда
					ПредставлениеОбъекта = ФильтруемоеПредставлениеСтрокиДерева(стрОбъект, Истина);
					ПремияФильтра = ирОбщий.НайтиФильтрВСловеЛкс(Ложь, ФильтрПредставления, ПредставлениеОбъекта, НРег(ПредставлениеОбъекта), Истина);
					//ПремияФильтра = Цел(ПремияФильтра / 2);
				КонецЕсли;
			КонецЕсли;
			...
			стрОбъектИнтерфейса.ПремияФильтра = ПремияФильтра;
		КонецЦикла;
		Если СловаФильтра <> Неопределено И ДочерниеСтроки <> Неопределено Тогда
			ДочерниеСтроки.Сортировать("ПремияФильтра Убыв");
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
	...
	Если мЕстьСправочникВнешнихОбработок Тогда
		...
		Если мИмяСправочникаДопОбработок = "ВнешниеОбработки" Тогда
			Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
			               ...
			               |ГДЕ
			               |	Не ВнешниеОбработки.ЭтоГруппа"; //@skip-check unknown-method-property
		Иначе 
			Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
			               |	ВнешниеОбработки.Наименование КАК Представление,
						   ...
						   |	И ВнешниеОбработки.Вид В (ЗНАЧЕНИЕ(Перечисление.ВидыДополнительныхОтчетовИОбработок.ДополнительныйОтчет), ЗНАЧЕНИЕ(Перечисление.ВидыДополнительныхОтчетовИОбработок.ДополнительнаяОбработка))"; //@skip-check unknown-method-property
		КонецЕсли;
...
Функция ЛиОбъектПроходитФильтры(Знач СтрокаДерева, Знач СтрокаПоиска = "", Знач ПроверятьПодсистему = Ложь, Знач ОбъектыВыбранныхПодсистем = Неопределено, КэшСловаФильтра = Неопределено,
	...
	Если ЗначениеЗаполнено(СтрокаПоиска) Тогда
		...
		Если ТипЗнч(СтрокаДерева) = Тип("СтрокаДереваЗначений") Тогда
			...
			Представление = НРег(ФильтруемоеПредставлениеСтрокиДерева(СтрокаДерева));
		Иначе
...
Функция ФильтруемоеПредставлениеСтрокиДерева(Знач СтрокаДерева, Знач ТолькоГлавное = Ложь) Экспорт
	Если флИмяСиноним Тогда
		Результат = СтрокаДерева.Имя;
		Если Не ТолькоГлавное Тогда
			Результат = Результат + " " + СтрокаДерева.Представление;
		КонецЕсли;
	Иначе
		Результат = СтрокаДерева.Представление;
		Если Не ТолькоГлавное Тогда
			Результат = Результат + " " + СтрокаДерева.Имя;
		КонецЕсли;
	КонецЕсли;
	Возврат Результат;
КонецФункции
...
Процедура кнНастройкаНажатие(Элемент)
	РезультатФормы = ПолучитьФорму("Настройки", ЭтаФорма).ОткрытьМодально();
...
Функция НайтиСсылкиВМетаданных(ОбъектМД)
	...
	#КонецЕсли
	Форма = НовыйОбъект.ПолучитьФорму("ОбъектМетаданных",, ОбъектМД.ПолноеИмя());
	Форма.ПараметрСтраница = "СсылкиНаОбъект";
	Возврат Форма;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник) Экспорт
	...
	Если ИмяСобытия = "ИзмененБуферОбмена" Тогда
		ОбновитьПредставлениеБуфераОбмена();
	ИначеЕсли ИмяСобытия = "ПолучитьФорму.ирИнтерфейснаяПанель" Тогда
...
Процедура КомандыПанелиОбновитьКоличествоСтрок(Кнопка)
	...
	Если Кнопка.Пометка Тогда
		...
		КонецЕсли;
	Иначе
		Дерево.Строки.Сортировать("Порядок, ПремияФильтра Убыв", Истина);
	КонецЕсли; 
...
Процедура ИзбранноеПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки) Экспорт
	...
	Если КолонкаВремя <> Неопределено И КолонкаВремя.Видимость Тогда
		ПоследнееИспользование = ДанныеСтроки.ПоследнееИспользование; //@skip-check unknown-method-property
		ОформлениеСтроки.Ячейки.Время.УстановитьТекст(Формат(ПоследнееИспользование, "ДЛФ=T"));
	КонецЕсли;
...
Процедура ВыгрузитьМетаданные(Знач ТекстСпискаОбъектов) Экспорт
	КаталогВыгрузки = ирКлиент.ГитРепозиторий(Истина);
	КаталогВыгрузкиКонфигурации = ирКлиент.ВыбратьКаталогВФормеЛкс(КаталогВыгрузки,, "Укажите каталог куда выгружать файлы метаданных");
	Если Не ЗначениеЗаполнено(КаталогВыгрузкиКонфигурации) Тогда
		Возврат;
	КонецЕсли;
	Иерархически = Истина; // Этот режим выгружает все компоненты метаданных, но для обычных форм не выделяет модуль (он внутри form.bin). А плоский режим не выгружает
	ТекстНепустойКаталог = "";
	Файлы = НайтиФайлы(КаталогВыгрузкиКонфигурации, "*");
	Если Файлы.Количество() > 0 Тогда
		//ТекстНепустойКаталог = "Выбранный каталог не пустой. ";
		Для Каждого Файл Из Файлы Цикл
			...
			КонецЕсли;
		КонецЦикла;
		Если Не Иерархически Тогда
			Ответ = Вопрос("Выбранный каталог не пустой. Удалить в нем файлы?", РежимДиалогаВопрос.ДаНетОтмена);
		...
		КонецЕсли;
	КонецЕсли;
	Успех = ирОбщий.ВыгрузитьОбъектыМетаданныхЛкс(ТекстСпискаОбъектов, КаталогВыгрузкиКонфигурации,,,,, Истина, Иерархически);
	Если Успех И Иерархически Тогда
		ИмяОбъекта = ирОбщий.ПервыйФрагментЛкс(ТекстСпискаОбъектов, Символы.ПС);
		ИмяФайла = ирКлиент.ИмяФайлаМодуляВПапкеГитаЛкс(ИмяОбъекта, КаталогВыгрузки, Ложь);
		ирКлиент.ОткрытьФайлВПроводникеЛкс(ИмяФайла);
	КонецЕсли;
КонецПроцедуры

Процедура КомандыПанелиВыгрузитьМетаданные(Кнопка = Неопределено) Экспорт
	...
	Для Каждого ТекущаяСтрока Из ЭлементыФормы.Дерево.ВыделенныеСтроки Цикл
		Если ТекущаяСтрока.Уровень() <> 1 Тогда
			Продолжить;
		КонецЕсли;
		ВыбранныеИмена.Добавить(ТекущаяСтрока.ПолноеИмяОбъекта);
	КонецЦикла; 
	ИменаТиповМетаданных = Новый Массив;
	ИменаТиповМетаданных.Добавить("КритерийОтбора");
	Форма = ирКлиент.ФормаВыбораОбъектаМетаданныхЛкс(,, ВыбранныеИмена, Истина, Истина, Истина, Истина, Истина, Истина,,, Истина,,, Истина,, Истина, Истина, Истина, Истина, Истина, Истина, Истина, ИменаТиповМетаданных);
	ВыбранныеИмена = Форма.ОткрытьМодально(); 
	Если ВыбранныеИмена = Неопределено Тогда
		Возврат;
	КонецЕсли;
	ТекстСпискаОбъектов = "";
	Для Каждого ПолноеИмяОбъекта Из ВыбранныеИмена Цикл
		ТекстСпискаОбъектов = ТекстСпискаОбъектов + ПолноеИмяОбъекта + Символы.ПС;
		Попытка
			МетаФормы = Метаданные.НайтиПоПолномуИмени(ПолноеИмяОбъекта).Формы;
		Исключение
	...
	Если Не ЗначениеЗаполнено(ТекстСпискаОбъектов) Тогда
		...
		Возврат;
	КонецЕсли; 
	ВыгрузитьМетаданные(ТекстСпискаОбъектов);
КонецПроцедуры

Процедура КомандыПанелиЗагрузитьМетаданные(Кнопка = Неопределено, Знач ЧерезЗеркалоБазы = Ложь) Экспорт
	ТекстовыйДокумент = Новый ТекстовыйДокумент;
	КаталогГита = ирКлиент.ГитРепозиторий(Истина);
	КаталогВыгрузкиКонфигурации = ирКлиент.ВыбратьКаталогВФормеЛкс(КаталогГита,, "Укажите корневой каталог конфигурации для поиска загружаемых метаданных");
	Если Не ЗначениеЗаполнено(КаталогВыгрузкиКонфигурации) Тогда 
		Возврат;
	КонецЕсли;
	РазделительПутиКФайлу = ирОбщий.РазделительПутиКФайлуЛкс();
	ФайлыКорневыхОбъектов = Новый Массив;
	Иерархически = Ложь;
	МаркерДочернегоОбъекта = СтрЗаменить("\Ext\", "\", ирОбщий.РазделительПутиКФайлуЛкс());
	МаркерФормы = СтрЗаменить("\Forms\", "\", ирОбщий.РазделительПутиКФайлуЛкс());
	ГлубинаКорня = СтрЧислоВхождений(КаталогВыгрузкиКонфигурации, РазделительПутиКФайлу) + 1;
	Для Каждого ФайлОбъекта Из НайтиФайлы(КаталогВыгрузкиКонфигурации, "*.xml", Истина) Цикл
		#Если Сервер И Не Сервер Тогда
			ФайлОбъекта = Новый файл;
		#КонецЕсли
		Если Ложь
			Или ФайлОбъекта.ЭтоКаталог() 
			Или Найти(ФайлОбъекта.Путь, МаркерДочернегоОбъекта)
			Или ФайлОбъекта.Имя = "Configuration.xml"
			Или ФайлОбъекта.Имя = "ConfigDumpInfo.xml"
			Или (Истина // Для ускорения
				И СтрЧислоВхождений(ФайлОбъекта.Путь, РазделительПутиКФайлу) - ГлубинаКорня > 1 
				И Найти(ФайлОбъекта.Путь, МаркерФормы) = 0)
		Тогда
			Продолжить;
		КонецЕсли;
		ФайлыКорневыхОбъектов.Добавить(ФайлОбъекта);
		Иерархически = ГлубинаКорня < СтрЧислоВхождений(ФайлОбъекта.ПолноеИмя, РазделительПутиКФайлу);
	КонецЦикла;
	СписокФайлов = Новый СписокЗначений;
	ВыделенныеИменаМД = Новый Соответствие;
	Для Каждого ВыделенныеЭлемент Из ЭлементыФормы.Дерево.ВыделенныеСтроки Цикл
		Если Иерархически Тогда
			ИмяФайлаМД = ирКлиент.ИмяФайлаМодуляВПапкеГитаЛкс(ВыделенныеЭлемент.ПолноеИмяОбъекта, КаталогВыгрузкиКонфигурации, Ложь) + ".xml";
		Иначе
			ИмяФайлаМД = мПлатформа.ИмяФайлаМодуляИзИмениМодуля(ВыделенныеЭлемент.ПолноеИмяОбъекта, "xml");
		КонецЕсли;
		ВыделенныеИменаМД[ИмяФайлаМД] = 1;
	КонецЦикла;
	ТекущийЭлементСписка = Неопределено;
	Для Каждого ФайлКорневогоОбъекта Из ФайлыКорневыхОбъектов Цикл
		ЭлементСписка = СписокФайлов.Добавить(ФайлКорневогоОбъекта.ПолноеИмя);
		Если Иерархически Тогда
			ИмяФайлаМД = ФайлКорневогоОбъекта.ПолноеИмя;
		Иначе
			ИмяФайлаМД = ФайлКорневогоОбъекта.Имя;
		КонецЕсли;
		ЭлементСписка.Пометка = ВыделенныеИменаМД[ИмяФайлаМД] <> Неопределено;
		Если ЭлементСписка.Пометка Тогда
			ТекущийЭлементСписка = ЭлементСписка;
		КонецЕсли;
	КонецЦикла;
	СписокФайлов.СортироватьПоЗначению();
	РезультатВыбора = ирКлиент.ВыбратьЭлементСпискаЗначенийЛкс(СписокФайлов, ТекущийЭлементСписка,, "Будут загружены объекты метаданных", Истина);
	Если РезультатВыбора = Неопределено Тогда
		Возврат;
	КонецЕсли;
	ТекстЛога = "";
	ФайлСпискаФайлов = Новый Файл(ПолучитьИмяВременногоФайла("txt"));
	ТекстСпискаОбъектов = ирОбщий.СтрСоединитьЛкс(ирОбщий.ВыгрузитьСвойствоЛкс(РезультатВыбора, "Значение"), Символы.ПС);
	ТекстовыйДокумент.УстановитьТекст(ТекстСпискаОбъектов);
	ТекстовыйДокумент.Записать(ФайлСпискаФайлов.ПолноеИмя);
	Формат = ирОбщий.ФорматВыгрузкиМетаданныхЛкс(Иерархически);
	Успех = ирОбщий.ВыполнитьКомандуКонфигуратораЛкс("/LoadConfigFromFiles """ + КаталогВыгрузкиКонфигурации + """ -listFile """ + ФайлСпискаФайлов.ПолноеИмя + """" + Формат,
		СтрокаСоединенияИнформационнойБазы(), ТекстЛога,,,,,,,,,, Истина,,, ЧерезЗеркалоБазы);
	УдалитьФайлы(ФайлСпискаФайлов.ПолноеИмя);
	Если Успех И Не ЧерезЗеркалоБазы Тогда
		...
		Для Каждого ИмяОбъекта Из ИменаОбъектов Цикл
			...
			Иначе
				...
				Если ОбъектМД <> Неопределено Тогда
					...
					ОписаниеОбъекта.Вид = ирОбщий.ИмяТипаМДМножественноеИзЕдинЛкс(ирОбщий.ПервыйФрагментЛкс(ИмяОбъекта));
...
Процедура КомандыПанелиПоискПоТекстамФорм(Кнопка)
	Коллекция = ирОбщий.ВсеСтрокиДереваЗначенийЛкс(Дерево);
	Для Каждого СтрокаДерева Из Дерево.Строки Цикл
		ирОбщий.УдалитьЗначениеИзМассиваЛкс(Коллекция, СтрокаДерева);
	КонецЦикла;
	ИменаОбъектовМД = ирОбщий.ВыгрузитьСвойствоЛкс(Коллекция, "ПолноеИмяОбъекта"); 
	ОткрытьПоискПоФормам(ИменаОбъектовМД);
КонецПроцедуры

//.
// Параметры:
//   ИменаОбъектовМД - Массив из Строка - 
Процедура ОткрытьПоискПоФормам(Знач ИменаОбъектовМД) Экспорт
	Форма = ирКэш.Получить().ПолучитьФорму("СтруктураФормы",, "ПоСпискуОбъектов");
	Форма.ИменаОбъектовМД = ИменаОбъектовМД;
	Форма.Открыть();
...
Иначе
	...
	КонецПопытки; 
КонецЕсли;
мНастройкиФормы = ПрочитатьНастройки(); 
//мНастройкиФормы.СтатистикаИспользования.Добавить().
