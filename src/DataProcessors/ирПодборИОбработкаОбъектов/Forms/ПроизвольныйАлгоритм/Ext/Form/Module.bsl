...
Функция КлсПолеТекстаПрограммыОбновитьКонтекст(Знач Компонента = Неопределено, Знач Кнопка = Неопределено) Экспорт 
	...
	Если ВладелецФормы <> Неопределено Тогда
		Компонента.УстановитьКлиентСерверКонтекст(ВладелецФормы.ВыполнятьНаСервере); 
	КонецЕсли; 
	// Локальный
	Для Каждого СтрокаПараметра Из мТабличноеПолеПараметры.Значение Цикл
...
Процедура вВыполнитьОбработку(Кнопка = Неопределено) Экспорт 
	...
	Тогда
		...
		Иначе
			//! мТабличноеПолеПараметры.Значение = 0 // ТабличнаяЧасть
		...
		Если РедакторАлгоритма.мФлагиКомпиляции.Сервер Тогда
			КонтекстПроверки = "Сервер";
		КонецЕсли;
		Если Открыта() И Не РедакторАлгоритма.ПроверитьПрограммныйКод(,, КонтекстПроверки) Тогда
			Возврат;
		КонецЕсли;
		Если Истина
...
Процедура ПодготовитьГлобальныйКонтекст()
	РедакторАлгоритма.ПодготовитьГлобальныйКонтекст();
КонецПроцедуры

Процедура ПриЗакрытии()
	
	мТабличноеПолеПараметры = Неопределено; // Для подстраховки от циклических ссылок
	Если ВладелецФормы <> Неопределено Тогда
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если Не РедакторАлгоритма.ЛиМожноЗакрытьФорму() Тогда
		Отказ = Истина;
		Возврат;
	КонецЕсли;
...
#Если Сервер И Не Сервер Тогда
	...
	ВладелецФормы = ПолучитьФорму();
#КонецЕсли
мИспользоватьНастройки = Истина;
//Реквизиты настройки и значения по умолчанию.
мНастройка = Новый Структура("ТекстПроизвольногоАлгоритма, ПринудительнаяЗапись, Параметры", "", Ложь);
мНастройка.Параметры = Новый ТаблицаЗначений; // см. Параметры.СкопироватьКолонки("Имя, Значение")
Если ирКэш.ЛиПортативныйРежимЛкс() Тогда
	...
	#КонецЕсли
	ЭлементыФормы.Удалить(ЭлементыФормы.Параметры);
	мТабличноеПолеПараметры = ЭлементыФормы.СкрытыеПараметры;
...
//Если ирКэш.ЛиПортативныйРежимЛкс() Тогда // Сделал безусловно чтобы анализатор кода знал колонки и для этого случая
