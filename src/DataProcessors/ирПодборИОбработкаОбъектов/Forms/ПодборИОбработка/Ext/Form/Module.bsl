...
Процедура НастроитьЭлементыФормы()
	...
	ирОбщий.ОбновитьТекстПослеМаркераЛкс(ЭтаФорма.Заголовок,, ирОбщий.ПредставлениеСОбрезкойДлиныЛкс(ОбластьПоиска, 50), ": ");
...
Функция ПолучитьИсполняемуюКомпоновку(выхНастройкаКомпоновки = Неопределено, выхСхемаКомпоновки = Неопределено, ТолькоВычислитьКоличество = Ложь, Знач ТолькоКлючевыеПоля = Ложь)
	...
	СписокПолейКлюча = ирОбщий.СтруктураКлючаТаблицыБДЛкс(ИмяПервойТаблицыБД(),, Ложь);
	...
	Иначе
		...
		КонецЦикла; 
		Если АвтозаполнениеПорядка Тогда
			ПостроительПорядка = ирОбщий.ОсновнойПорядокТаблицыБДЛкс(ИмяПервойТаблицыБД(), ирОбщий.ВыражениеПорядкаКомпоновкиНаЯзыкеЗапросовЛкс(выхНастройкаКомпоновки.Порядок),,, Истина);
...
Функция ПолучитьЗапрос(ПолноеИмяМД, ПредставленияПолей = Неопределено)
	...
	Если ПодключатьСвязанныеТаблицы Тогда 
		Если ирОбщий.ЛиКорневойТипСсылкиЛкс(ТипТаблицы) Тогда
			...
			КонецЕсли;
		ИначеЕсли ирОбщий.ЛиКорневойТипРегистраСведенийЛкс(ТипТаблицы) И Не МноготабличнаяВыборка Тогда
			...
			КонецЕсли; 
		ИначеЕсли ирОбщий.ЛиКорневойТипРегистраНакопленияЛкс(ТипТаблицы) И Не МноготабличнаяВыборка Тогда
			МетаРегистр = ОбъектыМД[0];
			#Если Сервер И Не Сервер Тогда
				МетаРегистр = Метаданные.РегистрыНакопления.ОстаткиДнейДополнительныхОтпусков;
			#КонецЕсли
...
Функция НастроитьКомпоновщик(СохранятьТекущиеНастройки = Истина, ЗагружатьТекущиеНастройки = Ложь, НастройкиНабораТаблицБД = Неопределено, ИмяПоляКоличестваСтрок = "")
	...
	лСхемаКомпоновки = ирОбщий.СоздатьСхемуКомпоновкиПоЗапросуЛкс(Запрос);
	#Если Сервер И Не Сервер Тогда
	...
	Если ИмяСиноним Тогда
		ВременнаяСхема = ирОбщий.СоздатьСхемуКомпоновкиПоЗапросуЛкс(Запрос);
	...
	КонецЕсли; 
	СхемаКомпоновки = ирОбщий.СоздатьСхемуКомпоновкиПоЗапросуЛкс(Запрос,, ПредставленияПолей,, ПредставленияПапок);
	#Если Сервер И Не Сервер Тогда
...
Процедура ПриОткрытии()
	...
	ирКлиент.Форма_ПриОткрытииЛкс(ЭтаФорма,,, Новый Структура("ПриОткрытии"), "por", СоставНастройкиФормы);
	#Если Сервер И Не Сервер Тогда
...
Процедура ПриЗакрытии()
	...
	мСтарыеКолонкиТабличногоПоляРезультата = Неопределено; // Разрываем странное кольцо ссылок, которое удерживает форму https://www.hostedredmine.com/issues/1002584
	...
	Иначе
		...
		Если ирОбщий.ЛиТаблицаБДСуществуетЛкс(ОбластьПоиска, Истина) Тогда
			...
			Если МетаданныеОбъекта <> Неопределено Тогда
				...
				мИскомыйОбъект.Вставить("МетаОбъект", МетаданныеОбъекта); // Массив из ОбъектМетаданных, ОбъектМетаданных -
				ЕстьТаблицаИзменений = Истина
...
Процедура КП_ДоступныеПоляКолонкаБД(Кнопка)
	...
	Тогда
		Возврат;
	КонецЕсли;
	ирКлиент.ОткрытьКолонкуБДДоступногоПоляКомпоновкиЛкс(ЭлементыФормы.КомпоновщикДоступныеПоля, ИмяПервойТаблицыБД());
...
Функция ПоследниеОтборыНажатие(Кнопка) Экспорт
	...
	НастройкаКомпоновки = ирКлиент.ВыбранныйЭлементПоследнихЗначенийЛкс(ЭтаФорма, ОбластьПоиска, Кнопка, "Отборы");
	#Если Сервер И Не Сервер Тогда
		НастройкаКомпоновки = Новый НастройкиКомпоновкиДанных;
	#КонецЕсли
...
Процедура КПОтборДобавитьЭлемент(Кнопка)
	ирКлиент.ДобавитьСтрокуВТабличноеПолеОтбораЛкс(ЭлементыФормы.КомпоновщикОтбор, 0);
КонецПроцедуры

Процедура КПОтборДобавитьГруппуИ(Кнопка)
	ирКлиент.ДобавитьСтрокуВТабличноеПолеОтбораЛкс(ЭлементыФормы.КомпоновщикОтбор, 2);
КонецПроцедуры

Процедура КПОтборДобавитьГруппуИли(Кнопка)
	ирКлиент.ДобавитьСтрокуВТабличноеПолеОтбораЛкс(ЭлементыФормы.КомпоновщикОтбор, 3);
КонецПроцедуры
