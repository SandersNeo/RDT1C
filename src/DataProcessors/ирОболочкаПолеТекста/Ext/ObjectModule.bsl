...
Процедура УстановитьГраницыВыделения(Знач НачальнаяСтрокаИлиПозиция, Знач НачальнаяКолонкаИлиКонечнаяПозиция, Знач КонечнаяСтрока = Неопределено, Знач КонечнаяКолонка = Неопределено,
	...
	Если ЛиПолеHTMLДокумента() Тогда
		...
		Если РедакторHTML = Неопределено Тогда
			...
			Иначе                                                               
				Выделение.Вставить("Начало", ПозицияВПолеТекстаПоНомеруСтрокиИКолонки(НачальнаяСтрокаИлиПозиция, НачальнаяКолонкаИлиКонечнаяПозиция));
				Выделение.Вставить("Конец", ПозицияВПолеТекстаПоНомеруСтрокиИКолонки(КонечнаяСтрока, КонечнаяКолонка));
			КонецЕсли;
...
Функция ПолучитьТекст(Знач ЛиСырой = Ложь, Знач ДляПоиска = Ложь) Экспорт
	...
	Если ЛиПолеHTMLДокумента() Тогда
		...
		ИначеЕсли РедакторHTML = Неопределено Тогда
			...
			КонецЕсли;
			//! ДокументHtml.documentElement = 0; // ОбработкаОбъект.ирТипHTMLElement
			Если Истина
				И ЛиСырой 
				И (Ложь
...
//   НачальныйТекст -  - для отображения изменений
// Возвращаемое значение:
//    - 
Функция УстановитьТекст(Знач Текст, Знач Активировать = Ложь, Знач НачальныйТекст = Неопределено, Знач СохранитьГраницыВыделения = Ложь) Экспорт 
	...
	Если ЛиПолеHTMLДокумента() Тогда
		...
		Иначе
			...
			Если НачальныйТекст <> Неопределено Тогда
				РедакторHTML.setOriginalText(НачальныйТекст);
			КонецЕсли;
			УстановитьДекорации();
		КонецЕсли; 
	ИначеЕсли ТипЗнч(ЭлементФормы) = Тип("ПолеВвода") Тогда
...
Процедура УстановитьДекорации(Знач МассивСтрок = Неопределено) Экспорт 
	...
	КонецЕсли;
	РедакторHTML = РедакторHTML();
	РедакторHTML.setLineNumbersDecorations(ирОбщий.ОбъектВТекстЖСОНЛкс(МассивСтрок));
...
// Устанавливает или читает выделенный текст.
...
Процедура ВставитьТекст(Знач НовыйТекст, Знач ДобавитьНовуюСтрокуПосле = Истина) Экспорт 
	...
	КонецЕсли;
	УстановитьГраницыВыделения(НачальнаяСтрока, НачальнаяКолонка, НачальнаяСтрока, НачальнаяКолонка); // Антибаг платформы https://www.hostedredmine.com/issues/1007564
...
//.
// Параметры:
//   ВКОбщая - ОбработкаОбъект.ирТипКомпонентаОбщая - 
//   Форма -  - 
//   Панель -  - 
//   СмещениеГориз -  - 
//   СмещениеВерт -  - 
Процедура ПолучитьПозициюКаретки(Знач ВКОбщая, Знач Форма, Знач Панель = Неопределено, Знач СмещениеГориз = 0, Знач СмещениеВерт = 0) Экспорт 
	...
	Иначе 
		...
		КонецЕсли;
		//ЧерезUIAutomation = ирКэш.НомерИзданияПлатформыЛкс() = "85"; // Для 8.5 требуется
		ЧерезUIAutomation = Истина;
		ВКОбщая.ПолучитьПозициюКаретки(СмещениеГориз, СмещениеВерт, ЧерезUIAutomation);
	КонецЕсли; 
...
Процедура Инициировать(ФормаВладелец) Экспорт 
	...
	// Убираем полосы прокрутки 1С. Комментарий нельзя вставлять в стиль!
	|body {
	|	overflow: hidden; 
	|}
	...
	|}
	|";
	мРедакторHTML.document.head.appendChild(ЭлементСтиля); //@skip-check unknown-method-property
...
Процедура Прочитать(Знач ИмяФайла, Знач ТипФайла = Неопределено) Экспорт 
	Если ТипЗнч(ЭлементФормы) = Тип("ТекстовыйДокумент") Тогда
		ЭлементФормы.Прочитать(ИмяФайла, ТипФайла);
	Иначе
		ТекстовыйДокумент = Новый ТекстовыйДокумент;
...
Функция РазметитьТекстРезультатамиПоиска(Знач Текст, Знач РезультатыПоиска, Знач ИндексГруппы = -1, Знач ЛиСыройТекст = Ложь) Экспорт
	...
	//! РезультатыПоиска.Колонки.Добавить("ЛиГиперссылка");
...
Процедура ВыделитьУзелХМТЛ(Знач ТегГруппы, Знач Центрировать) Экспорт
	ДокументHtml = ЭлементФормы.Документ;
	ТелоДокумента = ДокументHtml.body; // IHTMLBodyElement {HTMLFile}
	Если ирКэш.ДоступноБраузерWebKitЛкс() Тогда
...
// Параметры:
//   ВыделениеВТексте -  - 
//   РодительскийУзел - ОбработкаОбъект.ирТипHTMLElement - 
//   Центрировать - Булево - по родительскому элементу, если он передан
Процедура УстановитьВыделениеВДокументеHTML(Знач ВыделениеВТексте, РодительскийУзел = Неопределено, Знач Центрировать = Ложь) Экспорт 
	...
	ДокументHtml = ЭлементФормы.Документ;
	ТелоДокумента = ДокументHtml.body; // ОбработкаОбъект.ирТипHTMLElement
	...
	Если ирКэш.ДоступноБраузерWebKitЛкс() Тогда
		...
		НачалоНайдено = Ложь;
		СчетчикСимволов = 0;
		ОбходчикУзлов = ДокументHtml.createTreeWalker(РодительскийУзел, 4, null, false);
		Пока Истина Цикл
			ТекстовыйУзел = ОбходчикУзлов.nextNode();
			...
			Если Истина
				...
				И ВыделениеВТексте.Начало <= endCharCount
			Тогда 
		...
		Если РодительскийУзел = ТелоДокумента Тогда
			ЭлементХтмл = Диапазон.startContainer; // ОбработкаОбъект.ирТипHTMLElement
			ЭлементХтмл.parentElement.scrollIntoViewIfNeeded();
		КонецЕсли;
	Иначе
