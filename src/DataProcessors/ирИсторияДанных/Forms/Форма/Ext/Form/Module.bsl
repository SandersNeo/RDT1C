...
Процедура ПриОткрытии()
	...
	КПТипыОбновить();
	ОбновитьДоступность();
	ЭлементыФормы.КПТипы.Кнопки.ОткрытьЗапросСУБД.Доступность = Не ирКэш.ЛиФайловаяБазаЛкс();
	ЭлементыФормы.КонтекстноеМенюТиты.Кнопки.ОткрытьЗапросСУБД.Доступность = Не ирКэш.ЛиФайловаяБазаЛкс();
	ЭлементыФормы.ВычислятьРазмеры.Доступность = Не ирКэш.ЛиФайловаяБазаЛкс();
...
Процедура КПТипыОбновить(Кнопка = Неопределено) Экспорт 
	...
	#Если Сервер И Не Сервер Тогда
		РассчитатьИтогиИсторииПоТипам(ПараметрыЗадания);
...
Процедура ОбновитьТипыЗавершение(СостояниеЗадания = Неопределено, РезультатЗадания = Неопределено) Экспорт 
	...
	Тогда
		...
		Если ЗначениеЗаполнено(ПараметрКлючОбъекта) Тогда
			ПолноеИмяМД = Метаданные.НайтиПоТипу(ирОбщий.ТипОбъектаБДЛкс(ПараметрКлючОбъекта)).ПолноеИмя();
			НайденнаяСтрока = Типы.Найти(ПолноеИмяМД, "ПолноеИмяМД");
			Если НайденнаяСтрока <> Неопределено Тогда
				ЭлементыФормы.Типы.ТекущаяСтрока = НайденнаяСтрока;
			КонецЕсли; 
...
Процедура ТипыПриАктивизацииСтроки(Элемент)
	...
	ОчиститьСписокСсылок();
	
КонецПроцедуры

Процедура ОчиститьСписокСсылок()
	...
	ЭтоСсылочныйТип = ирОбщий.ЛиКорневойТипСсылкиЛкс(ОбъектМД.ПолноеИмя());
...
Процедура ДанныеПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки) Экспорт
	...
	Если ЭлементыФормы.Типы.ТекущаяСтрока <> Неопределено Тогда
		...
		ирКлиент.ЗаполнитьИзмененныеПоляВСтрокеВерсииДанныхЛкс(ДанныеСтроки,, ЭлементыФормы.Типы.ТекущаяСтрока.ПолноеИмяМД);
	КонецЕсли;
...
Функция КлючОбъектаВерсии(Знач СтрокаТаблицыВерсий = Неопределено)
	...
	Если СтрокаТаблицыВерсий = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;
	СсылкаОбъекта = ирКлиент.КлючОбъектаНайденнойСсылкиЛкс(СтрокаТаблицыВерсий, ЭлементыФормы.Типы.ТекущаяСтрока.ПолноеИмяМД);
...
Процедура ДанныеВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	...
	Если Колонка = ЭлементыФормы.Версии.Колонки.Данные Тогда
		...
		Иначе
			ОткрытьЗначение(КлючОбъектаВерсии);
		КонецЕсли; 
	ИначеЕсли Колонка = ЭлементыФормы.Версии.Колонки.Транзакция Тогда
		ирКлиент.ОткрытьЖурналРегистрацииПоТранзакцииЛкс(ВыбраннаяСтрока.Транзакция, ВыбраннаяСтрока.Данные);
	Иначе
...
Процедура КП_ОбновитьВерсии(Кнопка = Неопределено)
	...
	#Если Сервер И Не Сервер Тогда
		ВыбратьВерсииПоОбъектуМД(ПараметрыЗадания);
...
Процедура ОбновитьВерсииЗавершение(СостояниеЗадания = Неопределено, РезультатЗадания = Неопределено) Экспорт 
	...
	Если Ложь
		...
		Или СостояниеЗадания = СостояниеФоновогоЗадания.Завершено 
	Тогда
		СостояниеСтрокВерсии = ирКлиент.ТабличноеПолеСостояниеСтрокЛкс(ЭлементыФормы.Версии, "Данные, ДанныеСтрока, НомерВерсии"); 
		Версии.Очистить();
		...
		Для Каждого СтрокаВыборки Из РезультатЗадания.ТаблицаВерсий Цикл
			...
			Если НадоСериализоватьКлюч Тогда
				СтрокаВерсии.ДанныеСтрока = ЗначениеВСтрокуВнутр(СтрокаВыборки.Данные);
			КонецЕсли;
			СтрокаВерсии.ИзмененныеПоля = "?";
		КонецЦикла;
		ирКлиент.ТабличноеПолеВосстановитьСостояниеСтрокЛкс(ЭлементыФормы.Версии, СостояниеСтрокВерсии);
		Если ЗначениеЗаполнено(ПараметрКлючОбъекта) Тогда
			СтрокаВерсии = Версии.НайтиСтроки(Новый Структура("Данные, ДанныеСтрока", ПараметрКлючОбъекта));
		...
		Если ЗначениеЗаполнено(ПараметрНомерВерсии) Тогда
			СтрокаВерсии = Версии.НайтиСтроки(Новый Структура("Данные, ДанныеСтрока, НомерВерсии", ПараметрКлючОбъекта, ПараметрНомерВерсии));
			Если СтрокаВерсии.Количество() > 0 Тогда
				ЭлементыФормы.Версии.ТекущаяСтрока = СтрокаВерсии[0];
			Иначе
...
Процедура ПроверятьНаличиеВерсийПриИзменении(Элемент = Неопределено)
	...
	Если ВычислятьКоличествоОтобрано Тогда
		КПТипыОбновить();
		ЭлементыФормы.Типы.Колонки.ОтобраноВерсий.Видимость = Истина;
	КонецЕсли; 
...
Процедура КПТипыУдалитьВерсии(Кнопка)
	...
	Если Ответ = КодВозвратаДиалога.ОК Тогда
		...
		Для Каждого ВыделеннаяСтрока Из ВыделенныеСтроки Цикл
			...
			ВыделеннаяСтрока.ОтобраноВерсий = 0;
			ВыделеннаяСтрока.ЧислоВерсийГотовых = 0;
			ВыделеннаяСтрока.РазмерВерсийГотовыхКБ = 0;
		КонецЦикла;
		КП_ОбновитьВерсии();
	КонецЕсли;
...
Процедура ПоляИспользованиеПриИзменении(Элемент)
	...
	Для Каждого СтрокаПоля Из Поля Цикл
		...
		Если СтрокаПоля.Изменено Тогда
			ИспользованиеПолей.Вставить(СтрокаПоля.ИмяПоля, СтрокаПоля.Использование);
		КонецЕсли; 
	КонецЦикла;
	ТекущаяСтрока = ЭлементыФормы.Типы.ТекущаяСтрока;
	НастройкиИстории = ирОбщий.УстановитьИспользованиеИсторииДанныхЛкс(ТекущаяСтрока.ПолноеИмяМД,, ИспользованиеПолей);
	ОбъектМД = ирКэш.ОбъектМДПоПолномуИмениЛкс(ТекущаяСтрока.ПолноеИмяМД);
	ЗаполнитьНастройкиИсторииВСтрокеТипа(НастройкиИстории[0], ОбъектМД, ТекущаяСтрока, Истина);
...
Процедура ТипыИспользованиеПриИзменении(Элемент)
	...
	ТекущаяСтрока = ЭлементыФормы.Типы.ТекущаяСтрока;
	ирОбщий.УстановитьИспользованиеИсторииДанныхЛкс(ТекущаяСтрока.ПолноеИмяМД, ТекущаяСтрока.Использование);
	ОбновитьКолонкуИзмененоВСтрокеТипа(ТекущаяСтрока);
...
Процедура КП_ДанныеОтборВерсий(Кнопка)
	...
	КлючОбъекта = ирКлиент.КлючОбъектаНайденнойСсылкиЛкс(Версии[0], ЭлементыФормы.Типы.ТекущаяСтрока.ПолноеИмяМД);
...
Процедура СписокСсылокНачалоВыбора(Элемент, СтандартнаяОбработка)
	...
	ирКлиент.РедактироватьСписокЗначенийЛкс(Элемент.Значение);
...
Процедура ОбновитьДоступность()
	ЭлементыФормы.Типы.Колонки.РазмерВерсийГотовыхКБ.Видимость = ВычислятьРазмеры;
	ЭлементыФормы.Типы.Колонки.РазмерВерсийСырыхКБ.Видимость = ВычислятьРазмеры;
	ЭлементыФормы.Типы.Колонки.ЧислоВерсийГотовых.Видимость = ВычислятьРазмеры;
	ЭлементыФормы.Типы.Колонки.ЧислоВерсийСырых.Видимость = ВычислятьРазмеры;
КонецПроцедуры 
...
Процедура КонтекстноеМенюТипыОткрытьЗапросСУБД(Кнопка)
	Если ЭлементыФормы.Типы.ТекущаяСтрока = Неопределено Тогда
		Возврат;
	КонецЕсли;
	ПолноеИмяМД = ЭлементыФормы.Типы.ТекущаяСтрока.ПолноеИмяМД;
	ИдентификаторыОбъектовМД = ирКэш.ИдентификаторыОбъектовМДЛкс();
	СтрокаИД = ИдентификаторыОбъектовМД.Найти(ПолноеИмяМД, "ПолноеИмя");
	Если СтрокаИД = Неопределено Тогда
		ирОбщий.СообщитьЛкс("В кэше не найден идентификатор объекта метаданных: " + ПолноеИмяМД);
		Возврат;
	КонецЕсли;
	ТипСУБД = ирОбщий.ПараметрыСоединенияЭтойСУБДЛкс().ТипСУБД;
	ИдентификаторМД = ирОбщий.ПолучитьГУИДИнверсныйИзПрямогоЛкс(СтрокаИД.Идентификатор);
	Если ТипСУБД = "PostgreSQL" Тогда
		ТекстЗапросаВерсий = ПолучитьМакет("ВерсииПоТипуPostgreSQL").ПолучитьТекст();
		ТекстЗапросаВерсий = ирОбщий.СтрЗаменитьЛкс(ТекстЗапросаВерсий, "_MetadataId = $1", "_MetadataId = '\x" + ИдентификаторМД + "'");
	Иначе
		ТекстЗапросаВерсий = ПолучитьМакет("ВерсииПоТипуMSSQL").ПолучитьТекст();
		ТекстЗапросаВерсий = ирОбщий.СтрЗаменитьЛкс(ТекстЗапросаВерсий, "_MetadataId = ?", "_MetadataId = 0x" + ИдентификаторМД);
	КонецЕсли; 
	ирКлиент.ОткрытьЗапросСУБДЛкс(ТекстЗапросаВерсий, "Версии " + ПолноеИмяМД);
КонецПроцедуры

Процедура КПТипыОбновитьРазмеры(Кнопка)
	ВычислитьРазмерыТаблицВФорме();
КонецПроцедуры

Процедура ВычислитьРазмерыТаблицВФорме() Экспорт
	СостояниеТипы = ирКлиент.ТабличноеПолеСостояниеСтрокЛкс(ЭлементыФормы.Типы, "ПолноеИмяМД");
	ВычислитьРазмерыТаблиц();
	ирКлиент.ТабличноеПолеВосстановитьСостояниеСтрокЛкс(ЭлементыФормы.Типы, СостояниеТипы);
КонецПроцедуры
