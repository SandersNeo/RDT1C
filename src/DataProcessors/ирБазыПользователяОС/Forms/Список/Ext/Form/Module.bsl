...
Перем мХранимыеДанные;
Перем мНастройкаФормы;

Функция СохраняемаяНастройкаФормы(выхНаименование, выхИменаСвойств) Экспорт 
	СохранитьНаборПараметров();
	выхИменаСвойств = "";
	Результат = Новый Структура;
	Результат.Вставить("ПоказыватьОсобыйЗапуск", ПоказыватьОсобыйЗапуск);
...
Функция СвойстваГлобализуемые() Экспорт
	Результат = Новый Структура;
	Результат.Вставить("БазыПользователяСтрокаПоиска");
	Результат.Вставить("ДополнительныеПараметры");
	Результат.Вставить("ЗапретРегламентныхЗаданий");
	Результат.Вставить("ЗапускОтИмениАдминистратора");
	Результат.Вставить("ИзбранноеСверху");
	Результат.Вставить("КаталогиПоискаПлатформы");
	Результат.Вставить("КаталогПортативныхИнструментов");
	Результат.Вставить("КодРазрешения");
	Результат.Вставить("КодЯзыкаСистемы");
	Результат.Вставить("НаборПараметров");
	Результат.Вставить("ОтключитьАутентификациюОС");
	Результат.Вставить("ОткрытьОбработку");
	Результат.Вставить("ОткрытьПортативныеИнструменты");
	Результат.Вставить("ОчисткаКэшаКлиентСерверныхВызовов");
	Результат.Вставить("ПараметрЗапуска");
	Результат.Вставить("ПодключитьОтладчик");
	Результат.Вставить("ПоказыватьОсобыйЗапуск");
	Результат.Вставить("Пользователь");
	Результат.Вставить("РазделениеДанных");
	Результат.Вставить("РежимЗапуска");
	Результат.Вставить("РежимИнтерфейсаТакси");
	Результат.Вставить("СборкаПлатформы");
	Результат.Вставить("СообщитьКоманднуюСтроку");
	Результат.Вставить("СортировкаПоДате");
	Результат.Вставить("СписокНаборовПараметров");
	Результат.Вставить("ТолькоИзбранное");
...
Процедура ЗагрузитьНастройкуВФорме(НастройкаФормы, ДопПараметры) Экспорт
	мНастройкаФормы = НастройкаФормы;
КонецПроцедуры

Процедура ИзменитьСвернутостьПанельОсобыйЗапускОтложенно() Экспорт
	ИзменитьСвернутостьПанельОсобыйЗапуск(ПоказыватьОсобыйЗапуск);
КонецПроцедуры
...
Процедура ПриОткрытии()
	...
	СохранитьКопиюСпискаБаз(Истина);
	...
	ирКлиент.ЗагрузитьНастройкуФормыЛкс(ЭтаФорма, мНастройкаФормы);
	...
	КонецЕсли; 
	ОбновитьСписок();
	ЭтотОбъект.ИмяФайлаДопАтрибутов = мПлатформа.ИмяФайлаНастроекКомпьютера(Ложь, "IRInfobaseList.xml");
	ФайлБольшихДанных = ФайлБольшихДанных();
	мХранимыеДанные.ХранимыеАтрибуты = Неопределено;
	Если ФайлБольшихДанных.Существует() Тогда
		ДанныеИзФайла = ирОбщий.ЗагрузитьЗначениеИзФайлаЛкс(ФайлБольшихДанных.ПолноеИмя);
		Если ДанныеИзФайла <> Неопределено Тогда
			ЗаполнитьЗначенияСвойств(мХранимыеДанные, ДанныеИзФайла);
		КонецЕсли;
	КонецЕсли;
	Если мХранимыеДанные.ХранимыеАтрибуты = Неопределено Тогда
		ЗаполнитьЗначенияСвойств(мХранимыеДанные, мНастройкаФормы);
	КонецЕсли;
	Если мХранимыеДанные.РежимыЗапускаОбычногоПриложения <> Неопределено Тогда
		ЗагрузитьХранимыеАтрибутыБаз(мХранимыеДанные.ХранимыеАтрибуты);
	КонецЕсли;
	Если мХранимыеДанные.РежимыЗапускаОбычногоПриложения <> Неопределено Тогда
		Для Каждого СтрокаХранимая Из мХранимыеДанные.РежимыЗапускаОбычногоПриложения Цикл
			СтрокаБазы = БазыПользователя.Найти(СтрокаХранимая.Идентификатор, "Идентификатор");
			Если СтрокаБазы <> Неопределено Тогда
				ЗаполнитьЗначенияСвойств(СтрокаБазы, СтрокаХранимая);
			КонецЕсли;
		КонецЦикла;
		ирКлиент.ТабличноеПолеВосстановитьСостояниеСтрокЛкс(ЭлементыФормы.Папки, мХранимыеДанные.ПапкиСостояние);
		ирКлиент.ТабличноеПолеВосстановитьСостояниеСтрокЛкс(ЭлементыФормы.БазыПользователя, мХранимыеДанные.БазыПользователяСостояние);
	КонецЕсли;
	НастроитьЭлементыФормы();
	ОбновитьКомпоновкуСписка();
	ПодключитьОбработчикОжидания("ИзменитьСвернутостьПанельОсобыйЗапускОтложенно", 0.1, Истина);
	Если НачальноеЗначениеВыбора <> Неопределено Тогда
		...
		КонецЕсли;
		Если СтрокаСписка <> Неопределено Тогда
			ЭлементыФормы.Папки.ТекущаяСтрока = ПапкаВсе();
			ЭтаФорма.БазыПользователяСтрокаПоиска = "";
			ПрименитьФильтрПоПодстрокеБезСохранения();
...
Функция ПапкаВсе() Экспорт
	Результат = Папки.Строки.Найти("<Все>", "Имя");
	Возврат Результат;
КонецФункции
...
Процедура НастроитьЭлементыФормы() Экспорт
	Для Каждого Колонка Из ЭлементыФормы.БазыПользователя.Колонки Цикл
		...
		Тогда
			Продолжить;
		КонецЕсли;
		Колонка.Видимость = Не РежимСтартера;
	КонецЦикла;
...
Процедура ОбновитьСписок() 
	...
	Для Счетчик = 1 По 2 Цикл
		...
		Папки.Строки.Очистить();
		СтрокаПапкиВсе = Папки.Строки.Добавить();
		СтрокаПапкиВсе.Имя = "<Все>";
		СтрокаПапкиВсе.ИмяВСписке = СтрокаПапкиВсе.Имя;
		СтрокаПапкиВсе.Идентификатор = "_";
		СтрокаПапкиПустая = Папки.Строки.Добавить();
		СтрокаПапкиПустая.Имя = "<Пустая>";
		СтрокаПапкиПустая.ИмяВСписке = СтрокаПапкиПустая.Имя;
		...
		Для Каждого СтрокаТаблицы Из ТаблицаБазКлиента Цикл
			Если ПустаяСтрока(СтрокаТаблицы.Connect) Тогда
				ПолноеИмяПапки = СтрокаТаблицы.Folder;
				Если ПолноеИмяПапки <> "/" Тогда
					ПолноеИмяПапки = ПолноеИмяПапки + "/";
				КонецЕсли;
				СтрокаПапки = РодительИзИмениПапки(ПолноеИмяПапки + СтрокаТаблицы.ИмяВСписке);
				ЗаполнитьЗначенияСвойств(СтрокаПапки, СтрокаТаблицы,, "ИмяВСписке");
	...
	ОбновитьКомпоновкуСписка();
	ирКлиент.ТабличноеПолеДеревоЗначений_РазвернутьВсеСтрокиЛкс(ЭлементыФормы.Папки);
...
Процедура СортироватьПапки() Экспорт
	Папки.Строки.Сортировать("Имя", Истина);
КонецПроцедуры

Процедура ОбновитьПапкиПоБазам() Экспорт 
	Папки.Колонки.Удалить(Папки.Колонки.ЧислоБаз);
	Папки.Колонки.Добавить("ЧислоБаз", Новый ОписаниеТипов("Число"));
	СтрокаПапкиВсе = ПапкаВсе();
	СтрокаПапкиВсе.ЧислоБаз = БазыПользователя.Количество();
	Для Каждого БазыЭлемент Из БазыПользователя Цикл
		ИмяВСписке = БазыЭлемент.Folder;
		Если ИмяВСписке = "/" Тогда
			СтрокаПапки = Папки.Строки.Найти("<Пустая>", "Имя");
		Иначе
			СтрокаПапки = Папки.Строки.Найти(ИмяВСписке, "ИмяВСписке", Истина);
		КонецЕсли;
		Если СтрокаПапки = Неопределено Тогда
			// Битая ссылка на папку
			СтрокаПапки = РодительИзИмениПапки(ИмяВСписке);
			СтрокаПапки.Идентификатор = Новый УникальныйИдентификатор;
			СтрокаПапки.ID = СтрокаПапки.Идентификатор;
		КонецЕсли;
	...
	КонецЦикла;
	//ирОбщий.РассчитатьИтогиВДеревеЛкс(Папки, "ЧислоБаз"); // Это скорее вредно чем полезно
КонецПроцедуры

Функция РодительИзИмениПапки(Знач ИмяВСписке) Экспорт
	Родители = ирОбщий.СтрРазделитьЛкс(ИмяВСписке, "/");
	Результат = Папки;
	ТекущееИмяВСписке = "";
	Для Каждого ИмяРодителя Из Родители Цикл
		ТекущееИмяВСписке = ТекущееИмяВСписке + "/" + ИмяРодителя;
		Если ПустаяСтрока(ИмяРодителя) Тогда
			Продолжить;
		КонецЕсли;
		НовыйРезультат = Результат.Строки.Найти(ИмяРодителя, "Имя");
		Если НовыйРезультат = Неопределено Тогда
			НовыйРезультат = Результат.Строки.Добавить();
			НовыйРезультат.Имя = ИмяРодителя;
			ОбновитьПолноеИмяПапки(НовыйРезультат);
		КонецЕсли;
...
Процедура СписокБазПользователяВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	...
	ИначеЕсли Колонка = Элемент.Колонки.ВерсияПлатформы Тогда
		...
		КонецЕсли;
	ИначеЕсли Ложь
		Или Колонка = Элемент.Колонки.КаталогКэша 
		Или Колонка = Элемент.Колонки.КаталогНастроек 
		Или Колонка = Элемент.Колонки.КаталогДанных
		Или Колонка = Элемент.Колонки.ФайлСписка
	Тогда
...
Процедура ЗапуститьПриложение1С(РежимКонфигуратора)
	...
	КонецЕсли;
	ПоследниеВыбранныеДобавить();
	ирКлиент.ЗапуститьПриложение1СЛкс(РежимКонфигуратора,, ТекущаяСтрока.ИмяВСписке,, Ложь, ТекущаяСтрока.Пользователь, ТекущаяСтрока.РежимЗапуска, ТекущаяСтрока.Пароль);
КонецПроцедуры
...
// Обновляет иерархическое имя папки (ИмяВСписке) на основе ее собственного имени (Имя)
// Параметры:
//   ТекущиеДанные -  - 
// Возвращаемое значение:
//    - 
Функция ОбновитьПолноеИмяПапки(Знач ТекущиеДанные = Неопределено) Экспорт
	Если ТекущиеДанные = Неопределено Тогда
		ТекущиеДанные = ЭлементыФормы.Папки.ТекущиеДанные;
	КонецЕсли;
	Если ТекущиеДанные = Папки Тогда
		Возврат "";
	КонецЕсли;
	Результат = ОбновитьПолноеИмяПапки(ирОбщий.РодительСтрокиДереваЛкс(ТекущиеДанные)) + "/" + ТекущиеДанные.Имя;
	ТекущиеДанные.ИмяВСписке = Результат;
	Возврат Результат;
КонецФункции
...
Процедура ЗапуститьСПараметрамиНажатие(Элемент)
	...
	КонецЕсли; 
	ПараметрыЗапуска = ирОбщий.ПараметрыЗапускаПриложения1СЛкс(Пользователь, ТекущаяСтрока.Пароль, КодРазрешения, РежимКонфигуратора, РежимЗапуска, Ложь, ОчисткаКэшаКлиентСерверныхВызовов, 
	...
	КаталогПрограммы = "";
	Тонкий = Не РежимКонфигуратора И РежимЗапуска = "УправляемоеПриложениеТонкий";
	СтрокаСборки = СборкиПлатформы.Найти(СборкаПлатформы, "КлючСборки");
	Если Тонкий И СтрокаСборки = Неопределено Тогда
		СтрокаСборки = ПодобратьСборкуПлатформы(ТекущаяСтрока);
	КонецЕсли;
	Если СтрокаСборки <> Неопределено Тогда
		КаталогПрограммы = СтрокаСборки.Каталог + "bin" + ирОбщий.РазделительПутиКФайлуЛкс();
		ИмяИсполняемогоФайла = ирОбщий.ИмяИсполняемогоФайлаКлиентаПлатформыЛкс(, Тонкий, КаталогПрограммы);
	Иначе
...
Процедура ЗапуститьЧерезВерсиюПлатформыСтандартно(Знач РежимКонфигуратора = Ложь)
	ТекущаяСтрока = ЭлементыФормы.БазыПользователя.ТекущаяСтрока;
	ПараметрыЗапуска = ирОбщий.ПараметрыЗапускаПриложения1СЛкс(ТекущаяСтрока.Пользователь, ТекущаяСтрока.Пароль,, РежимКонфигуратора, ТекущаяСтрока.РежимЗапуска, Ложь,, ТекущаяСтрока.AdditionalParameters, Ложь,
...
Функция ПодобратьКаталогПлатформы(СтрокаБазы, Знач ТолькоЕстьВРеестре = Ложь)
	ПодходящаяСборка = ПодобратьСборкуПлатформы(СтрокаБазы, ТолькоЕстьВРеестре);
	Если ПодходящаяСборка = Неопределено Тогда
		Возврат Неопределено;
	Иначе
		Возврат ПодходящаяСборка.Каталог + ирОбщий.РазделительПутиКФайлуЛкс() + "bin" + ирОбщий.РазделительПутиКФайлуЛкс();
	КонецЕсли;
КонецФункции

Функция ПодобратьСборкуПлатформы(СтрокаБазы, Знач ТолькоЕстьВРеестре = Ложь) Экспорт
	ОбновитьДоступныеПлатформы(); 
	Если Истина
		И ЗначениеЗаполнено(СтрокаБазы.DefaultVersion) 
		И (Ложь
			Или ПустаяСтрока(СтрокаБазы.Version)
			Или ирОбщий.СтрНачинаетсяСЛкс(СтрокаБазы.DefaultVersion, СтрокаБазы.Version))
	Тогда
		КлючПоиска = Новый Структура("СборкаПлатформы", СтрокаБазы.DefaultVersion);
		Если ТолькоЕстьВРеестре Тогда
			КлючПоиска.Вставить("ЕстьВРеестре", Истина);
		КонецЕсли;
		ПодходящаяСборка = СборкиПлатформы.НайтиСтроки(КлючПоиска);
		Если ПодходящаяСборка.Количество() Тогда
			ПодходящаяСборка = ПодходящаяСборка[0];
		Иначе
			ПодходящаяСборка = Неопределено;
		КонецЕсли;
	КонецЕсли;
	Если ПодходящаяСборка = Неопределено Тогда
		Если ПустаяСтрока(СтрокаБазы.Version) Тогда
			ПодходящаяСборка = СборкиПлатформы[0];
		Иначе
			Для Каждого СборкиЭлемент Из СборкиПлатформы Цикл
				...
				Тогда
					ПодходящаяСборка = СборкиЭлемент;
					Прервать;
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
...
Процедура ПапкиПередУдалением(Элемент, Отказ)
	Отказ = Истина;
	СтрокаПапки = ЭлементыФормы.Папки.ТекущаяСтрока;
	Если Лев(СтрокаПапки.ИмяВСписке, 1) = "<" Тогда
		Возврат;
	КонецЕсли;
	Если Вопрос("Удалить папку из списка? Ее содержимое переместится в родительскую папку.", РежимДиалогаВопрос.ОКОтмена) <> КодВозвратаДиалога.ОК Тогда
		Возврат;
	КонецЕсли;
	СтрокаПапки.ПометкаУдаления = Истина;
	Для Каждого БазыЭлемент Из БазыПользователя.НайтиСтроки(Новый Структура("Папка", СтрокаПапки.Идентификатор)) Цикл
		Если СтрокаПапки.Родитель = Неопределено Тогда
			БазыЭлемент.Папка = "";
		Иначе
			БазыЭлемент.Папка = СтрокаПапки.Родитель.Идентификатор;
		КонецЕсли;
		БазыЭлемент.Изменена = Истина;
	КонецЦикла;
	Если СтрокаПапки.Родитель <> Неопределено Тогда
		ЭлементыФормы.Папки.ТекущаяСтрока = СтрокаПапки.Родитель;
	КонецЕсли;
	ЗаписатьСписокБаз();
КонецПроцедуры
...
Процедура ЗаписатьСписокБаз()  
	Для Каждого БазыЭлемент Из БазыПользователя Цикл
		...
		Если ЗначениеЗаполнено(БазыЭлемент.Папка) Тогда
			ПапкаИзменена = Ложь;
			ирОбщий.ПрисвоитьЕслиНеРавноЛкс(БазыЭлемент.Folder, Папки.Строки.Найти(БазыЭлемент.Папка, "Идентификатор", Истина).ИмяВСписке, ПапкаИзменена);
			БазыЭлемент.Изменена = БазыЭлемент.Изменена Или ПапкаИзменена;
		Иначе
			БазыЭлемент.Folder = "/"; 
		КонецЕсли;
	КонецЦикла;
	Для Каждого ПапкиЭлемент Из ирОбщий.ВсеСтрокиДереваЗначенийЛкс(Папки) Цикл
		ПапкиЭлемент.ID = ПапкиЭлемент.Идентификатор;
		ПапкиЭлемент.Folder = ирОбщий.ТекстБезПоследнегоФрагментаЛкс(ПапкиЭлемент.ИмяВСписке, "/"); 
	КонецЦикла;
	ИменаСтандартныхАтрибутов = ИменаСтандартныхАтрибутов();
			...
			Иначе
				...
				Если ирОбщий.СтрокиРавныЛкс(ИмяКолонки, "id") Тогда
					...
					Если НовоеОписаниеБазы <> Неопределено Тогда
						...
						КонецЕсли;
					КонецЕсли;
					НовоеОписаниеПапки = Папки.Строки.Найти(ПредставлениеЗначения, ИмяКолонки, Истина);
					Если НовоеОписаниеПапки <> Неопределено Тогда
						Если НовоеОписаниеПапки.Изменена Тогда
							НаименованиеБазы = НовоеОписаниеПапки.Имя;
	...
	КонецЦикла; 
	ЧтениеФайла.Закрыть(); 
	СериализоватьВыбранныеБазы(БазыПользователя.НайтиСтроки(Новый Структура("Изменена", Истина)), НовыеСтрокиФайла);
	ИзменныеПапки = Папки.Строки.НайтиСтроки(Новый Структура("Изменена", Истина), Истина);
	Для Каждого СтрокаНовойПапки Из ИзменныеПапки Цикл
		СтрокиАтрибутовБазы = Новый Массив;
		СтрокиАтрибутовБазы.Добавить("[" + СтрокаНовойПапки.Имя + "]");
		СтрокиАтрибутовБазы.Добавить("ID=" + СтрокаНовойПапки.Идентификатор);
		СтрокиАтрибутовБазы.Добавить("Folder=" + СтрокаНовойПапки.Folder);
		ирОбщий.ДополнитьМассивЛкс(НовыеСтрокиФайла, СтрокиАтрибутовБазы);
	КонецЦикла;
	...
	ирОбщий.УстановитьСвойствоВКоллекцииЛкс(ИзменныеПапки, "Изменена", Ложь);
	...
	СохранитьХранимыеАтрибуты();
	ОбновитьПапкиПоБазам();
	//ОбновитьКомпоновкуСписка(); // Часто вредно сортировать сразу https://devtool1c.ucoz.ru/forum/2-2812-1
КонецПроцедуры 

// Вынес отдельно чтобы не замедлять чтение/запись файла общих глобальных настроек
Процедура СохранитьХранимыеАтрибуты() Экспорт
	мХранимыеДанные.ХранимыеАтрибуты = ХранимыеАтрибуты();
	мХранимыеДанные.РежимыЗапускаОбычногоПриложения = БазыПользователя.Выгрузить(Новый Структура("РежимЗапуска", "ОбычноеПриложение"), "Идентификатор, РежимЗапуска");
	мХранимыеДанные.БазыПользователяСостояние = ирКлиент.ТабличноеПолеСостояниеСтрокЛкс(ЭлементыФормы.БазыПользователя, "Идентификатор");
	мХранимыеДанные.ПапкиСостояние = ирКлиент.ТабличноеПолеСостояниеСтрокЛкс(ЭлементыФормы.Папки, "Идентификатор");
	ирОбщий.СохранитьЗначениеВФайлЛкс(мХранимыеДанные, ФайлБольшихДанных().ПолноеИмя);
КонецПроцедуры 

Функция ФайлБольшихДанных() Экспорт
	Возврат Новый Файл(ИмяФайлаДопАтрибутов);
КонецФункции 

//.
// Возвращаемое значение:
//   Массив - 
Функция ИменаСтандартныхАтрибутов() Экспорт
	...
	Для Каждого РеквизитыЭлемент Из Метаданные().ТабличныеЧасти.БазыПользователя.Реквизиты Цикл
		...
		КонецЕсли;
	КонецЦикла;
	Возврат ИменаСтандартныхАтрибутов;
КонецФункции 

//.
// Параметры:
//   СтрокиВыбранныхБаз - Массив - 
//   НовыеСтрокиФайла - Массив, Неопределено - 
// Возвращаемое значение:
//   Массив - 
Функция СериализоватьВыбранныеБазы(знач СтрокиВыбранныхБаз, Знач НовыеСтрокиФайла = Неопределено) Экспорт
	Если НовыеСтрокиФайла = Неопределено Тогда
		НовыеСтрокиФайла = Новый Массив;
	КонецЕсли;
	ИменаСтандартныхАтрибутов = ИменаСтандартныхАтрибутов();
	Для Каждого СтрокаНовойБазы Из СтрокиВыбранныхБаз Цикл
		СтрокиАтрибутовБазы = Новый Массив;
...
Функция СлитьАтрибутБазы(Знач СтрокаИзФайла, Знач СтрокиБазы, Знач НовоеОписаниеБазы, Знач НовоеОписаниеПапки, Знач ДобавленныеАтрибуты) Экспорт
	...
	Если ЗначениеЗаполнено(ИмяКолонки) Тогда
		...
		ПредставлениеЗначения = ирОбщий.ТекстБезПервогоФрагментаЛкс(СтрокаИзФайла, "=");
		Если НовоеОписаниеБазы <> Неопределено Тогда
...
Процедура БазыПользователяНаименованиеПриИзменении(Элемент)
	...
	Если ПустаяСтрока(Элемент.Значение) Тогда
		Элемент.Значение = ТекущаяСтрока.СтрокаСоединения;
	КонецЕсли;
	Элемент.Значение = ирОбщий.АвтоУникальноеИмяВКоллекцииЛкс(БазыПользователя, ТекущаяСтрока, "ИмяВСписке", Ложь);
	ТекущаяСтрока.Изменена = Истина;
	ОбновитьПредставлениеБазы(ТекущаяСтрока);
...
Процедура ОбновитьКомпоновкуСписка(Знач ВключитьПоДате = Ложь)
	СтрокаСортировки = Новый Массив;
	Если ИзбранноеСверху Тогда
		СтрокаСортировки.Добавить("Избранное Убыв");
	КонецЕсли;
	Если СортировкаПоДате Тогда
		СтрокаСортировки.Добавить("ДатаЗапуска Убыв");
	КонецЕсли;
	СтрокаСортировки.Добавить("Представление");
	БазыПользователя.Сортировать(ирОбщий.СтрСоединитьЛкс(СтрокаСортировки));
	Если СортировкаПоДате И ВключитьПоДате Тогда
		ЭлементыФормы.Папки.ТекущаяСтрока = ПапкаВсе();
...
Процедура ЗагрузитьАтрибутыБазыВРедактор(Знач ФормаЭлемента) Экспорт
	...
	ФормаЭлемента.КаталогКэша = ТекущаяСтрока.КаталогКэша;
	ТекущаяСтрока = ЭлементыФормы.БазыПользователя.ТекущаяСтрока;
	СтрокаПапки = Папки.Строки.Найти(ТекущаяСтрока.Папка, "Идентификатор", Истина);
	Если СтрокаПапки <> Неопределено Тогда
...
Процедура БазыПользователяОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	КонецЕсли;
	СтрокаБазы.App = ?(СтрокаБазы.РежимЗапуска = "Авто", "", РежимаЗапускаВнутр);
	Если ЗначениеЗаполнено(ВыбранноеЗначение.ПапкаИдентификатор) Тогда
		СтрокаПапки = Папки.Строки.Найти(ВыбранноеЗначение.ПапкаИдентификатор, "Идентификатор", Истина);
		Если СтрокаПапки <> Неопределено Тогда
	...
	ОбновитьПредставлениеБазы(СтрокаБазы);
...
Процедура ПапкиПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Колонка)
	Если ТипЗнч(ПараметрыПеретаскивания.Значение) = Тип("Массив") Тогда
		СтандартнаяОбработка = Ложь;
		Для Каждого СтрокаБазы Из ПараметрыПеретаскивания.Значение Цикл // СтрокаТаблицыЗначений
			Если ирОбщий.ЛиСтрокаТабличнойЧастиЛкс(СтрокаБазы) Тогда
				//! СтрокаБазы = 0 // см. БазыПользователя[0]
				СтрокаБазы.Изменена = Истина;
				...
				КонецЕсли;
			ИначеЕсли ирОбщий.ЛиКолонкиТаблицСовпадаютЛкс(СтрокаБазы.Владелец(), Папки) Тогда
				//! СтрокаБазы = 0 // см. Папки.Строки[0]
				Если Ложь
					Или Строка = Неопределено
					Или Лев(Строка.Имя, 1) = "<"
				Тогда           
					НоваяСтрока = Папки.Строки.Добавить();
				Иначе
					НоваяСтрока = Строка.Строки.Добавить();
				КонецЕсли;
				ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаБазы);
				НоваяСтрока.Изменена = Истина;
				ирОбщий.РодительСтрокиДереваЛкс(СтрокаБазы).Строки.Удалить(СтрокаБазы);
				ОбновитьПолноеИмяПапки(НоваяСтрока);
...
Процедура ИзменитьСвернутостьПанельОсобыйЗапуск(Знач Видимость, Знач Запоминать = Истина)
	
	ирКлиент.ИзменитьСвернутостьЛкс(ЭтаФорма, Видимость, ЭлементыФормы.ПанельОсобыйЗапуск, ЭтаФорма.ЭлементыФормы.вРазделительСписокОсобыйЗапуск, ЭтаФорма.Панель, "право",, Ложь);
	ЭлементыФормы.ДействияФормы.Кнопки.СверткаОсобыйЗапуск.Пометка = Видимость;
	Если Запоминать Тогда
		ЭтаФорма.ПоказыватьОсобыйЗапуск = Видимость;
	КонецЕсли;
	
КонецПроцедуры

Процедура ДействияФормыСверткаОсобыйЗапуск(Кнопка)
	ИзменитьСвернутостьПанельОсобыйЗапуск(Не ПоказыватьОсобыйЗапуск);
КонецПроцедуры

Процедура ИзбранноеСверхуПриИзменении(Элемент)
	ОбновитьКомпоновкуСписка();
КонецПроцедуры

Процедура ПапкиПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки)
	ирКлиент.ТабличноеПолеПриВыводеСтрокиЛкс(ЭтаФорма, Элемент, ОформлениеСтроки, ДанныеСтроки);
КонецПроцедуры

Процедура ПапкиПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель)
	
	Если Лев(Родитель.Имя, 1) = "<" Тогда
		Отказ = Истина;
		СтрокаДерева = Папки.Строки.Добавить();
		СтрокаДерева.Идентификатор = Новый УникальныйИдентификатор;
		СтрокаДерева.ID = СтрокаДерева.Идентификатор;
		Элемент.ТекущаяСтрока = СтрокаДерева;
		ОбновитьИмяПапки();
...
Процедура ПапкиПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
...
Процедура ОбновитьИмяПапки() Экспорт
	ТекущиеДанные = ЭлементыФормы.Папки.ТекущиеДанные;
	ТекущиеДанные.Имя = ирОбщий.АвтоУникальноеИмяВКоллекцииЛкс(ирОбщий.РодительСтрокиДереваЛкс(ТекущиеДанные).Строки, ТекущиеДанные,,, "Папка");
	ТекущиеДанные.Изменена = Истина;
	ОбновитьПолноеИмяПапки();
	ПодключитьОбработчикОжидания("ЗаписатьСписокБаз", 0.1, Истина);
КонецПроцедуры

Процедура ДействияФормыСохранитьВНовыйСписокБаз(Кнопка)
	ИмяФайла = ирКлиент.ВыбратьФайлЛкс(Ложь, "v8i", "Список баз 1С");
	Если ПустаяСтрока(ИмяФайла) Тогда
		Возврат;
	КонецЕсли;
	Массив = СериализоватьВыбранныеБазы(ЭлементыФормы.БазыПользователя.ВыделенныеСтроки);
	ирОбщий.ЗаписатьТекстВФайлЛкс(ирОбщий.СтрСоединитьЛкс(Массив, Символы.ПС), ИмяФайла);
КонецПроцедуры

Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	ИзменитьСвернутостьПанельОсобыйЗапуск(Истина, Ложь);
КонецПроцедуры

Функция СохранитьКопиюСпискаБаз(Знач Авто = Ложь) Экспорт
	ФайлТекущегоСписка = Новый Файл(ирОбщий.ИмяФайлаСпискаИнфобазПользователяОСЛкс());
	Если Авто Тогда
		ДатаДляИмени = НачалоМесяца(ТекущаяДата());
		ирОбщий.СократитьГруппуФайловОткатаЛкс(НайтиФайлы(ФайлТекущегоСписка.Путь, ФайлТекущегоСписка.ИмяБезРасширения + "_*"));
	Иначе
		ДатаДляИмени = ТекущаяДата();
	КонецЕсли;
	ФайлаПриемника = Новый Файл(ФайлТекущегоСписка.Путь + ФайлТекущегоСписка.ИмяБезРасширения + "_" + ирОбщий.ЗаменитьНедопустимыеСимволыВИмениФайлаЛкс(ДатаДляИмени) + ФайлТекущегоСписка.Расширение);
	Если Не ФайлаПриемника.Существует() Тогда
		КопироватьФайл(ФайлТекущегоСписка.ПолноеИмя, ФайлаПриемника.ПолноеИмя);
		ирОбщий.СообщитьЛкс("Копия списка баз сохранена " + ФайлаПриемника.ПолноеИмя);
	КонецЕсли;
	Возврат ФайлТекущегоСписка;
КонецФункции

Процедура ДействияФормыЗагрузитьИзФайла(Кнопка)
	ИмяФайлаТекущегоСписка = ирОбщий.ИмяФайлаСпискаИнфобазПользователяОСЛкс();
	ИмяФайла = ирКлиент.ВыбратьФайлЛкс(, "v8i", "Список баз 1С", ИмяФайлаТекущегоСписка);
	Если ПустаяСтрока(ИмяФайла) Тогда
		Возврат;
	КонецЕсли;
	ТаблицаБазТекущая = ирОбщий.СписокБазПользователяОСЛкс();
	ТаблицаБазФайла = ирОбщий.СписокБазПользователяОСЛкс(ИмяФайла);
	ТекстВопроса = ирОбщий.СтрШаблонЛкс("В текущем списке %1 элементов. В списке из выбранного файла - %2 элементов. Хотите заменить список (копия текущего списка будет сохранена)?",
		ТаблицаБазТекущая.Количество(), ТаблицаБазФайла.Количество());
	Ответ = Вопрос(ТекстВопроса, РежимДиалогаВопрос.ОКОтмена);
	Если Ответ = КодВозвратаДиалога.ОК Тогда
		СохранитьКопиюСпискаБаз();
		КопироватьФайл(ИмяФайла, ИмяФайлаТекущегоСписка);
		ОбновитьСписок();
	КонецЕсли;
КонецПроцедуры

Процедура ДействияФормыСоздатьРезервнуюКопию(Кнопка)
	СохранитьКопиюСпискаБаз();
КонецПроцедуры
...
мМаксЧислоКлавиш = 5;
ПоказыватьОсобыйЗапуск = Истина;
мХранимыеДанные = Новый Структура;
мХранимыеДанные.Вставить("ХранимыеАтрибуты");
мХранимыеДанные.Вставить("РежимыЗапускаОбычногоПриложения");
мХранимыеДанные.Вставить("ПапкиСостояние");
мХранимыеДанные.Вставить("БазыПользователяСостояние");
