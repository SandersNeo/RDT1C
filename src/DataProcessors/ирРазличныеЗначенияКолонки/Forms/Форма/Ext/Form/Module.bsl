...
Функция ОбновитьДанные(РежимОтладки = Ложь, ИзменятьФлагУчитыватьОтбор = Ложь)
	...
	Если ИсточникДействий = Неопределено Тогда
		ПолноеИмяТаблицыБД = ирОбщий.ТекстБезПоследнегоФрагментаЛкс(ПараметрПолноеИмяКолонкиБД);
	...
	Иначе
		...
		ИначеЕсли ТипЗнч(ИсточникДействий) = Тип("ТаблицаФормы") Тогда
		...
		Если ТипИсточникаДанных = "ТаблицаЗначений" Тогда 
			...
			Если НастройкиСписка <> Неопределено Тогда
				...
				Если Отбор.ДоступныеПоляОтбора.Элементы.Количество() = 0 Тогда
					ирКлиент.ТабличноеПолеСОтборомПросмотраОбновитьКомпоновщикЛкс(Форма, ИсточникДействий);
				КонецЕсли;
			КонецЕсли; 
	...
	Тогда
		...
		Иначе
			...
			|ИЗ Т
			|УПОРЯДОЧИТЬ ПО НомерСтроки"; //@skip-check unknown-method-property
			...
			СсылкиБД = ирОбщий.ВыполнитьЗапросЛкс(Запрос.Текст, Запрос.Параметры,, ЭтаФорма,,, Ложь).ТаблицаРезультата;
	...
	Иначе
		...
		Тогда
			КорневоеПолеКолонки = ирОбщий.ТекстБезПоследнегоФрагментаЛкс(ВыражениеПоляКолонки, СуффиксПоляТипЗначения);
		...
		Иначе
			...
			ТипЗначенияГруппировки = ДоступноеПоле.ТипЗначения;
		КонецЕсли;
		...
		КонецЕсли;
		// Выполняем на сервере для ускорения получения представлений с серверными обработчиками, которые платформа извлекает внутри метода Выгрузить() https://www.hostedredmine.com/issues/971259
		ВыгрузкаРезультата = ирОбщий.ВыполнитьЗапросЛкс(Запрос.Текст, Запрос.Параметры,, ЭтаФорма,,, ирКэш.ПараметрыЗаписиОбъектовЛкс().ОбъектыНаСервере,,, Истина).ТаблицаРезультата;
			...
			Если ЗначениеТабличногоПоля <> Неопределено Тогда
				...
				Если НайденныеСтроки.Количество() > 0 Тогда
					...
					Иначе
						...
						Если ТипЗнч(ИсточникДействий) = Тип("ТаблицаФормы") Тогда
							//! ТекущаяСтрокаИсточника = 0; // ДанныеФормыЭлементКоллекции
							ТекущаяСтрокаИсточника = ТекущаяСтрокаИсточника.ПолучитьИдентификатор();
						КонецЕсли; 
					...
					Тогда
						...
						Для Каждого ПодчиненныеЭлемент Из ИсточникДействий.КоманднаяПанель.ПодчиненныеЭлементы Цикл
							Если Найти(ПодчиненныеЭлемент.Имя, "ОтменитьПоиск") > 0 Тогда
								...
								ПодчиненныеЭлемент.ПоложениеВКоманднойПанели = Вычислить("ПоложениеКнопкиВКоманднойПанели").ВКоманднойПанели; /// 8.3.15+
							КонецЕсли;
						КонецЦикла;
						ИсточникДействий.ПоложениеСостоянияПросмотра = Вычислить("ПоложениеСостоянияПросмотра").Верх;  /// 8.3+
...
Процедура ПриОткрытии()
	ирКлиент.Форма_ПриОткрытииЛкс(ЭтаФорма, ЭлементыФормы.ПредставлениеТипаЗначения.Видимость);
