...
Функция СохранитьКомпоновкиВФайл(ЗапрашиватьСохранение = Ложь, ЗапрашиватьИмяФайла = Ложь, Знач ИмяФайла = Неопределено, СброситьМодифицированность = Неопределено, Знач ЭтоФайлВосстановления = Ложь)
	...
	Тогда
		...
		ИначеЕсли ВладелецФормы = Неопределено Тогда
			ИмяФайлаСхемы = ирКлиент.ВыбратьФайлЛкс(Ложь, "xml",,,, "Схема компоновки из ИР", "Выберите файл хранения схемы компоновки");
			Если ИмяФайлаСхемы = Неопределено Тогда
				Возврат Ложь;
			КонецЕсли;
			ТекстовыйДокумент = Новый ТекстовыйДокумент;
			ТекстовыйДокумент.УстановитьТекст(ирОбщий.ОбъектВСтрокуXMLЛкс(мСхемаКомпоновкиДанных));
...
Функция МакетКомпоновки(ИмяТипаГенератора = Неопределено, Предпросмотр = Ложь, СтруктураВнешниеНаборыДанных = Неопределено, Знач РазрешитьЗаполнятьРасшифровки = Истина,
	...
	Для Каждого СтрокаНаборДанных Из ДеревоНаборовДанныхЗапросов.Строки[0].Строки Цикл
		...
		Если ЗначениеЗаполнено(ВременныеТаблицы[ВременныеТаблицы.Количество() - 1].Значение) Тогда
			ТекстСообщения = ирОбщий.ПодставитьПараметрыВСтрокуЛкс("В наборе данных %1 последний запрос помещает данные во временную таблицу, а должен возвращать их!", СтрокаНаборДанных.Имя);
	...
	Попытка
		МакетКомпоновкиДанных = ирОбщий.СобратьМакетКомпоновкиЛкс(ВременнаяСхемаКомпоновки, ВременныеНастройкиКомпоновки, СтруктураВнешниеНаборыДанных,,, ПроверятьДоступностьПолей,,
			РазрешитьЗаполнятьРасшифровки И Не НеЗаполнятьРасшифровки, ДанныеРасшифровкиЛ, МакетОформления, Тип(ИмяТипаГенератора), ВстраиватьЗначенияРесурсов, Ложь);
	Исключение
		ОписаниеОшибки = ОписаниеОшибки();
...
Функция ЕстьТаблицыВСтруктуреКомпоновки(Знач ВременныеНастройкиКомпоновки) Экспорт
	Результат = ирОбщий.НайтиЭлементКоллекцииЛкс(ирОбщий.ВсеЭлементыИерархииНастроекКомпоновкиЛкс(ВременныеНастройкиКомпоновки.Структура),,, Тип("ТаблицаКомпоновкиДанных")) <> Неопределено;
	Возврат Результат;
КонецФункции
...
Функция СкомпоноватьРезультатМой(Знач МакетКомпоновкиДанных = Неопределено, Знач СтруктураВнешниеНаборыДанных = Неопределено, Предпросмотр = Ложь, РазрешитьАсинхронно = Ложь,
	...
	Тогда
		...
		КонецЕсли; 
		КоллекцияВывода = Новый (ТипЗнч(ЭУВывода.Значение)); // ТаблицаЗначений
...
// Процедура - Вывести результат завершение
//
// Параметры:
//  СостояниеЗадания - 	 - 
//  РезультатЗадания - см. РезультатВывода - 
//
Процедура ВывестиРезультатЗавершение(СостояниеЗадания = Неопределено, РезультатЗадания = Неопределено) Экспорт 
	...
	Тогда
		...
		мЭлементыРезультата = РезультатЗадания.ЭлементыРезультата;
		#Если Сервер И Не Сервер Тогда
		    мАнализТехножурнала = Обработки.ирАнализТехножурнала.Создать();
		#КонецЕсли
		Если Не Предпросмотр Тогда
		...
		Если Ложь
			...
			Или СтраницаРезультата = ЭлементыФормы.ПанельРезультатов.Страницы.ТаблицаЗначений
		Тогда
			Если СтраницаРезультата = ЭлементыФормы.ПанельРезультатов.Страницы.ТаблицаЗначений Тогда
				ЭлементыФормы.ТаблицаЗначений.Значение = КоллекцияВывода;
				ЭтаФорма.ТаблицаЗначений = ЭлементыФормы.ТаблицаЗначений.Значение;
				ирКлиент.ТабличноеПолеСОтборомПросмотраОбновитьКомпоновщикЛкс(ЭтаФорма, ЭлементыФормы.ТаблицаЗначений);
				ТаблицаЗначенийИспользоватьОтборПриИзменении();
			Иначе
...
Процедура УстановитьМакетКомпоновкиДанных(Знач МакетКомпоновкиДанных = Неопределено)
	...
	Если мМакетКомпоновкиДанных <> Неопределено Тогда
		ирОбщий.ОбновитьТекстПослеМаркераЛкс(ЭлементыФормы.КП_МакетКомпоновкиДанных.Кнопки.ИсполняемыеЗапросы.Текст,, "" + мМакетКомпоновкиДанных.НаборыДанных.Количество() + ")", " (");
	КонецЕсли;
...
Процедура КоманднаяПанельРезультатаВперед(Кнопка)
...
Процедура КоманднаяПанельРезультатаКопия(Кнопка)
...
Процедура ДеревоОтчетовПриАктивизацииСтроки(Элемент)
	...
	Если мТекущаяСтрока <> НеОпределено Тогда
		...
		Если мТекущаяСтрока.ВнешниеНаборыДанных <> Неопределено Тогда
			...
			ВнешниеНаборыДанных.Сортировать("Имя");
		КонецЕсли; 
		...
		Если мТекущаяСтрока.События <> Неопределено Тогда
			ирОбщий.ПрочитатьХранимыеСобытияЛкс(мТекущаяСтрока.События, События);
		КонецЕсли; 
...
Функция ЗаполнитьСвойстваОтчетаПоУмолчанию(НоваяСтрока, НастройкаКомпоновки = Неопределено)
	...
	НоваяСтрока.СхемаКомпоновкиДанных = ирОбщий.СоздатьСхемуКомпоновкиЛкс();
	НоваяСтрока.ВнешниеНаборыДанных = НоваяТаблицаВнешниеНаборыДанных();
	НоваяСтрока.Настройки = НоваяТаблицаНастройкиОтчета();
	СтрокаНастроек = НоваяСтрока.Настройки.Добавить();
...
Процедура ПриОткрытии()
	...
	Тогда
		...
		Если мМакетКомпоновкиДанных <> Неопределено Тогда
			...
			ЭлементыФормы.ПанельРезультатов.ТекущаяСтраница = ЭлементыФормы.ПанельРезультатов.Страницы.МакетКомпоновки;
			Если мВнешниеНаборыДанных = Неопределено Тогда
				ПоказатьСвернутьНастройки(Ложь);
			КонецЕсли;
		КонецЕсли;
...
Процедура ОбработкаВыбора(ЗначениеВыбора, Источник)
	...
	Если ТипЗнч(ЗначениеВыбора) = Тип("СхемаКомпоновкиДанных") Тогда
		...
		Если РедактируемаяКонструкторомСтрока = Неопределено Тогда
			...
			КонецЕсли;
		КонецЕсли; 
		ЛиСтандартныеНастройкиИзменены = Истина;
		Если РедактируемаяКонструкторомСтрока.СхемаКомпоновкиДанных <> Неопределено Тогда
			ЛиСтандартныеНастройкиИзменены = ирОбщий.ОбъектВСтрокуXMLЛкс(ЗначениеВыбора.НастройкиПоУмолчанию) <> ирОбщий.ОбъектВСтрокуXMLЛкс(РедактируемаяКонструкторомСтрока.СхемаКомпоновкиДанных.НастройкиПоУмолчанию)
		КонецЕсли; 
		...
		//Если мРедактируемаяКонструкторомСтрока = мТекущаяСтрока Тогда
			...
			Если Ложь
				...
				Или ЛиСтандартныеНастройкиИзменены И Вопрос("Загрузить стандартные настройки?", РежимДиалогаВопрос.ДаНет) = КодВозвратаДиалога.Да 
			Тогда
			...
			ЭлементыФормы.КоманднаяПанельФормы.Кнопки.РедакторВыраженияКомпоновки.Доступность = Истина;
		//КонецЕсли;
		НомерыСтрок = Новый Структура;
		Для Каждого СтрокаНаборДанных Из ирОбщий.ВсеСтрокиДереваЗначенийЛкс(ДеревоНаборовДанныхЗапросов) Цикл
			НомерыСтрок.Вставить(СтрокаНаборДанных.Имя, СтрокаНаборДанных.НомерСтроки);
		КонецЦикла;
		мТекущаяСтрока.НомерыСтрокЗапросов = НомерыСтрок;
		ОбновитьТаблицыСхемы();
		ПослеЗагрузкиНастроекКомпоновки();
		ЭтаФорма.ИспользоватьПользовательскиеНастройки = Ложь;
	...
	ИначеЕсли ТипЗнч(ЗначениеВыбора) = Тип("Структура") Тогда
		...
		СтрокаДереваНаборов = ДеревоНаборовДанныхЗапросов.Строки.Найти(ЗначениеВыбора.НаборДанных.Имя, "Имя", Истина);
		Если СтрокаДереваНаборов <> Неопределено Тогда
			СтрокаДереваНаборов.НомерСтроки = ЗначениеВыбора.НомерСтроки;
		КонецЕсли;
...
Процедура ЗаполнитьСтрокиДереваНаборовДанныхЗапросовПоСхеме(Знач СтрокиДерева, Знач СхемаКомпоновки, Знач НомерыСтрок)
	ЗаполнитьСтрокиДереваНаборовДанныхЗапросов(СтрокиДерева, СхемаКомпоновки.НаборыДанных, НомерыСтрок);
	Для Каждого ВложеннаяСхемаКомпоновки Из СхемаКомпоновки.ВложенныеСхемыКомпоновкиДанных Цикл
		...
		ЗаполнитьСтрокиДереваНаборовДанныхЗапросовПоСхеме(СтрокаДереваВложенная.Строки, ВложеннаяСхемаКомпоновки.Схема, НомерыСтрок);
	КонецЦикла;
КонецПроцедуры

// Заполняет строки дерева наборами данных запросами схемы компоновки данных.
// Содержит рекурсивный вызов.
Процедура ЗаполнитьСтрокиДереваНаборовДанныхЗапросов(Знач СтрокиДерева, Знач НаборыДанных, Знач НомерыСтрок)
	Для Каждого НаборДанных Из НаборыДанных Цикл
		Если Ложь
			Или ТипЗнч(НаборДанных) = Тип("НаборДанныхЗапросСхемыКомпоновкиДанных") 
			Или ТипЗнч(НаборДанных) = Тип("НаборДанныхОбъектСхемыКомпоновкиДанных")
		Тогда
			НоваяСтрока = СтрокиДерева.Добавить();
			Если НомерыСтрок <> Неопределено Тогда
				НомерыСтрок.Свойство(НаборДанных.Имя, НоваяСтрока.НомерСтроки);
			КонецЕсли;
		ИначеЕсли ТипЗнч(НаборДанных) = Тип("НаборДанныхОбъединениеСхемыКомпоновкиДанных") Тогда
			НоваяСтрока = СтрокиДерева.Добавить();
			ЗаполнитьСтрокиДереваНаборовДанныхЗапросов(НоваяСтрока.Строки, НаборДанных.Элементы, НомерыСтрок);
		Иначе
			Продолжить;
		КонецЕсли;
...
Процедура ДеревоНаборовДанныхЗапросовВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	РедактироватьЗапросСхемы(ВыбраннаяСтрока);
	СтандартнаяОбработка = Ложь;
КонецПроцедуры

Процедура РедактироватьЗапросСхемы(Знач ВыбраннаяСтрока, Знач ИмяПоляКомпоновки = "")
	
	Если ТипЗнч(ВыбраннаяСтрока.НаборДанных) = Тип("НаборДанныхЗапросСхемыКомпоновкиДанных") Тогда
	...
	ИначеЕсли ТипЗнч(ВыбраннаяСтрока.НаборДанных) = Тип("НаборДанныхОбъектСхемыКомпоновкиДанных") Тогда
		ТаблицыВнешнегоНабора = ВнешниеНаборыДанных.НайтиСтроки(Новый Структура("Имя", ВыбраннаяСтрока.НаборДанных.ИмяОбъекта));
		Если ТаблицыВнешнегоНабора.Количество() Тогда
			ирКлиент.ОткрытьЗначениеЛкс(ТаблицыВнешнегоНабора[0].Значение);
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбновитьТаблицыСхемы()
	...
	Если мТекущаяСтрока <> Неопределено Тогда
		...
		Если мТекущаяСтрока.СхемаКомпоновкиДанных <> Неопределено Тогда
			...
			ЗаполнитьСтрокиДереваНаборовДанныхЗапросовПоСхеме(КорневаяСтрока.Строки, лСхемаКомпоновкиДанных, мТекущаяСтрока.НомерыСтрокЗапросов);
			СообщатьОМодификацииВнешнихНаборов = мВнешниеНаборыДанных <> Неопределено;
		    СтруктураВнешниеНаборыДанных = Новый Структура;
			//! СтруктураВнешниеНаборыДанных.Вставить("", Новый ТаблицаЗначений);
			...
			Для Каждого КлючИЗначение Из СтруктураВнешниеНаборыДанных Цикл
				...
				Если НоваяСтрока = Неопределено Тогда
					...
					НоваяСтрока.Имя = КлючИЗначение.Ключ;
					НоваяСтрока.Значение = КлючИЗначение.Значение;
					Если СообщатьОМодификацииВнешнихНаборов Тогда
						ирОбщий.СообщитьЛкс(ирОбщий.СтрШаблонЛкс(НСтр("ru='Добавлен недостающий внешний набор данных %1 '"), КлючИЗначение.Ключ));
					КонецЕсли;
				Иначе
					Для Каждого КолонкаПустышки Из КлючИЗначение.Значение.Колонки Цикл
						...
						Если ТипЗнч(ТаблицаВнешнегоНабора) = Тип("ТаблицаЗначений") Тогда
							...
							Если ТаблицаВнешнегоНабора.Колонки.Найти(КолонкаПустышки.Имя) = Неопределено Тогда
								...
								Если СообщатьОМодификацииВнешнихНаборов Тогда
									ирОбщий.СообщитьЛкс(ирОбщий.СтрШаблонЛкс(НСтр("ru='Во внешний набор данных (%1) добавлена недостающая колонка (%2)'"), КлючИЗначение.Ключ, КолонкаПустышки.Имя));
								КонецЕсли;
							КонецЕсли; 
						КонецЕсли; 
					КонецЦикла;
...
Процедура ДеревоНаборовДанныхЗапросовПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки) Экспорт
	...
	ИначеЕсли ТипНабораДанных = Тип("НаборДанныхОбъектСхемыКомпоновкиДанных") Тогда
		...
		ТаблицыВнешнегоНабора = ВнешниеНаборыДанных.НайтиСтроки(Новый Структура("Имя", ДанныеСтроки.НаборДанных.ИмяОбъекта));
		Если ТаблицыВнешнегоНабора.Количество() Тогда
			ОформлениеСтроки.Ячейки.Имя.Текст = ОформлениеСтроки.Ячейки.Имя.Текст + " = " + ЭлементыФормы.ВнешниеНаборыДанных.ОформлениеСтроки(ТаблицыВнешнегоНабора[0]).Ячейки.Значение.Текст;
		КонецЕсли;
...
Процедура КоманднаяПанельСтруктураСохранитьКакНастройкуПоУмолчанию(Кнопка)
	...
	//! СхемаXDTO.settingsVariant = 0 // СписокXDTO
...
Процедура ТабличноеПолеСтруктурыКомпоновкиПереставитьГруппировкиЛкс(Знач ТабличноеПоле, ПеремещаемыйЭлемент, ЭлементЦелевойПозиции, ПереместитьЭлементЦелевойПозицииНаМестоПеремещаемого = Истина,
	...
	СоответствиеИменКоллекций = Новый Структура;
	СоответствиеИменКоллекций.Вставить("Строки", "Row");
	СоответствиеИменКоллекций.Вставить("Колонки", "Column");
	СоответствиеИменКоллекций.Вставить("Структура", "Item");
	...
	Для Каждого СтрокаПути Из ПутьКСтроке1 Цикл
		ЭлементXDTO = ЭлементXDTO[СоответствиеИменКоллекций[СтрокаПути.ИмяКоллекции]][СтрокаПути.Индекс];
	...
	Потомки2 = РодительскийЭлементXDTO2[СоответствиеИменКоллекций[СтрокаПутиРодителя2.ИмяКоллекции]];
	...
	Если ПереместитьЭлементЦелевойПозицииНаМестоПеремещаемого Тогда
		...
		Иначе
			Потомки2 = РодительскийЭлементXDTO1[СоответствиеИменКоллекций[СтрокаПутиРодителя1.ИмяКоллекции]];
	...
	Иначе
		ЭлементXDTO1.Item.Добавить(ЭлементXDTO2);
		Потомки1 = РодительскийЭлементXDTO1[СоответствиеИменКоллекций[СтрокаПутиРодителя1.ИмяКоллекции]];
		Для Каждого БуферныйЭлемент Из Буфер Цикл
...
Процедура КоманднаяПанельТабличныйДокументИсследоватьРасшифровку(Кнопка)
	...
	Если ТипЗнч(Расшифровка) = Тип("ИдентификаторРасшифровкиКомпоновкиДанных") Тогда
		...
		ирОбщий.ИсследоватьЛкс(ЭлементРасшифровки);
...
Процедура ОбновлениеОтображения()
	...
	КонецЕсли; 
	ирКлиент.ТабличноеПолеСОтборомПросмотраОбновитьПредставлениеЛкс(ЭтаФорма, ЭлементыФормы.ТаблицаЗначений);
...
Процедура КоманднаяПанельФормыДанныеРасшифровки(Кнопка)
	Если ДанныеРасшифровки <> Неопределено Тогда
		ирОбщий.ИсследоватьЛкс(ДанныеРасшифровки);
	Иначе
		ирОбщий.СообщитьЛкс("Сначала нужно выполнить вывод результата в табличный документ с расшифровкой");
	КонецЕсли;
КонецПроцедуры
...
Функция ТабличноеПолеНастройкиКомпоновкиДанныхСгруппировать(Знач ТабличноеПоле, Знач ГруппируемыеЭлементы = Неопределено) Экспорт
	...
	Для Каждого СтрокаПути Из ПутьКСтроке Цикл
		лЭлементXDTO = лЭлементXDTO[СоответствиеИменКоллекций[СтрокаПути.ИмяКоллекции]][СтрокаПути.Индекс];
	...
	ИмяКоллекцииXDTO = СоответствиеИменКоллекций[СтрокаПутиНового.ИмяКоллекции];
...
Процедура КоманднаяПанель1ДобавитьЗапрос(Кнопка)
	...
	РедактироватьЗапросСхемы(НоваяСтрокаДерева);
...
Процедура ДоступныеПоляВыбораВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	ирКлиент.ДобавитьПолеВКоллекциюНастроекКомпоновкиПеретаскиваниемЛкс(ВыбраннаяСтрока, ЭлементыФормы.Выбор.Значение);
КонецПроцедуры
...
Процедура КоманднаяПанельДеревоЗначенийРазличныеЗначенияКолонки(Кнопка)
	
	ирКлиент.ОткрытьРазличныеЗначенияКолонкиЛкс(ТекущееТабличноеПолеРезультата());
...
Процедура СохранитьФайлДляВосстановления()
	...
	Тогда
		СохранитьКомпоновкиВФайл(,, "*",, Истина);
	КонецЕсли;
...
Процедура КПДоступныеПоляОбозначитьВЗапросе(Кнопка)
	...
	Если ТекущийЗапрос = Неопределено Тогда
		Возврат;
	КонецЕсли;
	РедактироватьЗапросСхемы(ТекущийЗапрос, ирОбщий.ПервыйФрагментЛкс(ТекущееДоступноеПоле.Поле));
...
Процедура ТаблицаЗначенийИспользоватьОтборПриИзменении(Элемент = Неопределено)
	ирКлиент.ТабличноеПолеСОтборомПросмотраОтобратьЛкс(ЭтаФорма, ЭлементыФормы.ТаблицаЗначений,,, Элемент = Неопределено);
КонецПроцедуры

Процедура ТаблицаЗначенийПодстрокаОтбораПриИзменении(Элемент)
	ирКлиент.ПолеВводаСИсториейВыбора_ПриИзмененииЛкс(Элемент, ЭтаФорма);
	ТаблицаЗначенийИспользоватьОтборПриИзменении();
КонецПроцедуры

Процедура ТаблицаЗначенийОтбораНачалоВыбораИзСписка(Элемент, СтандартнаяОбработка)
	ирОбщий.ПолеВводаСИсториейВыбора_ОбновитьСписокЛкс(Элемент, ЭтаФорма);
КонецПроцедуры

Процедура ТаблицаЗначенийПодстрокаОтбораАвтоПодборТекста(Элемент, Текст, ТекстАвтоПодбора, СтандартнаяОбработка)
	Если ирКлиент.ПромежуточноеОбновлениеСтроковогоЗначенияПоляВводаЛкс(ЭтаФорма, Элемент, Текст) Тогда 
		ТаблицаЗначенийИспользоватьОтборПриИзменении();
	КонецЕсли;
КонецПроцедуры

Процедура КоманднаяПанельТаблицаЗначенийВОтборКомпоновки(Кнопка)
	
	ирОбщий.СкопироватьЭлементыКомпоновкиЛкс(КомпоновщикНастроек.Настройки.Отбор, ТаблицаЗначенийКомпоновщик.Настройки.Отбор);
	ЭтаФорма.ТекущийЭлемент = ЭлементыФормы.Отбор;
	
КонецПроцедуры

Процедура КПОтборДобавитьЭлемент(Кнопка)
	ирКлиент.ДобавитьСтрокуВТабличноеПолеОтбораЛкс(ЭлементыФормы.Отбор, 0);
КонецПроцедуры

Процедура КПОтборДобавитьГруппуИ(Кнопка)
	ирКлиент.ДобавитьСтрокуВТабличноеПолеОтбораЛкс(ЭлементыФормы.Отбор, 2);
КонецПроцедуры

Процедура КПОтборДобавитьГруппуИли(Кнопка)
	ирКлиент.ДобавитьСтрокуВТабличноеПолеОтбораЛкс(ЭлементыФормы.Отбор, 3);
КонецПроцедуры
...
ДеревоНаборовДанныхЗапросов.Колонки.Добавить("НаборДанных"); // НаборДанныхЗапросСхемыКомпоновкиДанных, НаборДанныхОбъектСхемыКомпоновкиДанных, НаборДанныхОбъединениеСхемыКомпоновкиДанных
ДеревоНаборовДанныхЗапросов.Колонки.Добавить("ТипНабораДанных");
ДеревоНаборовДанныхЗапросов.Колонки.Добавить("НомерСтроки");
мСтруктураВосстановления = ирКлиент.НоваяСтруктураВосстановленияКонсолиЛкс("irComposConsole");
