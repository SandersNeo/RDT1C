...
Процедура ЗаполнитьПомеченныеНаУдаление()
	...
	Кнопка = ЭлементыФормы.ДействияФормы.Кнопки.Заполнить;
...
Процедура КонтролироватьЗавершение(СостояниеЗадания = Неопределено, РезультатЗадания = Неопределено) Экспорт 
	...
	Тогда
		...
		Если Истина
			...
			И ЭлементыФормы.ДействияФормы.Кнопки.Удалить.Доступность
		Тогда 
			ОсновныеДействияФормыУдалить(ЭлементыФормы.ДействияФормы.Кнопки.Удалить);
		КонецЕсли;
...
Процедура СсылкиНаУдаляемыеОбъектыПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки) Экспорт
	...
	//	Ячейки.Данные.Текст = ирОбщий.РасширенноеПредставлениеЛкс(КлючОбъекта);
	//КонецЕсли;
...
Процедура ПриОткрытии()
	...
	ирКлиент.Форма_ПриОткрытииЛкс(ЭтаФорма,,, Новый Структура("ПриОткрытии"), "dor", СоставНастройкиФормы);
	...
	КонецЕсли;
	ЭлементыФормы.ПаузаМеждуУдалениями.СписокВыбора.Добавить(1);
...
Процедура ДополнительныеПоляПрочитатьПоля(Кнопка = Неопределено) 
	Кнопка = ЭлементыФормы.КоманднаяПанельДопПоля.Кнопки.ПрочитатьПоля;
	мСтрокаПорядка = ирКлиент.ПрочитатьДопПоляСсылающихсяОбъектовЛкс(ЭлементыФормы.СсылкиНаКандидата, КомпоновщикДопПолей, ТаблицаСсылок,, ЭтаФорма, Кнопка, "ПослеЧтенияДопПолей"); 
КонецПроцедуры                                                      
...
Процедура ПослеЧтенияДопПолей(СостояниеЗадания = Неопределено, РезультатЗадания = Неопределено) Экспорт
	Если РезультатЗадания = Неопределено Тогда
		Возврат;
	КонецЕсли;
	ТаблицаДанных = РезультатЗадания.ТаблицаДанных; // см. ТаблицаСсылок
	Для Каждого СтрокаРезультата Из ТаблицаДанных Цикл
		СтрокаСсылки = ТаблицаСсылок.Найти(СтрокаРезультата.Данные, "Данные");
...
Процедура КоманднаяПанельУдаляемыхОбъектовПараметрыЗаписи(Кнопка)
	...
	ирКлиент.ОткрытьОбщиеПараметрыЗаписиЛкс(Кнопка, Истина);
