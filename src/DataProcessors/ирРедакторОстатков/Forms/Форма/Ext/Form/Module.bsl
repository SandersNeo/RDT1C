...
Функция ПараметрыВыбораОбъектаМетаданных()
	...
	Для Каждого ОписаниеТаблицы Из ирКэш.ТаблицаВсехТаблицБДЛкс() Цикл
		Если ирОбщий.СтрКончаетсяНаЛкс(ОписаниеТаблицы.ПолноеИмя, ".Остатки") Тогда
			ДоступныеОбъекты.Добавить(ирОбщий.ТекстБезПоследнегоФрагментаЛкс(ОписаниеТаблицы.ПолноеИмя));
		КонецЕсли;
	КонецЦикла;
...
Процедура ИмяТаблицыРегистраПриИзменении(Элемент = Неопределено)
	...
	Если ЗначениеЗаполнено(ИмяТаблицыРегистра) Тогда
		...
		#КонецЕсли
	КонецЕсли;
	ЭлементыФормы.КорСчет.Доступность = Истина
		И мОбъектМДРегистра <> Неопределено 
		И Метаданные.РегистрыБухгалтерии.Индекс(мОбъектМДРегистра) >= 0
		И мОбъектМДРегистра.Корреспонденция;
	Если ЭлементыФормы.КорСчет.Доступность Тогда
		ЭлементыФормы.КорСчет.ТолькоПросмотр = Не мОбъектМДРегистра.Корреспонденция;
	КонецЕсли;
	мИмяТаблицыОстатков = "";
	Если мОбъектМДРегистра <> Неопределено Тогда
		...
		мСхемаКомпоновки = ирОбщий.СоздатьСхемуКомпоновкиПоЗапросуЛкс(Запрос);
		//
		...
		Для Каждого ДоступноеПоле Из Компоновщик.Настройки.Выбор.ДоступныеПоляВыбора.Элементы Цикл
			...
			Если ЛиПолеОстатка(ДоступноеПоле.Поле) Тогда
				Если Не ирОбщий.СтрКончаетсяНаЛкс(ДоступноеПоле.Поле, "Остаток") Тогда
					...
					Продолжить;
				КонецЕсли;
				мИменаРесурсов.Добавить(ирОбщий.ТекстБезПоследнегоФрагментаЛкс(ДоступноеПоле.Поле, "Остаток"));
			Иначе
		...
		Если ЭлементыФормы.КорСчет.Доступность Тогда
			...
			КонецЕсли;
			Если ЛиРегистрБухлагтерииСКорреспонденцией() Тогда
...
Процедура УстановитьОстатки(Знач НоваяТаблица)
	...
	Если ЗначениеЗаполнено(мИмяТаблицыОстатков) Тогда
		мИменаИзмеренийСВидами = ирОбщий.СкопироватьКоллекциюЛкс(мИменаИзмерений); 
...
Функция ЛиРегистрБухлагтерииСКорреспонденцией() Экспорт
	Результат = ЭлементыФормы.КорСчет.Доступность И Не ЭлементыФормы.КорСчет.ТолькоПросмотр;
	Возврат Результат;
КонецФункции
...
Функция ИмяКолонкиДвиженияИзИмениРесурса(Знач ИмяРесурса, Знач Сторона = "Дт")
	...
	Суффикс = "";
	Если Истина
		И ЛиРегистрБухлагтерииСКорреспонденцией() 
		И Не мОбъектМДРегистра.Ресурсы[ИмяРесурса].Балансовый 
	Тогда
		Суффикс = Сторона;
	КонецЕсли;
...
Процедура НастроитьЭлементыФормы()
	ЭлементыФормы.КПНабораЗаписей.Кнопки.Записать.Доступность = ЗначениеЗаполнено(Дата) И ЗначениеЗаполнено(ТипРегистратора);
	ЭлементыФормы.КПНабораЗаписей.Кнопки.ОткрытьРегистратор.Доступность = ЗначениеЗаполнено(Дата) И ЗначениеЗаполнено(ТипРегистратора);
	ЭлементыФормы.КПНабораЗаписей.Кнопки.ДвиженияРегистратора.Доступность = ЗначениеЗаполнено(Дата) И ЗначениеЗаполнено(ТипРегистратора);
	ЭлементыФормы.ДействияФормы.Кнопки.ПрочитатьОстатки.Доступность = ЗначениеЗаполнено(Дата) И ЗначениеЗаполнено(ИмяТаблицыРегистра);
	ЭлементыФормы.КПНабораЗаписей.Кнопки.ДвиженияОстатка.Доступность = ЗначениеЗаполнено(Дата) И ЗначениеЗаполнено(ИмяТаблицыРегистра);
КонецПроцедуры
...
Процедура КПНабораЗаписейЗаписать(Кнопка)
	...
	#Если Сервер И Не Сервер Тогда
		КопияНабораЗаписей = Новый ТаблицаЗначений;
	#КонецЕсли
	КопияНабораЗаписей.Колонки.Удалить("МоментВремени");
	ЛиРегистрБухлагтерииСКорреспонденцией = ЛиРегистрБухлагтерииСКорреспонденцией();
...
Процедура ПриОткрытии()  
	...
	Если КлючУникальности <> Неопределено И ТипЗнч(КлючУникальности) = Тип("Строка") Тогда
		...
		Если ОбъектМД <> Неопределено Тогда
			...
			Если ТипЗнч(ПараметрОтбор) = Тип("Структура") Тогда
				ирОбщий.СкопироватьОтборЛюбойЛкс(Компоновщик.Настройки.Отбор, ПараметрОтбор);
				ЭтаФорма.Дата = Неопределено; 
				ДействияФормыПрочитатьОстатки();
			КонецЕсли;
		КонецЕсли;
	КонецЕсли; 
	НастроитьЭлементыФормы();
...
Процедура КПНабораЗаписейДвиженияРегистратора(Кнопка)
...
Процедура ПрочитатьОстаток(Знач ТекущаяСтрока)
	...
	Если ЭлементыФормы.КорСчет.Доступность Тогда 
		...
		Исключение
			ирОбщий.СообщитьЛкс("Не удалось прочитать остаток " + ирОбщий.РасширенноеПредставлениеЛкс(СтруктураКлюча), СтатусСообщения.Внимание);
			Возврат;
		КонецПопытки;
		#Если Сервер И Не Сервер Тогда
...
Функция КлючСтроки(Знач ТекущаяСтрока = Неопределено, Знач УдалитьNull = Истина)
	...
	Если УдалитьNull Тогда
		Для Каждого Ключ Из ирОбщий.ОтобратьКоллекциюЛкс(СтруктураКлюча, Новый Структура("Значение", Null), "Ключ") Цикл
			СтруктураКлюча.Удалить(Ключ);
