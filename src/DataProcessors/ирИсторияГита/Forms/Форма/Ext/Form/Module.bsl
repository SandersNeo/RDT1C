...
Перем мАктивироватьНомерСтроки;
Перем мЗагруженныйКоммит;
Перем мПолеТекстаПоиска Экспорт;

Функция СохраняемаяНастройкаФормы(выхНаименование, выхИменаСвойств) Экспорт 
	выхИменаСвойств = "Реквизит.ПапкаГита, Форма.ЧтоИскать, Форма.МаскаИмениФайла, Реквизит.ЧислоСтрокКонтекста, Форма.Ветка, Форма.ФильтрПодсистем, Форма.КромеВетки, Форма.ШаблонКомментария, Форма.СортироватьПоСтатусу";
...
Процедура ПриОткрытии()
	...
	СписокВыбора = ЭлементыФормы.ПорогСхожести.СписокВыбора;
	СписокВыбора.Добавить(50);
	СписокВыбора.Добавить(60);
	СписокВыбора.Добавить(70);
	СписокВыбора.Добавить(80);
	СписокВыбора.Добавить(90);
	ПриПовторномОткрытии(Ложь);
КонецПроцедуры

Процедура ПриПовторномОткрытии(СтандартнаяОбработка = Истина)
	Если Ложь
		Или ПустаяСтрока(ПапкаГита) И СтандартнаяОбработка
		Или ЗначениеЗаполнено(ПараметрИмяМодуля)
		Или ЗначениеЗаполнено(ПараметрЧтоИскать)
		Или ЗначениеЗаполнено(ПараметрКоммит)
		Или ЗначениеЗаполнено(ПараметрНомерСтроки)
	Тогда
		ЭтаФорма.ПапкаГита = ирКлиент.ГитРепозиторий();
	ИначеЕсли СтандартнаяОбработка Тогда
		// Приходим например в управляемом приложении после вывода сообщения
		Возврат;
	КонецЕсли;
	ОчиститьРезультаты();
	ЭтаФорма.МаскаИмениФайла = "*.bsl | *.os";
	МаскаИмениФайлаПриИзменении();
	Если ПараметрНомерСтроки > 0 Тогда
		ЭтаФорма.ЧтоИскать = Неопределено;
		ЭтаФорма.ФильтрПодсистем = Неопределено;
		ЭтаФорма.КромеВетки = Неопределено;
		ЭтаФорма.Ветка = Неопределено;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ПараметрИмяМодуля) Тогда
		...
		КонецЕсли;
		ИмяФайлаГита = СтрЗаменить(ИмяФайлаГита, "src\", "*"); // чтобы учесть нестабильное наличие сверху папки src
...
Процедура ФайлыКоммитаПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки)
	...
	Если Ложь
		Или Найти(ДанныеСтроки.Статус, "Удален") 
		Или Найти(ДанныеСтроки.Статус, "Переименован") 
	Тогда
		ОформлениеСтроки.Ячейки.Статус.ЦветФона = ирОбщий.ЦветФонаОшибкиЛкс();
	ИначеЕсли Найти(ДанныеСтроки.Статус, "Добавлен") Тогда
...
Функция ОформитьТекстИзмененийХтмл(Знач Текст, Знач ПолеТекстаМодуля = Неопределено, Знач ЛиСыройТекст = Ложь) Экспорт
	...
	КонецЕсли;
	Если ОтображатьКонцыСтрок Тогда
		...
		Для Каждого Строка Из Строки Цикл
			...
			ИначеЕсли ПолеТекстаМодуля <> Неопределено И Найти(Строка, "@@") = 1 Тогда
				...
				Если ПустаяСтрока(ИмяМетода) И НомерСтроки > 0 Тогда
					ИмяМетода = мПлатформа.НайтиМетодПоНомеруСтроки(ПолеТекстаМодуля, НомерСтроки,, ТекстыМодулей,,, Истина);
...
Функция ДобавитьБлокОтличий(Знач ИмяМетода, Знач Строка, Знач НомерСтроки) Экспорт
	...
	Если ПустаяСтрока(ИмяМетода) Тогда
		ИмяМетода = мПлатформа.ИмяМетодаВнеМетодов(); // ~ чтобы при сортировке по алфавиту в конец попадали
		НомерСтроки = 0;
		Строка = "";
	КонецЕсли;
...
Функция ПараметрыВыводаИзмененийДляКоманды() Экспорт
	...
	Результат = " -M" + ПорогСхожести + " --unified=" + ЧислоСтрокКонтекста + " ";
	Возврат Результат;
КонецФункции
...
Процедура ЗагрузитьФайлыКоммита() Экспорт
	...
	КонецЕсли;
	ТекстИзмененийКоммита = ирОбщий.ВыполнитьКомандуОСЛкс(НачалоКомандыГита() + КомандаСравнения + ПараметрыВыводаИзмененийДляКоманды() + ЭлементыФормы.Коммиты.ТекущаяСтрока.Коммит + ПараметрГитаМаскаФайла(),
		,,,, КодировкаТекста.UTF8,, Не ОтображатьКонцыСтрок);
	...
	МаркерРодителя = "commit " + ЭлементыФормы.Коммиты.ТекущаяСтрока.Коммит + " \(from ";
			...
			//КонецЕсли;
	// Заголовок части слияния
	//commit 7a672871a48d0e9dff4913c97032ede42d53dc71 (from 89854fb6a77bc95b59501e19b30d5c8f0daa80a9)
	//Merge: 89854fb6 d104c31f
	//Author: s.starykh s.starykh@com.com
	//Date: Thu Feb 19 09:03:44 2026 +0300
	//Merge remote-tracking branch 'origin/UI2' into main
	ВхожденияРодителей = ирОбщий.НайтиРегВыражениеЛкс(ТекстИзмененийКоммита, "(?:^|\n)commit " + ЭлементыФормы.Коммиты.ТекущаяСтрока.Коммит + " \(from ([\dA-F]+)\)");

	МаркерПредВерсии = "--- "; /// Строго равны по длине
	МаркерТекВерсии = "+++ "; /// Строго равны по длине
	МаркерОтсутствияВерсии = "dev/null";
	ВхожденияФайлов = ирОбщий.НайтиРегВыражениеЛкс(ТекстИзмененийКоммита, 
		"diff --git a/(?:[^\n]+) b/(?:[^\n]+)\n(?:similarity index ([^\n]+\n))?(?:(?:rename|new file|deleted file|index)[^\n]+\n)*(\-\-\- ([^\n]+)\n\+\+\+ ([^\n]+)\n)", "Схожесть,Соответствие,Файл1,Файл2");
	...
	Для Индекс = 0 По ВхожденияФайлов.Количество() - 2 Цикл
		...
		Если Истина
			И ВхожденияРодителей.Количество() > ИндексРодителя
			И ВхожденияРодителей[ИндексРодителя].ПозицияВхождения < ВхожденияЭлемент.ПозицияВхождения
		Тогда
			НомерРодителяОтображаемый = Родители.Найти(ВхожденияРодителей[ИндексРодителя].Группа0, "Коммит").Номер;
			ИндексРодителя = ИндексРодителя + 1;
		КонецЕсли;
		ИмяФайла1 = ирОбщий.ТекстБезПервогоФрагментаЛкс(ВхожденияЭлемент.Файл1, "/");
		ИмяФайла2 = ирОбщий.ТекстБезПервогоФрагментаЛкс(ВхожденияЭлемент.Файл2, "/");
		Если МаркерОтсутствияВерсии = ИмяФайла1 Тогда
			ТекущийФайл = ИмяФайла2;
		Иначе
			ТекущийФайл = ИмяФайла1;
		КонецЕсли;
		ЗаголовокСравнения = "";
		Если ЗначениеЗаполнено(ВхожденияЭлемент.Схожесть) Тогда
			ЗаголовокСравнения = "Схожесть содержимого файлов = " + ВхожденияЭлемент.Схожесть + ВхожденияЭлемент.Соответствие;
		КонецЕсли;
		Модуль = ирКлиент.ИмяМодуляИзИмениФайлаГитаЛкс(ТекущийФайл);
		Если ЗначениеЗаполнено(Модуль) Тогда
			ПолноеИмяМД = ирКлиент.ПолноеИмяМДИзИмениМодуляЛкс(Модуль);
			Если ФильтрПодсистем.Количество() > 0 Тогда
		...
		Если ТекущийФайл <> Неопределено Тогда
			...
			ТекстБлока = ЗаголовокСравнения + Сред(ТекстИзмененийКоммита, НачальныйНомер, ВхождениеСледующее.ПозицияВхождения - НачальныйНомер);
			...
			Тогда
				...
				КонецЕсли;
				Если ИмяФайла1 = МаркерОтсутствияВерсии Тогда
					СтатусФайла = "Добавлен";
					Если ЗначениеЗаполнено(МаскаИмениФайла) Или ЗначениеЗаполнено(ЧтоИскать) Тогда
						СтатусФайла = СтатусФайла + "*";
					КонецЕсли;
				ИначеЕсли ИмяФайла2 = МаркерОтсутствияВерсии Тогда
					СтатусФайла = "Удален";
					Если ЗначениеЗаполнено(МаскаИмениФайла) Или ЗначениеЗаполнено(ЧтоИскать) Тогда
						СтатусФайла = СтатусФайла + "*";
					КонецЕсли;
				ИначеЕсли ИмяФайла2 <> ИмяФайла1 Тогда
					СтатусФайла = "Переименован";
				Иначе
					СтатусФайла = "";
				КонецЕсли;
				СтрокаФайла.Статус = СтатусФайла;
				СтрокаФайла.НомерРодителя = НомерРодителяОтображаемый;
	...
	Если ФайлыКоммита.Количество() Тогда
		СортироватьФайлы();
		Если Истина
			И СтарыйФайл <> Неопределено 
			И (Ложь
				Или Не СортироватьПоСтатусу
				Или мЗагруженныйКоммит = ЭлементыФормы.Коммиты.ТекущаяСтрока.Коммит)
		Тогда
		...
		КонецЕсли;
	КонецЕсли;
	мЗагруженныйКоммит = ЭлементыФормы.Коммиты.ТекущаяСтрока.Коммит;

КонецПроцедуры

Процедура СортироватьФайлы() Экспорт
	Если СортироватьПоСтатусу Тогда
		ФайлыКоммита.Сортировать("Статус Убыв, Файл");
	Иначе
...
Функция КомандаВыводаАтрибутовКоммита() Экспорт
	КомандаВыводаКоммита = НачалоКомандыГита() + " log" + ПараметрыВыводаДанныхКоммита() + " -1 ";
...
Функция ПоследниеВыбранныеНажатие(Кнопка) Экспорт
	ирКлиент.ПоследниеВыбранныеНажатиеЛкс(ЭтаФорма, ЭлементыФормы.Коммиты, Метаданные().ТабличныеЧасти.Коммиты.Реквизиты.Коммит.Имя, Кнопка,, ПапкаГита);
КонецФункции

Процедура ПоследниеВыбранныеДобавить(Знач Строка = Неопределено) Экспорт 
	Если Строка = Неопределено Тогда
		Строка = ЭлементыФормы.Коммиты.ТекущаяСтрока;
	КонецЕсли;
	ПредставлениеЗначения = "" + Строка.Дата + " " + Строка.Автор + ", " + ирОбщий.ПредставлениеСОбрезкойДлиныЛкс(Строка.Комментарий);
	ирКлиент.ПоследниеВыбранныеДобавитьЛкс(ЭтаФорма, Строка.Коммит, ПредставлениеЗначения, ЭлементыФормы.Коммиты,, ПапкаГита);
	ОбновитьПодменюПоследниеВыбранные();
КонецПроцедуры   

Процедура ОбновитьПодменюПоследниеВыбранные()
	ирКлиент.ПоследниеВыбранныеЗаполнитьПодменюЛкс(ЭтаФорма, ЭлементыФормы.КПКоммиты.Кнопки.ПоследниеВыбранные, ЭлементыФормы.Коммиты,,, ПапкаГита);
КонецПроцедуры
...
Процедура ДействияФормыЗагрузить(Кнопка = Ложь)
	Если мФайлРезультата = Неопределено Тогда
		ТекущаяСтрока = ЭлементыФормы.Коммиты.ТекущаяСтрока;
		Если ЗначениеЗаполнено(ПараметрКоммит) Тогда
			мСтарыйКоммит = ПараметрКоммит;
			ПараметрКоммит = "";
		...
		КонецЕсли;
		КомандаСистемы = НачалоКомандыГита() + " log" + ПараметрыВыводаДанныхКоммита(, Истина);
		Если ЗначениеЗаполнено(ЧтоИскать) Тогда
			КомандаСистемы = КомандаСистемы + " -m -G " + ВТекстовыйЛитералГит(РегВыражениеПоиска());
		КонецЕсли;
		ОтборПоВеткам = "";
		...
		ирОбщий.ЗапуститьСкрытоеПриложениеИДождатьсяЗавершенияЛкс(КомандаСистемы, мФайлРезультата.Путь, мФайлРезультата.Имя, Ложь,,, Истина);
		ОчиститьРезультаты();
...
Функция ВТекстовыйЛитералГит(Знач Текст) Экспорт
	Текст = СтрЗаменить(Текст, "\\", "\\\\"); // Первый!
...
Процедура ЗагрузитьПорциюКоммитов()
	...
	Иначе
		...
		Если мФайлРезультата.Существует() Тогда
			Если мЧтениеТекста = Неопределено Тогда
				мЧтениеТекста = Новый ЧтениеТекста(мФайлРезультата.ПолноеИмя, КодировкаТекста.UTF8,,, Ложь);
			КонецЕсли;
			Попытка 
			...
			Если ПорцияРезультатов <> Неопределено Тогда
				...
				Если Вхождения = Неопределено Тогда
					ПоискЗакончен = Истина;   
				ИначеЕсли Вхождения.Количество() Тогда
				...
				Иначе
					мОстатокПорции = ТекстДляАнализа;
				КонецЕсли;
			КонецЕсли;
		ИначеЕсли ПрошлоСекунд > 10 Тогда
			// Вслепую решаем что процесс отбора коммитов завершен. TODO сделать отслеживание процесса выполнения скрипта
			ПоискЗакончен = Истина;
		КонецЕсли;
	КонецЕсли;
	Если ПоискЗакончен Тогда
		...
		//Если мФайлОшибокЗапуска.Существует() Тогда
		//	ТекстОшибки = Новый ТекстовыйДокумент;
		//	Попытка
		//		ТекстОшибки.Прочитать(мФайлОшибокЗапуска.ПолноеИмя, КодировкаТекста.UTF8);
		//	Исключение
		//		//
		//	КонецПопытки;
		//	ТекстСообщения = ТекстОшибки.ПолучитьТекст();
		//	Если ЗначениеЗаполнено(ТекстСообщения) Тогда
		//		ирОбщий.СообщитьЛкс("Ошибка скрипта загрузки: " + ТекстСообщения);
		//	КонецЕсли;
		//КонецЕсли; 
		мЧтениеТекста = Неопределено;
		Если мФайлРезультата <> Неопределено Тогда
			УдалитьФайлы(мФайлРезультата.ПолноеИмя);
			мФайлРезультата = Неопределено; 
		КонецЕсли;
	Иначе
...
Функция ЗагрузитьСписокКоммитов(Знач ТаблицаКоммитов, Знач ТекстДляАнализа, Знач Роль = "") Экспорт
	...
	Для Каждого ВхожденияЭлемент Из Вхождения Цикл
		...
		КонецЕсли;
		Если Ложь
			// Так затормозим загрузке коммитов
			//Или (Истина
			//	И Не АктивироватьСтарыйКоммит
			//	И ЭлементыФормы.Коммиты.ТекущаяСтрока = Неопределено)
			Или (Истина
...
Процедура ОбновитьПоискВТекстеФайла(Знач ПереносСлов = Ложь, Знач ЛиРасширениеФайлаКода1С = Неопределено, Знач ТекстВерсииФайла = Неопределено) Экспорт
	СтруктураОтличий.Очистить();
	Если ЛиРасширениеФайлаКода1С = Неопределено Тогда
		ЛиРасширениеФайлаКода1С = ЛиРасширениеФайлаКода1С(ЭлементыФормы.ФайлыКоммита.ТекущаяСтрока.ТипФайла);
	КонецЕсли;
	Если ЛиРасширениеФайлаКода1С Тогда
		ПолеТекстаМодуля = ирОбщий.НовыйАнализаторКодаЛкс(0);
		Если ТекстВерсииФайла = Неопределено Тогда
		...
		Для Каждого РегЭлемент Из мПлатформа.НайтиЗаголовкиИКонцыМетодов(ТекстИзмененийФайла, Ложь) Цикл
			ПропущенныйТекст = Сред(ТекстИзмененийФайла, ПредПозиция0 , РегЭлемент.FirstIndex - ПредПозиция0 +1);
			НомерСтрокиИзменений = НомерСтрокиИзменений + ирОбщий.СтрЧислоСтрокЛкс(ПропущенныйТекст) - 1;
			ДобавитьБлокОтличий(РегЭлемент.SubMatches(0), СокрЛП(РегЭлемент.Value), НомерСтрокиИзменений);
			ПредПозиция0 = РегЭлемент.FirstIndex;
		КонецЦикла;
	Иначе
		ПолеТекстаМодуля = Неопределено;
	КонецЕсли;
	мАктивироватьНомерСтроки = Неопределено;
	Если ЗначениеЗаполнено(ПараметрНомерСтроки) Тогда
		Вхождения = ирОбщий.НайтиРегВыражениеЛкс(ТекстИзмененийФайла, "@@ \-\d+,\d+ \+(\d+),\d+ @@");
		ПредВхождение = Неопределено;
		ПредНомерСтроки = 0;
		Для Каждого ВхожденияЭлемент Из Вхождения Цикл
			НомерСтрокиЦикл = Число(ВхожденияЭлемент.Группа0);
			Если НомерСтрокиЦикл > ПараметрНомерСтроки Тогда
				Прервать;
			КонецЕсли;
			ПредВхождение = ВхожденияЭлемент;
			ПредНомерСтроки = НомерСтрокиЦикл;
		КонецЦикла;
		Если ПредВхождение <> Неопределено Тогда
			СмещениеСтрокиВБлоке = ПараметрНомерСтроки - ПредНомерСтроки + 1;
			мАктивироватьНомерСтроки = ирОбщий.СтрЧислоСтрокЛкс(Лев(ТекстИзмененийФайла, ПредВхождение.ПозицияВхождения + 1));
			Текстовый = Новый ТекстовыйДокумент;
			Текстовый.УстановитьТекст(Сред(ТекстИзмененийФайла, ПредВхождение.ПозицияВхождения + СтрДлина(ПредВхождение.ТекстВхождения) + 2));
			Для НомерСтроки = 1 По Текстовый.КоличествоСтрок() Цикл
				мАктивироватьНомерСтроки = мАктивироватьНомерСтроки + 1;
				Строка = Текстовый.ПолучитьСтроку(НомерСтроки);
				Если Лев(Строка, 1) <> "-" Тогда
					СмещениеСтрокиВБлоке = СмещениеСтрокиВБлоке - 1;
					Если СмещениеСтрокиВБлоке = 0 Тогда
						Прервать;
					КонецЕсли;
				КонецЕсли; 
			КонецЦикла;
		КонецЕсли;
		ЭтаФорма.ПараметрНомерСтроки = "";
	КонецЕсли;
...
Процедура СледующееВхождениеНажатие(Элемент = Неопределено)
	Если мАктивироватьНомерСтроки <> Неопределено Тогда
		мПолеТекстаПоиска.УстановитьГраницыВыделения(мАктивироватьНомерСтроки, 1, мАктивироватьНомерСтроки, 30, , ЭтаФорма);
		мАктивироватьНомерСтроки = Неопределено;
	Иначе
...
Процедура ВеткаПриИзменении(Элемент)
	ирКлиент.ПолеВводаСИсториейВыбора_ПриИзмененииЛкс(Элемент, ЭтаФорма,,, ПапкаГита);
	Элемент.ЦветТекстаПоля = ирОбщий.ЦветТекстаИзмененныхДанныхЛкс();
КонецПроцедуры

Процедура ВеткаНачалоВыбораИзСписка(Элемент, СтандартнаяОбработка)
	ирОбщий.ПолеВводаСИсториейВыбора_ОбновитьСписокЛкс(Элемент, ЭтаФорма, ПапкаГита);
КонецПроцедуры

Процедура ВеткаНачалоВыбора(Элемент, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	СписокВеток = Новый ТаблицаЗначений;
	СписокВеток.Колонки.Добавить("Значение", Новый ОписаниеТипов("Строка"));
	СписокВеток.Колонки.Добавить("ДатаСоздания", Новый ОписаниеТипов("Дата"));
	РазделительДаты = " от ";
	Текст = ирОбщий.ВыполнитьКомандуОСЛкс(НачалоКомандыГита() + " for-each-ref --format=""%(refname:short)"+РазделительДаты+"%(creatordate:short)""",,,,, КодировкаТекста.UTF8);
	Текст = СтрЗаменить(Текст, "* ", "");
	Массив = ирОбщий.СтрРазделитьЛкс(Текст, Символы.ПС, Истина);
	Для Каждого МассивЭлемент Из Массив Цикл
		Фрагменты = ирОбщий.СтрРазделитьЛкс(МассивЭлемент, РазделительДаты, Истина);
		Строка = СписокВеток.Добавить();
		Строка.Значение = Фрагменты[0];
		Строка.ДатаСоздания = ирОбщий.СтрокаВДатуЛкс(Фрагменты[1]);
	КонецЦикла;
	СписокВеток.Сортировать("ДатаСоздания Убыв");
	ЭлементСписка = СписокВеток.Найти(Элемент.Значение, "Значение");
	ВыбранныйЭлемент = ирКлиент.ОткрытьТаблицуЗначенийЛкс(СписокВеток, ЭлементСписка, Истина, "Выбор ветки", Истина,,, "ВыборВеткиГита");
...
Процедура КПРезультатыПоискаСравнитьВСравнителеТекстов(Кнопка)
	...
	Если ТекущаяСтрока.Статус = "Добавлен" Тогда
		ТекстПредыдущий = "";
	Иначе
...
Функция НомерСтрокиВМодуле(Знач ЛиПредыдущаяВерсия = Ложь) Экспорт
	...
	Если ЗначениеЗаполнено(ТекстНомераСтроки) Тогда
		...
		ЧислоИзмененныхСтрок = СтрЧислоВхождений(НачалоБлока, Символы.ПС + НачальныйМаркер);
		НомерСтроки1 = Число(ТекстНомераСтроки) - 1 + ЧислоСтрокКонтекстаБлока + ЧислоИзмененныхСтрок;
	КонецЕсли;
	Возврат НомерСтроки1;
КонецФункции
...
Функция ТекстВерсииФайла(Знач ЛиПредыдущаяВерсия = Ложь) Экспорт
	...
	Если ЛиПредыдущаяВерсия Тогда
		...
		СтрокаРодителя = Родители.Найти(ЭлементыФормы.ФайлыКоммита.ТекущаяСтрока.НомерРодителя, "Номер");
		Если СтрокаРодителя = Неопределено Тогда
			Возврат "";
		КонецЕсли;
		Команда = Команда + СтрокаРодителя.Коммит;
	Иначе
		Если ПустаяСтрока(ЭлементыФормы.Коммиты.ТекущаяСтрока.Коммит) Тогда
...
Функция ПараметрыКоммитаДляКоманды() Экспорт
	ПараметрыКоммитаДляКоманды = ЭлементыФормы.Коммиты.ТекущаяСтрока.Коммит
		+ ?(Родители.Количество() < 2, "", "^^" + ЭлементыФормы.Родители.ТекущаяСтрока.Номер);
...
Процедура ФильтрФайловПриИзменении(Элемент)
	ОбновитьОтборПоИмениФайла();
	ирКлиент.ПолеВводаСИсториейВыбора_ПриИзмененииЛкс(Элемент, ЭтаФорма);
КонецПроцедуры

Процедура ОбновитьОтборПоИмениФайла() Экспорт
	ирОбщий.УстановитьОтборПоПодстрокеЛкс(ЭлементыФормы.ФайлыКоммита.ОтборСтрок.Файл, ФильтрФайлов);
КонецПроцедуры
...
Процедура ВыгрузитьФайлВерсии(Знач ИмяФайла, Знач СообщитьИмя = Истина) Экспорт
...
Процедура ДействияФормыИзмененныеМодули(Кнопка)
	ПолучитьФорму("ИзмененныеМодули").Открыть();
КонецПроцедуры

Процедура СтруктураОтличийПриАктивизацииСтроки(Элемент) Экспорт
	Если мРазрешитьПрыжокВНачалоБлока Тогда
		Если Элемент.ТекущаяСтрока <> Неопределено Тогда
			мПолеТекстаПоиска.НайтиФрагментТекста(Элемент.ТекущаяСтрока.Идентификатор);
		КонецЕсли;
	КонецЕсли;
...
Процедура АктивироватьБлокИзТекста(Знач ТабличноеПоле = Неопределено) Экспорт
	Если СтруктураОтличий.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	ВыделениеТекста = мПолеТекстаПоиска.ПолучитьВыделениеВДокументеHTML();
	НомерСтрокиТекущий = ирОбщий.СтрЧислоСтрокЛкс(Лев(мПолеТекстаПоиска.ПолучитьТекст(), ВыделениеТекста.Конец));
	...
	КонецЦикла;
	мРазрешитьПрыжокВНачалоБлока = Ложь;
	Если СтрокаАктивногоБлока <> Неопределено Тогда
		РасчетнаяСтрокаБлока = СтруктураОтличий.Найти(СтрокаАктивногоБлока.Блок, "Блок");
		Если ТабличноеПоле = Неопределено Тогда
			ТабличноеПоле = ЭлементыФормы.СтруктураОтличий;
		КонецЕсли;
		ТабличноеПоле.ТекущаяСтрока = РасчетнаяСтрокаБлока;
	КонецЕсли;
КонецПроцедуры
...
Процедура ФильтрФайловАвтоПодборТекста(Элемент, Текст, ТекстАвтоПодбора, СтандартнаяОбработка)
	ирКлиент.ПромежуточноеОбновлениеСтроковогоЗначенияПоляВводаЛкс(ЭтаФорма, Элемент, Текст);
	ОбновитьОтборПоИмениФайла();
КонецПроцедуры

Процедура СортироватьПоСтатусуПриИзменении(Элемент)
	СортироватьФайлы();
КонецПроцедуры

Процедура ПорогСхожестиПриИзменении(Элемент)
	ПодключитьОбработчикОжидания("ЗагрузитьФайлыКоммита", 0.1, Истина );
КонецПроцедуры
...
Процедура ФайлыКоммитаВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	Если ирОбщий.СтрКончаетсяНаЛкс(ВыбраннаяСтрока.Файл, "/form.txt") Тогда
		ИмяФайла = ПолучитьИмяВременногоФайла();
		ВыгрузитьФайлВерсии(ИмяФайла, Ложь);
		ТекстФайла = "{""#"",5c83cba4-7a20-4102-a5be-add0ee74f6a1," + ирОбщий.ПрочитатьТекстИзФайлаЛкс(ИмяФайла) + "}";
		СтруктураФормы = ирКлиент.ПолучитьФормуЛкс("Обработка.ирПлатформа.Форма.СтруктураФормы",,, ВыбраннаяСтрока.Файл + "," + ЭлементыФормы.Коммиты.ТекущаяСтрока.Коммит);
		СтруктураФормы.ДобавитьФормуИзСнимка(ТекстФайла, Тип("Форма"));
	КонецЕсли;
КонецПроцедуры
