...
Функция УстановитьИсследуемоеЗначение(Знач НоваяКоллекция, Знач ПутьКДанным = Неопределено, Знач пСтруктураТипаКоллекции = Неопределено, Знач БезСлужебныхКолонок = Ложь) Экспорт 
	...
	Иначе
		...
		//		СтрокаОбщегоТипа = мПлатформа.ОписаниеОбщегоТипаПоБазовому(СтрокаОписанияСлова.ТипЗначения);
...
Процедура ОбновитьДанные()
	...
	ирКлиент.ТабличноеПолеРезультатаЗапросаНастроитьКолонкиЛкс(ЭлементыФормы.ТаблицаКоллекции,,,, Ложь);
...
Процедура ТаблицаКоллекцииПриАктивизацииСтроки(Элемент)
	...
	Если Элемент.ТекущаяСтрока <> Неопределено Тогда
		Если Элемент.ТекущаяСтрока._ТипЭлементаКоллекции = Тип("КлючИЗначение") Тогда
			Ключ = Элемент.ТекущаяСтрока.Ключ; // Строка - @skip-check unknown-method-property
			ИндексЭлемента = """" + Ключ + """";
		Иначе
...
Процедура ТаблицаКоллекцииПередУдалением(Элемент, Отказ)
	Попытка
		_Значение_.Удалить(ЭлементыФормы.ТаблицаКоллекции.ТекущиеДанные._ЗначениеЭлементаКоллекции);
	Исключение
		Отказ = Истина;
		Возврат;
	КонецПопытки;
...
Процедура ТаблицаКоллекцииПередНачаломДобавления(Элемент, Отказ, Копирование)
	Отказ = Истина;
КонецПроцедуры

Функция ПредставлениеОбъектаДанных() Экспорт
	Результат = ирОбщий.ПредставлениеСОбрезкойДлиныЛкс(_Значение_);
	Возврат Результат;
КонецФункции
...
//! ВладелецФормы = ПолучитьФорму("ИсследовательОбъектов");
