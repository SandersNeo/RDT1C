...
Процедура ЗаполнитьСписокНайденных()
	...
	Если мПорцияРезультата.СлишкомМногоРезультатов() Тогда
		ирОбщий.СообщитьЛкс("Слишком много результатов, уточните запрос.");
	КонецЕсли;
	ДоступностьКнопок();
КонецПроцедуры
...
Процедура ПриОткрытии()
	...
	Если ирОбщий.РежимСовместимостиМеньше8_3_4Лкс() Тогда
		мИмяСвойстваНомера = "nameProp";
	Иначе
		мИмяСвойстваНомера = "sel_num";
	КонецЕсли;
	//! мИмяСвойстваНомера = "";
...
Процедура HTMLОтображениеonclick(Элемент, pEvtObj)
	...
	Если htmlElement.id = "FullTextSearchListItem" Тогда
		Если ирОбщий.РежимСовместимостиМеньше8_3_4Лкс() Тогда
			номерВСписке = Число(htmlElement[мИмяСвойстваНомера]);
		Иначе
			номерВСписке = Число(htmlElement.attributes[мИмяСвойстваНомера].value);
		КонецЕсли;
...
Функция ПроверитьДоступностьПоиска()
	...
	Если Не ПравоДоступа("Администрирование", Метаданные) Тогда
		ирОбщий.СообщитьЛкс(СтрОшибки + "Для включения поиска обратитесь к администратору.");
		Возврат Ложь;
	КонецЕсли;
	...
	Исключение
		ирОбщий.СообщитьЛкс("Ошибка при включении полнотекстового поиска:
				|" + ОписаниеОшибки());
		Возврат Ложь;
	КонецПопытки;
	...
	Если Не ПолнотекстовыйПоиск.ИндексАктуален() Тогда
		...
		Если Вопрос(СтрВопроса, РежимДиалогаВопрос.ДаНет) = КодВозвратаДиалога.Да Тогда
			...
			Исключение
				ирОбщий.СообщитьЛкс("Ошибка при обновлении индекса:
						|" + ОписаниеОшибки());
			КонецПопытки;
		КонецЕсли;
	КонецЕсли;
...
Процедура КоманднаяПанельКонсольОбработки(Кнопка)
	Если ЭлементыФормы.ПанельРезультат.ТекущаяСтраница = ЭлементыФормы.ПанельРезультат.Страницы.HtmlПорции Тогда
		ШаблонПоиска = мИмяСвойстваНомера + "=""(\d+)""";
