...
Процедура ТаблицаЗагружаемыхРеквизитовВыражениеНачалоВыбора(_Элемент = Неопределено, СтандартнаяОбработка = Истина)
	Элемент = ЭлементыФормы.СопоставлениеКолонокТЗ.Колонки.Алгоритм.ЭлементУправления;
	СтандартнаяОбработка = Ложь;
...
Процедура СопоставлениеКолонокТЗВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	...
	ИначеЕсли Колонка = ЭлементыФормы.СопоставлениеКолонокТЗ.Колонки.ОписаниеТипов Тогда
		...
		КонецЕсли;
	ИначеЕсли Колонка = ЭлементыФормы.СопоставлениеКолонокТЗ.Колонки.Алгоритм Тогда
		Элемент.ИзменитьСтроку();
		ТаблицаЗагружаемыхРеквизитовВыражениеНачалоВыбора();
...
Функция СтрокаКолонокКлючаБД()
	...
	СтрокаКолонокКлючаБД = ирОбщий.СтрСоединитьЛкс(ПоляПолногоКлюча,,,,, "Представление");
...
Процедура СопоставлениеКолонокТЗАлгоритмОчистка(Элемент, СтандартнаяОбработка)
	Если Не ПустаяСтрока(Элемент.Значение) Тогда
		Ответ = Вопрос("Очистить текст алгоритма?", РежимДиалогаВопрос.ОКОтмена);
...
Процедура ЗаполнитьСоответствиеТЗИИ(Кнопка)
	Если Кнопка.Картинка <> ирКэш.КартинкаПоИмениЛкс("ирОстановить") Тогда
		МассивИмен1 = СопоставлениеКолонокТЗ.Скопировать(Новый Структура("ИмяКолонкиИсточника", "")).ВыгрузитьКолонку("ИмяКолонкиПриемника");
		Если МассивИмен1.Количество() = 0 Тогда
			Возврат;
		КонецЕсли;
		МассивИмен2 = Новый Массив;
		Для Каждого КлючИЗначение Из ПолучитьКолонкиТабличногоДокумента() Цикл
			МассивИмен2.Добавить(КлючИЗначение.Значение.Имя + " - " + КлючИЗначение.Значение.Заголовок);
		КонецЦикла;
		ТекстПромта = ирОбщий.ТекстЗапросаСоответствияИменИИЛкс(МассивИмен1, МассивИмен2);
	КонецЕсли;
	БлокируемыеЭлементы = Новый Массив;
	БлокируемыеЭлементы.Добавить(ЭлементыФормы.СопоставлениеКолонокТЗ);
	ПараметрыЗадания = Новый Структура;
	ПараметрыЗадания.Вставить("Текст", ТекстПромта);
	мФоновыйЗапросХттп.ОтправитьЗапросИИ(ПараметрыЗадания, ЭтаФорма, "ЗаполнитьТЗИИКонец", БлокируемыеЭлементы, Кнопка);
КонецПроцедуры
...
Процедура ЗаполнитьТЗИИКонец(СостояниеЗадания, РезультатЗадания) Экспорт
	Если РезультатЗадания = Неопределено Тогда
		Возврат;
	КонецЕсли;
	Ответ = РезультатЗадания.Ответ;
	РезультатИИ = ирОбщий.ОбъектИзТекстаЖСОНЛкс(ирОбщий.ТекстМеждуМаркерамиЛкс(РезультатЗадания.Ответ, "```json", "```")); // Соответствие
	КолонкиТабличногоДокумента = ПолучитьКолонкиТабличногоДокумента();
	Для Каждого КлючИЗначение Из РезультатИИ Цикл
		Если ПустаяСтрока(КлючИЗначение.Значение) Тогда
			Продолжить;
		КонецЕсли;
		СтрокаСопоставления = СопоставлениеКолонокТЗ.Найти(КлючИЗначение.Ключ, "ИмяКолонкиПриемника");
		КолонкаИсточника = КолонкиТабличногоДокумента[ирОбщий.ПервыйФрагментЛкс(КлючИЗначение.Значение, " ")];
		Если КолонкаИсточника <> Неопределено Тогда
			СопоставитьКолонкуТЗ(СтрокаСопоставления, КолонкаИсточника);
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры

Процедура ЗаполнитьСоответствиеБДИИ(Кнопка)
	Если Кнопка.Картинка <> ирКэш.КартинкаПоИмениЛкс("ирОстановить") Тогда
		МассивИмен1 = Новый Массив;
		Для Каждого СтрокаТаблицы Из СопоставлениеКолонокБД.Скопировать(Новый Структура("ИмяКолонкиИсточника", "")) Цикл
			МассивИмен1.Добавить(СтрокаТаблицы.ИмяКолонкиПриемника + " - " + СтрокаТаблицы.СинонимКолонкиПриемника + " - " + СтрокаТаблицы.ОписаниеТиповПриемника);
		КонецЦикла;
		Если МассивИмен1.Количество() = 0 Тогда
			Возврат;
		КонецЕсли;
		...
		Для Каждого Колонка Из ТаблицаЗначений.Колонки Цикл
			МассивИмен2.Добавить(Колонка.Имя + " - " + Колонка.Заголовок + " - " + Колонка.ТипЗначения);
		КонецЦикла;
		ТекстПромта = ирОбщий.ТекстЗапросаСоответствияИменИИЛкс(МассивИмен1, МассивИмен2);
	КонецЕсли;
	БлокируемыеЭлементы = Новый Массив;
	БлокируемыеЭлементы.Добавить(ЭлементыФормы.СопоставлениеКолонокБД);
	ПараметрыЗадания = Новый Структура;
	ПараметрыЗадания.Вставить("Текст", ТекстПромта);
	мФоновыйЗапросХттп.ОтправитьЗапросИИ(ПараметрыЗадания, ЭтаФорма, "ЗаполнитьБДИИКонец", БлокируемыеЭлементы, Кнопка);
КонецПроцедуры
...
//   СостояниеЗадания -  -
//   РезультатЗадания - см. ирОбщий.ОтправитьЗапросИИЛкс -
Процедура ЗаполнитьБДИИКонец(СостояниеЗадания, РезультатЗадания) Экспорт
	Если РезультатЗадания = Неопределено Тогда
		Возврат;
	КонецЕсли;
	Ответ = РезультатЗадания.Ответ;
	...
	Для Каждого КлючИЗначение Из РезультатИИ Цикл
		Если ПустаяСтрока(КлючИЗначение.Значение) Тогда
			Продолжить;
		КонецЕсли;
		СтрокаСопоставления = СопоставлениеКолонокБД.Найти(ирОбщий.ПервыйФрагментЛкс(КлючИЗначение.Ключ), "ИмяКолонкиПриемника");
		КолонкаИсточника = ТаблицаЗначений.Колонки[ирОбщий.ПервыйФрагментЛкс(КлючИЗначение.Значение, " ")];
		Если КолонкаИсточника <> Неопределено Тогда
			СопоставитьКолонкуБД(СтрокаСопоставления, КолонкаИсточника);
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры

Процедура ОткрытьПараметрыИИ(Кнопка)
