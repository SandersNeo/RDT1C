...
Процедура ПриОткрытии()
	...
	КонецЕсли;
	ирКлиент.Форма_ПриОткрытииЛкс(ЭтаФорма, КлючСохраненияПоложенияОкна);
...
Процедура НаборыПолейПриАктивизацииСтроки(Элемент)
	...
	Если Истина
		...
		И ГлавноеТабличноеПоле() = ЭлементыФормы.Таблицы
		И ТипЗнч(СтрокаТаблиц.КоличествоСтрок) = Тип("Число")
...
Процедура ТаблицыВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	...
	КонецЕсли; 
	Если Ложь
		Или Колонка = ЭлементыФормы.Таблицы.Колонки.Метаданные 
		Или Колонка = ЭлементыФормы.Таблицы.Колонки.ИмяТаблицы 
	Тогда
		Если ирОбщий.СтрКончаетсяНаЛкс(ВыбраннаяСтрока.ИмяТаблицы, ".Изменения") Тогда
			Форма = ирКлиент.ПолучитьФормуЛкс("Обработка.ирРедакторИзмененийНаУзлах.Форма");
			Форма.ПараметрТаблицаБД = ирОбщий.ТекстБезПоследнегоФрагментаЛкс(ВыбраннаяСтрока.ИмяТаблицы);
			Форма.Открыть(); 
		Иначе
...
Процедура КонтекстноеМенюТаблицыОткрытьЗапросСУБД(Кнопка)
	...
	ирКлиент.ОткрытьЗапросСУБДЛкс(Запрос, "Выборка из " + ЭлементыФормы.Таблицы.ТекущаяСтрока.ИмяТаблицыХранения);
КонецПроцедуры
