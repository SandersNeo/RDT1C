...
Процедура ПередОткрытием(Отказ, СтандартнаяОбработка)
	...
	ЭтаФорма.ИмяФайлаНастроекБазы = мПлатформа.ИмяФайлаНастроекКомпьютера(Истина);
	ЭтаФорма.ИмяФайлаНастроекВсехБаз = мПлатформа.ИмяФайлаНастроекКомпьютера(Ложь);
	...
	ЭтаФорма.ИспользоватьЭмуляциюНажатияКлавиш = мПлатформа.ИспользоватьЭмуляциюНажатияКлавиш;
	ЭтаФорма.ВыделениеРезультатовПоиска = мПлатформа.ВыделениеРезультатовПоиска;
	ЭтаФорма.ЛиГлобальныеНастройкиВсехБаз = мПлатформа.ЛиГлобальныеНастройкиВсехБаз;
...
Процедура ОсновныеДействияФормыСохранитьНастройки(Кнопка = Неопределено)
	...
	ПрочитатьГлобальныеНастройки = Ложь;
	Если Не мПлатформа.ЛиГлобальныеНастройкиВсехБаз И ЛиГлобальныеНастройкиВсехБаз Тогда
		Файл = Новый Файл(ИмяФайлаНастроекВсехБаз);
		Если Файл.Существует() Тогда
			Ответ = Вопрос("Файл глобальных настроек всех баз существует. Перезаписать его настройками этой базы?", РежимДиалогаВопрос.ДаНетОтмена,, КодВозвратаДиалога.Нет);
			Если Ответ = КодВозвратаДиалога.Отмена Тогда
				Возврат;
			ИначеЕсли Ответ = КодВозвратаДиалога.Нет Тогда
				ПрочитатьГлобальныеНастройки = Истина;
			Иначе
				мПлатформа.ЛиГлобальныеНастройкиВсехБаз = ЛиГлобальныеНастройкиВсехБаз;
			КонецЕсли;
		КонецЕсли;
	ИначеЕсли мПлатформа.ЛиГлобальныеНастройкиВсехБаз И Не ЛиГлобальныеНастройкиВсехБаз Тогда
		мПлатформа.ЛиГлобальныеНастройкиВсехБаз = ЛиГлобальныеНастройкиВсехБаз;
	КонецЕсли;
	мПлатформа.АвторегистрацияComКомпонент = АвторегистрацияComКомпонент;
	мПлатформа.ИспользоватьЭмуляциюНажатияКлавиш = ИспользоватьЭмуляциюНажатияКлавиш;
	мПлатформа.ВыделениеРезультатовПоиска = ВыделениеРезультатовПоиска;
	...
	мПлатформа.ЛиГлобальныеНастройкиВсехБаз = ЛиГлобальныеНастройкиВсехБаз;
	ирОбщий.СохранитьЗначениеЛкс("ЛиГлобальныеНастройкиВсехБаз", ЛиГлобальныеНастройкиВсехБаз);
	Если ПрочитатьГлобальныеНастройки Тогда
		мПлатформа.ПрочитатьОбщиеНастройки();
	КонецЕсли;
	ирОбщий.СохранитьЗначениеЛкс("ПроверятьПодпискиКонфигурации", ЭтаФорма.ПроверятьПодпискиКонфигурации, Истина); // Для всех пользователей базы!
	Если Не ирКэш.ЛиПортативныйРежимЛкс() Тогда
		ирОбщий.СохранитьЗначениеЛкс("КаталогОбъектовДляОтладки", ЭтаФорма.КаталогОбъектовДляОтладки);
	КонецЕсли; 
	Если ирКэш.ЛиЭтоРасширениеКонфигурацииЛкс() Тогда 
		ирОбщий.СохранитьЗначениеЛкс("ДобавлятьРольИРВсемАдминистраторам", ЭтаФорма.ДобавлятьРольИРВсемАдминистраторам, Истина); // Для всех пользователей базы!
	КонецЕсли; 
	ЗаписатьНастройки();
	Если ЭлементыФормы.ЗапускатьПриСтарте.Доступность Тогда
...
Процедура ПриОткрытии()
	...
	ЭлементыФормы.НадписьКлиентСерверногоЗапуска.Видимость = ирКлиент.ЛиНедоступныйПутьКлиентСервераЛкс();
	Если ирКэш.ЛиПортативныйРежимЛкс() Тогда
		ЭлементыФормы.НадписьКлиентСерверногоЗапуска.ЦветТекста = WebЦвета.Красный;
	КонецЕсли;
...
Процедура ИмяФайлаНастроекПортативныйОткрытие(Элемент, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	ирКлиент.ОткрытьФайлВПроводникеЛкс(Элемент.Значение);
КонецПроцедуры

ирКлиент.ИнициироватьФормуЛкс(ЭтаФорма, "Обработка.ирПортативный.Форма.Настройки");
