	...
	#Иначе
		...
		Если ПрименитьТекущиеПараметрыЗапуска Тогда
			...
			Исключение
				...
				//АдресКомпоненты = "Обработка.ирПортативный.Макет.ВК32";
				...
				Если Результат Тогда
					ВК = Новый ("AddIn.ирОбщая.AddIn");
				Иначе
					Сообщить("Не удалось подключить внешнюю компоненту ""Общая"". Она требуется для флажка ""Применить текущие параметры запуска""");
				КонецЕсли;
			КонецПопытки;
...
Функция ОбработкаОбъект()
	ОбработкаОбъект = Вычислить("ЭтотОбъект"); // ВнешняяОбработкаОбъект
