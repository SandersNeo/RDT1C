	...
	Для каждого ПодчиненныйЭлемент Из ГруппаНастроек.ПодчиненныеЭлементы Цикл
		...
		Иначе
			Если ирОбщий.СтрКончаетсяНаЛкс(ПодчиненныйЭлемент.Имя, СловоИспользование) Тогда
				//! ПодчиненныйЭлемент = 0 // РасширениеПоляФормыДляПоляФлажка
...
Процедура УстановитьОбъектМетаданныхНаКлиенте(Знач ПолноеИмяТаблицы = Неопределено, Знач СохранитьНастройкиТаблицы = Ложь)
	...
	#КонецЕсли
	Если Истина
		И ирОбщий.ЛиКорневойТипПоследовательностиЛкс(ирОбщий.ПервыйФрагментЛкс(фОбъект.ПолноеИмяТаблицы))
		И ирОбщий.СтрКончаетсяНаЛкс(фОбъект.ПолноеИмяТаблицы, ".Границы")
	Тогда
		// Антибаг платформы 8.3.1-27 https://www.hostedredmine.com/issues/1005511
		ирОбщий.СообщитьЛкс(ирОбщий.СтрШаблонЛкс("Недопустимая таблица БД (%1)", фОбъект.ПолноеИмяТаблицы)); 
		Закрыть();
		Возврат;
	КонецЕсли;
...
Процедура ДинамическийСписокВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Иначе
		ОбъектМД = ирОбщий.ОбъектМДПоПолномуИмениТаблицыБДЛкс(фОбъект.ПолноеИмяТаблицы); // ОбъектМетаданныхРегистрСведений
		Если Не ирОбщий.ЛиМетаданныеНезависимогоРегистраЛкс(ОбъектМД) Тогда
			ПутьКДанным = ирОбщий.ПутьКДаннымЭлементаУправляемойФормыЛкс(Поле, , ЭтаФорма);
			Если ирОбщий.СтрКончаетсяНаЛкс(ПутьКДанным, ".Регистратор") Тогда
				Ссылка = Элемент.ТекущиеДанные.Регистратор;
				ирКлиент.ОткрытьЗначениеЛкс(Ссылка);
				СтандартнаяОбработка = Ложь;
			КонецЕсли;
		Иначе
