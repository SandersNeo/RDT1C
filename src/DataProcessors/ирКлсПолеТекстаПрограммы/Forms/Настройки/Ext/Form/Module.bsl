...
Процедура ОсновныеДействияФормыОК(Кнопка = Неопределено)
	...
	мПлатформа.ПараметрыМоделиИИ = ПараметрыМоделиИИ;
...
Процедура ПрочитатьОбновитьНастройки() Экспорт
	...
	ЭтаФорма.ПараметрыМоделиИИ = мПлатформа.ПараметрыМоделиИИ();
...
Процедура КоманднаяПанель1ПроверитьМодули(Кнопка)
	...
	Форма.ПоОтобраннымМодулям = Истина;
	Форма.Открыть()
...
Процедура КлючСервисаИИНачалоВыбора(Элемент, СтандартнаяОбработка)
	...
	ИначеЕсли СерверИИ = "https://api.z.ai/api/paas/v4" Тогда
		ЗапуститьПриложение("https://z.ai/manage-apikey/apikey-list");
	КонецЕсли;
КонецПроцедуры
...
Процедура МодельИИНачалоВыбора(Элемент, СтандартнаяОбработка)
	...
	ИначеЕсли СерверИИ = "https://api.z.ai/api/paas/v4" Тогда
		ЗапуститьПриложение("https://docs.z.ai/guides/overview/pricing");
	КонецЕсли;
КонецПроцедуры
...
Процедура ПроверитьИИНажатие(Элемент)
	мФоновыйЗапросХттп.ОтправитьЗапросИИ(Новый Структура("текст, проверка", "привет", Истина), ЭтаФорма,,, Элемент,, Истина, , КлючСервисаИИ, СерверИИ, МодельИИ, ПараметрыМоделиИИ);
КонецПроцедуры
...
Процедура СерверИИНачалоВыбора(Элемент, СтандартнаяОбработка)
	...
	СписокЗначений.Добавить("https://code.1c.ai",        "Бесплатно code.1c.ai - бесплатно до 01.10.2026, доступна оплата из РФ");
	СписокЗначений.Добавить("https://openrouter.ai/api", "Бесплатно openrouter.ai - 50 запросов В ДЕНЬ бесплатно, недоступна оплата из РФ");
	СписокЗначений.Добавить("https://chat.qwen.ai/api", "Бесплатно qwen.ai CLI - 1000 запросов В ДЕНЬ бесплатно без ключа с пустой моделью (qwen3-coder-plus)");
	СписокЗначений.Добавить("https://api.z.ai/api/paas/v4", "Бесплатно z.ai - Х запросов В ДЕНЬ бесплатно на некоторых моделях, доступна оплата из РФ");
	СписокЗначений.Добавить("https://api.smartbuddy.ru", "Платно smartbuddy.ru - 10 запросов бесплатно, доступна оплата из РФ");
	СписокЗначений.Добавить("https://api.vsegpt.ru",     "Платно vsegpt.ru - 10 запросов бесплатно, доступна оплата из РФ");
	СписокЗначений.Добавить("https://api.together.xyz",  "Платно together.ai - 1000 запросов бесплатно, недоступна оплата из РФ");
	СписокЗначений.Добавить("https://api.proxyapi.ru/openai", "ПЛАТНО proxyapi.ru - только платно, доступна оплата из РФ, прокси для openrouter.ai");
...
Процедура ПроверитьНапарникаНажатие(Элемент)
	Результат = СоединениеНапарника(КлючСервисаНапарник, Истина, Ложь);
	Если ТипЗнч(Результат) = Тип("HTTPСоединение") Тогда
...
Процедура ОтправитьМетаданныеНажатие(Элемент)
	ФормаОтправки = ПолучитьФорму("ОтправкаМетаданныхНапарнику");
	ФормаОтправки.Открыть();
КонецПроцедуры

Процедура ПараметрыМоделиИИНажатие(Элемент)
	Если ирКлиент.РедактироватьСписокЗначенийЛкс(ПараметрыМоделиИИ,, "Параметры модели ИИ") Тогда
