...
Функция СвойстваГлобализуемые() Экспорт
	...
	Результат.Вставить("ШиринаГенерируемогоТекста");
...
Процедура ПриОткрытии()
	...
	Иначе
		ЭлементыФормы.ТаблицаПараметров.Колонки.ЭтоРезультат.Видимость = Истина;
		ЭтаФорма.ТекущийЭлемент = ЭлементыФормы.ИмяМетода;
		ЭлементыФормы.ПолеПосле.УстановитьТекст(ТекстПосле);

		// Избавляемся от служебного параметра ТипВыхода, если возможно
		Если ПустаяСтрока(ТекстПосле) Тогда
			СтрокаПараметраТипаВыхода = ТаблицаПараметров.Найти(НРег(ИмяПараметраТипаВыхода), "НИмя");
			Если СтрокаПараметраТипаВыхода <> Неопределено Тогда
				ТекстТела = СтрЗаменить(ТекстТела, ИмяПараметраТипаВыхода + " = ""Возврат""; ", "");
				ТипыВнешнихПереходов.Очистить();
				ТаблицаПараметров.Удалить(СтрокаПараметраТипаВыхода);
			КонецЕсли;
		Иначе
			ШаблонВозвратов = ИмяПараметраТипаВыхода + " = ""(["+шБуква+"]+)""; Возврат ("+мПлатформа.шКонстанта+")?";
			Вхождения = ирОбщий.НайтиРегВыражениеЛкс(ТекстТела, ШаблонВозвратов, ИмяПараметраТипаВыхода + ", Результат",,, Истина);
			//! Вхождения.Колонки.Добавить("Результат");
			ВсеТипыВыхода = Вхождения.Скопировать(, ИмяПараметраТипаВыхода);
			ВсеТипыВыхода.Свернуть(ИмяПараметраТипаВыхода);
			ВсеТипыВыхода = ВсеТипыВыхода.ВыгрузитьКолонку(ИмяПараметраТипаВыхода);
			Если ВсеТипыВыхода.Количество() = 1 Тогда
				ВсеРезультаты = Вхождения.Скопировать(, "Результат");
				ВсеРезультаты.Свернуть("Результат");
				ВсеРезультаты = ВсеРезультаты.ВыгрузитьКолонку("Результат");
				ВыражениеРезультата = ВсеРезультаты[0];
				Если Истина
					И ВсеРезультаты.Количество() = 1 
					И ВыражениеРезультата <> Неопределено
				Тогда
					ТекстТела = ирОбщий.СтрЗаменитьЛкс(ТекстТела, ИмяПараметраТипаВыхода + " = """ + ВсеТипыВыхода[0] + """; ", "");
					СтрокаПараметраТипаВыхода = ТаблицаПараметров.Найти(НРег(ИмяПараметраТипаВыхода), "НИмя");
					ТаблицаПараметров.Удалить(СтрокаПараметраТипаВыхода);
					ЭтаФорма.ИмяПараметраТипаВыхода = ИмяПараметраВозврата();
					ТипыВнешнихПереходов[ВсеТипыВыхода[0]] = ВыражениеРезультата;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	...
	Для Каждого СтрокаПараметра Из ТаблицаПараметров Цикл
		Если СтрокаПараметра.Обязательный Тогда
			Попытка
				ОписаниеТипов = Новый ОписаниеТипов(СтрокаПараметра.ТипЗначения);
				СтрокаПараметра.Значение = ОписаниеТипов.ПривестиЗначение();
			Исключение
			КонецПопытки;
		КонецЕсли;
	КонецЦикла;
	СписокВыбора = ЭлементыФормы.ДирективаКомпиляции.СписокВыбора;
	...
	Найденный = ирОбщий.ОтобратьКоллекциюЛкс(СписокВыбора, "НРег(Э.Значение)=П1", "Значение", НРег(ДирективаКомпиляции));
	Если Найденный.Количество() > 0 Тогда
		ЭтаФорма.ДирективаКомпиляции = Найденный[0];
	КонецЕсли; 
...
Процедура ЗагрузитьТекущийМетод() Экспорт
	...
	ЭтаФорма.ДирективаКомпиляции = Сред(мМетодМодуля.Аннотация, 2);
...
Функция ОбновитьПолноеОпределение(Знач БезТела = Ложь) Экспорт
	ЭтаФорма.ПолноеОпределение = НачалоОпределения + ?(БезТела, "", СокрП(ЭлементыФормы.ПолеТела.ПолучитьТекст())) + Символы.ПС + КонецОпределения;
...
Процедура ОбновлениеОтображения(Знач ЗаполнятьТипы = Неопределено) Экспорт
	...
	КонецЕсли;
	ЧистыйКонтекст = ЭтаФорма.ЧистыйКонтекст Или ирОбщий.СтрКончаетсяНаЛкс(ДирективаКомпиляции, "БезКонтекста");
	...
	Тогда
		СтрокаРезультата = Неопределено;
	КонецЕсли;
	ИмяПараметраВозврата = ИмяПараметраВозврата();
	ТекстВозвратаРезультата = "";
	КонецОпределения = "";
	Если Ложь
		Или ТипыВнешнихПереходов.Количество() > 0 
	...
	ИначеЕсли Истина
		И ПустаяСтрока(ТекстПосле)
		И Не мРежимРедактора
		И ЛиВнутриФункции
	Тогда
		ТекстВозвратаРезультата = "Возврат ";
		НачалоОпределения = "Функция";
		КонецОпределения = "КонецФункции";
	ИначеЕсли Истина
		И ЭлементыФормы.ПолеТела.КоличествоСтрок() = 1
		И ирОбщий.СтрНачинаетсяСЛкс(СокрЛ(ТекстТела), "Возврат ")
		И Не мРежимРедактора
	Тогда
		Если Не ЭтоВыражение Тогда
			ТекстВозвратаРезультата = ИмяМетода + " = ";
		КонецЕсли; 
		НачалоОпределения = "Функция";
		КонецОпределения = "КонецФункции";
	Иначе
	...
	КонецЕсли; 
	ТекстДиректива = "";
	Если ЗначениеЗаполнено(ДирективаКомпиляции) Тогда
		ТекстДиректива = "&" + ДирективаКомпиляции + Символы.ПС;
	КонецЕсли;
	...
	Для Каждого КлючИЗначение Из ТипыВнешнихПереходов Цикл
		...
		Если КлючИЗначение.Значение = Неопределено Тогда
			ПроверяемоеЗначение = """" + КлючИЗначение.Ключ + """";
		Иначе
			ПроверяемоеЗначение = КлючИЗначение.Значение;
		КонецЕсли;
		ТекстУсловияПерехода = ТекстУсловияПерехода + "Если " + ИмяПараметраТипаВыхода + " = " + ПроверяемоеЗначение + " Тогда" + Символы.ПС;
		ТекстУсловияПерехода = ТекстУсловияПерехода + ТекстСмещения + Символы.Таб + КлючИЗначение.Ключ;
		Если КлючИЗначение.Ключ = "Возврат" И мМетодМодуля.ТипЗначения <> Неопределено Тогда
...
Функция ИмяПараметраВозврата() Экспорт
...
Процедура ДействияФормыЗаполнитьИИ(Кнопка)
	Если Кнопка.Картинка <> ирКэш.КартинкаПоИмениЛкс("ирОстановить") Тогда
		...
		Если ПараметрыЗаполненияМетода.ПередаватьКонтекст Тогда
			КонтекстИИ = КонтекстИИ(Описание, ТаблицаПараметров, ЧистыйКонтекст, ДирективаКомпиляции);
		КонецЕсли;
		ТекстЗапроса = ТекстЗапросаИИДляМетода(ТекстМодуля, КонтекстИИ, ШиринаГенерируемогоТекста, ПараметрыЗаполненияМетода);
...
Процедура ЗаполнитьИИКонец(Знач СостояниеЗадания, Знач РезультатЗадания) Экспорт
	...
	ТекстЖСОН = ирОбщий.ТекстМеждуМаркерамиЛкс(РезультатЗадания.Ответ, "```json", "```", Ложь,,, Ложь);
	Если ПустаяСтрока(ТекстЖСОН) Тогда
		ирОбщий.СообщитьЛкс("ИИ вернул ответ, не соответствующий запрошенному формату. Если проблема повторяется, выберите другую модель ИИ.");
		Возврат;
	КонецЕсли;
	РезультатИИ = ирОбщий.ОбъектИзТекстаЖСОНЛкс(ТекстЖСОН); // см. ПрочитатьJSON(ОбразецДокументацииМетодаЖСОН())
...
Процедура ДействияФормыОбсудитьИИ(Кнопка)
	ОткрытьЧатИИ(ОбновитьПолноеОпределение());
КонецПроцедуры
