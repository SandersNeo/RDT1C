...
Процедура ПередОткрытием(Отказ, СтандартнаяОбработка)
	...
	Если ЛиМетод И ЛиВЗаголовкеМетода() Тогда
		ТаблицаТиповКонтекста = ТаблицаТиповТекущегоВыражения();
		ЭтотОбъект.мМетодМодуля = Неопределено; // чтобы обязательно заполнился полный контекст модуля. Иначе сработает урезанный контекст через ЛиВЗаголовкеМетода()=Истина
	Иначе
	...
	ИначеЕсли Истина
		И Не ЛиМетод
		И мМетодМодуля.Имя = "<>" 
		И мМодульМетаданных.Переменные.Найти(НРег(СтароеСлово), "НИмя") <> Неопределено
	Тогда
	...
	Если Не ЛиМетод Тогда
		...
		Для Каждого СтрокаОбласти Из Области Цикл
			СтрокаОбласти.ПозицияНачала = мПозицияТекстаДляПоискаОпределения - мСмещениеНачалаТела + СтрокаОбласти.ПозицияНачала - 1; // -1 нужен для алгоритма в консоли кода
			СтрокаОбласти.ПозицияКонца = мПозицияТекстаДляПоискаОпределения - мСмещениеНачалаТела + СтрокаОбласти.ПозицияКонца - 1;
...
Функция ВыполнитьЗамену(Знач ПолеТекстаПрограммы, Знач АвтоНовоеСлово = Ложь, Знач ВыделениеДвумерное = Неопределено) Экспорт
	...
	Пока ПолеТекстаПрограммы.НайтиПоказатьСловоВТексте(СтароеСлово, ШаблонПоискаСлова(СтароеСлово),,, Ложь,, НачалоОбластиВМодуле, КонецОбластиВМодуле) Цикл
		НовоеВыражение = ирОбщий.ТекстБезПоследнегоФрагментаЛкс(ПолеТекстаПрограммы.ВыделенныйТекст());
...
Процедура ОбластиВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	...
	ИначеЕсли Колонка = ЭлементыФормы.Области.Колонки.СтрокаВозможногоСоздания Тогда
		Если ВыбраннаяСтрока.ПозицияПервогоПрисвоения > 0 Тогда
