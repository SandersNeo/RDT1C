...
Функция НайтиВходящие(Знач СтрокаРодителя)
	...
	Если Не ПустаяСтрока(СтрокаРодителя.Модуль) Тогда
		ИмяРодногоМодуляБезКонца = ирОбщий.ТекстБезПоследнегоФрагментаЛкс(ирОбщий.ТекстМеждуМаркерамиЛкс(СтрокаРодителя.Модуль, "[", "]"));
	...
	Если Лев(СтрокаРодителя.Метод, 1) = "<" Тогда
		...
		КонецЕсли;
		ТекстовыйДокумент = Новый ТекстовыйДокумент;
		ТекстовыйДокумент.Прочитать(мПлатформа.ФайлМодуляИзИмениМодуля(ИмяМодуляПсевдокодаПлатформы).ПолноеИмя);
...
Процедура УточнитьСтрокуРезультата(Знач ТекущаяСтрока, Знач Групповое = Истина) Экспорт
	...
	Если ИмяМодуля = мПлатформа.ИмяДинамическогоМодуля() Тогда
		...
		Иначе
			ТекущаяСтрока.Метод = мПлатформа.ИмяМетодаВнеМетодов();
		КонецЕсли;
...
Процедура ДействияФормыНайтиВызовыМетода(Кнопка)
	...
	ПолеТекстаВызывающее.ОткрытьПоискВызововСлова(ВыбраннаяСтрока.Метод + "(");
КонецПроцедуры
...
#Если Сервер И Не Сервер Тогда
	...
	ПараметрСтрокаМетода = мПлатформа.ПараметрыМетодаМодуля(0)[0];
#КонецЕсли
РасширениеФайлаМодуля = "txt";
ДеревоВызовов.Колонки.Добавить("Места", Новый ОписаниеТипов("ТаблицаЗначений")); // см. НайтиВходящие
