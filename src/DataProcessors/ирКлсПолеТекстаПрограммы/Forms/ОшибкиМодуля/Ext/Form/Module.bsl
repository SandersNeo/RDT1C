...
Процедура ПриЛюбомОткрытии() Экспорт
	...
	НовыйТекст = "";
	Если Не ПоОтобраннымМодулям Тогда
...
Процедура ОшибкиМодуляПриАктивизацииСтроки(Элемент)
	...
	ирКлиент.РазобратьПозициюМодуляВСтрокеТаблицыЛкс(ТекущаяСтрока, ИмяМодуляИзСтрокиОшибки(ТекущаяСтрока, Истина), ЭлементыФормы.ПолеТекстаВхождения);
КонецПроцедуры
...
//   Строка - 
Функция ИмяМодуляИзСтрокиОшибки(Знач СтрокаОшибки = Неопределено, Знач ДляСсылки = Ложь) Экспорт
	...
	Если Не ЗначениеЗаполнено(Модуль) Тогда
		...
		//Если Истина
		//	И ДляСсылки
		//	И ирОбщий.СтрНачинаетсяСЛкс(Модуль, "Внешн") 
		//	И ирОбщий.СтрКончаетсяНаЛкс(Модуль, ".Форма.Модуль")
		//Тогда
		//	Модуль = мМодульМетаданных.СтруктураТипа.Метаданные.ИмяФормы + ".Форма.Модуль";
		//КонецЕсли;
	КонецЕсли;
	Возврат Модуль;
КонецФункции
		...
		Если Не ирКлиент.ОткрытьСсылкуСтрокиМодуляЛкс(ВыбраннаяСтрока.Ссылка,, Автооткрытие) Тогда 
			...
			Если ИмяМодуля = мПлатформа.ИмяДинамическогоМодуля() Тогда 
				Если ФормаВладелец = Неопределено Тогда
					ирОбщий.СообщитьЛкс("Форма-источник текста не обнаружена");
					Возврат;
				КонецЕсли;
...
Процедура ОбновитьОшибки(Знач ПолныйПересчет = Ложь) Экспорт
	...
	Если мМодульМетаданных = Неопределено Тогда
		ЭтаФорма.ПоОтобраннымМодулям = Истина;
	КонецЕсли;
	ПриЛюбомОткрытии();
	...
	КонецЕсли; 
	ЭлементыФормы.ОшибкиМодуля.ВыделенныеСтроки.Очистить();
	Если ПоОтобраннымМодулям Тогда
		...
		Для Каждого Файл Из ФайлыМодулей Цикл
			...
			Если ФильтрПодсистем.Количество() > 0 Тогда
				...
				КонецЕсли;
			КонецЕсли;
			Если мДатыПроверенныхМодулей[ИмяМодуля] = ВремяИзменения Тогда
				Продолжить;
			КонецЕсли;
			Если ТекущаяДата() - ВремяИзменения < 3600 Тогда
		...
		Для Каждого ЭлементСписка Из ОтобранныеФайлы Цикл
			...
			Если АнализаторМодуля.ПроверитьВыраженияТекста(,, ПолныйПересчет, ПроверятьКороткиеЛитералы, Ложь) Тогда 
				Если мПлатформа.ЛиСохранятьОшибкиМодуля(АнализаторМодуля.мМодульМетаданных) Тогда
					//Сообщить(АнализаторМодуля.мМодульМетаданных.Имя);
					мПлатформа.ЗаписатьМодульВКэш(АнализаторМодуля.мМодульМетаданных);
				КонецЕсли;
			КонецЕсли;
			ирОбщий.УдалитьСтрокиТаблицыИлиДереваПоЗначениюЛкс(ОшибкиМодуля, "Модуль", ИмяМодуля);
			Если АнализаторМодуля.ОшибкиМодуля.Количество() > 0 Тогда
				...
				ДобавленныеСтроки = ирОбщий.ЗагрузитьВТаблицуЗначенийЛкс(АнализаторМодуля.ОшибкиМодуля, ОшибкиМодуля,, НовыеЗначения,,,, Не ПолныйПересчет);
				Если ОбновитьОтображение Тогда
					ОшибкиМодуля.Сортировать("Модуль, Метод");
					Если ДобавленныеСтроки <> Неопределено Тогда
						Для Каждого ДобавленныеЭлемент Из ДобавленныеСтроки Цикл
							Если ДобавленныеЭлемент.ДатаОбнаружения >= НачалоПроверки Тогда
								ЭлементыФормы.ОшибкиМодуля.ВыделенныеСтроки.Добавить(ДобавленныеЭлемент);
							КонецЕсли;
						КонецЦикла;
					КонецЕсли;
				...
				КонецЕсли;
			КонецЕсли;
			мДатыПроверенныхМодулей[ИмяМодуля] = Файл.ПолучитьВремяИзменения();
			ЭтаФорма.КоличествоНайдено = ОшибкиМодуля.Количество();
		КонецЦикла;
		ирОбщий.ОсвободитьИндикаторПроцессаЛкс();
		мПлатформа.ОчиститьКэшАнализатораЯзыка(); // Сразу и запишем результаты в кэш и освободим память
	Иначе
...
Процедура ОчиститьОшибки() Экспорт
	мДатыПроверенныхМодулей.Очистить();
...
Процедура ДействияФормыПодавитьПроверки(Кнопка)
	...
	Если ИмяМодуляИзСтрокиОшибки(ВыбраннаяСтрока) = мПлатформа.ИмяДинамическогоМодуля() Тогда 
		...
		ПолеТекста.ВыделенныйТекст(" " + ИнструкцияПодавленияПроверки(ВыбраннаяСтрока.ТипОшибки, Ложь));
	Иначе 
...
Процедура ИзмененныеПозжеПриИзменении(Элемент)
	ЭтаФорма.ПоОтобраннымМодулям = Истина;
	ОчиститьОшибки();
КонецПроцедуры

Процедура ПоОтобраннымМодулямПриИзменении(Элемент)
	Если ПоОтобраннымМодулям Тогда
		ОчиститьОшибки();
	КонецЕсли;
КонецПроцедуры
...
Автооткрытие = Истина; 
мДатыПроверенныхМодулей = Новый Соответствие;
