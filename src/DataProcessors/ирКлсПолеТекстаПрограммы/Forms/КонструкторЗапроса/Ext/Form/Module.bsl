...
Перем мСтрокаНачальногоВыбранногоПоля;
...
Функция СвойстваГлобализуемые() Экспорт
	...
	//Результат.Вставить("ПоказыватьИндексы"); // Много времени уходит на получение структуры БД
	Результат.Вставить("ИерархическийСтильЛогическихВыражений");
...
Процедура УстановитьДанные(НачальныйТокен = Неопределено, ТаблицаКомментариев = Неопределено, БылиПотери = Неопределено,
	...
	мСтрокаНачальногоВыбранногоПоля = Неопределено;
	...
	КонецЕсли; 
	Если Истина
		И ПустаяСтрока(ИмяВыбранногоПоляДляАктивации)
		И мСтрокаНачальногоВыбранногоПоля <> Неопределено 
	Тогда
		ИмяВыбранногоПоляДляАктивации = мСтрокаНачальногоВыбранногоПоля.Имя;
	КонецЕсли;
	мСтрокаНачальногоВыбранногоПоля = Неопределено;
...
Функция СообщитьОНеразобранныхКомментариях(ТаблицаКомментариев) Экспорт
	...
	Если Не ЗначениеЗаполнено(ТаблицаКомментариев) Тогда
		Возврат Ложь;
	КонецЕсли;
...
Функция СобратьСлужебныйТекстПакетаПостроителя(Знач пЗапросыПакета = Неопределено)
	...
	Для ИндексЗапроса = 0 По пЗапросыПакета.Количество() - 1 Цикл
		...
		Для ИндексЧасти = 0 По ЗапросПакета.ЧастиОбъединения.Количество() - 1 Цикл  
			...
			КонецЕсли; 
			ЧастьОбъединения.ТекстРасширения = "";
		КонецЦикла;
		ТекстЗапросаПакета = ТекстЗапросаПакета + "
...
Функция НайтиДобавитьВыбраннуюТаблицуВСвязи(СтрокаВыбраннойТаблицы, ЧастьОбъединения = Неопределено, Знач РодительДляНового = Неопределено)
	...
	Если СвязьТаблицы = Неопределено Тогда
		Если РодительДляНового = Неопределено Тогда
			РодительДляНового = ЧастьОбъединения.СвязиТаблиц;
		КонецЕсли;
		СвязьТаблицы = РодительДляНового.Строки.Добавить();
...
Процедура ОбработатьЗапросПослеУстановкиДанных(ЗапросПакета)
	...
	Тогда
		Если ЗапросПакета.ТипЗапроса = 1 И ЗапросПакета.ИндексыТаблицы.Количество() = 0 Тогда
			ДобавитьИндексТаблицыБД(ЗапросПакета);
		КонецЕсли;
		...
		Для Каждого ЧастьОбъединения Из ЗапросПакета.ЧастиОбъединения Цикл
			...
			Пока ИндексВыбранногоПоля < ЧастьОбъединения.ВыбранныеПоля.Количество() Цикл
				...
				Если ИмяПоля = "*" Тогда
					...
					Если мСтрокаНачальногоВыбранногоПоля = ЧастьОбъединения.ВыбранныеПоля[ИндексВыбранногоПоля] Тогда
						мСтрокаНачальногоВыбранногоПоля = Неопределено;
					КонецЕсли;
			...
			Если Истина
				И ТипЗапроса = 0
				И ПерваяЧасть = ЧастьОбъединения 
			Тогда
				СтрокаПодзапроса = ПерваяЧасть.ВыбранныеТаблицы.Найти("ИР_ЗащитаГруппировокОтКомпоновщика", "Имя");
				Если СтрокаПодзапроса <> Неопределено Тогда
					ВложеннаяЧасть = СтрокаПодзапроса.ВложенныйПакет[0].ЧастиОбъединения[0]; // см. ЧастиОбъединения[0]
					Для Каждого ВыбранныеЭлемент Из ВложеннаяЧасть.ВыбранныеТаблицы Цикл
						КлючПоиска = Новый Структура;
						КлючПоиска.Вставить("ТипЗапроса", 1);
						КлючПоиска.Вставить("ИмяОсновнойТаблицы", ВыбранныеЭлемент.Имя);
						Найденные = ЗапросыПакета.НайтиСтроки(КлючПоиска);
						Если Найденные.Количество() Тогда
							Найденные[0].ЗащититьГруппировкиОтКомпоновщика = Истина;
						КонецЕсли;
					КонецЦикла;
					УдалитьВыбраннуюТаблицу(СтрокаВыбраннойТаблицы, ЗапросПакета, ЧастьОбъединения);
				КонецЕсли;
			КонецЕсли;
			...
			Если ПерваяЧасть <> ЧастьОбъединения Тогда
				...
				Если ЧислоДобавленных > 0 Тогда
					ирОбщий.СообщитьЛкс(ирОбщий.СтрШаблонЛкс("В запросе[%1] добавлено %2 недостающих полей части[%3] объединения", ЗапросПакета.Имя, ЧислоДобавленных, 
						ирОбщий.ПервыйФрагментЛкс(ЧастьОбъединения.Имя, ",")), СтатусСообщения.Внимание);
				КонецЕсли;
			КонецЕсли; 
		КонецЦикла;
...
//
// Параметры:
//  Токен				 - ОбработкаОбъект.ирТипGoldParserToken - 
//  Стек				 - 	 - 
...
Функция ЗаполнитьПоТокену(Знач Токен, Стек = Неопределено, Знач ЗапросПакета = Неопределено, Знач ЧастьОбъединения = Неопределено, Индикатор = Неопределено,
	Знач РазборРасширений = Ложь, ТаблицаКомментариев = Неопределено, Знач ТекущаяСвязьТаблиц = Неопределено, Знач УровеньВложенности = 0) Экспорт
	...
	ИначеЕсли ИмяПравила = "<Top>" Тогда
		...
		Если Данные.TokenCount > 1 Тогда
			ТокенЧисла = Неопределено;
			Если Данные.Tokens(1).Type = 1 Тогда
				Если Данные.TokenCount = 2 Тогда
					ЧастьОбъединения.ПервыеКоличество = Данные.Tokens(1).Data;
				Иначе
					ТокенЧисла = Данные.Tokens(2).Data.Tokens(0);
				КонецЕсли;
			Иначе
				ТокенЧисла = Данные.Tokens(1).Data.Tokens(0);
			КонецЕсли; 
			Если ТокенЧисла <> Неопределено Тогда
				Если ТокенЧисла.text = "<Parameter>" Тогда
					ЧастьОбъединения.ПервыеКоличество = СобратьВыражениеЗапроса(ТокенЧисла,, ЗапросПакета, ЧастьОбъединения);  
				Иначе
					ЧастьОбъединения.ПервыеКоличество = Число(ТокенЧисла.Data);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли; 
	ИначеЕсли ИмяПравила = "<Limit>" Тогда
		Если Данные.TokenCount > 0 Тогда
			...
			Иначе
				ТокенЧисла = Данные.Tokens(1).Data.Tokens(0);
				Если ТокенЧисла.text = "<Parameter>" Тогда
					ЗапросПакета.ПервыеКоличество = СобратьВыражениеЗапроса(ТокенЧисла,, ЗапросПакета, ЧастьОбъединения);
				Иначе
					ЗапросПакета.ПервыеКоличество = Число(ТокенЧисла.Data);
				КонецЕсли; 
			КонецЕсли; 
		КонецЕсли; 
	...
	ИначеЕсли ИмяПравила = "<SelectExpr>" Тогда
		...
		Если Истина
			...
			И ЧастьОбъединения = мСтрокаНачальнойЧастиОбъединения
		Тогда 
			НачальныйТокен = Токен.GetBorderToken(0, Истина);
			КонечныйТокен = Токен.GetBorderToken(1, Истина);
			Если Истина
				И НачальныйТокен.LineNumber <= мНомерБазовойСтрокиВТексте
				И мНомерБазовойСтрокиВТексте <= КонечныйТокен.LineNumber 
			Тогда 
				мСтрокаНачальногоВыбранногоПоля = ТекущийУзел;
			КонецЕсли; 
		КонецЕсли; 
	...
	ИначеЕсли ИмяПравила = "<TotalsExpr>" Тогда
		...
		ТокенВыражения = ДанныеHierExpr.Tokens(0);
		Определение = СобратьВыражениеЗапроса(ТокенВыражения,, ЗапросПакета);
		НОпределение = НРег(Определение);
		Если Ложь
			Или НОпределение = "overall"
			Или НОпределение = "общие"
		Тогда
			ЗапросПакета.ОпцияОбщиеИтоги = Истина;
		Иначе
			ТекущийУзел = ЗапросПакета.ГруппирующиеПоляИтогов.Добавить();
			ТекущийУзел.Токен = ТокенВыражения;
			ТекущийУзел.Определение = Определение;
	...
	ИначеЕсли ИмяПравила = "<AgregatExpr>" Тогда
		...
		СобратьКомментарийТокена(Токен, ТаблицаКомментариев, ТекущийУзел.Комментарий);
		ЗаполнитьПсевдонимПоляИзТокена(ТекущийУзел, Данные.Tokens(1));
	ИначеЕсли ИмяПравила = "<IndexSet>" Тогда
		ТекущийУзел = ДобавитьИндексТаблицыБД(ЗапросПакета);
		Если Данные.Tokens(3).Data.Count Тогда
			ТекущийУзел.Уникальный = Истина;
		КонецЕсли;
		ЗаполнитьПоТокену(Данные.Tokens(1), Стек, ЗапросПакета,, Индикатор, РазборРасширений, ТаблицаКомментариев);
		СобратьКомментарийТокена(Токен, ТаблицаКомментариев, ТекущийУзел.Комментарий);
	ИначеЕсли ИмяПравила = "<IndexExpr>" Тогда
		Если ЗапросПакета.ИндексыТаблицы.Количество() = 0 Тогда
			ДобавитьИндексТаблицыБД(ЗапросПакета);
		КонецЕсли;
		ТекущийУзел = ЗапросПакета.ИндексыТаблицы[ЗапросПакета.ИндексыТаблицы.Количество() - 1].ПоляИндекса.Добавить();
...
// Параметры:
//   ЗапросПакета - см. ЗапросыПакета[0] - 
Функция ДобавитьИндексТаблицыБД(ЗапросПакета) Экспорт
	НовыйИндекс = ЗапросПакета.ИндексыТаблицы.Добавить();
	НовыйИндекс.Имя = "Индекс " + ЗапросПакета.ИндексыТаблицы.Количество();
	НовыйИндекс.ПоляИндекса = ПоляИндекса.СкопироватьКолонки();
	Возврат НовыйИндекс;
КонецФункции
...
Функция РасшифровкаЗначенияДляТекстаЗапроса(ЗначениеПоля, Знач НоваяФорма) Экспорт 
	...
	Иначе
		РасшифровкаПоля = ирОбщий.ПредставлениеСОбрезкойДлиныЛкс(ЗначениеПоля);
	...
	Тогда
		...
		Иначе
			...
			Тогда
				...
				ПростоеЧислоТипов = ПростоеЧислоТипов + 1;
			КонецЕсли; 
		КонецЕсли; 
		Смещение = ирОбщий.СтрокаПовторомЛкс(Символы.Таб, Отступ);
		ПредставлениеСписка = "";
		Если ТипыПоля.Количество() > ПростоеЧислоТипов Тогда
		...
		Если ВидСравнения = ВидСравненияКомпоновкиДанных.Заполнено Тогда
			Результат = СловоЯзыкаЗапросовВФорме("NOT") + " (" + Результат + ")";
		Иначе
			Результат = "(" + Результат + ")"; // https://www.hostedredmine.com/issues/1003555
		КонецЕсли;
	ИначеЕсли Ложь
		...
		Тогда
			Если Найти(Результат, ".") > 0 Тогда
				...
				Иначе	
					...
					КонецЕсли;
					ДоступнаяТаблица = ДоступнаяТаблицаПоЛюбомуИмени(СтрокаВыбраннойТаблицы.ПолноеИмя); 
					Если Ложь
...
Функция СобратьВыражениеЗапроса(Знач Токен, Знач Отступ = 0, Знач ЗапросПакета = Неопределено, Знач ЧастьОбъединения = Неопределено,
	...
	Иначе
		...
		Иначе
			...
			Тогда
				МаксИндексСНормализацией = 2;
			ИначеЕсли Истина
				И ИмяПравила = "<DateDiff>"
				И мПараметрыДиалектаSQL.Это1С 
			Тогда
				МаксИндексСНормализацией = 4;
			КонецЕсли;
			_РежимОтладки = Ложь;
			Если _РежимОтладки Тогда // Можно менять на Истина в точке останова, например условием ирОбщий.Пр(_РежимОтладки, 1, 1)
				...
				Для ИндексТокена = 0 По Данные.TokenCount - 1 Цикл
					ТокенВниз = Данные.Tokens(ИндексТокена); 
					Если МаксИндексСНормализацией < ИндексТокена Тогда
	...
	Иначе //! Если ТипЗнч(ЭлементОтбора) = Тип("ЭлементОтбораКомпоновкиДанных") Тогда
		...
		Если ЗначениеЗаполнено(ЭлементОтбора.Представление) Тогда
			...
			Если Истина
				...
				И Не ГруппаИзОдногоЭлемента 
				И ирОбщий.СтрНайтиЛкс(Представление, "ИЛИ ",,,, Ложь)
				И Лев(Представление, 1) <> "("
			Тогда
				ПредставлениеЭлемента = "(" + ПредставлениеЭлемента + ")";
			КонецЕсли; 
		Иначе
...
Функция СобратьВыражениеОтбораКомпоновки(ГруппаОтбора, Отступ = 0, ФильтрацияАгрегатов = 0, выхЕстьАгрегаты = Ложь, НастройкиКомпоновки = Неопределено, СтруктураПолей = Неопределено) Экспорт 
	...
	Если Истина
		...
		И ПараметрыДиалектаSQL.МногоТаблиц // WQL не поддерживает фиктивные сравнения 0=0
		И (Ложь
...
Функция ДобавитьЗапросПакета(Пакет = Неопределено, Позиция = Неопределено)
	...
	ЗапросПакета.ИндексыТаблицы = ИндексыТаблицы.СкопироватьКолонки();
...
Функция СобратьТекстПакета(Пакет = Неопределено, Смещение = "", СтруктураПолей = Неопределено)
	...
	Для Каждого ЗапросПакета Из Пакет Цикл
		...
		Если ИндексЗапроса = Пакет.Количество() - 1 Тогда
			...
			|			ВременнаяТаблица
			|		ГДЕ ЛОЖЬ И 1=1
			|		) КАК ИР_ЗащитаГруппировокОтКомпоновщика";
			ИменаТаблиц = Новый Массив;
			ИменаПолей = Новый Массив; 
			Для Каждого ЗапросДляЗащиты Из Пакет.НайтиСтроки(Новый Структура("ЗащититьГруппировкиОтКомпоновщика, ТипЗапроса", Истина, 1)) Цикл
				Если ЗапросПакета = ЗапросДляЗащиты Тогда
					Продолжить;
				КонецЕсли;
				ИменаТаблиц.Добавить(ЗапросДляЗащиты.ИмяОсновнойТаблицы);
				Для Каждого ПолеОбъединения Из ЗапросДляЗащиты.ПоляОбъединения Цикл
					ИменаПолей.Добавить(ЗапросДляЗащиты.ИмяОсновнойТаблицы + "." + ПолеОбъединения.Имя);
				КонецЦикла;
			КонецЦикла;
			Если ЗначениеЗаполнено(ИменаТаблиц) Тогда
				ТекстЗащиты = СтрЗаменить(ТекстЗащиты, "ВременнаяТаблица", ирОбщий.СтрСоединитьЛкс(ИменаТаблиц,,,, "%1 КАК %1"));
				ТекстЗащиты = СтрЗаменить(ТекстЗащиты, "И 1=1", ирОбщий.СтрСоединитьЛкс(ИменаПолей, "",,, "
				|			И %1 = NULL"));
			Иначе
				ТекстЗащиты = "";
			КонецЕсли;
			ТекстЗапроса = СобратьТекстЗапросаПакета(ЗапросПакета, Смещение, СтруктураПолей, ТекстЗащиты);
		Иначе
...
Функция СобратьТекстЗапросаПакета(ЗапросПакета = Неопределено, Смещение = "", СтруктураПолей = Неопределено, Знач ТекстЗащиты = "") Экспорт 
	...
	Для Каждого ЧастьОбъединения Из ЗапросПакета.ЧастиОбъединения Цикл
		...
		ТекстЧасти = СобратьТекстЧастиОбъединения(ЗапросПакета, ЧастьОбъединения, Смещение, ЗапросПакета.ЧастиОбъединения.Количество() > 1, СтруктураПолей, ТекстЗащиты);
	...
	Если ЗапросПакета.ТипЗапроса = 1 Тогда
		// INDEX BY
		СтруктураПолей.РазрешитьНормализациюИмен = Ложь;
		ТекстИндексы = Новый Массив;
		Для Каждого ИндексТаблицы Из ЗапросПакета.ИндексыТаблицы Цикл
			ТекстИндекс = "";
			Для Каждого ЭлементИндекса Из ИндексТаблицы.ПоляИндекса Цикл
				Если ТекстИндекс <> "" Тогда
					ТекстИндекс = ТекстИндекс + ", ";
				КонецЕсли; 
			...
			КонецЦикла;
			Если ЗначениеЗаполнено(ТекстИндекс) Тогда
				Если Истина
					И ЗапросПакета.ИндексыТаблицы.Количество() = 1
					И Не ИндексТаблицы.Уникальный
				Тогда
				...
				Иначе
					ТекстИндекс = Символы.Таб + "(" + ТекстИндекс + ")";
					Если ИндексТаблицы.Уникальный Тогда
						ТекстИндекс = ТекстИндекс + " " + СловоЯзыкаЗапросов("UNIQUE");
					КонецЕсли;
					ТекстИндексы.Добавить(ТекстИндекс);
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
		Если ТекстИндексы.Количество() Тогда
			Результат = Результат + Символы.ПС + Смещение + СловоЯзыкаЗапросовВФорме("INDEX") 
				+ " " + СловоЯзыкаЗапросовВФорме("BY") + " " + СловоЯзыкаЗапросовВФорме("SETS") + " (" + Символы.ПС + Смещение + ирОбщий.СтрСоединитьЛкс(ТекстИндексы, "," + Символы.ПС)
				+ Смещение + ")";
		КонецЕсли;
	КонецЕсли; 
	...
	Если ЗапросПакета.ТипЗапроса = 0 И Не РежимКомпоновкиДанных Тогда
		...
		ДобавитьТекстПредложенияПостроителя(Результат, СловоЯзыкаЗапросовВФорме("TOTALS") + " " + СловоЯзыкаЗапросовВФорме("BY"),
			ЗапросПакета.ПоляИтоговПостроителя, ЭлементыФормы.ПоляИтоговПостроителя, ЗапросПакета, , Смещение, Ложь);
	ИначеЕсли Ложь
...
Процедура ДобавитьТекстПредложенияПостроителя(ОбщийТекст, ПредложениеПостроителя, ТаблицаПолейПостроителя, ТабличноеПолеПостроителя, ЗапросПакета,
	ЧастьОбъединения = Неопределено, Смещение = "", Знач РазрешитьНормализациюИмен = Ложь)
	
	ТекстПостроителя = СобратьТекстПредложенияПостроителя(ЗапросПакета, ТаблицаПолейПостроителя, ТабличноеПолеПостроителя, ЧастьОбъединения, Смещение, РазрешитьНормализациюИмен);
...
Функция СобратьТекстПредложенияПостроителя(Знач ЗапросПакета, Знач ТаблицаПолейПостроителя, Знач ТабличноеПолеПостроителя = Неопределено, Знач ЧастьОбъединения = Неопределено, Знач Смещение = "",
	Знач РазрешитьНормализациюИмен = Истина)
	...
	Для Каждого ПолеПостроителя Из ТаблицаПолейПостроителя Цикл
		Если Не ЗначениеЗаполнено(ПолеПостроителя.Определение) Тогда
			Продолжить;
		КонецЕсли; 
		ТекстВыражения = ПолучитьОбновитьВыражениеПоляПостроителя(ПолеПостроителя, ТабличноеПолеПостроителя, ЗапросПакета, ЧастьОбъединения, РазрешитьНормализациюИмен);
...
Функция ПолучитьОбновитьВыражениеПоляПостроителя(ПолеПостроителя, ТабличноеПоле = Неопределено, Знач ЗапросПакета, Знач ЧастьОбъединения = Неопределено, Знач РазрешитьНормализациюИмен = Истина)

	ТекстВыражения = СокрП(ОбновитьВыражениеЗапроса(ПолеПостроителя, ТабличноеПоле, ЗапросПакета, ЧастьОбъединения,,,,, РазрешитьНормализациюИмен, 0));
...
Функция СобратьТекстЧастиОбъединения(ЗапросПакета = Неопределено, ЧастьОбъединения = Неопределено, Смещение = "", Знач РазвернутьАвтополя = Ложь, Знач СтруктураПолей = Неопределено,
	...
	Если ЧастьОбъединения = ЗапросПакета.ЧастиОбъединения[0] И ЗначениеЗаполнено(ТекстЗащиты) Тогда
		Если ЗначениеЗаполнено(ТекстИЗ) Тогда
			ТекстЗащиты = Символы.ПС + Смещение + Символы.Таб + ", " + ТекстЗащиты;
		КонецЕсли;
		ТекстИЗ = ТекстИЗ + ТекстЗащиты;
	КонецЕсли;
	Результат = Результат + ТекстИЗ;
	...
	ДобавитьТекстПредложенияПостроителя(Результат, СловоЯзыкаЗапросовВФорме("WHERE"),
		ЧастьОбъединения.ПоляОтбораПостроителя, ЭлементыФормы.ПоляОтбораПостроителя, ЗапросПакета, ЧастьОбъединения, Смещение, Истина);
		...
		Если СтрокаДоступнойТаблицы.Тип = "Параметр" Тогда
			...
			Иначе
				...
				Если СтрокаДоступнойТаблицыВТ = Неопределено Тогда
					...
					//КПВыбранныеПоляОбъявитьТипы(, ЧастьОбъединенияДоп);
...
//
// Параметры:
//  СтрокаДоступногоПоля		 - ДоступноеПолеКомпоновкиДанных, ДоступныеПоляКомпоновкиДанных - 
//  Автополе					 - 	 - 
//  ЗапросПакета				 - 	 - 
//  ЧастьОбъединения			 - 	 - 
//  СдвигИндексаПослеДобавления	 - 	 - 
...
Процедура ДобавитьДоступноеПолеСДочернимиВВыбранныеПоля(СтрокаДоступногоПоля, Автополе = Ложь, ЗапросПакета = Неопределено,
	...
	Если Ложь
		...
		Или СтрокаДоступногоПоля.Папка
	Тогда
		// Все дочерние поля
		Если Ложь
			Или ТипЗнч(СтрокаДоступногоПоля) = Тип("ДоступныеПоляКомпоновкиДанных")
			Или (Истина
				И ТипЗнч(СтрокаДоступногоПоля) = Тип("ДоступноеПолеКомпоновкиДанных")
				И Не ирОбщий.СтрокиРавныЛкс("ПараметрыДанных", "" + СтрокаДоступногоПоля.Поле))
		Тогда
			Если ПараметрыДиалектаSQL.МногоТаблиц Тогда
				ВыбраннаяТаблица = ЧастьОбъединения.ВыбранныеТаблицы.Найти(НРег(СтрокаДоступногоПоля.Поле), "НИмя");      
			Иначе
...
//
// Параметры:
//  Родитель					 - ДоступноеПолеКомпоновкиДанных, ДоступныеПоляКомпоновкиДанных - 
//  ЧастьОбъединения			 - 	 - 
//  СдвигИндексаПослеДобавления	 - 	 - 
//  Автополе					 - 	 - 
//  ЗапросПакета				 - 	 - 
...
Процедура ДобавитьВсеПоляВыбраннойТаблицыВВыбранныеПоля(Знач Родитель, Знач ЧастьОбъединения = Неопределено, Знач СдвигИндексаПослеДобавления = 0, Знач Автополе = Истина,
	...
	КонецЕсли;
	УжеДобавлены = Новый Соответствие; 
	Если ТипЗнч(Родитель) = Тип("ДоступноеПолеКомпоновкиДанных") Тогда
		СтрокаВременнойТаблицы = ДоступнаяТаблицаПоЛюбомуИмени(Родитель.Заголовок);
	КонецЕсли;
...
Процедура РазвернутьЕдинственнуюПапкуДоступныхПолей(Знач ТабличноеПолеПолей) Экспорт
	...
	Если СтрокаПапки.Поле = Новый ПолеКомпоновкиДанных("СистемныеПоля") Тогда
		Возврат;
	КонецЕсли;
...
Функция ОбновитьКомпоновщикЧастиОбъединения(ЧастьОбъединения = Неопределено, Принудительно = Истина)
	...
	Для Каждого ВыбраннаяТаблица Из ЧастьОбъединения.ВыбранныеТаблицы Цикл
		...
		Если ПоказыватьИндексы Тогда
			...
			ПервыйПараметр = "";
			Если ВыбраннаяТаблица.Параметры.Количество() Тогда
				ПервыйПараметр = ВыбраннаяТаблица.Параметры[0].Определение;
			КонецЕсли;
			СтруктураХраненияБД = ирОбщий.СтруктураХраненияТаблицыБДЛкс(ВыбраннаяТаблица.ПолноеИмя,,,,, Новый Структура("Выражение", ПервыйПараметр));
			Если СтруктураХраненияБД.Количество() > 0 Тогда
				СтрокаТаблицыХранения = СтруктураХраненияБД[0];
	...
	Если Параметры.Количество() > 0 Тогда
		...
		КонецЦикла;
	КонецЕсли; 
	СхемаКомпоновки = ирОбщий.СоздатьСхемуКомпоновкиПоДеревуПолейЛкс(ДоступныеПоляВыбора, ПоказыватьИндексы, ПараметрыДиалектаSQL.МногоТаблиц, Не ЭтоВыборкаИзТаблицыПараметра(), МаркерПоляФункцииТипЗначения(), Истина);
...
Процедура ПриОткрытии()
	...
	ЭлементыФормы.ДоступныеПоляВыбораПостроителя.Доступность = Не РежимКомпоновкиДанных;
...
Процедура ОбновитьДоступныеВременныеТаблицы(ИмяВременнойТаблицы = Неопределено, СортироватьПослеДобавления = Истина) Экспорт
	...
	Для Индекс = 0 По ИндексТекущегоЗапроса - 1 Цикл
		...
		Если ЗапросПакета.ТипЗапроса = 1 Тогда // Создание временной таблицы
			//Если ДоступныеТаблицы.Найти(НРег(ЗапросПакета.ИмяОсновнойТаблицы), "НПолноеИмя") = Неопределено Тогда
				СтруктураДобавленныхТаблиц.Вставить(ЗапросПакета.ИмяОсновнойТаблицы, ЗапросПакета);
			//КонецЕсли; 
	...
	Для Каждого КлючИЗначение Из СтруктураДобавленныхТаблиц Цикл
		Измерения = ИзмеренияВложеногоЗапросаТекстом(КлючИЗначение.Значение);
		НайтиДобавитьДоступнуюВременнуюТаблицу(КлючИЗначение.Ключ, КлючИЗначение.Значение.ИД, БылиДобавления, ИсточникДанных1С, Измерения);
	КонецЦикла;
...
Функция ИзмеренияВложеногоЗапросаТекстом(Знач ВложенныйЗапрос) Экспорт
	Измерения = ИзмеренияЗапроса(ВложенныйЗапрос);
	Если Измерения.Количество() = 0 Тогда
		Возврат "";
	КонецЕсли;
	Измерения = "," + ирОбщий.СтрСоединитьЛкс(ирОбщий.ВыгрузитьСвойствоЛкс(Измерения), ",") + ",";
	Возврат Измерения;
КонецФункции
...
Функция ПоляВыбраннойТаблицы(ИмяИлиСтрокаВыбраннойТаблицы, ЧастьОбъединения = Неопределено)
	...
	СтрокаДоступнойТаблицы = ДоступнаяТаблицаПоЛюбомуИмени(ВыбраннаяТаблица.ПолноеИмя);
	...
	ИначеЕсли СтрокаДоступнойТаблицы <> Неопределено Тогда
		Если Истина
			И ВыбраннаяТаблица.Параметры.Количество()
			И ПустаяСтрока(ВыбраннаяТаблица.Параметры[0].Имя) 
		Тогда
...
Процедура ОбновлениеОтображения()
	...
	Если ПараметрыДиалектаSQL <> Неопределено Тогда
		...
		ЭлементыФормы.КПВыбранныеПоля.Кнопки.ОбъявитьТипы.Доступность = ТипЗапроса = 1;
		...
		ЭлементыФормы.ЗащититьГруппировкиОтКомпоновщика.Доступность = ТипЗапроса = 1;
		...
		ЭлементыФормы.ЗапросОпцияПервые.Видимость = Истина
			И ПервыеНаУровнеЗапроса
			И ПараметрыДиалектаSQL.Первые 
			И ТипЗапроса <> 2
			И ТипЗапроса <> 6
			И (ТипЗапроса <> 3 Или ЗначенияИзВыборки);
		ЭлементыФормы.ЗапросПервыеКоличество.Видимость = ЭлементыФормы.ЗапросОпцияПервые.Видимость;
		...
		//	КонецЕсли; 
		//КонецЕсли; 
		ЭлементыФормы.ПанельОсновная.Страницы.ИндексыТаблицы.Доступность = ТипЗапроса = 1 И ЛиКорневойЗапрос();
...
Функция СкорректироватьИмяВыбраннойТаблицы(Знач ВыбраннаяТаблица, ПринудительноАвтоимя = Ложь, РазрешитьПереименованиеДублей = Истина)
	...
	Если Истина
		...
		И СтрокаДоступнойТаблицы.Тип <> "ВременнаяТаблица"
	Тогда
		БазовоеИмя = ОсновнойПсевдонимТаблицыБД(СтрокаДоступнойТаблицы, Английский1С);
	Иначе
...
Процедура СвязиТаблицПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки) Экспорт
	...
	Если ДанныеСтроки.Условие <> Неопределено Тогда
		...
		НайденныеЭлементы = ирОбщий.ВсеЭлементыИерархииНастроекКомпоновкиЛкс(КомпоновщикПроверкиУсловияСвязи.Настройки.Отбор);
	...
	Если ДанныеСтроки.Родитель <> Неопределено Тогда
		ВозможноРазмножениеСтрок = Не ЛиВСвязиОтобраныВсеИзмеренияПравойТаблицы(ДанныеСтроки);
		ОформлениеСтроки.Ячейки.ВозможноРазмножениеСтрок.УстановитьФлажок(ВозможноРазмножениеСтрок);
	КонецЕсли;
...
// При наличии связи с отбором табличной части надо вызывать ПриОткытии для лечения перескока каретки
...
//
// Параметры:
//  ВыражениеИлиТокен	 - ОбработкаОбъект.ирТипGoldParserToken - 
//  ЧастьОбъединения	 - 	 - 
...
//
// Параметры:
//  ТокенТипа		 - ОбработкаОбъект.ирТипGoldParserToken - 
//  ЧастьОбъединения - 	 - 
// 
// Возвращаемое значение:
//   - 
...
Процедура ДоступныеТаблицыПриАктивизацииСтроки(Элемент)
	...
	Если ирОбщий.НуженСдвигПоляСсылкаВНачалоСпискаЛкс(ТекущаяСтрока.Тип) Тогда
...
Процедура ОчиститьДанные() Экспорт
	...
	ЭтаФорма.ЗащититьГруппировкиОтКомпоновщика = Неопределено;
	...
	Если СтруктураПолей.Режим = "ЗаполнениеНастроекКомпоновки" Тогда
		...
		Для Каждого ВыбранноеПоле Из ЧастьОбъединения.ВыбранныеПоля Цикл
			...
			Тогда
				Продолжить;
			КонецЕсли;
			Если ЧастьОбъединения.ПоляОтбораПостроителя.НайтиСтроки(Новый Структура("Определение, НИмя", ВыбранноеПоле.Определение, ВыбранноеПоле.НИмя)).Количество() > 0 Тогда
				Продолжить;
			КонецЕсли;
			ДобавитьДоступноеПолеВТаблицуВыражений(ВыбранноеПоле, ЧастьОбъединения.ПоляОтбораПостроителя, ЗапросПакета, ЧастьОбъединения,, "Определение",, Новый Структура("Имя", ВыбранноеПоле.Имя), Ложь);
		КонецЦикла;
	КонецЕсли;
...
Процедура УстановитьДиалектSQL(НовыйДиалектSQL = Неопределено, ИменованныеПараметры = Неопределено) Экспорт
	...
	ЭлементыФормы.ИндексыТаблицы.Колонки.Комментарий.Видимость = РазрешеныКомментарии;
	...
	КонецЕсли; 
	ЭлементыФормы.ИндексыТаблицы.ТолькоПросмотр = ирКэш.НомерВерсииПлатформыЛкс() < 803025;
...
Процедура СохранитьЗапросПакета(РазрешитьДобавлениеАвтополей = Истина)
	...
	//мТекущаяСтрокаЗапросыПакета.ИндексыТаблицы = ИндексыТаблицы.Скопировать();
	...
	КонецЕсли;
	Если ТипЗапроса = 1 И ИндексыТаблицы.Количество() = 0 Тогда
		ДобавитьИндексТаблицыБД(мТекущийЗапросПакета);
	КонецЕсли;
	...
	мТекущийЗапросПакета.ЗащититьГруппировкиОтКомпоновщика = ЗащититьГруппировкиОтКомпоновщика;
...
Процедура ЗагрузитьЗапросПакета()
	...
	Если мТекущийЗапросПакета <> Неопределено Тогда
		...
		ЭтаФорма.ИндексыТаблицы = мТекущийЗапросПакета.ИндексыТаблицы;
		...
		ЭтаФорма.ЗащититьГруппировкиОтКомпоновщика = мТекущийЗапросПакета.ЗащититьГруппировкиОтКомпоновщика;
...
Функция ТабличныеПоляЗапроса()
	...
	СписокТаблиц.Добавить("ИндексыТаблицы");
...
Функция ПолучитьИндексКартинкиВыбраннойТаблицы(ДанныеСтроки, выхСтрокаДоступнойТаблицы = Неопределено)
	...
	Если ИндексКартинки = Неопределено Тогда
		выхСтрокаДоступнойТаблицы = ДоступнаяТаблицаПоЛюбомуИмени(ДанныеСтроки.ПолноеИмя);
...
Процедура ОбновитьНаименованиеЗапроса(ЗапросПакета = Неопределено) Экспорт
	...
	Если ЗапросПакета.ТипЗапроса = 0 Тогда
		...
		Иначе
			...
			КонецЕсли; 
		КонецЕсли; 
		ЗапросПакета.ЧислоВложенных = 0;
		Для Каждого ОбъединенияЭлемент Из ЗапросПакета.ЧастиОбъединения Цикл
			ЗапросПакета.ЧислоВложенных = ЗапросПакета.ЧислоВложенных + ОбъединенияЭлемент.ВыбранныеТаблицы.Количество() - ОбъединенияЭлемент.ВыбранныеТаблицы.НайтиСтроки(Новый Структура("ВложенныйПакет")).Количество();
		КонецЦикла;
		ЭлементыФормы.ЗапросыПакета.Колонки.ЧислоВложенных.Видимость = Истина
			И ЗапросыПакета.Количество() > 1
			И ЗапросыПакета.Количество() > ЗапросыПакета.НайтиСтроки(Новый Структура("ЧислоВложенных", 0)).Количество();
...
Процедура ВыбранныеТаблицыВыбор(Элемент = Неопределено, ВыбраннаяСтрока, Колонка = Неопределено, СтандартнаяОбработка = Истина)
...
Процедура ОбновитьТаблицуПараметровВыбраннойТаблицы(Знач ВыбраннаяТаблица = Неопределено)
	...
	ДоступнаяТаблица = ДоступнаяТаблицаПоЛюбомуИмени(ПолноеИмяТаблицы);
...
Функция КПВыбранныеТаблицыНайтиВСписке(Кнопка = Неопределено, ИмяВыбранногоПоляДляАктивации = Неопределено)
	...
	Если ТекущаяСтрока <> Неопределено Тогда
		СтрокаДоступнойТаблицы = ДоступнаяТаблицаПоЛюбомуИмени(ТекущаяСтрока.ПолноеИмя);
		Если СтрокаДоступнойТаблицы <> Неопределено Тогда
...
Функция КомпоновщикОтбораВПараметре(Знач ВыбраннаяСтрока, Знач ПолноеИмяТаблицы, выхПолноеИмяТаблицыПараметра = "", Знач МожноПостроитель = Ложь)
	...
	Если ЭтоПараметрОтбор1С(ВыбраннаяСтрока) Тогда
		...
		Тогда
			...
			Если Найти(ВыбраннаяСтрока.Имя, " счета Дт") Тогда
				ДоступныеПоля.Колонки.Добавить("СчетДт", Новый ОписаниеТипов(ирОбщий.ИмяТипаИзПолногоИмениМДЛкс(ОбъектМД.ПланСчетов)));
			ИначеЕсли Найти(ВыбраннаяСтрока.Имя, " счета Кт") Тогда
				ДоступныеПоля.Колонки.Добавить("СчетКт", Новый ОписаниеТипов(ирОбщий.ИмяТипаИзПолногоИмениМДЛкс(ОбъектМД.ПланСчетов)));
			...
			ИначеЕсли Найти(ВыбраннаяСтрока.Имя, " счета") Тогда
			...
			СхемаПараметра = ирОбщий.СоздатьСхемуКомпоновкиПоЗапросуЛкс(ТекстИмитатора);
...
Процедура ОсновныеДействияФормыКонструкторЗапроса(Кнопка)
	...
	КонецЕсли; 
	Компонента = ПолеТекстаПрограммы[ЭлементыФормы.ПолеТекстаЗапроса.Имя];
	Компонента.ОткрытьКонструкторЗапроса(Ложь);
...
Процедура КПДеревоУсловияСвязиДобавитьЭлемент(Кнопка)
	ирКлиент.ДобавитьСтрокуВТабличноеПолеОтбораЛкс(ЭлементыФормы.ДеревоУсловияСвязи, 0);
КонецПроцедуры

Процедура КПДеревоУсловияСвязиДобавитьГруппуИ(Кнопка)
	ирКлиент.ДобавитьСтрокуВТабличноеПолеОтбораЛкс(ЭлементыФормы.ДеревоУсловияСвязи, 2);
КонецПроцедуры

Процедура КПДеревоУсловияСвязиДобавитьГруппуИли(Кнопка)
	ирКлиент.ДобавитьСтрокуВТабличноеПолеОтбораЛкс(ЭлементыФормы.ДеревоУсловияСвязи, 3);
КонецПроцедуры

Процедура КПДеревоУсловияДобавитьЭлемент(Кнопка)
	ирКлиент.ДобавитьСтрокуВТабличноеПолеОтбораЛкс(ЭлементыФормы.ДеревоУсловия, 0);
КонецПроцедуры

Процедура КПДеревоУсловияСвязиДобавитьВыражение(Кнопка)
	ирКлиент.ДобавитьСтрокуВТабличноеПолеОтбораЛкс(ЭлементыФормы.ДеревоУсловияСвязи, 1);
КонецПроцедуры

Процедура КПДеревоУсловияДобавитьВыражение(Кнопка)
	ирКлиент.ДобавитьСтрокуВТабличноеПолеОтбораЛкс(ЭлементыФормы.ДеревоУсловия, 1);
КонецПроцедуры

Процедура КПДеревоУсловияДобавитьГруппуИ(Кнопка)
	ирКлиент.ДобавитьСтрокуВТабличноеПолеОтбораЛкс(ЭлементыФормы.ДеревоУсловия, 2);
КонецПроцедуры

Процедура КПДеревоУсловияДобавитьГруппуИли(Кнопка)
	ирКлиент.ДобавитьСтрокуВТабличноеПолеОтбораЛкс(ЭлементыФормы.ДеревоУсловия, 3);
КонецПроцедуры
...
Процедура КПВыбранныеТаблицыПреобразоватьВПодзапрос(Кнопка)
	...
	ДоступнаяТаблица = ДоступнаяТаблицаПоЛюбомуИмени(ТекущаяСтрока.ПолноеИмя);
	...
	Иначе
			...
			Если СтрокаВыбраннойТаблицы <> Неопределено Тогда
				...
				Иначе
					...
					Если КартинкаТипаТаблицы.Вид = ВидКартинки.Пустая Тогда
						СтрокаДоступнойТаблицы = ДоступнаяТаблицаПоЛюбомуИмени(СтрокаВыбраннойТаблицы.ПолноеИмя);
						Если СтрокаДоступнойТаблицы <> Неопределено Тогда
		...
		Иначе
			Если ДанныеСтроки.Родитель <> Неопределено Тогда
				...
				ТипТаблицы = "";
				Измерения = "";
				СтрокаПоля = ОписаниеДоступногоПоля(ДанныеСтроки, ТипТаблицы, Измерения);
				ПолеИспользовано = ВыбранныеПоля.Найти("" + ДанныеСтроки.Поле, "Определение") <> Неопределено;
				ирКлиент.ОформитьСтрокуДоступногоПоляЛкс(ОформлениеСтроки, СтрокаПоля, ТипТаблицы,, ЭтоИндекс, ПолеИспользовано, Измерения);
			КонецЕсли; 
		КонецЕсли; 
	КонецЕсли; 
...
Функция ОписаниеДоступногоПоля(Знач ДоступноеПоле, выхТипТаблицы = "", выхИзмерения = "")
	
	СтрокаПоля = Неопределено;
	СтрокаВыбраннойТаблицы = ВыбранныеТаблицы.Найти(НРег(ДоступноеПоле.Родитель.Поле), "НИмя");
	Если СтрокаВыбраннойТаблицы <> Неопределено Тогда
		Если ЗначениеЗаполнено(СтрокаВыбраннойТаблицы.ПолноеИмя) Тогда
			выхСтрокаДоступнойТаблицы = ДоступнаяТаблицаПоЛюбомуИмени(СтрокаВыбраннойТаблицы.ПолноеИмя);
			Если выхСтрокаДоступнойТаблицы <> Неопределено Тогда
				...
				выхИзмерения = выхСтрокаДоступнойТаблицы.Измерения;
				выхТипТаблицы = выхСтрокаДоступнойТаблицы.Тип;
			КонецЕсли;
		Иначе
			выхСтрокаДоступнойТаблицы = СтрокаВыбраннойТаблицы.ВложенныйПакет[0];
			Если выхСтрокаДоступнойТаблицы <> Неопределено Тогда
				СтрокаПоля = выхСтрокаДоступнойТаблицы.ПоляОбъединения.Найти(НРег(ирОбщий.ПоследнийФрагментЛкс(ДоступноеПоле.Поле)), "НИмя");
				выхИзмерения = ИзмеренияВложеногоЗапросаТекстом(выхСтрокаДоступнойТаблицы);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	Возврат СтрокаПоля;
...
Функция ТекстЗапросаСВыборомМетаданных(Знач ЛиТаблицыИзменения = Ложь) Экспорт
...
Процедура КПВыбранныеТаблицыОбъединениеСсылочныхТаблиц(Кнопка)
	...
	Текст = ТекстЗапросаСВыборомМетаданных();
...
Процедура ПоляДоступнойТаблицыПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки) Экспорт
	...
	Если ДоступнаяТаблица <> Неопределено Тогда
		...
		КонецЕсли;
		ирКлиент.ОформитьСтрокуДоступногоПоляЛкс(ОформлениеСтроки, ДанныеСтроки, ДоступнаяТаблица.Тип, "Имя",, ПолеИспользовано, ДоступнаяТаблица.Измерения);
	КонецЕсли;
	ирКлиент.ТабличноеПолеПриВыводеСтрокиЛкс(ЭтаФорма, Элемент, ОформлениеСтроки, ДанныеСтроки);
...
Процедура ФильтрДоступныхТаблицОчистка(Элемент = Неопределено, СтандартнаяОбработка = Ложь)
	СтандартнаяОбработка = Ложь;
	ОтборСтрок = ЭлементыФормы.ДоступныеТаблицы.ОтборСтрок;
	ОтборСтрок.Имя.Использование = Ложь;
	ОтборСтрок.ИмяДляОтбора.Использование = Ложь;
...
Функция ОтключитьОтборыЗапроса(ОпределенияТаблиц = Неопределено) Экспорт 
	...
	Для Каждого ЧастьОбъединения Из ЧастиОбъединения Цикл
		...
		Для Каждого СвязьТаблиц Из ирОбщий.ВсеСтрокиДереваЗначенийЛкс(СвязиТаблиц) Цикл
			...
			Тогда
				СтрокаТаблицы = ВыбранныеТаблицы.Найти(Нрег(СвязьТаблиц.Таблица), "НИмя");
				ТаблицыВнутреннихСоединений.Добавить(СвязьТаблиц.Таблица, СтрокаТаблицы.ПолноеИмя);
			КонецЕсли;
...
Процедура КПВыбранныеПоляОбъявитьТипы(Кнопка = Неопределено, ЧастьОбъединения = Неопределено)
...
Процедура КПЗапросыПакетаОчиститьНастройкиКомпоновки(Кнопка)
	СохранитьЗапросПакета();
		...
		Для Каждого ЭлементСписка Из ПоляДляПровеки Цикл
			...
			Для Каждого ЧастьОбъединения Из ЗапросПакета.ЧастиОбъединения Цикл
				...
				Для Каждого СтрокаПоля Из СтруктураОбработкиПолей.СоответствиеПолей Цикл
					...
					Иначе
						ДоступнаяТаблица = ДоступнаяТаблицаПоЛюбомуИмени(ВыбраннаяТаблица.ПолноеИмя);
						...
						Иначе  
							...
							Если ДоступнаяТаблица.Тип = "ВиртуальнаяТаблица" Тогда
								ПолноеИмяПоля = ирОбщий.ТекстБезПоследнегоФрагментаЛкс(ПолноеИмяПоля);
...
Процедура ОсновныеДействияФормыНастройки(Кнопка)
	...
	ФормаНастроек = ПолучитьФорму("НастройкиКонструктораЗапроса", ЭтаФорма);
...
Процедура ВыбранныеПоляИмяНачалоВыбораИзСписка(Элемент, СтандартнаяОбработка)
	...
	Для Каждого ПолеОбъединения Из ПоляОбъединения Цикл
		Если ПолеОбъединения.Имя <> "" И ВыбранныеПоля.Найти(НРег(ПолеОбъединения.Имя), "НИмя") = Неопределено Тогда
			СписокВыбора.Добавить(ПолеОбъединения.Имя, ПолеОбъединения.Имя + " - " + ирОбщий.РасширенноеПредставлениеЛкс(ПолеОбъединения.ТипЗначения));
		КонецЕсли;
	КонецЦикла;
...
Процедура ОчиститьТокеныВНастройкахКомпоновки(Знач НастройкиКомпоновки)
	...
	//ВсеЭлементыОтбора = ирОбщий.ВсеЭлементыИерархииНастроекКомпоновкиЛкс(НастройкиКомпоновки.Отбор, Ложь, Истина);
...
Процедура КППоляДоступнойТаблицыПерейтиКТаблицеТипа(Кнопка)
	ТекущаяСтрока = ЭлементыФормы.ПоляДоступнойТаблицы.ТекущаяСтрока;
	Если ТекущаяСтрока = Неопределено Тогда
		Возврат;
	КонецЕсли;
	Тип = ТекущаяСтрока.ТипЗначения.Типы()[0];
	Если Не ирОбщий.ЛиТипСсылкиБДЛкс(Тип, Ложь) Тогда
		Возврат;
	КонецЕсли;
	СтрокаДоступнойТаблицы = ДоступнаяТаблицаПоЛюбомуИмени(ирКэш.ИмяТаблицыИзМетаданныхЛкс(Метаданные.НайтиПоТипу(Тип).ПолноеИмя()));
	Если СтрокаДоступнойТаблицы = Неопределено Тогда
		Возврат;
	КонецЕсли;
	ЭлементыФормы.ДоступныеТаблицы.ТекущаяСтрока = СтрокаДоступнойТаблицы;
...
Процедура ЗапросыПакетаВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	Если Колонка = Элемент.Колонки.ЧислоВложенных Тогда
		ЭлементыФормы.ПанельЧасти.ТекущаяСтраница = ЭлементыФормы.ПанельЧасти.Страницы.ВыбранныеТаблицы;
		Вложенные = ирОбщий.ОтобратьКоллекциюЛкс(ВыбранныеТаблицы, "Э.ВложенныйПакет <> Неопределено");
		Если Вложенные.Количество() = 1 Тогда 
			ЭлементыФормы.ВыбранныеТаблицы.ТекущаяСтрока = Вложенные[0];
			ВыбранныеТаблицыВыбор(, Вложенные[0]);
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры

Процедура ИндексыТаблицыПередУдалением(Элемент, Отказ)
	Отказ = Элемент.Значение.Количество() = 1;
КонецПроцедуры

Процедура ИндексыТаблицыПриАктивизацииСтроки(Элемент)
	ЭтаФорма.ПоляИндекса = Элемент.ТекущаяСтрока.ПоляИндекса;
	ирКлиент.ТабличноеПолеПриАктивизацииСтрокиЛкс(ЭтаФорма, Элемент);
КонецПроцедуры

Процедура ИндексыТаблицыПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки)
	ирКлиент.ТабличноеПолеПриВыводеСтрокиЛкс(ЭтаФорма, Элемент, ОформлениеСтроки, ДанныеСтроки);
КонецПроцедуры

Процедура ИндексыТаблицыПриИзмененииФлажка(Элемент, Колонка)
	ирКлиент.ТабличноеПолеПриИзмененииФлажкаЛкс(ЭтаФорма, Элемент, Колонка);
КонецПроцедуры

Процедура КПДеревоУсловияСвязиМаксДата(Кнопка)
	ЛеваяТаблица = "";
	ПраваяТаблица = "";
	ПолеДатыЛевойТаблицы = "";
	ПолеДатыПравойТаблицы = "";
	ЭлементОтбораДат = Неопределено; // ЭлементОтбораКомпоновкиДанных
	СохранитьСвязьТаблиц();
	ТекущаяСвязь = ЭлементыФормы.СвязиТаблиц.ТекущаяСтрока;
	ОтобраныВсеИзмеренияПравойТаблицы = ЛиВСвязиОтобраныВсеИзмеренияПравойТаблицы(ТекущаяСвязь, ЛеваяТаблица, ПраваяТаблица, ПолеДатыЛевойТаблицы, ПолеДатыПравойТаблицы, ЭлементОтбораДат);
	Если ЭлементОтбораДат = Неопределено Тогда
		Предупреждение("В условии связи не найден элемент, связывающий даты.");
		Возврат;
	КонецЕсли;
	Если Не ОтобраныВсеИзмеренияПравойТаблицы Тогда
		ТекстВопроса = "В условии связи использованы не все измерения правой таблицы. Продолжить?";
	...
	ВложенныйЗапрос = ДобавитьЗапросПакета(ЗапросыВложенногоПакета);
	ВложеннаяЧастьОбъединения = ДобавитьЧастьОбъединения(ВложенныйЗапрос);
	СтрокаВыбраннойТаблицыЛевая = ВложеннаяЧастьОбъединения.ВыбранныеТаблицы.Добавить();
	ЗаполнитьЗначенияСвойств(СтрокаВыбраннойТаблицыЛевая, ВыбранныеТаблицы.Найти(НРег(ЛеваяТаблица), "НИмя")); // Опасно. Копируем ссылку на таблицу Параметры
	СтрокаСвязиЛевая = НайтиДобавитьВыбраннуюТаблицуВСвязи(СтрокаВыбраннойТаблицыЛевая, ВложеннаяЧастьОбъединения);
	СтрокаВыбраннойТаблицыПравая = ВложеннаяЧастьОбъединения.ВыбранныеТаблицы.Добавить();
	ЗаполнитьЗначенияСвойств(СтрокаВыбраннойТаблицыПравая, ВыбранныеТаблицы.Найти(НРег(ПраваяТаблица), "НИмя")); // Опасно. Копируем ссылку на таблицу Параметры
	СтрокаСвязиПравая = НайтиДобавитьВыбраннуюТаблицуВСвязи(СтрокаВыбраннойТаблицыПравая, ВложеннаяЧастьОбъединения, СтрокаСвязиЛевая);
	СтрокаСвязиПравая.ТипСвязи = "LEFT";
	СтрокаСвязиПравая.Условие = КомпоновщикУсловияСвязи.ПолучитьНастройки();
	ЭлементОтбора = ирОбщий.НайтиЭлементОтбораЛкс(СтрокаСвязиПравая.Условие.Отбор, "" + ЭлементОтбораДат.ЛевоеЗначение);
	ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных(ПолеДатыЛевойТаблицы);
	ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.БольшеИлиРавно;
	ЭлементОтбора.ПравоеЗначение = Новый ПолеКомпоновкиДанных(ПолеДатыПравойТаблицы);
	ОбновитьКомпоновщикЧастиОбъединения(ВложеннаяЧастьОбъединения);  
	ВыбранноеПолеПраваяДата = ВложеннаяЧастьОбъединения.ВыбранныеПоля.Добавить();
	ВыбранноеПолеПраваяДата.ИД = Новый УникальныйИдентификатор;
	ВыбранноеПолеПраваяДата.Определение = "Максимум(" + ПолеДатыПравойТаблицы + ")";
	ВыбранноеПолеПраваяДата.Имя = ирОбщий.ПоследнийФрагментЛкс(ПолеДатыПравойТаблицы);
	ПриИзмененииВыраженияВыбранногоПоля(ВыбранноеПолеПраваяДата, ВложенныйЗапрос, ВложеннаяЧастьОбъединения);
	ВыбраннаяТаблицаВложенныйЗапрос = ДобавитьВыбраннуюТаблицу(мТекущаяЧастьОбъединения);
	ВыбраннаяТаблицаВложенныйЗапрос.Имя = ирОбщий.АвтоУникальноеИмяВКоллекцииЛкс(ВыбранныеТаблицы, "МаксДаты" + ПраваяТаблица);
	УсловиеСвязиСМаксДатами = Новый НастройкиКомпоновкиДанных;
	Для Каждого ЭлементОтбора Из ТекущаяСвязь.Условие.Отбор.Элементы Цикл
		Если Ложь
			Или ТипЗнч(ЭлементОтбора) = Тип("ГруппаЭлементовОтбораКомпоновкиДанных")
			Или ЗначениеЗаполнено(ЭлементОтбора.Представление)
		Тогда
			Продолжить;
		КонецЕсли;    
		ПолеПравойТаблицы = ИмяПоляВыбраннойТаблицыИзЭлементаОтбора(ЭлементОтбора, ПраваяТаблица, Истина);
		Если Не ЗначениеЗаполнено(ПолеПравойТаблицы) Тогда
			Продолжить;
		КонецЕсли;
		ПолеЛевоеТаблицы = ИмяПоляВыбраннойТаблицыИзЭлементаОтбора(ЭлементОтбора, ЛеваяТаблица, Истина);
		ВыбранноеПоле = ВложеннаяЧастьОбъединения.ВыбранныеПоля.Добавить();
		ВыбранноеПоле.ИД = Новый УникальныйИдентификатор;
		Если ЭлементОтбора = ЭлементОтбораДат Тогда
			ВыбранноеПоле.Определение = ПолеЛевоеТаблицы;
		Иначе
			ВыбранноеПоле.Определение = ПолеПравойТаблицы;
		КонецЕсли;
		ВыбранноеПоле.Имя = ирОбщий.ПоследнийФрагментЛкс(ВыбранноеПоле.Определение);
		ПриИзмененииВыраженияВыбранногоПоля(ВыбранноеПоле, ВложенныйЗапрос, ВложеннаяЧастьОбъединения);
		Если ЗначениеЗаполнено(ПолеЛевоеТаблицы) Тогда
			ирОбщий.НайтиДобавитьЭлементОтбораКомпоновкиЛкс(УсловиеСвязиСМаксДатами.Отбор, ПолеЛевоеТаблицы, Новый ПолеКомпоновкиДанных(ВыбраннаяТаблицаВложенныйЗапрос.Имя + "." + ирОбщий.ПоследнийФрагментЛкс(ВыбранноеПоле.Определение)));
		КонецЕсли;
		Если ЭлементОтбора = ЭлементОтбораДат Тогда
			ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных(ВыбраннаяТаблицаВложенныйЗапрос.Имя + "." + ирОбщий.ПоследнийФрагментЛкс(ПолеПравойТаблицы));
			ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
			ЭлементОтбора.ПравоеЗначение = Новый ПолеКомпоновкиДанных(ПолеПравойТаблицы);
		КонецЕсли;
	КонецЦикла;
	ЗагрузитьСвязьТаблиц();
	ВыбраннаяТаблицаВложенныйЗапрос.ВложенныйПакет = ЗапросыВложенногоПакета;
	ирОбщий.ОбновитьКопиюСвойстваВНижнемРегистреЛкс(ВыбраннаяТаблицаВложенныйЗапрос);
	ОбновитьКомпоновщикЧастиОбъединения();
	ПанельЧастиОбновитьТекущуюСтраницу();
	СтрокаСвязиНовая = НайтиДобавитьВыбраннуюТаблицуВСвязи(ВыбраннаяТаблицаВложенныйЗапрос,, ЭлементыФормы.СвязиТаблиц.ТекущаяСтрока.Родитель);
	СтрокаСвязиНовая.ТипСвязи = "LEFT";
	СоседниеСвязи = СтрокаСвязиНовая.Родитель.Строки;
	СоседниеСвязи.Сдвинуть(СтрокаСвязиНовая, СоседниеСвязи.Индекс(ЭлементыФормы.СвязиТаблиц.ТекущаяСтрока) - СоседниеСвязи.Индекс(СтрокаСвязиНовая));
	СтрокаСвязиНовая.Условие = УсловиеСвязиСМаксДатами;
КонецПроцедуры
...
//   ЛеваяТаблица - Строка - 
//   ПраваяТаблица - Строка - 
//   ПолеДатыЛевойТаблицы - ПолеКомпоновкиДанных, Строка - 
//   ПолеДатыПравойТаблицы - Строка - 
//   ЭлементОтбораДат - ГруппаЭлементовОтбораКомпоновкиДанных, ЭлементОтбораКомпоновкиДанных, Неопределено - 
// Возвращаемое значение:
//   Булево - 
Функция ЛиВСвязиОтобраныВсеИзмеренияПравойТаблицы(Знач ТекущаяСтрока, ЛеваяТаблица = Неопределено, ПраваяТаблица = Неопределено, ПолеДатыЛевойТаблицы = Неопределено, ПолеДатыПравойТаблицы = Неопределено, ЭлементОтбораДат = Неопределено) Экспорт
	Если ТекущаяСтрока = Неопределено Тогда
		ТекущаяСтрока = ЭлементыФормы.СвязиТаблиц.ТекущаяСтрока;
	КонецЕсли;
	ПраваяТаблица = ТекущаяСтрока.Таблица;
	ЛеваяТаблица = ТекущаяСтрока.Родитель.Таблица;
	ИменаИзмерений = Новый Структура;
	СтрокаПравойВыбраннойТаблицы = ВыбранныеТаблицы.Найти(НРег(ПраваяТаблица), "НИмя");
	ПолноеИмяПраваяТаблица = СтрокаПравойВыбраннойТаблицы.ПолноеИмя;
	Если ЗначениеЗаполнено(ПолноеИмяПраваяТаблица) Тогда
		ОписаниеТаблицыБД = ирОбщий.ОписаниеТаблицыБДЛкс(ПолноеИмяПраваяТаблица);
		Если ОписаниеТаблицыБД <> Неопределено Тогда
			ИменаИзмерений = ирОбщий.СтруктураКлючаТаблицыБДЛкс(ПолноеИмяПраваяТаблица,,, Ложь, Ложь,, Ложь);
		Иначе
			ДоступнаяТаблица = ДоступнаяТаблицаПоЛюбомуИмени(ПолноеИмяПраваяТаблица);
			Если ДоступнаяТаблица <> Неопределено Тогда
				ИменаИзмерений = Новый Структура(ДоступнаяТаблица.Измерения);
			КонецЕсли;
		КонецЕсли;
	Иначе
		ВложенныйПакет = СтрокаПравойВыбраннойТаблицы.ВложенныйПакет;
		ИменаИзмерений = ИзмеренияЗапроса(ВложенныйПакет[0]);
	КонецЕсли;
	ЭлементОтбораДат = Неопределено;
	ИменаПравыхПолейУсловия = Новый Структура;
	Для Каждого ЭлементОтбора Из ТекущаяСтрока.Условие.Отбор.Элементы Цикл
		Если Ложь
			Или ТипЗнч(ЭлементОтбора) = Тип("ГруппаЭлементовОтбораКомпоновкиДанных")
			Или ЗначениеЗаполнено(ЭлементОтбора.Представление)
		...
		Тогда
			Продолжить;
		КонецЕсли;
		ИмяПоляПравойТаблицы = ИмяПоляВыбраннойТаблицыИзЭлементаОтбора(ЭлементОтбора, ПраваяТаблица);
		Если Не ЗначениеЗаполнено(ИмяПоляПравойТаблицы) Тогда
			Продолжить;
		КонецЕсли;
		Если ИменаИзмерений.Свойство(ИмяПоляПравойТаблицы) Тогда
			ИменаПравыхПолейУсловия.Вставить(ИмяПоляПравойТаблицы);
		КонецЕсли;
		Если ТекущаяСтрока = ЭлементыФормы.СвязиТаблиц.ТекущаяСтрока Тогда
			ПолеЛевоеТаблицы = ИмяПоляВыбраннойТаблицыИзЭлементаОтбора(ЭлементОтбора, ЛеваяТаблица, Истина);
			Если ТипЗнч(ЭлементОтбора.ЛевоеЗначение) = Тип("ПолеКомпоновкиДанных") Тогда
				ЛевоеДоступноеПоле = КомпоновщикУсловияСвязи.Настройки.ДоступныеПоляОтбора.НайтиПоле(ЭлементОтбора.ЛевоеЗначение);
			Иначе
				ЛевоеДоступноеПоле = Неопределено;
			КонецЕсли;
			Если ТипЗнч(ЭлементОтбора.ПравоеЗначение) = Тип("ПолеКомпоновкиДанных") Тогда
				ПравоеДоступноеПоле = КомпоновщикУсловияСвязи.Настройки.ДоступныеПоляОтбора.НайтиПоле(ЭлементОтбора.ПравоеЗначение);
			Иначе
				ПравоеДоступноеПоле = Неопределено;
			КонецЕсли;
			Если Истина
				И ЛевоеДоступноеПоле <> Неопределено
				И ПравоеДоступноеПоле <> Неопределено
				И ЛевоеДоступноеПоле.ТипЗначения.СодержитТип(Тип("Дата"))
				И ПравоеДоступноеПоле.ТипЗначения.СодержитТип(Тип("Дата"))
			Тогда
				ЭлементОтбораДат = ЭлементОтбора;
				ПолеДатыПравойТаблицы = ПраваяТаблица + "." + ИмяПоляПравойТаблицы;
				ПолеДатыЛевойТаблицы = ПолеЛевоеТаблицы;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	ОтобраныВсеИзмеренияПравойТаблицы = ИменаИзмерений.Количество() = ИменаПравыхПолейУсловия.Количество();
	Возврат ОтобраныВсеИзмеренияПравойТаблицы;
КонецФункции

Функция ИзмеренияЗапроса(Знач ВложенныйЗапрос) Экспорт
	ИменаИзмерений = Новый Структура;
	ЧастиОбъединенияПодзапроса = ВложенныйЗапрос.ЧастиОбъединения;
	Если ЧастиОбъединенияПодзапроса.Количество() = 1 Тогда
		Если ЧастиОбъединенияПодзапроса[0].ОпцияРазличные Тогда
			Для Каждого ВыбранныеПоляЭлемент Из ЧастиОбъединенияПодзапроса[0].ВыбранныеПоля Цикл
				ИменаИзмерений.Вставить(ВыбранныеПоляЭлемент.Имя);
			КонецЦикла;
		Иначе
			Для Каждого ГруппировкиЭлемент Из ЧастиОбъединенияПодзапроса[0].Группировки Цикл
				ВыбранноеПоле = ЧастиОбъединенияПодзапроса[0].ВыбранныеПоля.Найти(ГруппировкиЭлемент.Определение, "Определение");
				Если ВыбранноеПоле <> Неопределено Тогда
					ИменаИзмерений.Вставить(ВыбранноеПоле.Имя);
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
	КонецЕсли;
	Если ИменаИзмерений.Количество() = 0 Тогда
		СтрокаТаблицы = ВложенныйЗапрос.ИндексыТаблицы.Найти(Истина, "Уникальный");
		Если СтрокаТаблицы <> Неопределено Тогда
			Для Каждого ПоляИндексаЭлемент Из СтрокаТаблицы.ПоляИндекса Цикл
				ИменаИзмерений.Вставить(ПоляИндексаЭлемент.Имя);
			КонецЦикла;
		КонецЕсли;
	КонецЕсли;
...
//   ИмяВыбраннойТаблицы - Строка - 
// Возвращаемое значение:
//   Строка - 
Функция ИмяПоляВыбраннойТаблицыИзЭлементаОтбора(Знач ЭлементОтбора, Знач ИмяВыбраннойТаблицы, Знач ВместеСРодителем = Ложь) Экспорт
	Если ирОбщий.СтрНачинаетсяСЛкс(ЭлементОтбора.ПравоеЗначение, ИмяВыбраннойТаблицы + ".") Тогда
		ИмяПоля = ирОбщий.СтрРазделитьЛкс(ЭлементОтбора.ПравоеЗначение)[1];
	ИначеЕсли ирОбщий.СтрНачинаетсяСЛкс(ЭлементОтбора.ЛевоеЗначение, ИмяВыбраннойТаблицы + ".") Тогда
		ИмяПоля = ирОбщий.СтрРазделитьЛкс(ЭлементОтбора.ЛевоеЗначение)[1];
	Иначе
		ИмяПоля = "";
	КонецЕсли;
	Если ЗначениеЗаполнено(ИмяПоля) И ВместеСРодителем Тогда
		ИмяПоля = ИмяВыбраннойТаблицы + "." + ИмяПоля;
	КонецЕсли;
	Возврат ИмяПоля;
КонецФункции

Процедура ОсновныеДействияФормыЗаполнитьИИ(Кнопка)
	Если Кнопка.Картинка <> ирКэш.КартинкаПоИмениЛкс("ирОстановить") Тогда
		СохранитьЗапросПакета();
		ИндексыЗапросовПакета = Новый Массив;
		ИндексыЗапросовПакета = ирОбщий.ВыгрузитьСвойствоЛкс(ЭлементыФормы.ЗапросыПакета.ВыделенныеСтроки, "Индекс");
		ТекстПромта = ТекстЗапросаГенерироватьИменаЗапросаИИ(СобратьПолныйТекст(), ИндексыЗапросовПакета);
	КонецЕсли;
	БлокируемыеЭлементы = Новый Массив;
	БлокируемыеЭлементы.Добавить(ЭтаФорма);
	ПараметрыЗадания = Новый Структура;
	ПараметрыЗадания.Вставить("Текст", ТекстПромта);
	мФоновыйЗапросХттп.ОтправитьЗапросИИ(ПараметрыЗадания, ЭтаФорма, "ЗаполнитьИИКонец", БлокируемыеЭлементы, Кнопка);
КонецПроцедуры  
...
Процедура ЗаполнитьИИКонец(СостояниеЗадания, РезультатЗадания) Экспорт
	Если РезультатЗадания = Неопределено Тогда
		Возврат;
	КонецЕсли;
	Ответ = РезультатЗадания.Ответ;
	РезультатИИ = ирОбщий.ОбъектИзТекстаЖСОНЛкс(ирОбщий.ТекстМеждуМаркерамиЛкс(РезультатЗадания.Ответ, "```json", "```")); // см. ПрочитатьJSON(ОбразецРезультатаИИЖСОН())
	Для Каждого РезультатЭлемент Из РезультатИИ Цикл
		ЗапросПакета = ЗапросыПакета[Число(РезультатЭлемент.index)];
		Если ЗапросПакета.ТипЗапроса = 1 Тогда
			ПереименоватьТаблицуИлиПараметр(ЗапросыПакета, ЗапросПакета.ИмяОсновнойТаблицы, РезультатЭлемент.name);
		Иначе
			ЗапросПакета.ИмяОсновнойТаблицы = РезультатЭлемент.name;
			ОбновитьНаименованиеЗапроса(ЗапросПакета);
		КонецЕсли;
	КонецЦикла;
	ЗагрузитьЗапросПакета();
...
Функция ТекстЗапросаГенерироватьИменаЗапросаИИ(Знач ТекстЗапроса, Знач ИндексыЗапросов, Знач ДопИнфоКонтекст = "")
	Результат =  
	"Generate new name no longer than 50 characters for selected queries of batch identifying by number (from 0):
	|" + ирОбщий.СтрСоединитьЛкс(ИндексыЗапросов) + ".
	|
	|# Output Format
	...
	|[
	|    {
	|      ""index"": <номер запроса в пакете, начиная с 0>,
	|      ""name"": ""<новое имя запроса пакета 1>""
	|	}
	|]
	|```
	|
	...
	|**Example Input:**
	|query list: 1.
	|ВЫБРАТЬ 1 КАК Доход, 2 КАК Контрагент
	|ПОМЕСТИТЬ Т1;
	|ВЫБАТЬ МАКСИМУМ(Т1.Доход) 
	|ИЗ Т1 КАК Т1
	|СГРУППИРОВАТЬ ПО Т.Контрагент
	|
	|**Example Output:**
	|```json
	|" + ОбразецРезультатаИИЖСОН() + "
	|```
	|
	|# Context
	|<knowledge>
...
Функция ОбразецРезультатаИИЖСОН() Экспорт
	Результат = "[
		|    {
		|      ""index"": ""1"",
		|      ""name"": ""МаксДоходы"",
...
Процедура ДействияФормыОткрытьПараметрыИИ(Кнопка)
	ирКлиент.ОткрытьПараметрыИИЛкс();
КонецПроцедуры

Процедура ДействияФормыОбсудитьИИ(Кнопка)
	ОткрытьЧатИИ(СобратьПолныйТекст());
КонецПроцедуры
...
ЭлементыФормы.ЗапросыПакета.Колонки.ЧислоВложенных.Ширина = 3;
ЭлементыФормы.ЗапросыПакета.Колонки.ЧислоВложенных.ИзменениеРазмера = ИзменениеРазмераКолонки.НеИзменять;
...
ПоляОбъединения.Колонки.Добавить("Метаданные"); // - для передачи строки в ирКлиент.ОформитьСтрокуДоступногоПоляЛкс 
...
ИндексыТаблицы.Колонки.Добавить("ПоляИндекса"); // см. ПоляИндекса
//! СвязиТаблиц.Колонки.Добавить("ТипСвязи"); // Строка - LEFT, RIGHT, FULL, INNER
...
ЗапросыПакета.Колонки.Добавить("ИндексыТаблицы"); // см. ИндексыТаблицы
...
ЗапросыПакета.Колонки.Добавить("ЗащититьГруппировкиОтКомпоновщика", Новый ОписаниеТипов("Булево"));
...
ИерархическийСтильЛогическихВыражений = Истина;
