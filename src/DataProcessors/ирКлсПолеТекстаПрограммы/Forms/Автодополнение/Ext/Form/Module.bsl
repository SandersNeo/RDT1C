...
Функция ЗапуститьЗапросНапарникаВФорме()
	...
	Если Ложь
		...
		Или мФоновыйЗапросХттп.мБраузер = Неопределено
...
Процедура СлужебноеПолеХТМЛonclick(Элемент, pEvtObj) Экспорт
	...
	Если Ложь
		Или СтрокаСловаНапарника = Неопределено
		Или СтрокаСловаНапарника.Слово = СтатусИИНеЗнает() 
	Тогда
		Возврат;
	КонецЕсли;
...
Процедура КоманднаяПанельФормыПерейтиКОпределению(Кнопка)
	...
	Если СтруктураТипа <> Неопределено И СтруктураТипа.СтрокаОписания <> Неопределено Тогда
		Если Истина
			И Найти(ТипКонтекста, ".") > 0
			И ирОбщий.ИмяТипаМДМножественноеИзЕдинЛкс(ирОбщий.ПервыйФрагментЛкс(ТипКонтекста)) <> Неопределено
		Тогда
