...
Функция СохраняемаяНастройкаФормы(выхНаименование, выхИменаСвойств) Экспорт 
	// Нельзя брать из редактора формы! Почему то он не все реквизиты формы показывает
	выхИменаСвойств = ""; 
КонецФункции

Функция СвойстваГлобализуемые() Экспорт
	Результат = Новый Структура;
	Результат.Вставить("ПоказыватьВедомуюСтруктуру");
	Результат.Вставить("ПоказыватьРезультат");
	Результат.Вставить("ПоказыватьРезультатПакета");
	Результат.Вставить("ПоказыватьСвойства");
	Результат.Вставить("ВключатьДеревоЗапросаПриОтладке");
	Результат.Вставить("ВыполнятьВТранзакции");
	Результат.Вставить("ЗагружатьЭлементов");
	Результат.Вставить("МаксСтрокДляСложнойВедомойСтруктуры");
...
Процедура КлсПолеТекстаПрограммыНажатие(Кнопка)
	...
	Если Кнопка = ирКлиент.КнопкаКоманднойПанелиЭкземпляраКомпонентыЛкс(ПолеТекстаЗапроса, "КопироватьВБуферОбменаТекстВВидеКода") Тогда
		ЛиВыделенВесьТекст = Ложь;
		НовыйТекстЗапроса = ПолеТекстаЗапроса.ПолучитьВыделенныйИлиВесьТекст(ЛиВыделенВесьТекст);
		...
		Если ЛиВыделенВесьТекст Тогда
			...
			Для Каждого КлючИЗначение Из ИменованныеЗапросы Цикл
				ПрограммныйКод = ПрограммныйКод + "Позиция" + КлючИЗначение.Ключ + " = " + XMLСтрока(КлючИЗначение.Значение) + ";" + Символы.ПС;
			КонецЦикла;
		Иначе
...
Процедура ВнешнееСобытие(Источник, Событие, Данные) Экспорт
	...
	Если Истина
		//И ЭкземплярКомпоненты = ПолеТекстаЗапроса // Почему то в портативном режиме здесь возникала Ложь https://www.hostedredmine.com/issues/966081
	...
	Тогда 
		Если мСтруктурноеРедактирование Тогда 
			...
			ИначеЕсли Истина
				И (Ложь
					Или ирОбщий.ЛиНажатаПечатнаяКлавишаЛкс(Данные)
					Или ирОбщий.ПроверитьНажатиеКлавишЛкс(Данные, "CTRL+V"))
				И мТекущаяСтрокаДереваЗапроса <> мРедактируемыйУзел 
			Тогда
				ирОбщий.СообщитьЛкс("Ведущая структура запроса допускает редактирование только одного узла одновременно. Используй кнопку ""Перейти"" для перехода к редактируемому узлу.");
			КонецЕсли;
		Иначе
			ЛиКомандаНавигации = ирОбщий.ПроверитьНажатиеКлавишЛкс(Данные, "CTRL+F3, F3, F2, CTRL+End, CTRL+Home, Up, Down, PgUp, PgDown");
			#Если Сервер И Не Сервер Тогда
...
Функция СохранитьВФайл(ЗапрашиватьСохранение = Ложь, ЗапрашиватьИмяФайла = Ложь, Знач ИмяФайла = Неопределено, СброситьМодифицированность = Неопределено, Знач ЭтоФайлВосстановления = Ложь)
	...
	Тогда
		...
		//	Возврат;
		//КонецПопытки;
		ЭтаФорма.РезультатФормы = РезультатФормы();
...
// Возвращаемое значение:
//    - 
Функция РезультатФормы() Экспорт
	...
	СтруктураПараметров.Вставить("НомерСтроки", мТекущаяСтрокаДереваЗапросов.НачальнаяСтрока);
	...
	Если мРезультатЗапроса <> Неопределено Тогда
		...
		ИначеЕсли ЭлементыФормы.ПанельРезультата.ТекущаяСтраница = ЭлементыФормы.ПанельРезультата.Страницы.РезультатТабличныйДокумент Тогда
			Если Не ирКлиент.ПолучитьГотовностьДанныхСтраницыЛкс(ЭтаФорма, ЭлементыФормы.ПанельРезультата.ТекущаяСтраница) Тогда
				...
				ПараметрыВыводаЗапроса.Вставить("ВыводитьРезультатыЗапроса", ЗагружатьСтрокЗапросаВТД);
				ПараметрыВыводаЗапроса.Вставить("КоличествоСтрок");
				Для Каждого СтрокаПакета Из ТаблицаРезультатаПакета Цикл
					Если ДеревоЗапроса.Строки.Количество() > 0 Тогда
						Если СтрокаПакета.Индекс = ДеревоЗапроса.Строки[0].Строки.Количество() Тогда
							Прервать;
						КонецЕсли;        
						ПредставлениеЗапроса = ДеревоЗапроса.Строки[0].Строки[СтрокаПакета.Индекс].Представление;
					Иначе
						ПредставлениеЗапроса = "Запрос " + СтрокаПакета.Индекс;
					КонецЕсли;
					Если СтрокаПакета.Имя <> "" Тогда
						РезультатЗапроса = ВыполнитьЗапросИсточникаДанных("ВЫБРАТЬ ПЕРВЫЕ "+XMLСтрока(ЗагружатьСтрокЗапросаВТД)+" * ИЗ " + СтрокаПакета.Имя,, Ложь,)[0];
					Иначе
						РезультатЗапроса = мРезультатПакета[СтрокаПакета.Индекс]; // РезультатЗапроса
					КонецЕсли;
					Если РезультатЗапроса <> Неопределено Тогда
						РезультатЗапросаТаблица = Новый ТаблицаЗначений;
						Выборка = РезультатЗапроса.Выбрать();
						ПараметрыВыводаЗапроса.КоличествоСтрок = Выборка.Количество();
						ирКлиент.СчитатьПорциюВыборкиВТаблицуЛкс(Выборка, РезультатЗапросаТаблица, ЗагружатьСтрокЗапросаВТД, Ложь);
						ТД = ВывестиРезультатОдногоЗапроса(ПредставлениеЗапроса, РезультатЗапросаТаблица, ПараметрыВыводаЗапроса, МассивШириныКолонок);
						ЭлементыФормы.РезультатТабличныйДокумент.Вывести(ТД);
						// мДанныеРасшифровки
					КонецЕсли;
				КонецЦикла; 
				ирКлиент.УстановитьГотовностьДанныхСтраницыЛкс(ЭтаФорма, ЭлементыФормы.ПанельРезультата.ТекущаяСтраница, Истина);
...
//    * КоличествоСтрок - Число - всего строк в результате
...
Функция ВывестиРезультатОдногоЗапроса(ИмяЗапроса, РезультатЗапроса, ПараметрыВыводаЗапроса, МассивШириныКолонок)
	...
	ФрагментДетали = ВывестиДетали(РезультатЗапроса, ПараметрыВыводаЗапроса);
	Если Истина
		И ПараметрыВыводаЗапроса.ВыводитьРезультатыЗапроса >= 0
		И ПараметрыВыводаЗапроса.КоличествоСтрокВыведено < ПараметрыВыводаЗапроса.КоличествоСтрок
	Тогда
	...
	КонецЕсли;
	ФрагментЗаголовок = ВывестиЗаголовокЗапроса(ИмяЗапроса, КоличествоСтрокВЗаголовок);
	МакетОдногоЗапроса.НачатьАвтогруппировкуСтрок();
	МакетОдногоЗапроса.Вывести(ФрагментЗаголовок, УровеньВерхний);
	МакетОдногоЗапроса.Вывести(ЗаголовкиКолонок, УровеньЗаголовкаИДеталей,, Открыта);
	МакетОдногоЗапроса.Вывести(ФрагментДетали, УровеньЗаголовкаИДеталей,, Открыта);
...
Функция ВывестиЗаголовокЗапроса(ИмяЗапроса, КоличествоСтрок)
	ФрагментЗаголовок = Новый ТабличныйДокумент;
	...
	ОбластьЗаголовок.Параметры.Расшифровка = Новый Структура("ИмяЗапроса", ИмяЗапроса);
	ФрагментЗаголовок.Вывести(ОбластьЗаголовок);
	Возврат ФрагментЗаголовок;
КонецФункции

Процедура РезультатТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка, ДополнительныеПараметры)
	СтандартнаяОбработка = Ложь;
	Если ТипЗнч(Область.Расшифровка) = Тип("Структура") Тогда
		ЭлементыФормы.ДеревоЗапроса.ТекущаяСтрока = ДеревоЗапроса.Строки[0].Строки.Найти(Область.Расшифровка.ИмяЗапроса, "Представление");
	ИначеЕсли ирОбщий.ЛиСсылкаНаОбъектБДЛкс(Область.Расшифровка, Ложь) Тогда
		ОткрытьЗначение(Область.Расшифровка);
	КонецЕсли;
КонецПроцедуры

Процедура КоманднаяПанельТабличныйДокументРедакторОбъектаБД(Кнопка)
	ЗначениеРасшифровки = ЭлементыФормы.РезультатТабличныйДокумент.ТекущаяОбласть.Расшифровка;
	Если ирОбщий.ЛиСсылкаНаОбъектБДЛкс(ЗначениеРасшифровки, Ложь) Тогда
...
Функция ЗагрузитьРезультатВДеревоРезультатаПоВыборкамИтогов(ПолнаяЗагрузка = Ложь)
	...
	РазмерЗагружено = ВывестиВыборкуЗапросаРекурсивно(СчетчикРекурсии, мРезультатЗапроса, РезультатКоллекция.Строки, Индикатор, ПолнаяЗагрузка);
	Если РазмерЗагружено = 0 Тогда
		РазмерКоллекции = Индикатор.Счетчик;
		РазмерЗагружено = РазмерКоллекции;
	Иначе
		РазмерКоллекции = XMLСтрока(РазмерРезультата) + "+";
		РазмерЗагружено = Индикатор.Счетчик;
	КонецЕсли; 
	ирОбщий.ОсвободитьИндикаторПроцессаЛкс(Индикатор);
	//ирОбщий.ОбновитьЧислоЗагруженныхЭлементовВыборкиЛкс(ЭлементыФормы.РезультатКоллекция, ЭлементыФормы.КоманднаяПанельКоллекция,
	//	ЭлементыФормы.РазмерКоллекции, РазмерЗагружено, РазмерКоллекции, Истина);
	Возврат Неопределено;
	...
	Тогда
		Для Каждого ЭлементРезультата Из ТаблицаРезультатаПакета Цикл
			Если ДеревоЗапроса.Строки[0].Строки.Количество() = ЭлементРезультата.Индекс Тогда
				// Когда последний запрос на создание временной таблицы, мы добавлен служебный запрос на выборку из нее
				Прервать;
			КонецЕсли; 
...
Процедура ОбновитьЧистуюДлительность(СтрокаДереваЗапроса)
	...
	ЧистуюДлительностьМожноРассчитать = Истина;
	Для Каждого ДочерняяСтрока Из СтрокаДереваЗапроса.Строки Цикл
		Если ТипЗнч(ДочерняяСтрока.ДлительностьПолная) <> Тип("Число") Тогда
			Если Не ирОбщий.СтрокиРавныЛкс(ДочерняяСтрока.ТипУзла, "<TableWithName>") Тогда
				ЧистуюДлительностьМожноРассчитать = Ложь;
				Прервать;
			КонецЕсли; 
			Продолжить;
		КонецЕсли; 
	...
	КонецЦикла;
	Если ЧистуюДлительностьМожноРассчитать Тогда
...
Процедура ЗагрузитьТаблицуПараметровЗапроса()
	...
	Если ИсходнаяТаблицаПараметров <> Неопределено Тогда
		//! ИсходнаяТаблицаПараметров.Колонки.Добавить("ЗначениеПараметра"); // Старый формат
		Для каждого СтрокаИсходнойТаблицы из ИсходнаяТаблицаПараметров Цикл
			...
			Если Истина
				...
				И СтрокаИсходнойТаблицы.Владелец().Колонки.Найти("ЗначениеПараметра") <> Неопределено
			Тогда
...
Процедура ПриОткрытии()
	...
	Если мРежимОтладки Тогда
		...
		//ПодключитьОбработчикОжидания("ВключитьРежимДереваЗапросаОтложено", 0.1, Истина);
...
Процедура ВключитьРежимДереваЗапросаОтложено()
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	КонецЕсли; 
	Если Не Отказ Тогда
		ИзменитьСвернутостьПанельДеревоЗапроса(Истина, Ложь);
		ИзменитьСвернутостьПанельРезультата(Истина, Ложь);
		ИзменитьСвернутостьПанельСвойства(Истина, Ложь);
		ИзменитьСвернутостьРезультатПакета(Истина, Ложь);
...
Процедура КнопкаВыполнитьОбработкуТаблицыРезультата(Элемент)
	...
	Если мРезультатЗапроса = Неопределено Тогда
		УведомитьСначалаНужноВыполнитьЗапрос();
		Возврат;
	КонецЕсли; 
...
Процедура УведомитьСначалаНужноВыполнитьЗапрос() Экспорт
...
Процедура ПриЗакрытии()
	...
	Если мСтруктурноеРедактирование И мТекущаяСтрокаДереваЗапроса <> Неопределено Тогда
		УничтожитьВременныеТаблицыПодзапроса(мТекущаяСтрокаДереваЗапроса);
	КонецЕсли;
	мСтарыеКолонкиТабличногоПоляРезультата.Очистить(); // Разрываем странное кольцо ссылок, которое удерживает форму https://www.hostedredmine.com/issues/1002584
...
Функция ВыполнитьЛокальноКодПередВыполнениемЗапроса(ТекстДляВыполнения, ЛиСинтаксическийКонтроль = Ложь) Экспорт
	...
	Тогда
		...
		Если мТекущаяСтрокаДереваЗапросов.КопироватьМенеджерНаСервер Тогда
			СнимокМенеджера = ирОбщий.ПолВТ(мОбъектЗапроса.МенеджерВременныхТаблиц);
			СтруктураПараметров.Вставить("МенеджерВременныхТаблиц", СнимокМенеджера);
			ПредТекст = ПредТекст + "
			|мОбъектЗапроса.МенеджерВременныхТаблиц = ирОбщий.ВосстановитьМенеджерВременныхТаблицЛкс(СтруктураЗапроса.МенеджерВременныхТаблиц);";
		КонецЕсли;
		ТекстДляВыполнения = ПредТекст + "
		|" + ТекстДляВыполнения + "
...
Функция ПроверитьКорректностьТекстаЗапросаИЗаполнитьПараметры(ЛиДобавлятьНовыеПараметры = Истина, ОбновитьНастройки = Истина, СтартоваяСтрока = 0, Знач ТекстЗапроса = "",
	...
	Если ЛиДобавлятьНовыеПараметры Тогда
		...
		Для каждого ПараметрЗапроса Из выхПараметрыИЗТекстаЗапроса Цикл
			...
			ЗаполнитьТипЗначенияПараметраЗапроса(СтрокаПараметров, Ложь);
			...
			Если Не ЗначениеЗаполнено(СтрокаПараметров.Значение) Тогда
				Если ирОбщий.ЛиСсылкаНаПеречислениеЛкс(СтрокаПараметров.Значение) Тогда
					ОбъектМД = СтрокаПараметров.Значение.Метаданные(); // ОбъектМетаданныхПеречисление
				...
				ИначеЕсли ирОбщий.ЛиСсылкаНаОбъектБДЛкс(СтрокаПараметров.Значение) Тогда
					ОбъектМД = СтрокаПараметров.Значение.Метаданные(); // ОбъектМетаданныхСправочник
					Если Истина
...
Функция ПолучитьПараметрыИзТекстаЗапроса(Знач СтартоваяСтрока, Знач ТекстЗапроса = "", ПривестиИменаКНижнемуРегистру = Ложь, выхИнформацияОбОшибке = Неопределено, СообщитьОшибку = Истина)
...
Процедура КоманднаяПанельФормыПолучитьИзЗапроса(Кнопка)
	...
	ДобавитьПараметрыВКомпонентуПередВыполнениемЗапроса(ПоляТекстовПрограммы.КодПередВыполнениемЗапроса);
	ОбновитьТекстВОбъектеЗапроса(, Ложь);
	Если ВыполнитьОбработчикПередВыполнениемЗапроса() Тогда
		ПроверитьКорректностьТекстаЗапросаИЗаполнитьПараметры(, Ложь,, мОбъектЗапроса.Текст);
	КонецЕсли;
...
Процедура ПараметрыЗначениеПараметраНачалоВыбора(Элемент, СтандартнаяОбработка)
	...
	Если ирКлиент.ПолеВводаКолонкиРасширенногоЗначения_НачалоВыбораЛкс(ЭтаФорма, ЭлементыФормы.Параметры, СтандартнаяОбработка, , ИспользоватьОграничениеТипа,,,, ТекущиеДанные.ИмяПараметра) Тогда 
		ПривестиЗначениеПараметраКОграничениюТипов();
...
Процедура ДеревоЗапросаВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	...
	Тогда
		...
		КонецЕсли; 
	ИначеЕсли мТекущаяСтрокаДереваЗапросов.ЗаменитьПредставленияЗУП Тогда
		// https://www.hostedredmine.com/issues/1007396 
	Иначе
...
Функция КоманднаяПанельТекстЗапросаИзменитьИлиСохранитьУзелДерева(Кнопка = Неопределено, ПринудительноИзменить = Неопределено)
	ЛиПрямойВызов = Кнопка <> Неопределено;
	...
	ИначеЕсли Кнопка.Текст = "Перейти" Тогда
		...
		Если ПринудительноИзменить = Истина Тогда
			...
			КонецЕсли;
		ИначеЕсли Не ЛиПрямойВызов Тогда 
...
Функция РазобратьТекстЗапроса(Знач ТекстЗапроса = Неопределено, Знач СтроитьДеревоЗапроса = Истина, Знач ПолеТекста = Неопределено, Знач СтартоваяСтрокаДляОтображенияОшибок = 0,
	...
	Если ДеревоЗапроса.Строки.Количество() > 0 Тогда
		...
		Если ЛиВедущая Тогда
			...
			Если СтрокаДанных <> Неопределено Тогда
				ПолеТекста.УстановитьГраницыВыделения(
					Макс(1, НачальнаяСтрока - СтрокаДанных.НачальнаяСтрока + 1), НачальнаяКолонка,
					Макс(1, КонечнаяСтрока - СтрокаДанных.НачальнаяСтрока + 1), КонечнаяКолонка);
			КонецЕсли; 
		КонецЕсли;
...
Функция ОбработатьСтрокуДереваЗапроса(СтрокаДерева, НачальнаяСтрока = 1, НачальнаяКолонка = 1, НесуществующиеТаблицы = Неопределено)
	...
	Для Каждого ДинамическоеИмя Из МассивДинамическихИмен Цикл
		...
		КонецЦикла;
		Если ВГраница > 0 И Не мСтруктурноеРедактирование Тогда
			ДочерняяСтрока.Текст = ирОбщий.ПервыйФрагментЛкс(ДочерняяСтрока.Текст, "ИНДЕКСИРОВАТЬ ПО"); // Грязно
		КонецЕсли;
	КонецЦикла;
...
// Функция - Обновить статус временной таблицы
//
// Параметры:
...
Процедура МенюДереваЗапросаПеренестиВНовыйЗапрос(Кнопка)
	...
	Если мВременныеТаблицыПакета[Нрег(ИмяВременнойТаблицы)] <> Неопределено Тогда
		Ответ = Вопрос("В пакете уже есть запрос создания временной таблицы с таким именем. Хотите продолжить (Да) или сравнить запросы (Нет)?", РежимДиалогаВопрос.ДаНетОтмена,, КодВозвратаДиалога.Да);
...
Процедура КоманднаяПанельТекстЗапросаОбъединениеСсылочныхТаблицИзменений(Кнопка)
	...
	Текст = ТекстЗапросаСВыборомМетаданных(Истина);
	Если Не ПустаяСтрока(Текст) Тогда
		ирКлиент.УстановитьТекстСОткатомЛкс(ПолеТекстаЗапроса.ПолеТекста, Текст);
	КонецЕсли; 
...
Функция ПолучитьКодПолученияРезультатаЗапроса(СтруктураНатуральныхПараметров = Неопределено)
	...
	ИначеЕсли ирОбщий.СтрокиРавныЛкс(мОписаниеРезультата.ТипЗапроса, "ADO") Тогда 
		СоединениеФакт = "";
		ПодключитьИсточникADO(ПолучитьИсточникДанныхADO(),,,,,, СоединениеФакт);
...
Функция КомментарийЗначенияПараметраДляПрограммногоКода(Знач ЗначениеЭлемента)
	...
	Если ТипЗначения <> Тип("Неопределено") Тогда
		...
		Если ОбъектМД <> Неопределено Тогда
			...
			МножественноеИмяМД = ирОбщий.ИмяТипаМДМножественноеИзЕдинЛкс(ирОбщий.ПервыйФрагментЛкс(ПолноеИмяМД));
			Если Ложь
...
// Перед вызовом нужно вызывать ОбновитьТекстВОбъектеЗапроса()
...
Процедура ОбновитьКнопкиПоляЗначенияПараметра()
	...
	ПолеВвода = ТабличноеПоле.Колонки.Значение.ЭлементУправления;
	ПолеВвода.КнопкаОткрытия = ирОбщий.ЛиТипСсылкиБДЛкс(ТипЗначения);
	ПолеВвода.КнопкаРегулирования = ирОбщий.ЛиТипЗначенияДопускаетРегулированиеВПолеВводаЛкс(ТипЗначения);
			...
			Для Каждого МетодWMI из ЗначениеWMI.Methods_ Цикл
				...
				Если МетодWMI.InParameters <> Неопределено Тогда 
					Для Каждого ПараметрWMI из МетодWMI.InParameters.Properties_ Цикл
						КвалификаторыПараметра = ирОбщий.СтруктураИзЗначенияWMIЛкс(ПараметрWMI.Qualifiers_);
...
Процедура НастроитьКомпоновщикWMI(ЗагружатьСписокКлассов = Ложь)
	...
	КонецЦикла;
	мСхемаКомпоновки = ирОбщий.СоздатьСхемуКомпоновкиПоТаблицамЗначенийЛкс(Новый Структура("Основной", ТаблицаКласса), , , , Ложь);
...
Процедура ИзменитьСвернутостьПанельДеревоЗапроса(Видимость, Запоминать = Истина)
...
Процедура ИзменитьСвернутостьПанельСвойства(Видимость, Запоминать = Истина)
	...
	ЭлементыФормы.КоманднаяПанельКоллекция.Кнопки.ПоказыватьСвойства.Пометка = Видимость;
...
Процедура ИзменитьСвернутостьРезультатПакета(Видимость, Запоминать = Истина) Экспорт 
	...
	ЭлементыФормы.КоманднаяПанельКоллекция.Кнопки.ПоказыватьРезультатПакета.Пометка = Видимость;
	Если Запоминать Тогда
...
Процедура ИзменитьСвернутостьПанельРезультата(Видимость, Запоминать = Истина)
	...
	ЭлементыФормы.КоманднаяПанельФормы.Кнопки.ПоказыватьРезультат.Пометка = Видимость;
	Если Запоминать Тогда
...
Функция ВыполнитьЗапросADO(ИсточникДанных, СоединениеADO, ОбъектЗапрос, Опции, ЗапросТекст, пЗначенияПараметров, ТипыПараметров, ИнфоОшибки, выхРезультатПакета = Неопределено, ПредыдущийТекстПолногоЗапроса = "") Экспорт
	...
	Если СоединениеADO = Неопределено ИЛИ Переподключиться = Истина Тогда
		...
		Если НЕ ПодключитьИсточникADO(ИсточникДанных, СоединениеADO, Опции, Еррорс) Тогда
		...
		Если ДС0 > 0 Тогда
			...
			Пока СтрДлина(ТекстЗапросаК) > 0 Цикл
				...
				Если ТекПоз1 > 0 И ТекПоз2 > 0 Тогда
					...
					КонецЕсли; 
				Иначе
					// нет одной из скобок - прерываем обработку
					Прервать;
				КонецЕсли; 
			КонецЦикла;
...
Функция ПолучитьСоединениеADO(Переустановить = Ложь, Отключиться = Ложь, ВывестиСтрокуСоединения = Ложь)
	...
	Тогда
		...
		Еррорс = Неопределено; // Массив
		СтрокаСоединенияДляСообщения = "";
		Если НЕ ПодключитьИсточникADO(ИсточникДанных, мСоединениеADO, , Еррорс, СтрокаСоединенияДляСообщения) Тогда
			//ОчиститьСообщения();
...
Процедура ПлатформаADOПриИзменении(Элемент =  Неопределено) 
	...
	//// переинициируем контекстное меню для вставки ключевых слов строку соединений:
...
Процедура ФормаКонтроль_ТипИДADO()
	...
	//		ЦветТекста=Новый Цвет(128, 0, 128);		// лиловым жирным
	//	Иначе
	//		ЦветТекста=Новый Цвет(255, 0, 0);		// красный жирным
	//	КонецЕсли; 
...
Процедура ЗаполнитьТипЗначенияПараметраЗапроса(Знач СтрокаПараметра, Знач РазрешитьДолгийАнализ = Истина) Экспорт
	ирОбщий.ЗаполнитьТипЗначенияПоИмениЛкс(СтрокаПараметра, РазрешитьДолгийАнализ);
	ПривестиЗначениеПараметраКОписаниюТипов(СтрокаПараметра); 
КонецПроцедуры
...
Процедура МенюДереваЗапросаНайтиРезультат(Кнопка = Неопределено)
	Если Ложь
		Или ТаблицаРезультатаПакета.Количество() = 0
		Или ЭлементыФормы.ДеревоЗапроса.ТекущаяСтрока = Неопределено 
	Тогда
		Возврат;
	КонецЕсли; 
	СтрокаПакета = Неопределено;
	Если ЗначениеЗаполнено(мТекущаяСтрокаДереваЗапроса.НИмяОсновнойТаблицы) Тогда
		// Нужно в первую очередь для ЗаменитьПредставленияЗУП
		СтрокаТаблицыЗначений = ВременныеТаблицы.Найти(НРег(мТекущаяСтрокаДереваЗапроса.НИмяОсновнойТаблицы), "НИмя");
		Если СтрокаТаблицыЗначений <> Неопределено Тогда
			ИмяТаблицы = СтрокаТаблицыЗначений.Имя;
			СтрокаПакета = ТаблицаРезультатаПакета.Найти(ИмяТаблицы, "Имя");
		КонецЕсли;
	КонецЕсли;
	Если СтрокаПакета = Неопределено Тогда
		...
		Если ИндексЗапроса >= 0 И ТаблицаРезультатаПакета.Количество() > ИндексЗапроса Тогда
			СтрокаПакета = ТаблицаРезультатаПакета[ИндексЗапроса];
		КонецЕсли;
	КонецЕсли;
	Если СтрокаПакета <> Неопределено Тогда
		ЭлементыФормы.ТаблицаРезультатаПакета.ТекущаяСтрока = СтрокаПакета;
	КонецЕсли;
КонецПроцедуры
...
Функция НайтиПозициюТекстаВДеревеЗапроса(Знач СтрокиДерева, Знач НачальнаяСтрока, Знач НачальнаяКолонка, Знач КонечнаяСтрока, Знач КонечнаяКолонка, Знач ИгнорироватьМелкиеУзлы = Ложь,
	ЭтоПервыйУзелРекурсия = Неопределено)
...
Процедура КоманднаяПанельПараметрыЗаменитьЛитералами(Кнопка)
	...
	Тогда 
		Возврат;
	КонецЕсли; 
	Для Каждого ВыделеннаяСтрока Из ирКлиент.ВыделенныеСтрокиТабличногоПоляЛкс(ЭлементыФормы.Параметры) Цикл
...
Процедура УстановитьРежимВыполненияКодаНаСервере(НовыйРежим = Неопределено)
	...
	ЭлементыФормы.КопироватьМенеджерНаСервер.Доступность = НаСервере;
			...
			Для Каждого ОписаниеТаблицы Из ТаблицыЗапроса Цикл
				...
				Иначе
					...
					Если ирОбщий.ТипТаблицыБДЛкс(ИмяОсновнойТаблицы) = "ВиртуальнаяТаблица" Тогда
						ИмяОсновнойТаблицы = ирОбщий.ТекстБезПоследнегоФрагментаЛкс(ИмяОсновнойТаблицы);
					КонецЕсли;
				КонецЕсли;
...
Процедура РедакторHTMLonclickДинамический(ДанныеСобытия) Экспорт
	...
	Если Событие <> Неопределено Тогда
		...
		ИначеЕсли Событие.event = "EVENT_KEY_BINDING_2082" Тогда // CTRL+D
			...
			Кнопка = ЭлементыФормы.КоманднаяПанельФормы.Кнопки.ПоказыватьРезультат;
...
Процедура КоманднаяПанельОбработкаСтрокиРезультатаПараметрыЗаписи(Кнопка)
	ирКлиент.ОткрытьОбщиеПараметрыЗаписиЛкс(Кнопка);
КонецПроцедуры
...
Процедура КоманднаяПанельКоллекцияВТекстCSV(Кнопка)
	Если мРезультатЗапроса = Неопределено Тогда
		УведомитьСначалаНужноВыполнитьЗапрос();
		Возврат;
	КонецЕсли;
	...
	Если Не ВвестиСтроку(Разделитель, "Введите разделитель значений") Тогда 
		Возврат;
	КонецЕсли;
	Текст = ирОбщий.ТаблицаИлиРезультатЗапросаВТекстCSVЛкс(мРезультатЗапроса, Разделитель);
	ирКлиент.ОткрытьТекстЛкс(Текст);
КонецПроцедуры

Процедура ПараметрыИмяПараметраПриИзменении(Элемент)
	ЗаполнитьТипЗначенияПараметраЗапроса(ЭлементыФормы.Параметры.ТекущаяСтрока);
КонецПроцедуры

Процедура КоманднаяПанельТекстЗапросаОбсудитьИИ(Кнопка)
	ПолеТекстаЗапроса.ОткрытьЧатИИ();
КонецПроцедуры
...
#Если Сервер И Не Сервер Тогда
	...
	ПолеТекстаЗапроса = Обработки.ирКлсПолеТекстаПрограммы.Создать();
	ЭтаФорма.РезультатФормы = 0; // см. РезультатФормы
#КонецЕсли
ЭтаФорма.ЗагружатьЭлементов = 100000;
ЭтаФорма.ЗагружатьСтрокЗапросаВТД = 2;
...
ВключатьДеревоЗапросаПриОтладке = Истина;
мТекущаяСтрокаДереваЗапросов = Неопределено;
мРазрешитьАктивациюРедактора = Истина;
