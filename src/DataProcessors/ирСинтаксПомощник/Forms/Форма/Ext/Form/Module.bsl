...
Функция ЗагрузитьНовуюСтраницу(ПутьКЭлементуБезПрефиксаАрхива, ЛиЗагружатьПовторно = Ложь, ИмяАрхива = "")
	...
	НовыйАдрес = РаспаковатьЭлементАрхиваСинтаксПомощника(ПутьКЭлементу);
	...
	Иначе
		...
		КонецЕсли; 
	КонецЕсли; 
	ирОбщий.ЗаписатьТекстВФайлЛкс(ТекстДокумента, НовыйАдрес);
...
Процедура ПриОткрытии()
	...
	КнопкиКоманднойПанели = ЭлементыФормы.КоманднаяПанельХтмл.Кнопки.ПоследниеВыбранные.Кнопки;
	Если Истина
		И ПустаяСтрока(ИскомоеСлово)
...
Функция ПоследниеВыбранныеНажатие(Кнопка) Экспорт
	КлючСтроки = ирКлиент.ВыбранныйЭлементПоследнихЗначенийЛкс(ЭтаФорма, "История", Кнопка);
	ОткрытьАдрес(КлючСтроки);
КонецФункции 

Процедура ОбновитьПодменюПоследниеВыбранные()
	ирКлиент.ПоследниеВыбранныеЗаполнитьПодменюЛкс(ЭтаФорма, ЭлементыФормы.КоманднаяПанельХтмл.Кнопки.ПоследниеВыбранные, "История");
КонецПроцедуры

Процедура ПоследниеВыбранныеДобавить() Экспорт 
	ирКлиент.ПоследниеВыбранныеДобавитьЛкс(ЭтаФорма, ПутьКЭлементу, ПолноеИмяСвойства(), "История");
	ОбновитьПодменюПоследниеВыбранные();
КонецПроцедуры   
...
Процедура ОбновитьРезультатыПоиска(БезусловнаяАктивизацияРезультатов = Истина) Экспорт
...
Функция ПолноеИмяСвойства()
	...
	УзлыСтраницы = ДОМ.getElementsByClassName("V8SH_title");
	Если УзлыСтраницы.length > 0 Тогда
		ПолноеИмяСвойства = ирОбщий.ПервыйФрагментЛкс(УзлыСтраницы.Item(0).innerText, " (");
		УзлыСтраницы = ДОМ.getElementsByClassName("V8SH_heading");
		Если УзлыСтраницы.length > 0 Тогда
			ПолноеИмяСвойства = ПолноеИмяСвойства + "." + ирОбщий.ПервыйФрагментЛкс(УзлыСтраницы.Item(0).innerText, " (");
		КонецЕсли;
	Иначе
		Тело = ДОМ.body; // ОбработкаОбъект.ирТипHTMLElement
		Если Тело.childNodes.length > 0 Тогда
			ПолноеИмяСвойства = ирОбщий.ПервыйФрагментЛкс(Тело.childNodes.Item(0).innerText, " ");
		КонецЕсли;
	КонецЕсли;
...
Процедура КоманднаяПанельХтмлСсылкиНаСлово(Кнопка)
	...
	КонецЕсли;
	ПолеТекстаПрограммы = ирОбщий.НовыйАнализаторКодаЛкс(Истина);
	#Если Сервер И Не Сервер Тогда
...
Процедура КоманднаяПанельХтмлСсылкиВПсевдокоде(Кнопка)
	ПолноеИмяСвойства = ПолноеИмяСвойства();
	ПолеТекстаПрограммы = ирОбщий.НовыйАнализаторКодаЛкс(Истина);
	#Если Сервер И Не Сервер Тогда
		ПолеТекстаПрограммы = Обработки.ирКлсПолеТекстаПрограммы.Создать();
	#КонецЕсли
	Ссылка = ПолеТекстаПрограммы.СсылкаВызоваСобытияИзПсевдокодаПлатформы(ирОбщий.ПоследнийФрагментЛкс(ПолноеИмяСвойства), ирОбщий.ПервыйФрагментЛкс(ПолноеИмяСвойства));
	ПолеТекстаПрограммы.ПерейтиПоСсылкеСтрокиМодуля(Ссылка, Ложь);
КонецПроцедуры
