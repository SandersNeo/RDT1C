...
Функция ПолучитьЗапросПоискаДублей(ЗапросСОтборомОбщим, выхТекстИдексов = "", выхПоискПоПохожимСловам = Неопределено)
	...
	КопияПорядка = ирОбщий.СкопироватьНастройкиКомпоновкиЛкс(КомпоновщикСравнения.Настройки,,,, Истина);
	//ирОбщий.ЗапросИзКомпоновкиЛкс(мСхемаКомпоновкиОбщая,КомпоновщикСравнения.Настройки)
	СоединенияХарактеристик = Новый Массив;
	...
	Для Каждого ПолеСравнения Из мПоляСравнения Цикл 
		...
		ИмяДопРеквизита = ирОбщий.ТекстМеждуМаркерамиЛкс(ПолноеИмяПоля, ".[", "]", Ложь);
		ПервыйФрагмент = ирОбщий.ПервыйФрагментЛкс(ПолноеИмяПоля);
		...
		Если ЗначениеЗаполнено(ИмяДопРеквизита) Тогда
			ПутьКДаннымРодителя = ирОбщий.ПервыйФрагментЛкс(ПолноеИмяПоля, ".[");
			ДоступноеПолеРодителя = КомпоновщикСравнения.Настройки.ДоступныеПоляПорядка.НайтиПоле(Новый ПолеКомпоновкиДанных(ПутьКДаннымРодителя));
			ОбъектМД = Метаданные.НайтиПоТипу(ДоступноеПолеРодителя.ТипЗначения.Типы()[0]);
			Для Каждого ХарактеристикиЭлемент Из ОбъектМД.Характеристики Цикл
				ПоляТаблицыБД = ирОбщий.ПоляТаблицыМДЛкс(ХарактеристикиЭлемент.ЗначенияХарактеристик);
				ТипХарактеристики = ПоляТаблицыБД.Найти(ХарактеристикиЭлемент.ПолеКлюча.Имя, "Имя").ТипЗначения.Типы()[0];
				ИмяПлана = Метаданные.НайтиПоТипу(ТипХарактеристики).Имя;
				СвойствоСсылка = ПланыВидовХарактеристик[ИмяПлана].НайтиПоНаименованию(ИмяДопРеквизита, Истина);
				Если Не СвойствоСсылка.Пустая() Тогда
					ИмяТаблицыВидовХарактеристик = ирКэш.ИмяТаблицыИзМетаданныхЛкс(ХарактеристикиЭлемент.ВидыХарактеристик.ПолноеИмя());
					ПостроительЗапроса = Новый ПостроительЗапроса("ВЫБРАТЬ * ИЗ " + ИмяТаблицыВидовХарактеристик + " КАК Т");
					ПостроительЗапроса.ЗаполнитьНастройки();
					ирОбщий.НайтиДобавитьЭлементОтбораЛкс(ПостроительЗапроса, ХарактеристикиЭлемент.ПолеВида.Имя,, СвойствоСсылка);
					ирОбщий.НайтиДобавитьЭлементОтбораЛкс(ПостроительЗапроса, ХарактеристикиЭлемент.ПолеОтбораВидов.Имя,, ХарактеристикиЭлемент.ЗначениеОтбораВидов);
					Если ПостроительЗапроса.Результат.Пустой() Тогда
						Продолжить;
					КонецЕсли;
					ИмяТаблицыЗначений = ИмяПоля + "Т";
					ЗапросПоиска.Параметры.Вставить(ИмяПоля, СвойствоСсылка);
					ИмяТаблицыЗначенийХарактеристик = ирКэш.ИмяТаблицыИзМетаданныхЛкс(ХарактеристикиЭлемент.ЗначенияХарактеристик.ПолноеИмя());
					СоединенияХарактеристик.Добавить(
					"ЛЕВОЕ СОЕДИНЕНИЕ " + ИмяТаблицыЗначенийХарактеристик + " КАК " + ИмяТаблицыЗначений + "
					|ПО ИСТИНА
					|И " + мПсевдонимТаблицы + "." + ПутьКДаннымРодителя + " = " + ИмяТаблицыЗначений + "." + ХарактеристикиЭлемент.ПолеОбъекта.Имя + "
					|И " + ИмяТаблицыЗначений + "." + ХарактеристикиЭлемент.ПолеВида.Имя + " = &" + ИмяПоля);
					ВыражениеПоля = мПсевдонимТаблицы + "." + ИмяПоля;
					ТекстВЫБРАТЬВложенный = ТекстВЫБРАТЬВложенный + ", " + ИмяТаблицыЗначений + "." + ХарактеристикиЭлемент.ПолеЗначения.Имя + " КАК " + ИмяПоля;
				КонецЕсли;
			КонецЦикла;
			Если Не ЗначениеЗаполнено(ВыражениеПоля) Тогда
				ПолеСравнения.Использование = Ложь;
				ирОбщий.СообщитьЛкс(ирОбщий.СтрШаблонЛкс(НСтр("ru='Поле ""%1"" пропущено, т.к. в планах видов характеристик не найден элемент с наименованием ""%2""'"), ПолноеИмяПоля, ИмяДопРеквизита));
				Продолжить;
			КонецЕсли;
			ирОбщий.НайтиЭлементНастроекКомпоновкиПоПолюЛкс(КопияПорядка.Порядок.Элементы, ПолеСравнения.Поле).Использование = Ложь; // Временно
		ИначеЕсли КомпоновщикСравнения.Настройки.ДоступныеПоляПорядка.НайтиПоле(Новый ПолеКомпоновкиДанных(ПервыйФрагмент)).Папка Тогда
			Если ирКэш.НомерВерсииПлатформыЛкс() < 803020 Тогда
				ПолеСравнения.Использование = Ложь;
				ирОбщий.СообщитьЛкс(НСтр("ru='Поле %1 отключено, т.к. для него требуется платформа 8.3.20+'"));
				Продолжить;
			КонецЕсли;
			ирОбщий.НайтиЭлементКоллекцииЛкс(КопияПорядка.Порядок.Элементы, "Поле", ПолеСравнения.Поле).Использование = Ложь;
			ПоляТЧ = Неопределено;
			Если Не ТабличныеЧасти.Свойство(ПервыйФрагмент, ПоляТЧ) Тогда
				ПоляТЧ = Новый Массив;
				ТабличныеЧасти.Вставить(ПервыйФрагмент, ПоляТЧ);
			КонецЕсли;
			ПоляТЧ.Добавить(ирОбщий.ТекстБезПервогоФрагментаЛкс(ПолноеИмяПоля));
			Если ПоляТЧ.Количество() > 1 Тогда
				Продолжить;
			КонецЕсли;
			ВыражениеПоля = ПервыйФрагмент + "Хеши.Хеш";
			ОписаниеТипов = Новый ОписаниеТипов("Число");
			ПредставлениеПоля = ПервыйФрагмент;
		Иначе
			ВыражениеПоля = ирОбщий.ОбренутьВыражениеЗапросаДляГруппировкиЛкс(мПсевдонимТаблицы + "." + ПолноеИмяПоля, ОписаниеТипов, УчитыватьПравыеПробелыСтрок);
			ТекстВЫБРАТЬВложенный = ТекстВЫБРАТЬВложенный + ", " + ВыражениеПоля + " КАК " + ИмяПоля; // запрещенные имена например "Соединение" так вызывают ошибку?
		КонецЕсли;
		мПредставления.Вставить(ИмяПоля, ПредставлениеПоля);
		...
		ВыражениеПоляПравильный = ирОбщий.ОбренутьВыражениеЗапросаДляГруппировкиЛкс("Правильный." + ПолноеИмяПоля, ОписаниеТипов, УчитыватьПравыеПробелыСтрок);
		ТекстСоединенияПравильный = ТекстСоединенияПравильный + Символы.ПС + "И " + ВыражениеПоляПравильный + " = ПервичныеГруппыДублей." + ИмяПоля;
		ВыражениеПоляНеправильный = ирОбщий.ОбренутьВыражениеЗапросаДляГруппировкиЛкс("Неправильный." + ПолноеИмяПоля, ОписаниеТипов, УчитыватьПравыеПробелыСтрок);
	...
	КонецЦикла;
	Если СоединенияХарактеристик.Количество() Тогда
		Если Лев(ТекстИсточника, 1) = "(" Тогда
			ВызватьИсключение "Применение характеристик вместе с произвольным запросом не поддерживается";
		КонецЕсли;
		ТекстИсточника = "(ВЫБРАТЬ " + мПсевдонимТаблицы + ".Ссылка" + ТекстВЫБРАТЬВложенный + " ИЗ " + ПолноеИмяОбъекта + " КАК " + мПсевдонимТаблицы + Символы.ПС
		+ ирОбщий.СтрСоединитьЛкс(СоединенияХарактеристик, Символы.ПС) + ")"; 
	КонецЕсли;
	...
	ТекстЗапросаПоиска = "";
	// Для табличных частей применяем свертку в хеш функцию по способу №3 https://infostart.ru/1c/articles/1903939/
	Для Каждого КлючИЗначение Из ТабличныеЧасти Цикл
		ПоляТЧ = КлючИЗначение.Значение;
		ИмяТаблицыСочетанийТЧ = КлючИЗначение.Ключ + "РазличныеСочетания";
		ТекстЗапросаПоиска = ТекстЗапросаПоиска + "ВЫБРАТЬ РАЗЛИЧНЫЕ
		|" + ирОбщий.СтрСоединитьЛкс(ПоляТЧ,,,, "	ТаблицаТЧ.%1 КАК %1," + Символы.ПС) + "
		|	АВТОНОМЕРЗАПИСИ() КАК НомерСочетания
		|ПОМЕСТИТЬ " + ИмяТаблицыСочетанийТЧ + "
		|ИЗ
		|	" + ТекстИсточника + "." + КлючИЗначение.Ключ + " КАК ТаблицаТЧ
		|ГДЕ " + ТекстОтбораДублей + ";
		|ВЫБРАТЬ
		|	ТаблицаТЧ.Ссылка КАК Ссылка,
		|	СУММА(COS(РазличныеСочетания.НомерСочетания)) КАК Хеш
		|ПОМЕСТИТЬ " + КлючИЗначение.Ключ + "Хеши
		|ИЗ
		|	" + ТекстИсточника + "." + КлючИЗначение.Ключ + " КАК ТаблицаТЧ
		|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ " + ИмяТаблицыСочетанийТЧ + " КАК РазличныеСочетания
		|	ПО " + ирОбщий.СтрСоединитьЛкс(ПоляТЧ,,,, "ТаблицаТЧ.%1 = РазличныеСочетания.%1") + "
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаТЧ.Ссылка
		|;
		|";
	...
	|" + ирОбщий.СтрСоединитьЛкс(ирОбщий.ВыгрузитьСвойствоЛкс(ТабличныеЧасти),,,, "	ЛЕВОЕ СОЕДИНЕНИЕ %1Хеши КАК %1Хеши ПО " + мПсевдонимТаблицы + ".Ссылка = %1Хеши.Ссылка" + Символы.ПС) + "
	...
	ТекстУпорядочитьГруппы = ирОбщий.ВыражениеПорядкаКомпоновкиНаЯзыкеЗапросовЛкс(КопияПорядка.Порядок, ?(выхПоискПоПохожимСловам, мПутьКДаннымПоляНечеткогоСравнения, "") , "_");
	...
	Иначе
		...
		|ИЗ " + ТекстИсточника  + " КАК " + мПсевдонимТаблицы + "
		|" + ирОбщий.СтрСоединитьЛкс(ирОбщий.ВыгрузитьСвойствоЛкс(ТабличныеЧасти),,,, "	ЛЕВОЕ СОЕДИНЕНИЕ %1Хеши КАК %1Хеши ПО " + мПсевдонимТаблицы + ".Ссылка = %1Хеши.Ссылка" + Символы.ПС) + "
		|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ " + ПрефиксВременныхТаблиц + "ПервичныеГруппыДублей КАК ПервичныеГруппыДублей
		|	ПО " + ТекстСоединения + "
		|ГДЕ" + ТекстОтбораДублей + " 
...
Функция ПрефиксВременныхТаблиц()
	...
	Если ирОбщий.ЛиКорневойТипВнешнегоИсточникаДанныхЛкс(ирОбщий.ПервыйФрагментЛкс(ПолноеИмяОбъекта)) Тогда
		Результат = ирОбщий.ТекстБезПоследнегоФрагментаЛкс(ПолноеИмяОбъекта, ".Таблица.") + ".ВременнаяТаблица.";
	КонецЕсли; 
	Возврат Результат;
КонецФункции
...
Процедура АвтозаменаЭлементов(ОбработатьНеправильные, НепосредственноеУдаление = Истина) Экспорт
	...
	Для Каждого СтрокаПравильного Из ПравильныеСтроки Цикл
		...
		#КонецЕсли
		РезультатОбработки = ирОбщий.ВыполнитьЗаданиеФормыЛкс("ВыполнитьЗаменуЭлементов", ПараметрыЗадания, ЭтаФорма, "ЗаменаСсылок",,, "ВыполнитьЗаменуЗавершение", Ложь,, Истина);
...
Функция ПоследниеОтборыПоискаНажатие(Кнопка) Экспорт
	...
	НастройкаКомпоновки = ирКлиент.ВыбранныйЭлементПоследнихЗначенийЛкс(ЭтаФорма, ПолноеИмяОбъекта, Кнопка, "Отборы");
	#Если Сервер И Не Сервер Тогда
...
Процедура ОбновитьСхемуКомпоновки()
	...
	ВременнаяСхема = ирОбщий.СоздатьСхемуКомпоновкиПоЗапросуЛкс(ЗапросОбщий);
	...
	Для Каждого ДоступноеПоле Из ВременныйКомпоновщикЗапроса.Настройки.ДоступныеПоляВыбора.Элементы Цикл
		Если Истина
			...
			И "" + ДоступноеПоле.Поле = "СистемныеПоля"
		Тогда
			Продолжить;
		КонецЕсли; 
	...
	//мСхемаКомпоновкиОбщая = ирОбщий.СоздатьСхемуКомпоновкиПоЗапросуЛкс(ЗапросОбщий,, ПредставленияПолей, Ложь);
	мСхемаКомпоновкиОбщая = ирОбщий.СоздатьСхемуКомпоновкиПоЗапросуЛкс(ЗапросОбщий,, ПредставленияПолей);
	...
	мСхемаКомпоновкиПар = ирОбщий.СоздатьСхемуКомпоновкиПоЗапросуЛкс(ЗапросПар,,, Ложь);
	#Если Сервер И Не Сервер Тогда
	...
	ирОбщий.УстановитьОграниченияПоляНабораДанныхСхемыКомпоновкиЛкс(ПолеНабора, Истина);
	ПолеНабора = мСхемаКомпоновкиПар.НаборыДанных[0].Поля.Добавить(Тип("ПолеНабораДанныхСхемыКомпоновкиДанных"));
	...
	//ирОбщий.УстановитьОграниченияПоляНабораДанныхСхемыКомпоновкиЛкс(ПолеНабора, Истина, Ложь, , , Ложь);
...
Процедура НастроитьЭлементыФормы()
	...
	ИначеЕсли ЭлементыФормы.Панель.ТекущаяСтраница = ЭлементыФормы.Панель.Страницы.НайденныеГруппы Тогда
		ПустаяСсылка = Новый (ирОбщий.ИмяТипаИзПолногоИмениМДЛкс(ПолноеИмяОбъекта));
		Если НайденныеГруппы.НайтиСтроки(Новый Структура(мИмяПоляПравильныйЭлемент, ПустаяСсылка)).Количество() > 0 Тогда
...
// Параметры:
//   СостояниеЗадания -  -
//   РезультатЗадания - Структура, см. НайтиДубли -:
//     *НачалоЗадания
	...
	Тогда
		...
		Если ИзмененныеПроведенныеДокументы.Количество() > 0 Тогда
			...
			Если Ответ = КодВозвратаДиалога.Да Тогда
				...
				ТаблицаДокументов.Колонки.Документ.Имя = "Ссылка";
				ирКлиент.ОткрытьМассивОбъектовВПодбореИОбработкеОбъектовЛкс(ТаблицаДокументов,,,,,, "ПровестиДокументы");
			КонецЕсли;
		КонецЕсли; 
		Если ИзменятьПоляПоСвязямПараметровВыбора Тогда
...
Функция ПрочитатьДополнительныеПоляСсылающихсяОбъектов() Экспорт
	...
	ЛиАсинхронно = Ложь;
	мСтрокаПорядкаНайденныхСсылок = ирКлиент.ПрочитатьДопПоляСсылающихсяОбъектовЛкс(ЭлементыФормы.НайденныеСсылки, КомпоновщикДопПолей,,, 
		ЭтаФорма, ЭлементыФормы.КоманднаяПанельДопПоля.Кнопки.ПрочитатьПоля, "ПослеЧтенияДопПолей", ЛиАсинхронно);
	Если Не ЛиАсинхронно Тогда // Загрузку доп. реквизитов делаем по позициям строк. Поэтому нельзя тут сортировать их
...
Процедура ПослеЧтенияДопПолей(СостояниеЗадания = Неопределено, РезультатЗадания = Неопределено) Экспорт
	Если РезультатЗадания = Неопределено Тогда
		Возврат;
	КонецЕсли;
	ТаблицаДанных = РезультатЗадания.ТаблицаДанных; // см. НайденныеСсылки
	Для Каждого СтрокаРезультата Из ТаблицаДанных Цикл
		//Если СтрокаРезультата.Данные = Неопределено Тогда
		//	Продолжить;
		//КонецЕсли;
		СтрокаСсылки = НайденныеСсылки[ТаблицаДанных.Индекс(СтрокаРезультата)];
...
Процедура СортироватьНайденные() Экспорт
	Если Не ЗначениеЗаполнено(мСтрокаПорядкаНайденныхСсылок) Тогда
		Возврат;
	КонецЕсли;
	НайденныеСсылки.Сортировать(мСтрокаПорядкаНайденныхСсылок);
...
Процедура КоманднаяПанельНайденныеГруппыКонсольКомпоновки(Кнопка)
	...
	СхемаКомпоновкиДанных = ирОбщий.СоздатьСхемуКомпоновкиПоТаблицамЗначенийЛкс(НаборыДанныхОбъекты);
	#Если Сервер И Не Сервер Тогда
...
Процедура ПриОткрытии()
	...
	ирКлиент.Форма_ПриОткрытииЛкс(ЭтаФорма,,,, "fdr", СоставНастройкиФормы);
	#Если Сервер И Не Сервер Тогда
...
Процедура ОбновитьКнопкуПараметровПоискаСсылок()
	...
	Кнопка = ЭлементыФормы.ДействияФормы.Кнопки.ПараметрыПоиска;
...
Функция ПрочитатьДопПоляГрупп(Знач ТолькоТекущаяСтрока = Ложь) Экспорт
	СтрокаПорядка = ирКлиент.ПрочитатьДопПоляОбъектовЛкс(ЭлементыФормы.НайденныеГруппы, КомпоновщикГрупп,, мИмяПоляПравильныйЭлемент,, ТолькоТекущаяСтрока, 
		ЭтаФорма, ЭлементыФормы.КоманднаяПанельДопПоляГрупп.Кнопки.ПрочитатьПоля, "ПослеЧтенияДопПолейГрупп");
	Если ЗначениеЗаполнено(СтрокаПорядка) Тогда
		НайденныеГруппы.Сортировать(СтрокаПорядка);
	КонецЕсли;
КонецФункции

Процедура ПослеЧтенияДопПолейГрупп(СостояниеЗадания = Неопределено, РезультатЗадания = Неопределено) Экспорт
	Если РезультатЗадания = Неопределено Тогда
		Возврат;
	КонецЕсли;
	ТаблицаДанных = РезультатЗадания.ТаблицаДанных; // см. НайденныеГруппы
	Для Каждого СтрокаРезультата Из ТаблицаДанных Цикл
		СтрокаСсылки = НайденныеГруппы.Найти(СтрокаРезультата[мИмяПоляПравильныйЭлемент], мИмяПоляПравильныйЭлемент);
		ЗаполнитьЗначенияСвойств(СтрокаСсылки, СтрокаРезультата);
	КонецЦикла;
КонецПроцедуры
...
Процедура ШкалаЦенностиПередНачаломДобавления(Элемент, Отказ, Копирование)
	Если ирКэш.НомерИзданияПлатформыЛкс() = 82 Тогда
		// Антибаг платформы https://www.hostedredmine.com/issues/1008029
		Отказ = Истина;
		Элемент.Значение.Добавить();
	КонецЕсли;
КонецПроцедуры

ирКлиент.ИнициироватьФормуЛкс(ЭтаФорма, "Обработка.ирПоискДублейИЗаменаСсылок.Форма.Форма");
#Если Сервер И Не Сервер Тогда
	ЭтоПоискПоПохожимСловам(); 
#КонецЕсли
мПроизвольныйЗапрос = Новый Запрос;
