...
Процедура ПолучитьАктуальнуюВерсиюОтложенно()
	...
	Если ирКэш.НомерВерсииПлатформыЛкс() < 802018 Тогда
		...
		Возврат;
	КонецЕсли; 
	ЭтаФорма.АктуальнаяВерсия = ирОбщий.АктуальнаяВерсияПодсистемыЛкс(НазваниеВарианта, СсылкаНаСтраницуСкачивания);
	Если ЗначениеЗаполнено(АктуальнаяВерсия) Тогда
		ЭтаФорма.АктуальнаяВерсия = НазваниеВарианта + " " + АктуальнаяВерсия;
	КонецЕсли;
	Если Истина
	...
	Если мФоновыйЗапросХттп.мБраузер = Неопределено Тогда
		СлужебноеПолеХТМЛДокументСформирован();
	КонецЕсли;

КонецПроцедуры

Процедура ПрочитатьДатыВерсийИзОтветаСервера(Знач ТелоКакСтроку) Экспорт
	ПостроительDOM = Новый ПостроительDOM;
	ЧтениеXML = Новый ЧтениеXML;
	ЧтениеXML.УстановитьСтроку(ТелоКакСтроку);
...
Процедура ОбновитьНажатие(Элемент)
	...
	Если НазваниеВарианта = "Портативный" Тогда
		...
		Иначе
			...
			КонецЕсли;
		КонецЕсли; 
		ДвоичныеДанные = СкачатьАктуальнуюВерсию();
	...
	Иначе
		...
		КонецЕсли; 
		ДвоичныеДанные = СкачатьАктуальнуюВерсию();
		ИмяФайла = Каталог + "\ИР " + АктуальнаяВерсия + ".cf";
...
Функция СкачатьАктуальнуюВерсию() Экспорт
	Возврат ирОбщий.СкачатьФайлАктуальнойВерсииПодсистемыЛкс(СсылкаНаСтраницуСкачивания, НазваниеВарианта, ирОбщий.ПоследнийФрагментЛкс(АктуальнаяВерсия, " "));
КонецФункции
...
Процедура СлужебноеПолеХТМЛonclick(Элемент, pEvtObj) Экспорт
	ТекстОтвета = мФоновыйЗапросХттп.ПрочитатьЧастьОтвета();
	Если ирОбщий.СтрНачинаетсяСЛкс(ТекстОтвета, "Ошибка") Тогда
		Возврат;
	КонецЕсли;
	ПрочитатьДатыВерсийИзОтветаСервера(ТекстОтвета);
КонецПроцедуры

Процедура СлужебноеПолеХТМЛДокументСформирован(Элемент = Неопределено)
	мФоновыйЗапросХттп.ОтправитьЗапрос(ирОбщий.АдресСайтаЗадачЛкс() + "/versions.xml",,,,,,, "get", ЭтаФорма);
КонецПроцедуры
