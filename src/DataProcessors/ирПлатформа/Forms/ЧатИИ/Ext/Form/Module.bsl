...
Процедура ДействияФормыОтправить(Кнопка = Неопределено)
	...
	Если ЗначениеЗаполнено(КодЗапроса) Тогда
		...
		|Блоки кода всегда обрамляй строкой (```).
		|Непосредственно перед началом полного блока кода вставляй строку (" + мПрефиксПолногоКода + ").
	...
	Если Кнопка.Картинка <> ирКэш.КартинкаПоИмениЛкс("ирОстановить") Тогда
		ЭлементыФормы.HTMLГотовый.Документ.defaultView._1CAPI.setText("");
	КонецЕсли;
	мНовыйКод = "";
	...
	мФоновыйЗапросХттп.ОтправитьЗапросИИ(ПараметрыЗапроса, ЭтаФорма, "ОтправитьКонец",, Кнопка, Истина,, РежимНапарника,,,,, РазрешитьДозапросы);
...
Процедура ОтправитьКонец(Знач СостояниеЗадания, Знач РезультатЗадания) Экспорт
	...
	КонецЕсли;
	ЭлементыФормы.HTMLГотовый.Документ.defaultView._1CAPI.setText(ТекстОтвета,, Истина); // Долго на большом тексте
КонецПроцедуры   
