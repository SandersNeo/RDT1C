...
Процедура ЗаполнитьСтрокуДерева(Знач СтрокаДерева, Знач УстановитьПометкиРодителей = Истина, Знач УстановитьИзвестность = Истина)
...
Процедура ЗаполнитьДеревоИсточников(Знач Фильтр = Неопределено, МассивДоступныхОбъектов = Неопределено, Принудительно = Ложь) Экспорт
	...
	ЭтаФорма.ДеревоИсточников = ДеревоОбъектовМД().Скопировать(); 
	...
	Для СчетчикТиповМД = 1 По НачальноеКоличествоТиповМД Цикл
		...
		Для СчетчикОбъектовМД = 1 По НачальноеКоличествоОбъектовМД Цикл
			...
			КонецЕсли;
			Если мМножественныйВыбор Тогда
				ЗаполнитьСтрокуДерева(СтрокаОбъектаМД, Ложь); 
			КонецЕсли; 
		КонецЦикла;
		Если Истина
...
Функция УзелПодходитФильтру(Знач СтрокаОбъектаМД, Знач Фильтр, Знач СловаПоиска, Знач ИскатьСРодителем, выхПрерватьРодителя = Ложь)
	...
	Если СтрокаОбъектаМД.ПолноеИмяОбъекта = Фильтр Тогда
		УзелПодходитФильтру = Истина;
	Иначе
		ПредставлениеОбъекта = ФильтруемоеПредставлениеСтрокиДерева(СтрокаОбъектаМД);
		УзелПодходитФильтру = ирОбщий.ЛиСтрокаСодержитВсеПодстрокиЛкс(ПредставлениеОбъекта, СловаПоиска, ?(ИскатьСРодителем, СтрокаОбъектаМД.Родитель.Имя, ""), выхПрерватьРодителя);
		Если УзелПодходитФильтру И ЗначениеЗаполнено(СловаПоиска) Тогда
			ФильтрПредставления = ирОбщий.СтрСоединитьЛкс(СловаПоиска, " ");
			Если СтрДлина(ФильтрПредставления) > 1 Тогда
				ПредставлениеОбъекта = ФильтруемоеПредставлениеСтрокиДерева(СтрокаОбъектаМД, Истина);
				СтрокаОбъектаМД.ПремияФильтра = ирОбщий.НайтиФильтрВСловеЛкс(Ложь, ФильтрПредставления, ПредставлениеОбъекта, НРег(ПредставлениеОбъекта), Истина);
...
Процедура СортироватьДерево()
	...
	Если РежимИмяСиноним Тогда
		СтрокаСортировки = "Имя";
	Иначе
		СтрокаСортировки = "Представление";
	КонецЕсли;
	Если ЗначениеЗаполнено(мПоследнийФильтр) Тогда
		СтрокаСортировки = "ПремияФильтра Убыв, " + СтрокаСортировки;
	КонецЕсли;
	//ДеревоИсточников.Строки.Сортировать(СтрокаСортировки, Истина);
	Для Каждого КорневаяСтрока Из ДеревоИсточников.Строки Цикл
		КорневаяСтрока.Строки.Сортировать(СтрокаСортировки);
	КонецЦикла;
