...
Процедура ОбновитьСостояниеЗадания(Знач ФоновоеЗадание) Экспорт
	...
	КонецЕсли;
	ТекстСостояния = ирОбщий.ПредставлениеДлительностиЛкс(Конец - ФоновоеЗадание.Начало) + "с ";
	Если ирКэш.ЛиФайловаяБазаЛкс() Тогда
		...
		Для Каждого Задание Из АктивныеЗадания Цикл
			Если Задание.Начало < ФоновоеЗадание.Начало Тогда
				ЧислоВОчереди = ЧислоВОчереди + 1;
			КонецЕсли;
		КонецЦикла;
		Если ЧислоВОчереди > 0 Тогда
			ТекстСостояния = ТекстСостояния + ", ждем №" + ЧислоВОчереди + " в очереди, ";
		КонецЕсли;
	КонецЕсли;
	ЭтаФорма.Состояние = ТекстСостояния + ФоновоеЗадание.Состояние;
...
Процедура СостояниеНажатие(Элемент)
	Если ирКэш.ЛиФайловаяБазаЛкс() Тогда
		ФормаКонсоли = ирКлиент.ОткрытьКонсольЗаданийЛкс(, Истина);
	КонецЕсли;
	ФормаФоновогоЗадания = ирКлиент.ПолучитьФормуЛкс("Обработка.ирКонсольЗаданий.Форма.ДиалогФоновогоЗадания",, ФормаКонсоли, ИдентификаторЗадания);
