...
Функция ИмяТипаИзПредставления(Знач Представление, Знач ЛиИмяОдногоТипа = Ложь) 
	Результат = мСинтаксПомощник.ИмяТипаИзПредставления(Представление);
...
Процедура ЗаполнитьВсе()
	...
	шТип = мСинтаксПомощник.шТип;
	шТипСтрогий = мСинтаксПомощник.шТипСтрогий;
	шКонецТегаКлюч = мСинтаксПомощник.шКонецТегаКлюч;
	шТег = мСинтаксПомощник.шТег;
	шТегКлюч = мСинтаксПомощник.шТегКлюч;
	шТегНеКлюч = мСинтаксПомощник.шТегНеКлюч;
	...
	НоваяСтрока = ТаблицаШаблоновКонтекстов.Добавить();
	НоваяСтрока.Слово = "<Имя элемента отбора>";
	НоваяСтрока.ТипКонтекста = "НастройкаОтбора";
	НоваяСтрока.ТипСлова = "Свойство";
	НоваяСтрока.ТипЗначения = "ЭлементУправленияОтбором";
	
	НоваяСтрока = ТаблицаШаблоновКонтекстов.Добавить();
	НоваяСтрока.Слово = "<Имя варианта>";
	НоваяСтрока.ТипКонтекста = "ВариантыНастроекКомпоновкиДанных";
	НоваяСтрока.ТипСлова = "Свойство";
	НоваяСтрока.ТипЗначения = "ВариантНастроекКомпоновкиДанных";
	...
	Для ЯзыкПрограммы = 1 По 2 Цикл
		...
		Для Счетчик = 1 По СписокПараметров.Количество() Цикл
			...
			НоваяСтрокаПараметра.Необязательный = Счетчик > 3;
		КонецЦикла;
	...
	ИмяКниги = "dcsui_ru";
	ФайлАрхиваДанных = мСинтаксПомощник.АрхивСинтаксПомощникаПоИмени(, ИмяКниги);
...
// {ОписаниеРегулярногоВыражения.Конец}
	...
	Для Каждого КлючИЗначение Из ИменаФайлов Цикл
		...
		КонецЦикла;
	КонецЦикла; 
    КаталогОписанияВстроенногоЯзыка = мСинтаксПомощник.ПутьКэшаСинтаксПомощника();
	// Синтаксис встроенного языка
	ФайлАрхиваСодержания = мСинтаксПомощник.АрхивСинтаксПомощникаПоИмени(, "shlang_ru");
	ПараметрыРаспаковки = "";
	мСинтаксПомощник.ЛиАктивнаРаспаковкаАрхиваСинтаксПомощника(ФайлАрхиваСодержания, ПараметрыРаспаковки);
	Если ЗначениеЗаполнено(ПараметрыРаспаковки) Тогда
		мСинтаксПомощник.ЗапуститьРаспаковкуАрхиваСинтаксПомощника(ПараметрыРаспаковки, Истина);
	КонецЕсли;
	...
	НоваяСтрока.ТипЗначения = "Произвольный";
	ЗаполнитьОсобыйМетодВстроенногоЯзыка(НоваяСтрока, КаталогОписанияВстроенногоЯзыка, мСинтаксПомощник);
	НоваяСтрока = ТаблицаКонтекстов.Добавить();
	...
	ФайлАрхиваСодержания = мСинтаксПомощник.АрхивСинтаксПомощникаПоИмени(1, "shcntx_ru");
	ПараметрыРаспаковки = "";
	мСинтаксПомощник.ЛиАктивнаРаспаковкаАрхиваСинтаксПомощника(ФайлАрхиваСодержания, ПараметрыРаспаковки);
	Если ЗначениеЗаполнено(ПараметрыРаспаковки) Тогда
		мСинтаксПомощник.ЗапуститьРаспаковкуАрхиваСинтаксПомощника(ПараметрыРаспаковки, Истина);
	...
	ФайлАрхиваДанных = мСинтаксПомощник.АрхивСинтаксПомощникаПоИмени(, "shcntx_ru");
	ПараметрыРаспаковки = "";
	мСинтаксПомощник.ЛиАктивнаРаспаковкаАрхиваСинтаксПомощника(ФайлАрхиваСодержания, ПараметрыРаспаковки);
	Если ЗначениеЗаполнено(ПараметрыРаспаковки) Тогда
		мСинтаксПомощник.ЗапуститьРаспаковкуАрхиваСинтаксПомощника(ПараметрыРаспаковки, Истина);
	...
	Для Каждого Item Из Результат Цикл
		...
		Если ТекстПрочитан Тогда
			...
			Если Найти(НРегПуть, "/tables/") > 0 Тогда 
				...
				Если Результат.Количество() > 0 Тогда
					НовоеСлово = мСинтаксПомощник.ИзвлечьИзФрагментаHTMLОбычныйТекст(Результат[0].SubMatches(0));
				КонецЕсли;
			КонецЕсли;
			...
			Иначе
				...
				КонецЕсли;
				РегВыражение.Global = Ложь;
				РегВыражение.Pattern = "Тип:" + шТегНеКлюч + "?((?:" + шТип + ")+)(?:<[^>""]+>" + мСинтаксПомощник.шТипВложенный + ")?";
				СмещениеИндекса = 0;
				...
				Если Результат.Количество() > 0 Тогда
					...
					ТипЗначения = мСинтаксПомощник.ИзвлечьИзФрагментаHTMLОбычныйТекст(СтрокаТипаЗначения);
					ТипЗначения = ИмяТипаИзПредставления(ТипЗначения);
					Если Результат[0].SubMatches(4) <> Неопределено Тогда
						ТипЗначенияВложенный = ИмяТипаИзПредставления(мСинтаксПомощник.ИзвлечьИзФрагментаHTMLОбычныйТекст(Результат[0].SubMatches(4)));
			...
			Если Результат.Количество() > 0 Тогда
				...
				КонецЦикла;
				ТипЭлементаКоллекции = мСинтаксПомощник.ИзвлечьИзФрагментаHTMLОбычныйТекст(Сред(СтрокаТипаЗначения, 3));
		...
		Если ТипКонтекста = Неопределено Тогда
			...
			Тогда
				...
				Если Результат.Количество() > 0 Тогда
					ТипКонтекста = мСинтаксПомощник.ИзвлечьИзФрагментаHTMLОбычныйТекст(Результат[0].SubMatches(1));
				КонецЕсли;
			КонецЕсли;
		...
		КонецЕсли;
		ОписаниеМетодаСырое = ""; 
		мСинтаксПомощник.ЗагрузитьОписаниеМетода(ТипКонтекста, НовоеСлово, ОписаниеХтмл, ТипСлова, ЯзыкПрограммы, ТаблицаПараметров, КаталогОписанияВстроенногоЯзыка,, ОписаниеМетодаСырое);
		Если ТипКонтекста = "" Тогда
			...
			мСинтаксПомощник.СкорректироватьЭлементыСтруктуры(СтруктураКлюча);
			...
			НоваяСтрока.ЕстьЧисловойИндекс = Ложь
				...
				Или Найти(ОписаниеХтмл, "оператора [...]. В качестве аргумента передается позиция") > 0 // БуферДвоичныхДанных
		...
		Иначе
			...
			мСинтаксПомощник.СкорректироватьЭлементыСтруктуры(СтруктураСтроки);
			...
			Тогда
				...
				ВхожденияТипов = ирОбщий.НайтиРегВыражениеЛкс(ОписаниеМетодаСырое, мСинтаксПомощник.шТипСтрогий);
				Для Каждого ВхождениеТипа Из ВхожденияТипов Цикл
					ИмяТипаРезультата = мСинтаксПомощник.ИзвлечьИзФрагментаHTMLОбычныйТекст(ВхождениеТипа.Группа0);
					Если Ложь
	...
	СтрокаОбщийМодуль = ТаблицаШаблоновКонтекстов.НайтиСтроки(Новый Структура("ТипКонтекста, Слово", "Глобальный", "<Имя общего модуля>"))[0];
	НоваяСтрока = ТаблицаШаблоновКонтекстов.Добавить();
	ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаОбщийМодуль);
	НоваяСтрока.ЯзыкПрограммы = 2;
	...
	СтрокаПараметра = ТаблицаПараметров.Добавить();
	СтрокаПараметра.ТипКонтекста = "ЭлементDOM";
	СтрокаПараметра.Слово = "ПолучитьЭлементыПоИмени";
	СтрокаПараметра.ВариантСинтаксиса = "По имени";
	СтрокаПараметра.Параметр = "Имя";
	СтрокаПараметра.Номер = 1;
	СтрокаПараметра.ТипЗначения = "Строка";  
	СтрокаПараметра.ЯзыкПрограммы = ВременныйНомерВстроенногоЯзыка();	
	СтрокаПараметра = ТаблицаПараметров.Добавить();
	СтрокаПараметра.ТипКонтекста = "КоллекцияКолонокДереваЗначений";
	СтрокаПараметра.Слово = "Удалить";
	СтрокаПараметра.Параметр = "Индекс";
	СтрокаПараметра.Номер = 1;
	СтрокаПараметра.ТипЗначения = "Число, КолонкаДереваЗначений, Строка"; // Забыли тип Строка. Но похоже не работает такое замещение.
	СтрокаПараметра.ЯзыкПрограммы = ВременныйНомерВстроенногоЯзыка();
	...
	НайденныеСтроки = ТаблицаШаблоновКонтекстов.НайтиСтроки(СтруктураКлюча);
	НайденныеСтроки[0].ТипЗначения = "НаборЗаписейРегистра";
	...
	НайденныеСтроки = ТаблицаКонтекстов.НайтиСтроки(СтруктураКлюча);
	НайденныеСтроки[0].ТипЗначения = "Число";
	СтруктураКлюча = Новый Структура("ТипКонтекста, Слово, ЯзыкПрограммы", "COMSafeArray", "Выгрузить", 0);
	НайденныеСтроки = ТаблицаКонтекстов.НайтиСтроки(СтруктураКлюча);
	НайденныеСтроки[0].ТипЗначения = "Массив[Произвольный, Массив]";
	СтруктураКлюча = Новый Структура("ТипКонтекста, Слово, ЯзыкПрограммы", "ОписаниеХарактеристик", "ЗначенияХарактеристик", 0);
	НайденныеСтроки = ТаблицаКонтекстов.НайтиСтроки(СтруктураКлюча);
	НайденныеСтроки[0].ТипЗначения = "ОбъектМетаданныхРегистрСведений, ОбъектМетаданныхТабличнаяЧасть";
	СтруктураКлюча = Новый Структура("ТипКонтекста, Слово, ЯзыкПрограммы", "ЭлементОтбора", "Значение", 0);
	НайденныеСтроки = ТаблицаКонтекстов.НайтиСтроки(СтруктураКлюча);
	НайденныеСтроки[0].ТипЗначения = "Произвольный, СписокЗначений";
	СтруктураКлюча = Новый Структура("ТипКонтекста, Слово, ЯзыкПрограммы", "ЭлементОтбораКомпоновкиДанных", "ПравоеЗначение", 0);
	НайденныеСтроки = ТаблицаКонтекстов.НайтиСтроки(СтруктураКлюча);
	НайденныеСтроки[0].ТипЗначения = "Произвольный, СписокЗначений";
	СтруктураКлюча = Новый Структура("ТипКонтекста, Слово, ЯзыкПрограммы", "ОписаниеМакетаОбластиМакетаКомпоновкиДанных", "Макет", 0);
	НайденныеСтроки = ТаблицаКонтекстов.НайтиСтроки(СтруктураКлюча);
	НайденныеСтроки[0].ТипЗначения = СтрЗаменить(НайденныеСтроки[0].ТипЗначения, "ЯчейкаМакетаКоллекцииЗначенийОбластиКомпоновкиДанных", "МакетКоллекцииЗначенийОбластиКомпоновкиДанных");
	
	/// В режиме выполнения развернем виртуальные типы УзелHTML/УзелDOM в список всех типов HTML
	НоваяСтрока = ТаблицаОбщихТипов.Добавить();
	НоваяСтрока.Слово = "УзелHTML";
	ЗаполнитьСтрокуТипа(НоваяСтрока);
	НоваяСтрока = ТаблицаОбщихТипов.Добавить();
	НоваяСтрока.Слово = "УзелDOM";
	ЗаполнитьСтрокуТипа(НоваяСтрока);
	СписокСвойствДОМ = Новый Массив;
	СписокСвойствДОМ.Добавить("РодительскийУзел");
	СписокСвойствДОМ.Добавить("ПервыйДочерний");
	СписокСвойствДОМ.Добавить("ПоследнийДочерний");
	СписокСвойствДОМ.Добавить("ДобавитьДочерний");
	СписокСвойствДОМ.Добавить("ЗаменитьДочерний");
	СписокСвойствДОМ.Добавить("КлонироватьУзел");
	СписокСвойствДОМ.Добавить("УдалитьДочерний");
	Для Каждого ИмяСвойства Из СписокСвойствДОМ Цикл
		СтруктураКлюча = Новый Структура("Слово, ЯзыкПрограммы", ИмяСвойства, 0);
		НайденныеСтроки = ТаблицаКонтекстов.НайтиСтроки(СтруктураКлюча);
		Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл 
			ЕстьНеопределено = Найти(НайденнаяСтрока.ТипЗначения, "Неопределено") > 0;
			Если Найти(НайденнаяСтрока.ТипЗначения, "HTML") Тогда
				НайденнаяСтрока.ТипЗначения = "УзелHTML";
			Иначе
				НайденнаяСтрока.ТипЗначения = "УзелDOM";
			КонецЕсли;
			Если ЕстьНеопределено Тогда
				НайденнаяСтрока.ТипЗначения = НайденнаяСтрока.ТипЗначения + ", Неопределено";
			КонецЕсли;
		КонецЦикла;
	КонецЦикла; 
	...
	СтруктураКлюча = Новый Структура("ТипКонтекста, Слово, ТипСлова, ЯзыкПрограммы", "ОбъектМетаданныхКонфигурация", "Представление", "Метод", 0);
	НайденныеСтроки = ТаблицаКонтекстов.НайтиСтроки(СтруктураКлюча);
	Если НайденныеСтроки.Количество() = 0 Тогда
		// 8.0-8.3.27
		НоваяСтрока = ТаблицаКонтекстов.Добавить();
	...
	СтруктураКлюча = Новый Структура("ТипКонтекста, Слово, ТипСлова, ЯзыкПрограммы", "Глобальный", "ПолитикиПаролейПользователей", "Свойство", 0);
	НоваяСтрока = ТаблицаКонтекстов.НайтиСтроки(СтруктураКлюча)[0];
	НоваяСтрока.ТипЗначения = "МенеджерПолитикПаролейПользователей"; /// пусто в 8.3.27
	...
	СтруктураКлюча = Новый Структура("ТипКонтекста, Слово, ТипСлова, ЯзыкПрограммы", "ФормаКлиентскогоПриложения", "ВладелецФормы", "Свойство", 0);
	НоваяСтрока = ТаблицаКонтекстов.НайтиСтроки(СтруктураКлюча)[0];
	НоваяСтрока.ТипЗначения = НоваяСтрока.ТипЗначения + ", Неопределено";  
	ТипЗначенияВладельцаУправляемойФормы = НоваяСтрока.ТипЗначения;  
	СтруктураКлюча = Новый Структура("ТипКонтекста, Слово, ТипСлова, ЯзыкПрограммы", "Форма", "ВладелецФормы", "Свойство", 0);
	НоваяСтрока = ТаблицаКонтекстов.НайтиСтроки(СтруктураКлюча)[0];
	НоваяСтрока.ТипЗначения = НоваяСтрока.ТипЗначения + ", " + ТипЗначенияВладельцаУправляемойФормы;  
	...
	// В 8.3.21 доступность на клиенте синхронных методов спрятали из синтакс-помощника
	СтруктураКлюча = Новый Структура("ТипКонтекста, ТипСлова, ЯзыкПрограммы", "HTTPСоединение", "Метод", 0);
	Для Каждого НоваяСтрока Из ТаблицаКонтекстов.НайтиСтроки(СтруктураКлюча) Цикл
		НоваяСтрока.НеТолстыйКлиент = Ложь; 
		НоваяСтрока.НеТонкийКлиент = Ложь;
	КонецЦикла;
	
	//СтруктураКлюча = Новый Структура("ТипКонтекста, Слово, ТипСлова, ЯзыкПрограммы, НеТонкийКлиент", "ЭлементыФормы", "<Имя элемента управления>", "Свойство", 0, Истина);
	//НоваяСтрока = ТаблицаШаблоновКонтекстов.НайтиСтроки(СтруктураКлюча)[0];
	//Если ПустаяСтрока(НоваяСтрока.ТипЗначения) Тогда
	//	НоваяСтрока.ТипЗначения = "ЭлементОбычнойФормы"; /// Зачем виртуальный тип?
	//КонецЕсли;
	...
	НайденныеСтроки = ТаблицаКонтекстов.НайтиСтроки(СтруктураКлюча);
	НайденныеСтроки[0].ТипЗначения = "ОбъектXDTO";
	СтруктураКлюча = Новый Структура("ТипКонтекста, Слово", "ФабрикаXDTO", "Создать");
	НайденныеСтроки = ТаблицаКонтекстов.НайтиСтроки(СтруктураКлюча);
	НайденныеСтроки[0].ТипЗначения = "ЗначениеXDTO, ОбъектXDTO";
	...
	СтруктураКлюча = Новый Структура("ТипКонтекста, Слово", "ДанныеФормыСтруктура", "<Имя свойства>");
	НайденныеСтроки = ТаблицаШаблоновКонтекстов.НайтиСтроки(СтруктураКлюча);
	НайденныеСтроки[0].ТипЗначения = "Произвольный";
	СтруктураКлюча = Новый Структура("ТипКонтекста, Слово", "ДанныеФормыСтруктураСКоллекцией", "<Имя свойства>");
	НайденныеСтроки = ТаблицаШаблоновКонтекстов.НайтиСтроки(СтруктураКлюча);
	НайденныеСтроки[0].ТипЗначения = "Произвольный";
	СтруктураКлюча = Новый Структура("ТипКонтекста, Слово", "ДанныеФормыЭлементКоллекции", "<Имя свойства>");
	НайденныеСтроки = ТаблицаШаблоновКонтекстов.НайтиСтроки(СтруктураКлюча);
	НайденныеСтроки[0].ТипЗначения = "Произвольный";
	СтруктураКлюча = Новый Структура("ТипКонтекста, Слово", "ДанныеФормыЭлементДерева", "<Имя свойства>");
	НайденныеСтроки = ТаблицаШаблоновКонтекстов.НайтиСтроки(СтруктураКлюча);
	НайденныеСтроки[0].ТипЗначения = "Произвольный";
	СтруктураКлюча = Новый Структура("ТипКонтекста, Слово", "РасширениеЭлементовУправленияРасположенныхВФорме", "Значение");
	НайденныеСтроки = ТаблицаКонтекстов.НайтиСтроки(СтруктураКлюча);
	НайденныеСтроки[0].ТипЗначения = "Произвольный";
	...
	Если ВключитьАнализСрабатыванияЗамен Тогда
		ирОбщий.ИсследоватьЛкс(мСинтаксПомощник.СрабатываниеЗамен);
	КонецЕсли; 
	
КонецПроцедуры

Процедура ЗаполнитьОсобыеОбщиеТипы() Экспорт
	...
	СтруктураКлюча = Новый Структура("Слово, ЯзыкПрограммы", "ВнешнийОбъект", 0);
	НайденныеСтроки = ТаблицаОбщихТипов.НайтиСтроки(СтруктураКлюча);
	НайденныеСтроки[0].НеТонкийКлиент = Ложь;
	НайденныеСтроки[0].НеТолстыйКлиент = Ложь;
	...
	НайденныеСтроки[0].ТипЭлементаКоллекции = "Произвольный";
	НайденныеСтроки[0].ЕстьЧисловойИндекс = Истина;
	СтруктураКлюча = Новый Структура("Слово, ЯзыкПрограммы", "СписокПолнотекстовогоПоиска", 0);
	НайденныеСтроки = ТаблицаОбщихТипов.НайтиСтроки(СтруктураКлюча);
	НайденныеСтроки[0].ЕстьЧисловойИндекс = Истина;
	СтруктураКлюча = Новый Структура("ТипКонтекста, Слово, ЯзыкПрограммы", "ВыделенныеСтрокиТабличногоПоля", "Получить", 0);
	НайденныеСтроки = ТаблицаКонтекстов.НайтиСтроки(СтруктураКлюча);
	НайденныеСтроки[0].ТипЗначения = "Произвольный";
	...
	СтруктураКлюча = Новый Структура("Слово, ЯзыкПрограммы", "КритерииОтбораМенеджер", 0);
	НайденныеСтроки = ТаблицаОбщихТипов.НайтиСтроки(СтруктураКлюча);
	НайденныеСтроки[0].ТипЭлементаКоллекции = "КритерийОтбораМенеджер.<Имя критерия отбора>";
	СтруктураКлюча = Новый Структура("Слово, ЯзыкПрограммы", "СерииДиаграммы", 0);
	НайденныеСтроки = ТаблицаОбщихТипов.НайтиСтроки(СтруктураКлюча);
	НайденныеСтроки[0].ЕстьЧисловойИндекс = Истина;
	СтруктураКлюча = Новый Структура("Слово, ЯзыкПрограммы", "СписокЭлементовDOM", 0);
	ТаблицаОбщихТипов.НайтиСтроки(СтруктураКлюча)[0].ТипЭлементаКоллекции = "ЭлементDOM";
КонецПроцедуры
...
Процедура ЗаполнитьОсобыйМетодВстроенногоЯзыка(Знач НоваяСтрока, Знач РабочийКаталог, Знач мСинтаксПомощник)
	...
	мСинтаксПомощник.ЗагрузитьОписаниеМетода(НоваяСтрока.ТипКонтекста, НоваяСтрока.Слово, Текст.ПолучитьТекст(), НоваяСтрока.ТипСлова, НоваяСтрока.ЯзыкПрограммы, ТаблицаПараметров, РабочийКаталог);
...
Процедура ТаблицаПараметровВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	...
	мСинтаксПомощник = ирОбщий.СоздатьОбъектПоИмениМетаданныхЛкс("Обработка.ирСинтаксПомощник");
	#Если Сервер И Не Сервер Тогда
		мСинтаксПомощник = Обработки.ирСинтаксПомощник.Создать();
	#КонецЕсли
	СтрокаДляПодсветкиПараметра = мСинтаксПомощник.СтрокаДляПодсветкиПараметра(ВыбраннаяСтрока.Параметр, ВыбраннаяСтрока.ЯзыкПрограммы);
...
Функция ОтобратьТаблицуЗначенийПоТипуЗначения(Знач ТаблицаЗначений, Знач НастройкаКомпоновки, Знач ИмяКолонкиТипа = "ТипЗначения") Экспорт 
	...
	СхемаКомпоновки = ирОбщий.СоздатьСхемуКомпоновкиПоТаблицеЗначенийЛкс(ТаблицаЗначений, НаборыДанных);
	#Если Сервер И Не Сервер Тогда
