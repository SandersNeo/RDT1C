...
Процедура ЗагрузитьЭлементыСписка(Знач НовоеЗначение, Знач ОчиститьПередЗагрузкой = Истина, Знач РазрешитьПропускНеуникальных = Истина)
	...
	ЭлементыФормы.Таблица.Колонки.Пометка.Видимость = Ложь
		Или ЕстьПометки 
		Или РежимВыбора И МножественныйВыбор;
...
Процедура ПриОткрытии()
	...
	Если мРежимМассива Тогда 
		...
		СлужебныеДанные.ОригинальныйЗаголовок = ирОбщий.СтрЗаменитьЛкс(СлужебныеДанные.ОригинальныйЗаголовок, "Список", ТРег(ТипЗнч(НачальноеЗначениеВыбора)));
	Иначе
	...
	Если РежимВыбора Тогда
		...
		КонецЦикла;
		Если Не КолонкиТП.Пометка.ТолькоПросмотр Тогда
