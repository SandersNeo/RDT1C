...
Перем СубПутьКФайлуЗаголовкаВнешнегоОтчета;
Перем СубПутьККонтрольномуФайлуВнешнейОбработки;
Перем СубПутьККонтрольномуФайлуВнешнегоОтчета;
...
Перем мСкриптКопированиеВБуферОбменаCTRLКлик;
Перем мСкриптРаскраскиКодаВстроенногоЯзыка;
...
Перем мРазрешеноСохранениеВХранилища Экспорт; /// https://www.hostedredmine.com/issues/1006941
Перем КлючСервисаНапарник Экспорт;
Перем КлючСервисаИИ Экспорт;
Перем МодельИИ Экспорт;
Перем ПараметрыМоделиИИ Экспорт;
Перем СерверИИ Экспорт;
Перем ПредлагатьОчиститьМенеджерВременныхТаблицЧерезМинут Экспорт; // Структура
Перем мКонтекстНапарника;
Перем мДатаАктивностиПользователя Экспорт;
Перем мРазделительПутиКФайлу; /// ускороение
Перем мКэшФайловПоИменамМодулей; 
Перем мГлубинаСТека Экспорт;
...
// Опасно вызывать в файловой базе в транзакции https://www.hostedredmine.com/issues/1006941
Процедура ПрочитатьСтатистикуВыбораАвтодополнения() Экспорт
	...
	КонецЕсли;
	Если ТаблицаСтатистикиВыбора.Колонки.Найти("БезЗначения") = Неопределено Тогда
		ТаблицаСтатистикиВыбора.Колонки.Добавить("БезЗначения", Новый ОписаниеТипов("Булево"));
	КонецЕсли;
	...
	ирОбщий.ДобавитьИндексВТаблицуЛкс(ТаблицаСтатистикиВыбора, "ТипКонтекста, ЯзыкПрограммы, БезЗначения");
...
Функция НоваяТаблицаСлов() Экспорт 
	...
	ВнутрТаблицаСлов.Колонки.Добавить("НСлово"); // Строка, Неопределено
	...
	//ВнутрТаблицаСлов.Индексы.Добавить("НСлово, ТипСлова");
...
// Возвращаемое значение:
//   ПостроительЗапроса. - 
Функция ПостроительЗапросаПоСтруктуреТипа(Знач СтрокаОбщегоТипа, Знач ВиртуальнаяТаблица = Неопределено)
			...
			Если ТипЗнч(МетаданныеРодителя) = Тип("ОбъектМетаданных") Тогда
				...
				Иначе
					...
					Попытка
						...
						ТаблицаРезультата = ирОбщий.ПустаяТаблицаИзТекстаЗапросаЛкс(ОпределениеТаблицы,,, Ложь);
					Исключение
						Успех = Ложь;
						Если ВиртуальнаяТаблица <> Неопределено Тогда
							...
							Если ПозицияСкобки > 0 Тогда
								...
								Попытка
									//ПостроительЗапроса.Текст = ОпределениеТаблицы;
									ТаблицаРезультата = ирОбщий.ПустаяТаблицаИзТекстаЗапросаЛкс(ОпределениеТаблицы);
									Успех = Истина;
								Исключение
								КонецПопытки; 
							КонецЕсли; 
...
// Возвращаемое значение:
//
Функция РазвернутаяТаблицаТиповИзСтруктурыТипа(Знач СтруктураТипа, Знач ЭтоЛокальныйКонтекст, Знач ДляФиксированныхСлов = Истина, Знач ДоступноКэширование = Ложь, Знач ЯзыкПрограммы = 0) Экспорт 
	...
	ИначеЕсли Ложь
		Или СтруктураТипа.ИмяОбщегоТипа = "ПолеТабличногоДокумента" 
		Или СтруктураТипа.ИмяОбщегоТипа = "РасширениеПоляФормыДляПоляТабличногоДокумента"
	Тогда
		НоваяСтрока = МассивОбщихТипов.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрока, СтруктураТипа, "Метаданные");
	...
	ИначеЕсли СтруктураТипа.ИмяОбщегоТипа = "ЭлементHTML" Тогда
		Если ДоступноКэширование Тогда
			СтруктураКлюча = Новый Структура("Слово, ЯзыкПрограммы", "СписокУзловHTML", 0);
			ИменаТиповУзловХТМЛ = ирОбщий.СтрРазделитьЛкс(ТаблицаОбщихТипов.НайтиСтроки(СтруктураКлюча)[0].ТипЭлементаКоллекции, ",", Истина);
			Для Каждого ИмяТипа Из ИменаТиповУзловХТМЛ Цикл
				Если Ложь
					Или ИмяТипа = "ЭлементHTML" 
					Или ИмяТипа = "АтрибутHTML" 
					Или ИмяТипа = "ДокументHTML" 
					Или ИмяТипа = "ТекстHTML" 
					Или ИмяТипа = "КомментарийHTML" 
				Тогда
					Продолжить;
				КонецЕсли;
				НоваяСтрока = МассивОбщихТипов.Добавить();
				НоваяСтрока.ИмяОбщегоТипа = ИмяТипа;
			КонецЦикла;
		КонецЕсли;
	ИначеЕсли СтруктураТипа.ИмяОбщегоТипа = "УзелHTML" Тогда
		Если Не ирКэш.ЛиПортативныйРежимЛкс() Тогда
			СтруктураТипа = СтруктураТипаИзКонкретногоТипа(Тип("ОбработкаОбъект.ирТипУзелHTML"));
			НоваяСтрока = МассивОбщихТипов.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, СтруктураТипа);
		КонецЕсли;
		//Если ДоступноКэширование Тогда
		//	//МассивОбщихТипов.Очистить();
		//	СтруктураКлюча = Новый Структура("Слово, ЯзыкПрограммы", "СписокУзловHTML", 0);
		//	ИменаТиповУзловХТМЛ = ирОбщий.СтрРазделитьЛкс(ТаблицаОбщихТипов.НайтиСтроки(СтруктураКлюча)[0].ТипЭлементаКоллекции, ",", Истина);
		//	Для Каждого ИмяТипа Из ИменаТиповУзловХТМЛ Цикл
		//		НоваяСтрока = МассивОбщихТипов.Добавить();
		...
		//	КонецЦикла;
		//КонецЕсли;
	ИначеЕсли СтруктураТипа.ИмяОбщегоТипа = "УзелDOM" Тогда
		Если Не ирКэш.ЛиПортативныйРежимЛкс() Тогда
			СтруктураТипа = СтруктураТипаИзКонкретногоТипа(Тип("ОбработкаОбъект.ирТипУзелDOM"));
			НоваяСтрока = МассивОбщихТипов.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, СтруктураТипа);
		КонецЕсли;
		//Если ДоступноКэширование Тогда
		//	//МассивОбщихТипов.Очистить();
		//	СтруктураКлюча = Новый Структура("Слово, ЯзыкПрограммы", "СписокУзловDOM", 0);
		//	ИменаТиповУзловХТМЛ = ирОбщий.СтрРазделитьЛкс(ТаблицаОбщихТипов.НайтиСтроки(СтруктураКлюча)[0].ТипЭлементаКоллекции, ",", Истина);
		//	Для Каждого ИмяТипа Из ИменаТиповУзловХТМЛ Цикл
		//		НоваяСтрока = МассивОбщихТипов.Добавить();
		//		НоваяСтрока.ИмяОбщегоТипа = ИмяТипа;
	...
	ИначеЕсли СтруктураТипа.ИмяОбщегоТипа = "ЛюбаяСсылка" Тогда
		...
		Для Каждого СтрокаСлова Из ТаблицаКонтекстов.НайтиСтроки(Отбор) Цикл
			НоваяСтрока = МассивОбщихТипов.Добавить();
			ИмяОбщегоТипа = СтрокаСлова.ТипЗначения; 
			Если ЯзыкПрограммы = 1 Тогда
				ИмяОбщегоТипа = СтрЗаменить(ИмяОбщегоТипа, "Ссылка.", ".");
			КонецЕсли;
			НоваяСтрока.ИмяОбщегоТипа = ИмяОбщегоТипа;
		КонецЦикла;
	ИначеЕсли СтруктураТипа.ИмяОбщегоТипа = "СсылочныйОбъект" Тогда
		...
		КонецЦикла;
	ИначеЕсли Ложь
		Или СтруктураТипа.ИмяОбщегоТипа = "СправочникСсылка" 
		Или СтруктураТипа.ИмяОбщегоТипа = "ДокументСсылка" 
		Или СтруктураТипа.ИмяОбщегоТипа = "ПеречислениеСсылка" 
	Тогда
		// Когда сюда приходим? Вроде бы ТаблицаТиповИзТекста нормализует эти имена и они сюда уже не доходят. Наверное в описании платформы иногда такие типы встречаются и приходим из СловаКонтекстаПредопределенные.
		СтрокаОбщегоТипа = ОписаниеОбщегоТипаПоБазовому(СтруктураТипа.ИмяОбщегоТипа);
		Если СтрокаОбщегоТипа <> Неопределено Тогда
			НоваяСтрока = МассивОбщихТипов.Добавить();
			ИмяОбщегоТипа = СтрокаОбщегоТипа.Слово; 
			Если ЯзыкПрограммы = 1 Тогда
				ИмяОбщегоТипа = СтрЗаменить(ИмяОбщегоТипа, "Ссылка.", ".");
			КонецЕсли;
			НоваяСтрока.ИмяОбщегоТипа = ИмяОбщегоТипа;
...
Процедура ОчиститьКэшАнализатораЯзыка() Экспорт 
	Если мСвязанныйКонфигуратор <> Неопределено Тогда
		Для Каждого КлючИЗначение Из мМодулиМетаданных Цикл
			Модуль = КлючИЗначение.Значение; // см. МодульМетаданных
			ЗаписатьМодульВКэш(Модуль,, Истина); 
		КонецЦикла;
	КонецЕсли;
	мМодулиМетаданных.Очистить();
	мПустойМодуль = Неопределено;
	мПоляТекстаПрограммы.Очистить();
	мВызовыВсехСловПоМодулям.Очистить();
	мКэшСловГлобальногоКонтекста.Очистить();
	мПассивныеФормы.Очистить();
	мПрямыеВызовыМетодов.Очистить();
	ОбновитьПовторноИспользуемыеЗначения(); // Для ирКэш.ПолеТекстаМодуляЛкс
КонецПроцедуры
...
Функция СловаКонтекстаПредопределенные(Знач РодительскаяСтруктураТипа, Знач ПараметрыЗаполнения, КлючКэширования = Неопределено) Экспорт 
	...
	МассивОбщихТипов = РазвернутаяТаблицаТиповИзСтруктурыТипа(РодительскаяСтруктураТипа, ЭтоЛокальныйКонтекст, Истина, Не ЗначениеЗаполнено(ТаблицаСлов), ЯзыкПрограммы);
	...
	Для Каждого СтрокаОбщегоТипа Из МассивОбщихТипов Цикл
		...
		Тогда
			...
			КлючКэширования = КлючКэшированияСловИзОбщегоТипа(ИмяОбщегоТипаРодителяБезДеталей) + ";" + НаСервере + ";" + ДляЗаписи + ";" + ПараметрыЗаполнения.ТолькоСоЗначениями;
		...
		МетаданныеРодителя = СтрокаОбщегоТипа.Метаданные;
		Если Ложь
			Или ЛиМетаданныеОбработкиОписанияТипа(МетаданныеРодителя)
			Или (Истина
		...
		Для Каждого СтрокаСлова Из НайденныеСтроки Цикл
			...
			Если Ложь
				...
				Или (Истина
					...
					И (Ложь
						...
						Или ФлагиКомпиляции.КлиентУправляемоеПриложение И Не ФлагиКомпиляции.НеТонкийКлиент И СтрокаСлова.НеТонкийКлиент))
			Тогда
				Продолжить;
			КонецЕсли;
			...
			Если ВычислятьТипыЦикл Тогда
				...
				ИначеЕсли Истина                                    
					...
					//И Не ирОбщий.СтрНачинаетсяСЛкс(ИмяОбщегоТипаРодителяБезДеталей, "КоллекцияКолонок") // Закомментировал, т.к. ломает расчет возможных значений ТаблицаЗначений.Колонки.Удалить("")
					И Не (Истина
				...
				Для Каждого ИмяОбщегоТипаСлова Из МассивТиповЗначения Цикл
					...
					ИначеЕсли ИмяОбщегоТипаСлова = "Отбор" Тогда 
						...
						КонецЕсли;  
					ИначеЕсли Ложь
						Или ИмяОбщегоТипаСлова = "Число"
						Или ИмяОбщегоТипаСлова = "Булево"
						Или ИмяОбщегоТипаСлова = "Дата"
					Тогда
						// Например ТаблицаЗначений.Количество(), Структура.Свойство()
						СтруктураТипа.ДержательМетаданных = СтруктураТипа.Метаданные;
						СтруктураТипа.Метаданные = Неопределено;
					КонецЕсли;
					...
					Тогда
						Если Ложь 
							Или (Истина
								...
								И ИмяОбщегоТипаРодителяБезДеталей <> "ЭлементОтбора" // Для защиты от передачи построителя в значение
								)
							...
							Или ИмяОбщегоТипаРодителяБезДеталей = "ТаблицаЗначений" // Против захвата колонок с именами Индексы/Колонки из структуры
							Или ИмяОбщегоТипаРодителяБезДеталей = "ДеревоЗначений" // Против захвата колонок с именами Индексы/Колонки из структуры
						Тогда
						...
						Иначе
							Если Истина
								...
								#КонецЕсли 
								И (Ложь
									Или СловоЦикл = "ТекущийРодитель" // Антибаг платформы 8.2.16. Вывести в чистом виде не удалось. Падает при вычислении свойства ТекущийРодитель в форме списка РС.ООП_ВаучерыТез
									Или ИмяОбщегоТипаРодителяБезДеталей = "ПараметрыПеретаскивания" И СловоЦикл = "Значение" // Против проброса ТабличноеПоле.Значение->ПараметрыПеретаскивания.Значение
									)
							Тогда
							...
							ИначеЕсли мМассивТиповСМетаданными.Найти(ТипЗнч(ЗначениеСвойства)) <> Неопределено Тогда 
								...
								Иначе
									...
									Если СтрокаСлова.ТипЗначения = "Произвольный" Тогда
										СтруктураТипа.ИмяОбщегоТипа = ИмяОбщегоТипаИзТипа(ТипЗнч(ЗначениеСвойства));
									КонецЕсли;
								КонецЕсли;
					...
					Тогда
						СтруктураТипа.ИмяОбщегоТипа = ОписаниеОбщегоТипаПоБазовому(РодительскаяСтруктураТипа.ИмяОбщегоТипа).ТипЭлементаКоллекции;
						ДобавитьВТаблицуТипов(ТаблицаТипов, СтруктураТипа);
						Продолжить;
					ИначеЕсли Истина
					...
					ИначеЕсли Истина
						...
						//И МетаданныеРодителя <> Неопределено // Пришлось отключить чтобы не образовывалась двойная вложенность таблицы типов в случае слияния ТЧ/ТЗ.НайтиСтроки()
					Тогда         
						...
						СтруктураТипа = ТаблицаТиповИзТекста[0];
						СтруктураТипа.ИмяОбщегоТипа = "Массив";
						//! СтруктураТипа.Метаданные = 0 // см. НоваяТаблицаТипов
						Если МетаданныеРодителя <> Неопределено Тогда
							СтруктураТипа.Метаданные.ЗаполнитьЗначения(МетаданныеРодителя, "Метаданные"); // TODO проверить подобные места, чтобы там обновлялась детальность
							СтруктураТипа.Метаданные.ЗаполнитьЗначения(РодительскаяСтруктураТипа.ДержательМетаданных, "ДержательМетаданных"); // Для Запрос.ВыполнитьПакет()
							Для Каждого СтрокаТипаЭлемента Из СтруктураТипа.Метаданные Цикл
					...
					Тогда
						...
						Для Каждого КлючИЗначение Из ДочерниеСвойстваДанныхФормы(МетаданныеРодителя,, РодительскаяСтруктураТипа.ДержательМетаданных, СловоФильтр) Цикл
							...
							Если ТипЗнч(ОписаниеТипов) = Тип("ТаблицаЗначений") Тогда
								ОписаниеТипов = ОписаниеТиповИзТаблицыТипов(ОписаниеТипов);
							КонецЕсли;
							ТаблицаЗначений.Колонки.Добавить(КлючИЗначение.Ключ, ОписаниеТипов);
						КонецЦикла;
		...
		Пока Истина
			...
			И ТаблицаСлов[0].Слово = СловоФильтр
...
Функция ЛиМетаданныеОбработкиОписанияТипа(ОбъектМД) Экспорт
	Результат = Истина
		И ТипЗнч(ОбъектМД) = Тип("ОбъектМетаданных")
		И ирОбщий.СтрНачинаетсяСЛкс(ОбъектМД.Имя, "ирТип");
	Возврат Результат;
КонецФункции
...
Функция СловаКонтекстаМетаданные(Знач РодительскаяСтруктураТипа, Знач ПараметрыЗаполнения, КлючКэширования = Неопределено) Экспорт 
	...
	ЭтоЛокальныйКонтекст = ПараметрыЗаполнения.ЭтоЛокальныйКонтекст;
	ЯзыкПрограммы = ПараметрыЗаполнения.ЯзыкПрограммы; 
	ДляОтображения = ПараметрыЗаполнения.ДляОтображения;
	Вытеснять = ПараметрыЗаполнения.Вытеснять;
	Если РодительскаяСтруктураТипа.ТипЯзыка = "ЗначениеВЗапросе" Тогда
		ЯзыкПрограммы = 1;
	КонецЕсли;
	...
	//	Если Истина // Мультиметка260213_092040
	//		И Найти(ирОбщий.ПервыйФрагментЛкс(КонкретныйТип, "["), "<") > 0 
	//		И Найти(РодительскаяСтруктураТипа.Метаданные, ".") = 0
	//	Тогда
	//		// Такой конкретный тип не разрешен
	//		Возврат ТаблицаСлов; // Но тогда будет обнаруживаться ошибка, например <СправочникСсылка>.Договор  
	//	КонецЕсли;
	//КонецЕсли;
	...
	Если ВычислятьТипыМетодовМодулей = Неопределено Тогда
		ВычислятьТипыМетодовМодулей = ВычислятьТипы И СловоФильтр <> Неопределено;
	КонецЕсли;
	Если СловоФильтр <> Неопределено И ТекущийИндекс <> Неопределено Тогда 
		...
		Если Не ЛиСтруктура Тогда
			...
			Если ОписаниеОбщегоТипа <> Неопределено Тогда
				Если ОписаниеОбщегоТипа.ТипЭлементаКоллекции = "КлючИЗначение" Тогда  
					Если Истина
					...
					Возврат ТаблицаСлов;
				ИначеЕсли ОписаниеОбщегоТипа.ТипТипа = "Перечисление" Тогда  
					ТаблицаТипов = ДобавитьВТаблицуТипов(ТаблицаТипов, НоваяСтруктураТипа(ирОбщий.ТекстБезПервогоФрагментаЛкс(ОписаниеОбщегоТипа.Слово, "Перечисление")));
					ДобавитьВТаблицуСлов(ТаблицаСлов, "*", "Свойство", ТаблицаТипов,, "Метаданные1");
					Возврат ТаблицаСлов;
				КонецЕсли;
			КонецЕсли;
	...
	ТаблицаОбщихТиповСтруктурыТипа = РазвернутаяТаблицаТиповИзСтруктурыТипа(РодительскаяСтруктураТипа, ЭтоЛокальныйКонтекст, Ложь,, ЯзыкПрограммы);
	ЕдинственнаяСтрокаВида = Неопределено;
	ЕдинственнаяСтрокаГотова = Ложь;
	ПроверятьДоступностьВКонтекстеКомпиляции = ПроверятьДоступностьВКонтекстеКомпиляции(ФлагиКомпиляции, РодительскаяСтруктураТипа, ЭтоЛокальныйКонтекст);
	Для Каждого СтрокаОбщегоТипа Из ТаблицаОбщихТиповСтруктурыТипа Цикл
			...
			Если РодительскаяСтруктураТипа.ТипЯзыка <> "ИмяТипа" Тогда 
				...
				Если Ложь 
					...
					Или (Истина
						И ЭтоИнтеграция
						И (Ложь
							Или ТипМетаданныхРодителя = Тип("СправочникСсылка.МетаданныеИис") //@skip-check unknown-method-property
							Или ТипМетаданныхРодителя = Тип("СправочникСсылка.СвойстваМетаданныхИис"))) //@skip-check unknown-method-property
				Тогда
					Если ТипМетаданныхРодителя = Тип("ОбъектМетаданных") Тогда
						КорневойТипРодителя = ирОбщий.КорневойТипКонфигурацииЛкс(МетаданныеРодителя);
					Иначе
		...
		Для Каждого СтрокаСлова Из НайденныеСтроки Цикл
			...
			//	Продолжить; 
			//КонецЕсли;
			Если СтрокаСлова.Слово = "<Имя реквизита>" Тогда
				// Мультиметка260213_092040
				ЕдинственнаяСтрокаГотова = Истина;
				ЕдинственнаяСтрокаВида = СтрокаСлова;
			ИначеЕсли ЕдинственнаяСтрокаВида = Неопределено Тогда
				ЕдинственнаяСтрокаВида = СтрокаСлова;
			ИначеЕсли Не ЕдинственнаяСтрокаГотова Тогда
			...
			МетаданныеЭлементов = Новый Соответствие; // Нужно для перехода к определению
			...
			Если НайденныеСтрокиКоллекций.Количество() > 0 Тогда
				...
				Если НайденныеСтрокиКоллекций.Количество() > 1 Тогда
					...
					Тогда 
						// Устранение неоднозначности одноименных типов "ЭлементыФормы" обычной и управляемой формы
			...
			Если СтрокаОписанияВида = Неопределено Тогда
				...
				Если НайденныеСтроки.Количество() = 1 Тогда
					//СтрокаОбщегоТипаПолная = ОписаниеОбщегоТипаПоБазовому(ИмяОбщегоТипаРодителя);
					Если Истина
						И Найти(ИмяОбщегоТипаРодителя, ".") = 0
						И Найти(ИмяОбщегоТипаРодителя, "<") = 0
						И ТипЗнч(МетаданныеРодителя) = Тип(ИмяОбщегоТипаРодителя)
					Тогда
						//КоллекцияЗначений = МетаданныеРодителя; // Тогда еще и СтрокаОписанияВида придется создавать
						СхемаКоллекции = МетаданныеРодителя;
					КонецЕсли;
				КонецЕсли;
				Если Истина
					И КоллекцияЗначений = Неопределено 
			...
			ИначеЕсли Истина
				...
				И ТипЗнч(МетаданныеРодителя) = Тип("ОбъектМетаданных") //
			Тогда
				СхемаКоллекции = ирКэш.ИменаПредопределенныхЭлементовЛкс(МетаданныеРодителя.ПолноеИмя(), ДляОтображения);
			ИначеЕсли Истина
			...
			ИначеЕсли Истина
				...
				И ТипЗнч(МетаданныеРодителя) = Тип("ОбъектМетаданных")
				И (Ложь
					Или Найти(ИмяОбщегоТипаРодителя, ".")
					Или ИмяОбщегоТипаРодителя = "СтрокаТабличнойЧасти")
			Тогда
			...
			Тогда
				...
				Для Каждого ОбщийРеквизит Из ОбщиеРеквизиты Цикл
					Если ирОбщий.ЛиОбщийРеквизитПодключенВОбъектеМетаданныхЛкс(ОбщийРеквизит, МетаданныеРодителя) Тогда
			...
			Тогда
				...
				КонецПопытки;
				Если Истина
					И ЗначениеРеквизита <> Неопределено 
				...
				Тогда
					...
					МетаданныеЭлементов = МетаданныеРодителя.Реквизиты;
				Иначе
					СхемаКоллекции = МетаданныеРодителя.Реквизиты;
				КонецЕсли;
			ИначеЕсли Истина
			...
			ИначеЕсли Истина
				...
				И ТипЗнч(РодительскаяСтруктураТипа.СтрокаОписания) = Тип("СтрокаТаблицыЗначений")
				И РодительскаяСтруктураТипа.СтрокаОписания.Владелец().Колонки.Найти("ТипКонтекста") <> Неопределено
			...
			ИначеЕсли СтрокаОписанияВида.ИмяКоллекции = "ПодчиненныеЭлементы" Тогда
				...
				Если ирОбщий.ЛиФормаИлиИмитаторЛкс(ФормаИлиИмитатор, Ложь) Тогда
					...
					Если СлужебныеДанные.Свойство("Элементы") Тогда
						...
						КонецЦикла;
						Если ЛиТолькоТипы Тогда
							СхемаКоллекции["_"] = 1; // Чтобы описания типов в значении у всех элементов считались как тип элемента
						КонецЕсли;
					КонецЕсли; 
				КонецЕсли;
			#КонецЕсли
			...
			Тогда
				...
				КонецЕсли;
				Если Прав(СловоФильтр, 1) = "_" И СтрДлина(СловоФильтр) > 1 Тогда
			...
			ИначеЕсли Истина
				...
				И ЭтоИнтеграция
				И (Ложь
					Или ТипЗнч(МетаданныеРодителя) = Тип("СправочникСсылка.КонфигурацииМетаданныхИис") //@skip-check unknown-method-property
					Или ТипЗнч(МетаданныеРодителя) = Тип("СправочникСсылка.МетаданныеИис")) //@skip-check unknown-method-property
			Тогда
				...
				Если ЗначениеЗаполнено(ИмяКоллекцииОбъектовМетаданных) Тогда
					...
					Если СтрокаКорневогоТипа <> Неопределено Тогда
						...
						Если ИмяКоллекцииОбъектовМетаданных = "Реквизиты" Тогда
							...
							|ГДЕ КонфигурацияМетаданных = &Владелец И Вид <> ЗНАЧЕНИЕ(Перечисление.ВидыСвойствМетаданныхИис.Свойство)";  //@skip-check unknown-method-property
						...
						Иначе
							...
							|ГДЕ КонфигурацияМетаданных = &Владелец И ТипМетаданных.ИмяМножественное = &ИмяРодителя"; //@skip-check unknown-method-property
			...
			Иначе
				...
				КонецЕсли; 
			КонецЕсли;
			ДобавитьЭлементыКоллекцииВТаблицуСлов(РодительскаяСтруктураТипа, ВычислятьТипы, ИмяОбщегоТипаРодителя, МетаданныеРодителя, СловоФильтр, ТаблицаСлов, ЭтоЛокальныйКонтекст, ЯзыкПрограммы, ИмяТекущегоСлова,
				ИмяЭлементаКоллекции, КоллекцияЗначений, ЛиТипСКвалификаторами, МетаданныеЭлементов, ПостроительЗапросаРодителя, СтрокаОписанияВида, СтрокаСлова, СхемаКоллекции, ИмяКолонкиФлагаТаблицыТипов, Вытеснять);
		...
		Тогда
			...
			Иначе
				...
				Если Истина
					...
					И МетаданныеРодителя <> Неопределено
					И (Ложь
						Или ТипЗнч(МетаданныеРодителя) <> Тип("Строка") // Если включено, то блокирует доступ к модулю свеже добавленного объекта в конфигураторе
						Или ИмяОбщегоТипаРодителя <> "Строка")
					И МетаданныеРодителя <> "?"
					И (Ложь
						Или МодульМетаданныхКомпонента = Неопределено 
						Или (Истина 
							// Для вычисления колонок по модулю объекта в модуле формы в вызове из ВычислитьТипДочернегоЭлемента
							И МодульМетаданныхКомпонента = мПустойМодуль
							И ЗначениеЗаполнено(СловоФильтр)
							И Найти(ИмяОбщегоТипаРодителя, "Объект.")
							И РодительскаяСтруктураТипа.ИмяОбщегоТипа <> ИмяОбщегоТипаРодителя)
						)
				Тогда
				...
				Если МодульМетаданныхКомпонента <> Неопределено Тогда
					...
					КонецЕсли;
					Если МодульМетаданныхКомпонента <> мПустойМодуль Тогда 
						Если Ложь
					...
					Если Ложь
						...
						Или МетаданныеРодителя <> Неопределено
					Тогда
						...
						Для Каждого ВидимоеРасширение Из ВидимыеРасширения Цикл
							...
							Если Истина
								И МодульМетаданныхРасширение <> Неопределено 
								И МодульМетаданныхРасширение <> мПустойМодуль
								И МодульМетаданныхРасширение <> МодульМетаданныхКомпонента 
							Тогда
								ТаблицаСлов = ДобавитьВТаблицуСловЭлементыМодуляМетаданных(СтрокаОбщегоТипа, СловоФильтр, ТипСловаФильтр, ТаблицаСлов, МодульМетаданныхРасширение, ВычислятьТипыМетодовМодулей, ВычислятьТипы,
									Не ЭтоЛокальныйКонтекст, ФлагиКомпиляции, ВнутриГруппыОбщихМодулей, ДляСвойства, ЛиВместеСЛокальнымКонтекстом,, ТолькоСоЗначениями);
							КонецЕсли;
						КонецЦикла;
					КонецЕсли;
				КонецЕсли;
	...
	Пока Истина
		И СловоФильтр <> Неопределено
		И ТаблицаСлов.Количество() > 1
		И ТаблицаСлов[0].Слово = "<>"
		И ТаблицаСлов[0].ТипСлова = ПеречТипСлова.Метод
	Цикл
		// Например, модуль обычной формы + модуль объекта, модуль расширения + модуль базовый
		ТаблицаСлов.Удалить(0);
	КонецЦикла;
	Если Истина
		И СловоФильтр <> Неопределено
	...
	Тогда 
		...
		Для Каждого ТипЗначения Из МассивТиповЗначения Цикл
			...
			ИначеЕсли Ложь 
				Или ТипЗнч(МетаданныеРодителя) = Тип("ОбъектМетаданных") 
				Или ирОбщий.ИмяТипаМДЕдинственноеИзМножЛкс(ИмяОбщегоТипаРодителя) <> Неопределено 
				Или ЯзыкПрограммы > 0 И ирОбщий.ИмяТипаМДМножественноеИзЕдинЛкс(ИмяОбщегоТипаРодителя) <> Неопределено // Опасно. В языке 0 могло захватывать тип "Форма"
			...
			ИначеЕсли Истина
				И ЕдинственнаяСтрокаВида.Слово = "<Имя элемента управления>" 
				И ТипЗнч(МетаданныеРодителя) = Тип("Структура")
				И Не МетаданныеРодителя.Свойство("Элементы")
			Тогда
				// Такая ошибка не нужна 
			Иначе
...
Процедура ДобавитьЭлементыКоллекцииВТаблицуСлов(Знач РодительскаяСтруктураТипа, Знач ВычислятьТипы, Знач ИмяОбщегоТипаРодителя, Знач МетаданныеРодителя, Знач СловоФильтр, Знач ТаблицаСлов, Знач ЭтоЛокальныйКонтекст,
		Знач ЯзыкПрограммы, Знач ИмяТекущегоСлова, Знач ИмяЭлементаКоллекции, Знач КоллекцияЗначений, Знач ЛиТипСКвалификаторами, Знач МетаданныеЭлементов, Знач ПостроительЗапросаРодителя, Знач СтрокаОписанияВида,
		Знач СтрокаСлова, Знач СхемаКоллекции, Знач ИмяКолонкиФлагаТаблицыТипов, Знач Вытеснять = Истина) Экспорт
	Если КоллекцияЗначений = Неопределено Тогда
	...
	Для Каждого ЭлементКоллекции Из КоллекцияЗначений Цикл
		...
		Если ВычислятьТипыЦикл Тогда
			...
			Тогда
				ТаблицаТипов = ЗначениеСвойства;
				Если РодительскаяСтруктураТипа.ДержательМетаданных <> Неопределено Тогда
					ТаблицаТипов.ЗаполнитьЗначения(РодительскаяСтруктураТипа.ДержательМетаданных, "ДержательМетаданных"); // нужно для ВыгрузкаРезультатаЗапроса.НайтиСтроки()[0].Колонка1
				КонецЕсли;
			Иначе
				ТаблицаТипов = НоваяТаблицаТипов();
				...
				Тогда
					...
					ПередаватьМетаданныеРодителя = Истина
						...
						И ИмяОбщегоТипаРодителя <> "ВременнаяТаблица"
						...
						И СтрокаСлова.Слово <> "<Имя реквизита>" // Чтобы в реквизиты формы не передавалась форма
					...
					Если МассивТипов.Количество() > МаксЧислоТиповДляАнализа Тогда
						...
						Если Ложь
							Или ЯзыкПрограммы = 1
							Или Найти(СтруктураТипаСвойства.ИмяОбщегоТипа, "Ссылка.") 
						Тогда
							ИмяНабораТипов = ирОбщий.ИмяНабораТиповИзОписанияТиповЛкс(ОписаниеТиповЗначения, Ложь);
							Если ЗначениеЗаполнено(ИмяНабораТипов) Тогда
								СтруктураТипаСвойства.ИмяОбщегоТипа = ИмяНабораТипов;
							КонецЕсли;
						КонецЕсли;
						...
						Для Каждого Тип Из МассивТипов Цикл
							...
							Тогда
								...
								Если МетаданныеЭлемента <> Неопределено Тогда
									ШаблонСтруктурыТипа.МетаданныеСвойства = МетаданныеЭлемента;
								КонецЕсли;
				...
				Иначе
					...
					ИначеЕсли Не ЭтоЛокальныйКонтекст Тогда // Защита от наследования расширения между общим модулем расширения к общему модулю конфигурации
		...
		Иначе
			//
		КонецЕсли;
		ДобавитьВТаблицуСлов(ТаблицаСлов, КонечноеСлово, СтрокаСлова.ТипСлова, ТаблицаТипов, "??" + ТипЗначенияСлова, "Метаданные", Вытеснять,,,, ТипЗначенияСлова);
	КонецЦикла;
КонецПроцедуры
...
// Возвращаемое значение:
//    - 
Функция ЛиСловоНесовместимоСФлагамиКомпиляции(СтрокаСлова, ФлагиКомпиляции) Экспорт
	// Мультиметка251205_002737
	Возврат Ложь
		Или ФлагиКомпиляции.Сервер И СтрокаСлова.НеСервер
		Или ФлагиКомпиляции.КлиентОбычноеПриложение И СтрокаСлова.НеТолстыйКлиент
...
// Возвращаемое значение:
//   Структура - 
//     *Значение - см. НоваяТаблицаТипов, Неопределено - 
Функция СхемаКоллекцииПараметровФормы(Знач РодительскаяСтруктураТипа, Знач Слово = Неопределено) Экспорт
	...
	Если ТипЗнч(МетаданныеРодителя) = Тип("ДанныеФормыСтруктура") Тогда
		...
		КонецЕсли;
	Иначе
		Если ТипЗнч(РодительскаяСтруктураТипа.ДержательМетаданных) = Тип("Структура") Тогда
		...
		ИначеЕсли ТипЗнч(МетаданныеРодителя) = Тип("Строка") Тогда
			СтруктураТипаФормы = НоваяСтруктураТипа(МетаданныеРодителя);
		КонецЕсли;
		...
		ИначеЕсли ТипЗнч(МетаданныеРодителя) = Тип("Структура") Тогда
			// Вредно. Для неуспешных отсутствующих в кэше форм будет возвращать структуру с ИмяФормы и другими служебными свойствами
			//СхемаКоллекции = МетаданныеРодителя;
		КонецЕсли;
	КонецЕсли;
	...
	Если СтруктураТипаФормы <> Неопределено Тогда
		...
		Если МодульФормы <> Неопределено Тогда
			...
			Если МетодМодуля <> Неопределено Тогда
				...
				СтруктураИзКода = АнализаторМодуля.СобратьСвойстваСтруктуры("Параметры",,,,,, Слово, АнализаторМодуля.ИмяФиктивногоСвойства(),,, Истина);
				Если СтруктураИзКода <> Неопределено Тогда
					...
					Для Каждого СтрокаСлова Из АнализаторМодуля.ТаблицаСлов Цикл
						Если Не СхемаКоллекции.Свойство(СтрокаСлова.Слово) Тогда
...
Функция ТаблицаТиповИзИмитатораЭлементаФормы(Знач СтруктураЭлемента, Знач ИмитаторФормы, Знач Слово) Экспорт
	...
	Если Истина
		...
		И СтруктураЭлемента.Тип <> Неопределено
	Тогда
...
Функция СловаКонтекстаМетаданныеCOM(Знач Слово, Знач ТипСлова, Знач ИмяОбщегоТипаРодителя, Знач МетаданныеРодителя, Знач ТаблицаСлов, Знач НаборыСлов, КлючКэширования) Экспорт
	...
	Тогда
			...
			Иначе
				...
				Для Каждого ИнфоТипа Из МассивИнфоТипа Цикл
					Для Каждого Член Из ИнфоТипа.Members Цикл
						...
						СтруктураТипа.Метаданные = МетаданныеРодителя;
						ТаблицаТипов = ДобавитьВТаблицуТипов(, СтруктураТипа);
						ДобавитьВТаблицуСлов(ТаблицаСловОтдельная, ИмяЧлена, ТипСловаЧленаКласса, ТаблицаТипов,, "Метаданные",,, Истина);
						Если Истина    
		...
		Если ЗначениеЗаполнено(Слово) Тогда
			Если Истина
				И ТаблицаСловОтдельная.Количество() > 1 
				И ЗначениеЗаполнено(ТипСлова)
			Тогда
				// Например defaultView {HTMLFile}
				ОтобранныеТипомСлова = ТаблицаСловОтдельная.Скопировать(Новый Структура("ТипСлова", ТипСлова));
				Если ОтобранныеТипомСлова.Количество() Тогда
					ТаблицаСловОтдельная = ОтобранныеТипомСлова;
				КонецЕсли;
			КонецЕсли;
...
Функция ДобавитьВТаблицуСловЭлементыГлобальныхМодулей(Знач СтрокаОбщегоТипа, Знач Слово = Неопределено, Знач ТипСлова = Неопределено, Знач ВычислятьТипы = Истина, Знач ВычислятьТипыМетодовМодулей = Ложь,
	...
	Если ПустаяСтрока(ТипСлова) Или ТипСлова = ПеречТипСлова.Метод Тогда
		...
		Для Каждого ОбщийМодульМД Из ирКэш.ГлобальныеОбщиеМодулиЛкс(НаСервере, КлиентОбычноеПриложение, КлиентУправляемоеПриложение) Цикл
			СтруктураТипаМодуля = НоваяСтруктураТипа("ОбщийМодуль");
		...
		Если ТипыЭлементовКоллекции.Количество() > 0 Тогда
			...
			Для Каждого БазовыйТипЭлемента Из ТипыЭлементовКоллекции Цикл
				...
				Если КонкретныйТип = Неопределено И НужнаПроверкаТипов Тогда
					СтрокаОбщегоТипа = ОписаниеОбщегоТипаПоБазовому(БазовыйТипЭлемента, ЯзыкПрограммы);
					Если СтрокаОбщегоТипа <> Неопределено Тогда
						СтруктураТипаЭлемента.СтрокаОписания = СтрокаОбщегоТипа;
						СтруктураТипаЭлемента.ИмяОбщегоТипа = СтрокаОбщегоТипа.Слово;
...
Функция ДобавитьВременныеТаблицыПараметровВТекстЗапроса(Знач ТекстЗапроса, Знач Параметры, Знач ТекстыЗапросов = Неопределено, Знач КлючПакета = 0) Экспорт
	...
	Для Каждого КлючИЗначение Из Параметры Цикл
		ЗначениеПараметра = ТаблицаИзОписанияПараметраЗапроса(КлючИЗначение.Значение);
...
Функция ТаблицаИзОписанияПараметраЗапроса(Знач ЗначениеПараметра, выхОписаниеТипов = Неопределено) Экспорт
	Если Истина
	...
	Тогда
		...
		Если ЛиДетальностьТиповДостаточна(ЗначениеПараметра, 4, Истина, ЛучшийТип) Тогда  // 4 - захватывает ТЧ
			ЗначениеПараметра = ЛучшийТип.Метаданные;
			Если ТипЗнч(ЗначениеПараметра) = Тип("ОбъектМетаданных") Тогда
				Если Найти(ЛучшийТип.ИмяОбщегоТипа, "Ссылка.") Тогда
				ИначеЕсли Найти(ЛучшийТип.ИмяОбщегоТипа, "ТабличнаяЧасть.") Тогда
					// Отдельная ветка, т.к. у некоторых нет отражения в БД
					ЗначениеПараметра = ирОбщий.УстановитьМетаданныеКоллекцииЛкс(ЗначениеПараметра);
				ИначеЕсли Найти(ЛучшийТип.ИмяОбщегоТипа, ".") Тогда
					ЗначениеПараметра = ирОбщий.ПустаяТаблицаИзТаблицыБДЛкс(ирКэш.ИмяТаблицыИзМетаданныхЛкс(ЗначениеПараметра.ПолноеИмя()));
				КонецЕсли;
			КонецЕсли;
		Иначе
...
Процедура ПодготовитьМетаданныеПостроителяЗапроса(Знач ПостроительЗапроса, Знач ДержательМетаданных = Неопределено, Знач ТекстыЗапросов = Неопределено) Экспорт  
	...
	Если ПостроительЗапроса.ВыбранныеПоля.Количество() = 0 И ирКэш.ДоступноСхемаЗапросаЛкс() Тогда
		...
		КонецЕсли;
		Попытка
			ПустаяТаблица = ирОбщий.ПустаяТаблицаИзТекстаЗапросаЛкс(Текст,, Истина);
		Исключение
...
Функция ДобавитьВТаблицуСловЭлементыМодуляМетаданных(Знач СтруктураТипаМини, Знач СловоФильтр = Неопределено, Знач ТипСловаФильтр = Неопределено, Знач ТаблицаСловПриемник = Неопределено,
	...
	ЛиСеансТолстогоКлиентаУП = ирКэш.ЛиСеансТолстогоКлиентаУПЛкс();
	КлиентУправляемоеПриложение = ирОбщий.СвойствоСтруктурыЛкс(ФлагиКомпиляции, "КлиентУправляемоеПриложение", ЛиСеансТолстогоКлиентаУП);
	НеТонкийКлиент = ирОбщий.СвойствоСтруктурыЛкс(ФлагиКомпиляции, "НеТонкийКлиент", Не ЛиСеансТолстогоКлиентаУП);
	Если Истина
	...
	Тогда
		...
		КлиентОбычноеПриложение = ирОбщий.СвойствоСтруктурыЛкс(ФлагиКомпиляции, "КлиентОбычноеПриложение", Не ЛиСеансТолстогоКлиентаУП);
		КлючКэша = "" + ТолькоЭкспорт + ";" + БезПараметров + ";" + ТолькоСоЗначениями + ";" + НаСервере + ";" + БезКонтекста + ";" + КлиентОбычноеПриложение + ";" + КлиентУправляемоеПриложение + ";" + НеТонкийКлиент;
	...
	КонецЕсли;
	Если МодульМетаданных.Свойство("Родители") Тогда
		Для Каждого ТипРодителя Из МодульМетаданных.Родители Цикл
			ТаблицаТиповРодителей = ирКэш.ПолеТекстаПрограммы().ТаблицаТиповИзТекста(ТипРодителя);
			ПараметрыЗаполнения = НовыеПараметрыЗаполненияСлов(ТипСловаФильтр, , , , , ВычислятьТипы, ВнутриГруппыОбщихМодулей, , СловоФильтр, , ВычислятьТипыМетодовМодулей, ТаблицаСлов, , , СтрогийФильтр, ,
				ТолькоСоЗначениями, , , ДляСвойства, ЛиВместеСЛокальнымКонтекстом,,,, Ложь);
			Для Каждого СтруктураТипа Из ТаблицаТиповРодителей Цикл
				ТаблицаСлов = ТаблицаСловИзСтруктурыТипа(СтруктураТипа, ПараметрыЗаполнения);
			КонецЦикла;
		КонецЦикла;
	КонецЕсли;
		...
		Для Каждого ПеременнаяМодуля Из ПодходящиеПеременные Цикл
			...
			Если ВычислятьТипы Тогда
				Если Истина
					...
					//И ПеременнаяМодуля.ПозицияОпределения > 0 // так отбросим все специальные переменные модуля
				Тогда
	...
	Тогда 
		...
		КонецЕсли;
	КонецЕсли; 
	Если КлиентУправляемоеПриложение И Не НеТонкийКлиент Тогда
		ОтборСловМодуля.Вставить("НеТонкийКлиент", Ложь);
	КонецЕсли;
	Если Ложь
		Или ТипСловаФильтр = Неопределено
	...
	Тогда
		...
		Для Каждого МетодМодуля Из ПодходящиеМетоды Цикл
			...
			Если Ложь 
				...
				Или Лев(ИмяМетода, 1) = "<" И ПустаяСтрока(СловоФильтр) 
			Тогда
				Продолжить;
			КонецЕсли;
...
Функция ДобавитьВТаблицуСловЭлементМодуляМетаданных(Знач ТаблицаСлов, Знач Слово, Знач ВычислятьТипы, Знач ТаблицаИзКэша, Знач ТаблицаТипов, Знач ТипЗначения, Знач ТипСлова,
	...
	Если Лев(Слово, 1) <> "<" И ТипЗначения <> Неопределено Тогда
		Если Истина
			И ВычислятьТипы 
		...
		Тогда
			...
			Если НовыйТекст <> Неопределено Тогда 
				...
				Если СтарыйТекст = Неопределено И ФайлМодуля.Существует() Тогда
					ТекстовыйДокумент.Прочитать(ФайлМодуля.ПолноеИмя, КодировкаТекста.UTF8);
				...
				Если СтарыйТекст <> НовыйТекст Тогда
					...
					Если ПустаяСтрока(НовыйТекст) Тогда
						#Если Клиент Тогда
							ДобавитьСообщениеВнешнемуКлиенту("Пустой текст помещен в кэш модуля " + выхИмяМодуля);
						#КонецЕсли
					КонецЕсли;
			...
			Тогда
				МодульМетаданных = ирОбщий.ЗагрузитьЗначениеИзФайлаЛкс(ФайлОписанияМодуля.ПолноеИмя,, "Внутр", Ложь); // см. МодульМетаданных() - временно читаем без вывода ошибок для перехода между форматами сериализации
			...
			Если ОбновитьФайлОписания Тогда
				Если Не ФайлМодуля.Существует() И НовыйТекст = Неопределено Тогда
					Если СообщениеОНеобходимостиОбновитьКэшМодулейВыводилось <> Истина Тогда
						...
						Если ФайлыМодулей.Количество() = 0 Тогда
							#Если Клиент Тогда
								ирКлиент.ОткрытьНастройкиПоляПрограммыЛкс();
							#КонецЕсли
						КонецЕсли; 
				...
				Иначе 
					Если НовыйТекст = Неопределено Тогда
						...
						МодульМетаданных = МодульМетаданных(НовыйТекст, СтруктураТипа, выхИмяМодуля);
						ЗаписатьМодульВКэш(МодульМетаданных, Ложь);
					Иначе
						СтарыйМодуль = МодульМетаданных; 
						МодульМетаданных = МодульМетаданных(НовыйТекст, СтруктураТипа, выхИмяМодуля, Ложь, МодульМетаданных, ТекущаяПозиция);
			...
			Если МодульМетаданных <> Неопределено И СтарыйМодуль <> МодульМетаданных Тогда
				...
				Тогда
					...
					Для Каждого СтрокаСлова Из ТаблицаСлов Цикл // TODO добавить фильтр по типу 
						...
						Если СтрокаСлова.ТаблицаТипов <> Неопределено Тогда
							МетаданныеСвойства = СтрокаСлова.ТаблицаТипов[0].МетаданныеСвойства;
							Если ТипЗнч(МетаданныеСвойства) = Тип("ОбъектМетаданных") Тогда
								СтрокаПеременной.Описание = "-" + МетаданныеСвойства.ПолноеИмя(); // Мультиметка260316_083443
							КонецЕсли;
						КонецЕсли;
...
Функция ВерсияФорматаКэшаМодулей() Экспорт
	Возврат 30; // Мультиметка91237881041 Каждый раз при изменении структуры увеличиваем на 1
КонецФункции
...
// Возвращает таблицу вызовов слова через точку.
// Параметры:
//    ИмяМодуля - Строка - 
...
Функция НепрямыеВызовыСловаВМодуле(Знач ИмяМодуля, Знач ТекстовыйДокумент, Знач Файл, Знач ПолеТекстаПрограммы, Знач ИскатьСлово, Знач ЛиМетод = Истина) Экспорт
	...
	Если ИндексВызововМодуля = Неопределено Тогда
		...
		Иначе
			...
			ИмяФайлаИндекса = ФайлМодуляИзИмениМодуля(ИмяМодуля, Расширение).ПолноеИмя;
...
Процедура ДобавитьВхожденияСловаВИндекс(Знач Вхождения, Знач РегВыражение, Знач ТаблицаВызовов, Знач СВложенными = Ложь, Знач Смещение = 0, Знач ИндексГруппыВШаблоне = 0) Экспорт
			...
			Если СВложенными Тогда
				...
				Иначе 
					...
					Пока ЧислоТочек > 0 Цикл
						...
						Текст = ирОбщий.ТекстБезПоследнегоФрагментаЛкс(Текст);
						Слово = ирОбщий.ПоследнийФрагментЛкс(Текст);
						Если СтрЧислоВхождений(Слово, ")") > СтрЧислоВхождений(Слово, "(") Тогда
							// Точка внутри () - пропускаем все внутри скобок
							Текст = ирОбщий.ПервыйФрагментЛкс(Текст, "(") + "(";
							Слово = ирОбщий.ПоследнийФрагментЛкс(Текст,, Ложь);
						КонецЕсли;
						Если СтрЧислоВхождений(Слово, "]") > СтрЧислоВхождений(Слово, "[") Тогда
							// Точка внутри []
							Текст = ирОбщий.ПервыйФрагментЛкс(Текст, "[");
						...
						КонецЕсли;
						Если Найти(Слово, "(") Или Слово = "" Тогда
							Продолжить;
						КонецЕсли;
	...
	Иначе
		// Однострочный код использован для ускорения при разрешенной отладке. Выше расположен оригинал. Выполняйте изменения синхронно в обоих вариантах.
		Для Каждого Вхождение Из Вхождения Цикл  			СтрокаРезультата = ТаблицаВызовов.Добавить();  			СтрокаРезультата.ПозицияВхождения = Смещение + Вхождение.FirstIndex;  			СтрокаРезультата.ДлинаВхождения = Вхождение.Length;  			СтрокаРезультата.Слово = Вхождение.SubMatches(ИндексГруппыВШаблоне);  			Если СВложенными Тогда  				Если Вхождение.SubMatches.Count > ИндексГруппыПараметровВШаблоне Тогда  					ПараметрыВызова = Вхождение.SubMatches(ИндексГруппыПараметровВШаблоне);  					Если Найти(ПараметрыВызова, "(") > 0 Тогда  						ВхожденияВложенные = РегВыражение.НайтиВхождения(" " + ПараметрыВызова,, Истина);  						ДобавитьВхожденияСловаВИндекс(ВхожденияВложенные, РегВыражение, ТаблицаВызовов, Истина, Вхождение.FirstIndex + Найти(Вхождение.Value, ПараметрыВызова) - 2, ИндексГруппыВШаблоне);  					КонецЕсли;  				Иначе  					Если Вхождение.FirstIndex > 0 Тогда  						Текст = Сред(Вхождение.Value, 2);  						СмещениеДлины = 1;  					Иначе  						Текст = Вхождение.Value;  						СмещениеДлины = 0;  					КонецЕсли;  					ЧислоТочек = СтрЧислоВхождений(Текст, ".");  					Пока ЧислоТочек > 0 Цикл  						ЧислоТочек = ЧислоТочек - 1;  						Текст = ирОбщий.ТекстБезПоследнегоФрагментаЛкс(Текст);  						Слово = ирОбщий.ПоследнийФрагментЛкс(Текст);  						Если СтрЧислоВхождений(Слово, ")") > СтрЧислоВхождений(Слово, "(") Тогда    							Текст = ирОбщий.ПервыйФрагментЛкс(Текст, "(") + "(";  							Слово = ирОбщий.ПоследнийФрагментЛкс(Текст,, Ложь);  						КонецЕсли;  						Если СтрЧислоВхождений(Слово, "]") > СтрЧислоВхождений(Слово, "[") Тогда    							Текст = ирОбщий.ПервыйФрагментЛкс(Текст, "[");  							Слово = ирОбщий.ПоследнийФрагментЛкс(Текст);  						Иначе  							Слово = ирОбщий.ПервыйФрагментЛкс(Слово, "[");  						КонецЕсли;  						Если Найти(Слово, "(") Или Слово = "" Тогда  							Продолжить;  						КонецЕсли;  						СтрокаВложенная = ТаблицаВызовов.Добавить();  						СтрокаВложенная.ПозицияВхождения = СтрокаРезультата.ПозицияВхождения;  						СтрокаВложенная.ДлинаВхождения = СтрДлина(Текст) + СмещениеДлины;  						СтрокаВложенная.Слово = Слово;  					КонецЦикла;  				КонецЕсли;  			КонецЕсли;  		КонецЦикла;  
	КонецЕсли;
КонецПроцедуры
...
Функция ИмяМетодаВнеМетодов() Экспорт
	Возврат "<ВнеМетодов>";
КонецФункции

// Записывает кэш модуля из памяти в файл.
// Параметры:
//    МодульМетаданных - см. ПодготовитьМодульМетаданных - 
//    ЧерезКопию - Булево - гарантирует успех, т.к. очишает все несериализуемые данные
Процедура ЗаписатьМодульВКэш(Знач МодульМетаданных, Знач ЧерезКопию = Истина, Знач ТолькоЕслиОтсутствует = Ложь) Экспорт 
	Если МодульМетаданных = мПустойМодуль Тогда
		Возврат;
	КонецЕсли;  
	ФайлОписанияМодуля = ФайлМодуляИзИмениМодуля(МодульМетаданных.Имя, "ixd");
	Если Истина
		И ТолькоЕслиОтсутствует
		И ФайлОписанияМодуля.Существует()
	Тогда
		Возврат;
	КонецЕсли;
	Если ЧерезКопию Тогда
		...
		КопияМодуля.Методы.ЗаполнитьЗначения(Неопределено, "КэшПоиска, ТелоБезВозвратов, СтарыеТипы, ЗависимыеМетоды");      
...
Функция ФайлМодуляИзИмениМодуля(Знач ИмяМодуля, Знач Расширение = "txt", Знач ЗаменитьНедопустимыеСимволы = Ложь) Экспорт    
	ФайлМодуля = мКэшФайловПоИменамМодулей[ИмяМодуля+Расширение];
	Если ФайлМодуля = Неопределено Тогда
		ИмяФайла = ИмяФайлаМодуляИзИмениМодуля(ИмяМодуля, Расширение, ЗаменитьНедопустимыеСимволы);
		ФайлМодуля = Новый Файл(ИмяФайла);
		мКэшФайловПоИменамМодулей.Вставить(ИмяМодуля+Расширение, ФайлМодуля);
	КонецЕсли;
	Возврат ФайлМодуля;
КонецФункции
...
Функция ИмяФайлаМодуляИзИмениМодуля(Знач ИмяМодуля, Знач Расширение = "txt", Знач ЗаменитьНедопустимыеСимволы = Ложь) Экспорт
	...
	КонецЕсли;
	ИмяФайла = ПапкаКэшаМодулей.ПолноеИмя + мРазделительПутиКФайлу + ИмяМодуля + "." + Расширение;
	Возврат ИмяФайла;
КонецФункции
...
Функция ИмяМодуляИзСтруктурыТипа(Знач СтруктураТипа, ТипМодуля = "", СтрокаСозданияРодителя = "", ФлагиКомпиляции = Неопределено) Экспорт 
	...
	Иначе
		ФлагиКомпиляции = НовыеФлагиКомпиляции(,, Ложь);
	КонецЕсли;
	...
	Тогда
		...
		ФлагиКомпиляции.КлиентУправляемоеПриложение = Истина;
	...
	ИначеЕсли ирОбщий.ЛиИмяТипаФормыЛкс(ИмяОбщегоТипа, Истина) Тогда 
		...
		Тогда
			...
			ФлагиКомпиляции.КлиентОбычноеПриложение = Истина;
			ФлагиКомпиляции.КлиентУправляемоеПриложение = Ложь;
		...
		ИначеЕсли ирОбщий.ЛиФормаИлиИмитаторЛкс(МетаданныеРодителя, Ложь) Тогда
			//! МетаданныеРодителя = 0; // ФормаКлиентскогоПриложения
			МетаданныеРодителя = ирОбщий.ПолноеИмяФормыЛкс(МетаданныеРодителя);
			ФлагиКомпиляции.КлиентОбычноеПриложение = Ложь;
			ФлагиКомпиляции.КлиентУправляемоеПриложение = Истина;
		КонецЕсли;
		Если ТипЗнч(МетаданныеРодителя) <> Тип("Строка") Тогда
	...
	Тогда
		...
		//! МетаданныеРодителя = 0; // см. ОбработкаОбъект.ирКлсПолеТекстаПрограммы.ИмитаторОбъектаМД      
	...
	Если Не ЗначениеЗаполнено(Результат) Тогда
		Если ЗначениеЗаполнено(ТипМодуля) Тогда
			...
			Тогда
				Результат = ирОбщий.ПервыйФрагментЛкс(СтруктураТипа.ДержательМетаданных, "Объект.") + "." + ирОбщий.ПоследнийФрагментЛкс(Результат, "::");
			КонецЕсли;
		ИначеЕсли ТипЗнч(МетаданныеРодителя) = Тип("Строка") Тогда
	...
	Тогда 
		...
		КонецЕсли;
	КонецЕсли;
	Если Результат <> "" И ТипЗнч(СтруктураТипа.ДержательМетаданных) = Тип("Строка") Тогда 
...
Функция ОбновитьКэшСтруктурыФормыИзБуфера(Знач СлужебныеДанные, Знач ТекстБуфераОбмена, Знач ТекстМодуляФормы = "", Знач Форма = Неопределено) Экспорт 
	...
	Попытка                          
		...
		КонецЕсли;
		ТекстФайла = СтрЗаменить(ТекстФайла, ирОбщий.ТекстВВыражениеВстроенногоЯзыкаЛкс(ТекстМодуляФормы, Ложь, Ложь), ""); /// Делаем безусловно, т.к. внутри могут быть поломки при конвертации в XML
	...
	Исключение
		...
		Если Не ЛиОбычнаяФорма И ЗначениеЗаполнено(ТекстБуфераОбмена) Тогда
			ТекстСообщения = "Ошибка разбора снимка формы " + ирОбщий.ПолноеИмяФормыЛкс(СлужебныеДанные) + ": " + ОписаниеОшибки;
			ПроверитьТекстОшибкиРазбораСнимкаФормы(ТекстСообщения);
			ирОбщий.СообщитьЛкс(ТекстСообщения);
			#Если Клиент Тогда
				ДобавитьСообщениеВнешнемуКлиенту(ТекстСообщения);
			#КонецЕсли
		КонецЕсли;
		Возврат Ложь;
	КонецПопытки;
...
// Возвращаемое значение:
//   Булево - если изменен текст
Функция ПроверитьТекстОшибкиРазбораСнимкаФормы(ТекстСообщения) Экспорт
	Если Найти(ТекстСообщения, "Opening and ending") Тогда
		ТекстСообщения = ТекстСообщения + " В текстах формы не допускаются непарные фигурные скобки";
		Возврат Истина;
	КонецЕсли;
	Возврат Ложь;
КонецФункции
...
Функция ИмитаторУправляемойФормыИзДокументаДОМ(Знач ДокументDOM, Знач Форма = Неопределено) Экспорт
	...
	Пока УзелDOM <> Неопределено Цикл 
		...
		Если ОписаниеТипов = Новый ОписаниеТипов("ДинамическийСписок") Тогда
			...
			//УзелОсновнойТаблицы = УзелДинамическийСписок.ДочерниеУзлы[5];
			//ИД = УзелОсновнойТаблицы[1].ТекстовоеСодержимое; // Идентификатор типа УникальныйИдентификатор
			//ИД = УзелОсновнойТаблицы[2].ТекстовоеСодержимое; // Идентификатор объекта метаданных
			//ДинамическийСписок.ОсновнаяТаблица = ТипЗначенияИзУзлаФормы(УзелДинамическийСписок.ДочерниеУзлы[5]);
			УзелДинамическийСписок.ДочерниеУзлы[3].УдалитьДочерний(УзелДинамическийСписок.ДочерниеУзлы[3].ПервыйДочерний);
			ДинамическийСписок.ТекстЗапроса = ТекстИзМногострочногоЛитералаУзлаДОМ(УзелДинамическийСписок.ДочерниеУзлы[3].ПервыйДочерний);
	...
	Для Счетчик = 0 По 8 Цикл
		...
		Пока УзелDOM <> Неопределено Цикл 
			...
			СписокУзлов = УзелDOM.РодительскийУзел.РодительскийУзел.ДочерниеУзлы;
			Если СписокУзлов.Количество() <= 12 Тогда
				УзлыПутиКДанным = Новый Массив;
			Иначе 
				ВерхнийУзелПутиКДанным = СписокУзлов[12]; // - Режим совместимости 8.3.18+
				Если ВерхнийУзелПутиКДанным.ИмяУзла <> "e" Тогда
					ВерхнийУзелПутиКДанным = СписокУзлов[11]; // - Режим совместимости 8.3.12-
				КонецЕсли;
				УзлыПутиКДанным = ВерхнийУзелПутиКДанным.ДочерниеУзлы;
			КонецЕсли;
...
//.
// Параметры:
//   ТекущийУзел - УзелDOM - первый узел литерала
//   выхСледующийУзел -  - 
//   ЛиКорень -  - 
// Возвращаемое значение:
//    - 
Функция ТекстИзМногострочногоЛитералаУзлаДОМ(Знач ТекущийУзел, выхСледующийУзел = Неопределено, Знач ЛиКорень = Истина) Экспорт
	Результат = "";
	// Мультиметка260214_084229 Компенсируем разбивку литерала на куски символами ",{}" в ирОбщий.СтрокаВнутрВХМЛТелоЛкс
	Если Лев(ТекущийУзел.ТекстовоеСодержимое, 1) = """" Или Не ЛиКорень Тогда
		Фрагменты = Новый Массив;
		ЧислоКавычек = 0;
		ПредУзел = Неопределено;
		Пока Истина Цикл
			Если ТекущийУзел.ДочерниеУзлы.Количество() < 2 Тогда
				Если ТекущийУзел.ИмяУзла = "e" Тогда
					Фрагмент = "{" + ТекстИзМногострочногоЛитералаУзлаДОМ(ТекущийУзел.ПервыйДочерний, выхСледующийУзел, Ложь) + "}";
				Иначе
					Фрагмент = ТекущийУзел.ТекстовоеСодержимое;
				КонецЕсли;
			Иначе
				Фрагмент = ТекстИзМногострочногоЛитералаУзлаДОМ(ТекущийУзел.ПервыйДочерний, выхСледующийУзел, Ложь);
			КонецЕсли;
			Если Истина
				И ПредУзел <> Неопределено
				И ПредУзел.ИмяУзла = "d"
				И ТекущийУзел.ИмяУзла = "d"
			Тогда
				Фрагменты.Добавить(",");
			КонецЕсли;
			Фрагменты.Добавить(Фрагмент);
			ЧислоКавычек = ЧислоКавычек + СтрЧислоВхождений(Фрагмент, """");
			ТекущийУзел = ТекущийУзел.СледующийСоседний;
			Если Ложь
				Или Прав(Фрагмент, 1) = """" И ЧислоКавычек % 2 = 0
				Или ТекущийУзел = Неопределено
			Тогда
				Прервать;
			КонецЕсли;
			ПредУзел = ТекущийУзел;
		КонецЦикла;
		Если Фрагменты.Количество() = 1 Тогда
			Результат = Фрагмент;
		Иначе
		...
		КонецЕсли;
		Если ЛиКорень Тогда
			Результат = ирОбщий.ТекстИзВстроенногоЯзыкаЛкс(Результат,, Ложь);
		КонецЕсли;
	КонецЕсли;
...
Функция НовыеФлагиКомпиляции(Знач Сервер = Истина, Знач КлиентОбычноеПриложение = Истина, Знач КлиентУправляемоеПриложение = Истина, Знач БезКонтекста = Ложь) Экспорт
	...
	ФлагиКомпиляции.Вставить("НеТонкийКлиент", Ложь); // Применяется только при КлиентУправляемоеПриложение
	ФлагиКомпиляции.Вставить("ЕстьЛокальноеУсловие", Ложь);
...
Функция ПолучитьРазделыМодуля1С(Знач ТекстМодуля = "") Экспорт
	...
	Если Истина
		И ПозицияНачалаПервогоМетода < ПозицияКонцаПоследнегоМетода 
		И ПозицияКонцаПоследнегоМетода <= СтрДлина(ТекстМодуля) // Разрешаем выйти на 1 за размер текста
	Тогда
...
шСтруктураКомментаВозвр = "Возвращаемое значение(?:\s*:\s*)?((([^\n]\n)*|.|\n)*)(?:\n\s*Параметры\s*\:?\s*\n|$)";
шЭкспорт = "(?:Экспорт|Export)";
шПерем = "(?:Перем|Var)";
шВозврат = "(?:Возврат|Return)\s+(.+?)(?:;| КонецЕсли|\n|\r|$)";
шСтруктураКомментаОписаниеПараметров = "(" + шИмя + ")(?:\s*[-–])([^\n]*(?:\n|$)(\s{4}[^\n]*(?:\n|$))*)";
шКомментМетода = "((?:[ \f\r\t\v]*//[^\n]*\n)+)" + шМетодНачало + шРазделитель + "+(" + шИмя + ")";
шОписаниеПеременной = "(" + шИмя + ")(" + шРазделитель + "+"+шЭкспорт+")?";
шСтрокаПеременныхБезКомментов = шПерем + "((?:" + шРазделитель + "*" + шОписаниеПеременной + шРазделитель + "*[;,])+)";
шСтрокаПеременных = "" + шПустоеНачалоСтроки + шСтрокаПеременныхБезКомментов + "([ \f\r\t\v]*" + шБлокКомментов + ")?";
шПараметрыМетода = "" + шРазделитель + "*((?:Знач|Val)\s)?" + шРазделитель + "*(" + шИмя + ")\s*=?((?:\s*(" + шЛитералПрограммы + ")|(?:[^,\n/]*))+)(" + шКомментарий + ")?(?:(?:\n|"+шРазделитель+")+|,|$)";
шДирективаИлиАннотация = "(?:" + шПустоеНачалоСтроки + ")?" + шПустоеНачалоСтроки + "[#&][^\n]*(?=\n|$)";
шМетодЧистый = "([\t ]*(?:(?:Асинх|Async)\s+)?(" + шМетодНачало + ")" + шРазделитель + "+(" + шИмя + ")" + шРазделитель + "*\(([^\)]*)\)(" + шРазделитель + "*"+шЭкспорт+")?(?:[\t ]*(" + шКомментарий + "))?(?:\s*?(?=\n))?(?:\n\s*"+шПерем+" .*(?=\n))*)"
	+ "((?:" + шБлокКомментов + "|\." + шМетодКонец + "|(?:\r|\n|.)(?!" + шМетодКонец + "(?![^\n]*(?:""|\n\s*\|))))*)(?:$|[^а-яё_a-z0-9\.""]" + шМетодКонец + "(?=(?:" + шКомментарий + "|[^а-яё_a-z0-9\.""]|$)))";
...
// {ОписаниеРегулярногоВыражения.Конец}
	...
	Результат.Вставить("ОписаниеПараметра", ".*(?:\n(?:[/]{2,})?(?:(?:[\t ]{" + МинДлинаОтступаВнутриОписанияПараметра() + ",}|[\t ]+[*\-]).*|[\t ]*(?=\n)))*");
	мШаблоныДляАнализаВстроенногоЯзыка = Результат;
    Возврат Результат;
КонецФункции

// Функция - Модуль метаданных. Для пустого текста и НЕ пустого имени возвращает единый модуль мПустойМодуль!
// Параметры:
//  ТекстМодуля								 - Строка	 - 
//  СтруктураТипа							 - см. НоваяСтруктураТипа -
//  ИмяМодуля								 - Строка	 - 
...
// Возвращаемое значение:
//  см. МодульМетаданных() - таким объявлением отключаем расчет с фактическими параметрами
Функция МодульМетаданных(Знач ТекстМодуля, Знач СтруктураТипа = Неопределено, Знач ИмяМодуля = "", Знач ПодготовитьОписанияРезультатовМетодов = Истина, Знач СтарыйМодуль = Неопределено,
	...
	Если РазрешитьЕдиныйПустойМодуль И мПустойМодуль <> Неопределено Тогда
		// Опасно! Для ускорения заполнения списка в Обработка.ирКлсПолеТекстаПрограммы.Форма.МетодыМодулей
		Возврат мПустойМодуль;
	КонецЕсли; 
	Если СтарыйМодуль = мПустойМодуль Тогда
		СтарыйМодуль = Неопределено;
	КонецЕсли;
	...
	Тогда     
		...
		Если ИндексТекущегоМетода = -1 Тогда
			...
			Для Каждого Метод Из МетодыСтарые Цикл
				Если Метод.ДлинаОпределения = 0 Тогда 
					Продолжить;
				КонецЕсли;
		...
		КонецЕсли;
		Если Истина
			И ТекущийМетод <> Неопределено 
			И ТекущийМетод.ПозицияСОписанием > 0
		Тогда   
		...
		Тогда
			...
			Иначе 
				...
				Тогда 
					...
					ЛиКонечныйДиапазонСовпал = Ложь;
				Иначе
					ЛиКонечныйДиапазонСовпал = Истина;
				КонецЕсли;
			КонецЕсли;
	...
	ОбластиКомпиляции.Колонки.Добавить("НеТонкийКлиент", Новый ОписаниеТипов("Булево"));
	...
	Если ЛиНачальныйДиапазонСовпал Тогда
		...
		КонецЕсли;
		Методы = Модуль.Методы;
		Ошибки = Модуль.Ошибки; // ОбработкаТабличнаяЧасть.ирКлсПолеТекстаПрограммы.ОшибкиМодуля, Массив, Неопределено
		ЕстьОшибки = ЗначениеЗаполнено(Ошибки);
		Если ЕстьОшибки И ТекущийМетод <> Неопределено Тогда
		...
		Если ЛиКонечныйДиапазонСовпал Тогда
			...
			Если ТекущийМетод <> Неопределено Тогда
				...
				Методы[0].КэшПоиска = Неопределено;
				Если Истина
					И СтрДлина(ТекстМодуля) > 100000 // Мультиметка260404_210235
					И ИменаМетодовИнициацииБольшогоМодуля().Свойство(ТекущийМетод.Имя) 
				Тогда
					Модуль.ТекстПоискаОпределенияСловМодуля = Неопределено;
					МетодИнициацииМодуля(Модуль).КэшПоиска = Неопределено;
				КонецЕсли;
				ТекущийАдресГруппировки = ТекущийМетод.АдресГруппировки;
				ТекущийАдресКомпиляции = ТекущийМетод.АдресКомпиляции;
				...
				// Сбрасываем кэши структуры модуля, если заголовок метода изменился
				ПозицияВТелеМетода = 10; // Если дописали слово "Экспорт", то чтобы его начало наверняка захватить, берем первые 10 символов тела
				...
				Если Сред(ТекстМодуля, ПозицияНачалаСтарая, ДлинаСверяемойЧасти) <> Сред(СтарыйМодуль.Текст, ПозицияНачалаСтарая, ДлинаСверяемойЧасти) Тогда
					...
					ТекущийМетод.ХешСигнатуры = "";
					Модуль.ТаблицыСлов.Очистить();
					Модуль.ХешСигнатур = "";
					Если ТекущийМетод.ЗависимыеМетоды <> Неопределено Тогда
						Для Каждого ЗависимыеЭлемент Из ТекущийМетод.ЗависимыеМетоды Цикл // СтрокаТаблицыЗначений: см. НоваяТаблицаМетодовМодуля
							ЗависимыеЭлемент.Параметры = ЗависимыеЭлемент.ПараметрыТекст;
						КонецЦикла;
					КонецЕсли;
				КонецЕсли;
				...
				Если Истина
					И ТекущаяПозиция > ТекущийМетод.ПозицияТела 
					И ТипыВыраженийТекущегоМетода <> Неопределено
				Тогда
					...
					ДлинаСверяемойЧасти = ТекущийМетод.ПозицияТела + ПозицияВТелеМетода - ТекущийМетод.ПозицияСОписанием  - 1;
					Если Сред(ТекстМодуля, ПозицияНачалаСтарая, ДлинаСверяемойЧасти) <> Сред(СтарыйМодуль.Текст, ПозицияНачалаСтарая, ДлинаСверяемойЧасти) Тогда
						ДлинаОтступаНазад = 300;
						ПозицияВТелеМетода = ПозицияВТелеМетода - ДлинаОтступаНазад;
						ДлинаСверяемойЧасти = ДлинаСверяемойЧасти - ДлинаОтступаНазад;
						Если Ложь
							Или ПозицияВТелеМетода < 0 
							Или Сред(ТекстМодуля, ПозицияНачалаСтарая, ДлинаСверяемойЧасти) <> Сред(СтарыйМодуль.Текст, ПозицияНачалаСтарая, ДлинаСверяемойЧасти)
						Тогда
							ПозицияВТелеМетода = 0;
						КонецЕсли;
						ПозицияВТелеМетода = ПозицияВТелеМетода - 100; // Учет обрезанного выражения
					КонецЕсли;
					Если ПозицияВТелеМетода > 0 Тогда
						// Шапка и начало тела метода совпали
						...
						КонецЦикла;
						ТекущийМетод.ТипыВыражений = ТипыВыраженийТекущегоМетода; // Из-за массовой очистки выше тут уже будет Неопределено вместо таблицы. Поэтому переустанавливаем ссылку
						//Если ПозицияВТелеМетода = ПозицияПроверкиИзменений - ТекущийМетод.ПозицияТела Тогда
						//	УдаляемыеКлючи = Новый Массив;
						//	Для Каждого КлючИЗначение Из КэшПоискаТекущегоМетода Цикл
						//		ВхожденияКэша = КлючИЗначение.Значение;
						//		Если Ложь
						//			Или ТипЗнч(ВхожденияКэша) <> Тип("ТаблицаЗначений")
						//			Или (Истина
						//				И ВхожденияКэша.Колонки[0].Ширина > 0 
						//				И ВхожденияКэша.Колонки[0].Ширина < ПозицияВТелеМетода)
						//		Тогда
						//			УдаляемыеКлючи.Добавить(КлючИЗначение.Ключ);
						//			Продолжить;
						//		КонецЕсли;
						//		Для Индекс = 1 - ВхожденияКэша.Количество() По 0 Цикл // Обратный обход
						//			Элемент = ВхожденияКэша[-Индекс];
						//			Если Элемент.Позиция > ПозицияВТелеМетода Тогда
						//				ВхожденияКэша.Удалить(-Индекс);
						//			КонецЕсли;
						//		КонецЦикла;
						//		ВхожденияКэша.Колонки[0].Ширина = ПозицияВТелеМетода;
						//	КонецЦикла;
						//	Для Каждого УдаляемыеЭлемент Из УдаляемыеКлючи Цикл
						//		ТипыВыраженийТекущегоМетода.Удалить(УдаляемыеЭлемент);
						//	КонецЦикла;
						//	ТекущийМетод.КэшПоиска = КэшПоискаТекущегоМетода;
						//КонецЕсли;
					Иначе
						// На будущее
						ТипыВыраженийТекущегоМетода.Очистить();
					КонецЕсли;
				КонецЕсли; 
				...
				Если Лев(ТекущийМетод.Имя, 1) <> "<" Тогда
					...
					Если _РежимОтладки Тогда // Можно менять на Истина в точке останова, например условием ирОбщий.Пр(_РежимОтладки, 1, 1)
						...
						Для ИндексМетодаЦикл = ИндексТекущегоМетода + 1 По Методы.Количество() - 1 Цикл
							...
							Если СтрокаМетода.ДлинаОпределения = 0 Тогда
							...
							Если ЕстьОшибки И СтрокаМетода.ЕстьОшибки = Истина Тогда
					...
					Иначе
						// Однострочный код использован для ускорения при разрешенной отладке. Выше расположен оригинал. Выполняйте изменения синхронно в обоих вариантах.
						Для ИндексМетодаЦикл = ИндексТекущегоМетода + 1 По Методы.Количество() - 1 Цикл  					СтрокаМетода = Методы[ИндексМетодаЦикл];  					Если СтрокаМетода.ДлинаОпределения = 0 Тогда  						Продолжить;  					КонецЕсли;  					СтрокаМетода.ПозицияСОписанием = СтрокаМетода.ПозицияСОписанием + СмещениеКонечногоДиапазона;  					СтрокаМетода.ПозицияОпределения = СтрокаМетода.ПозицияОпределения + СмещениеКонечногоДиапазона;  					СтрокаМетода.ПозицияТела = СтрокаМетода.ПозицияТела + СмещениеКонечногоДиапазона;  					Если ЕстьОшибки И СтрокаМетода.ЕстьОшибки = Истина Тогда  						Для Каждого СтрокаОшибки Из Ошибки.НайтиСтроки(Новый Структура("Метод", СтрокаМетода.Имя)) Цикл  							СтрокаОшибки.Позиция = СтрокаОшибки.Позиция + СмещениеКонечногоДиапазона;  						КонецЦикла;  					КонецЕсли;  				КонецЦикла;  
					КонецЕсли;
		...
		Иначе
			// Конечный диапазон не совпал
			СброситьКэшТиповВыраженийМодуля(Модуль, Истина); 
			Модуль.ХешСигнатур = Неопределено;
			Модуль.ТаблицыСлов.Очистить();
			...
			Если ИндексТекущегоМетода > 0 Тогда
				...
				КонецЕсли;
			КонецЕсли;
			УстановитьПрямыеВызовыМетодовМодуля(ИмяМодуля); // TODO Вместо полной очистки можно сделать обновление по второй части модуля
		КонецЕсли;
	...
	Иначе 
		...
		Если СтруктураТипа <> Неопределено Тогда
			// Для указателя на обобщенный метод модуля и глобальных кэшей
	...
	Для Каждого Вхождение Из ВхожденияМетодов Цикл
		...
		Если Заголовок <> Неопределено Тогда
			...
			Если ТекущийМетод <> Неопределено Тогда
				...
				СтароеИмя = СтрокаМетода.Имя; // - после долгого простоя бывает ошибка "строка удалена"
			...
			СтрокаМетода.НеТонкийКлиент = Истина
					И ТекущаяОбластьКомпиляции <> Неопределено
					И ТекущаяОбластьКомпиляции.НеТонкийКлиент;
		...
		ИначеЕсли Вхождение.SubMatches(9) <> Неопределено Тогда
			...
			Если Лев(НоваяАннотация, 1) = "#" Тогда
				...
				Если Модуль.ПозицияПрограммы > ПозицияАннотации Тогда
	...
	Если Модуль.ПозицияМетодов > 30 Тогда
		Родители = Новый Массив;
		МаркерНаследования = ЗаголовокНаследоватьТипы();
		МаркерЗапретаПроверки = "@skip-check";
		Для Счетчик = 1 По 10 Цикл
			Строка = СокрЛП(СтрПолучитьСтроку(ТекстМодуля, Счетчик));
			Если Лев(Строка, 2) <> "//" Тогда
				Прервать;
			КонецЕсли;
			Строка = СокрЛ(Сред(Строка, 3));
			Если Найти(Строка, МаркерНаследования) Тогда
				ТипРодителя = СокрЛП(Сред(Строка, СтрДлина(МаркерНаследования) + 1));
				Родители.Добавить(ТипРодителя);
			КонецЕсли;
			Если Строка = МаркерЗапретаПроверки Тогда
				Модуль.Вставить("ЗапретитьПроверки");
			КонецЕсли;
		КонецЦикла;
		Если Родители.Количество() Тогда
			Модуль.Вставить("Родители", Родители);
		КонецЕсли;
	КонецЕсли;
	...
	КонецЕсли;
	МетодИнициацииМодуля(Модуль, Истина);
	Возврат Модуль;
КонецФункции
...
// Возвращаемое значение:
//    - 
Функция МетодИнициацииМодуля(Знач Модуль, Знач ОбновитьПозицию = Ложь) Экспорт
	ИмяМетодаИнициация = ИмяМетодаИнициация();
	Результат = Модуль.Методы.Найти(НРег(ИмяМетодаИнициация), "НИмя");
	Если Результат = Неопределено Тогда
		Результат = Модуль.Методы.Добавить();
		Результат.ТипыВыражений = Новый Соответствие;
		Результат.Сервер = Истина;
		Результат.Клиент = Истина; 
		Результат.Имя = ИмяМетодаИнициация;
		ирОбщий.ОбновитьКопиюСвойстваВНижнемРегистреЛкс(Результат);
		ОбновитьПозицию = Истина
	КонецЕсли;
	Если ОбновитьПозицию Тогда
		НачальнаяПозиция = Модуль.ПозицияПрограммы;
		Результат.ПозицияСОписанием = НачальнаяПозиция;
		Результат.ДлинаСОписанием = СтрДлина(Модуль.Текст) - НачальнаяПозиция + 1;
		Результат.ПозицияОпределения = Результат.ПозицияСОписанием;
		Результат.ПозицияТела = Результат.ПозицияСОписанием;
		Результат.ДлинаОпределения = Результат.ДлинаСОписанием;
		Результат.ДлинаТела = Результат.ДлинаСОписанием;
	КонецЕсли;
	Возврат Результат;
КонецФункции

Функция ИменаМетодовИнициацииБольшогоМодуля() Экспорт
	ИменаМетодов = Новый Структура;
	ИменаМетодов.Вставить("ПриСозданииНаСервере");
	ИменаМетодов.Вставить("ПередОткрытием");
	ИменаМетодов.Вставить("ПриОткрытии");
	Возврат ИменаМетодов;
КонецФункции

Процедура ОбработатьАннотациюОбласти(Знач НоваяАннотация, Знач ПозицияАннотации, Знач ОбластиГруппировки, ТекущаяОбластьГруппировки, ТекущийАдресГруппировки, Знач ОбластиКомпиляции,
		ТекущаяОбластьКомпиляции, ТекущийАдресКомпиляции)
	...
	ИначеЕсли Найти(НоваяАннотация, "КонецОбласти") = 1 Тогда
		...
		Тогда    
			...
			ТекущийАдресГруппировки = ирОбщий.ТекстБезПоследнегоФрагментаЛкс(ТекущийАдресГруппировки);
		КонецЕсли;
	ИначеЕсли Найти(НоваяАннотация, "Если ") = 1 Тогда
		...
		КонецЕсли;
		НоваяАннотацияНрег = НРег(?(ПустаяСтрока(ТекущийАдресКомпиляции), "", ТекущаяОбластьКомпиляции.Условие + "/") + НоваяАннотация);
		...
		ТекущаяОбластьКомпиляции.НеТонкийКлиент = Ложь
			Или Найти(НоваяАннотацияНрег, "не тонкийклиент")
			Или Найти(НоваяАннотацияНрег, "толстыйклиент");
	ИначеЕсли Найти(НоваяАннотация, "КонецЕсли") = 1 Тогда
		...
		Тогда    
			...
			Если ТекущаяОбластьКомпиляции = ОбластиКомпиляции Тогда
				ТекущаяОбластьКомпиляции = Неопределено;
			КонецЕсли;
			ТекущийАдресКомпиляции = ирОбщий.ТекстБезПоследнегоФрагментаЛкс(ТекущийАдресКомпиляции);
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
// Возвращаемое значение:
//  ТаблицаЗначений:
//    *КэшПоиска - Соответствие Из КлючИЗначение - куку
//      **Значение - см. ОбработкаОбъект.ирОболочкаРегВыражение.НайтиВхождения, см. ОбработкаОбъект.ирКлсПолеТекстаПрограммы.НоваяТаблицаВхожденийКэшаПоиска
Функция НоваяТаблицаМетодовМодуля() Экспорт 
	...
	Методы.Колонки.Добавить("ПараметрыТекст"); // Строка - только для ссылающегося описания параметров
	...
	Методы.Колонки.Добавить("ТипЗначения"); // Строка, Неопределено
	...
	Методы.Колонки.Добавить("НеТонкийКлиент", Новый ОписаниеТипов("Булево")); // - Применяется только при КлиентУправляемоеПриложение
	...
	Методы.Колонки.Добавить("ЕстьОшибки"); // Булево, Неопределено
	...
	Методы.Колонки.Добавить("ЗависимыеМетоды"); // Массив из см. НоваяТаблицаМетодовМодуля[0], Неопределено
	Методы.Колонки.Добавить("АдресГруппировки", Новый ОписаниеТипов("Строка"));
	Методы.Колонки.Добавить("АдресКомпиляции", Новый ОписаниеТипов("Строка"));
	Методы.Колонки.Добавить("ХешСигнатуры", Новый ОписаниеТипов("Строка"));
	Возврат Методы;
				...
				Если ЗначениеЗаполнено(СтрокаМетода.Описание) Тогда
					...
					Если Найти(СтрокаМетода.Описание, ":") = 0 Тогда
						...
						Если УдалитьПрефиксИзТекстаТипов(СсылкаНаОписание) Тогда
							// Ссылка на описание другого метода. Параметры сопоставляются по позиции.
							...
							Иначе
								...
								КонецЕсли;
								КраткоеИмяМодуля = ирОбщий.ТекстБезПоследнегоФрагментаЛкс(СсылкаНаОписание);
								Если ЗначениеЗаполнено(КраткоеИмяМодуля) Тогда
							...
							Если Методы <> Неопределено Тогда
								...
								Если СтрокаБазовогоМетода <> Неопределено Тогда
									Если СтрокаБазовогоМетода.ЗависимыеМетоды = Неопределено Тогда
										СтрокаБазовогоМетода.ЗависимыеМетоды = Новый Массив;
									КонецЕсли;
									СтрокаБазовогоМетода.ЗависимыеМетоды.Добавить(СтрокаМетода);
									СтрокаМетода.ПараметрыТекст = СтрокаМетода.Параметры;
					...
					Если Не ОписанияПараметровЗагружены Тогда
						...
						ПозицияОписания = Найти(СтрокаМетода.Описание, ЗаголовокРазделаПараметры()) - (СтрокаМетода.ПозицияТела - СтрокаМетода.ПозицияСОписанием);
						Для каждого ВхождениеОписания Из ВхожденияОписаний Цикл
							...
							Если СтрокаПараметра <> Неопределено Тогда
								...
								СтрокаПараметра.ПозицияСОписанием = ПозицияОписания + ВхождениеОписания.ПозицияВхождения 
									+ СтрДлина(ВхождениеОписания.Разделитель1) + СтрДлина(ВхождениеОписания.Имя) + СтрДлина(ВхождениеОписания.Разделитель2); /// Позиция описания типов параметра
								...
								КонецЕсли;
								ЧастьСоВторойСтроки = ирОбщий.ТекстБезПервогоФрагментаЛкс(СтрокаПараметра.Описание, Символы.ПС);
								Если Истина
									И ЗначениеЗаполнено(ЧастьСоВторойСтроки)
									И Найти(ЧастьСоВторойСтроки, "-") // хорошо бы и второй вариант тире проверять
								Тогда
									// Если каждый тип описан с новой строки описания, то нужен анализ описания https://www.hostedredmine.com/issues/984526
						...
						КонецЕсли;
					КонецЕсли;
					СтрокаМетода.Описание = СтрЗаменить(СтрокаМетода.Описание, Символы.ПС + "   ", Символы.ПС); // Компенсируем замену "//" на "  ", т.к. позиции уже не важны
				КонецЕсли; 
...
Функция ЗаголовокНаследоватьТипы() Экспорт
	Возврат "Наследовать:";
КонецФункции
...
Функция НоваяТаблицаПараметровМетодаМодуля() Экспорт
	...
	Результат.Колонки.Добавить("ПозицияСОписанием", Новый ОписаниеТипов("Число")); // Начиная с 1. Позиция описания типов.
...
Функция МинДлинаОтступаВнутриОписанияПараметра() Экспорт
	Возврат 8; /// 10 не хватает для ПодключаемыеКомандыПереопределяемый.ПриОпределенииКомандПодключенныхКОбъекту
КонецФункции
...
Функция ПараметрыМетодаИзКомментария(Знач ОписаниеМетода, Шаблоны = Неопределено) Экспорт
	...
	МаксДлинаОтступа = МинДлинаОтступаВнутриОписанияПараметра(); /// Возможно стоит заменить на МинДлинаОтступаВнутриОписанияПараметра()
	шОписаниеПараметра = "(\n[\t ]{0," + МаксДлинаОтступа + "})(" + шИмя + ")(\*?[\t ]*(?:[" + шТире + "][\t ]*)([^" + шТире + "\n]*?))?([\t ]*[" + шТире + "]\s*?" + Шаблоны.ОписаниеПараметра + ")";
...
// В многострочном тексте сохраняет позиции, т.е. заменяет "//" на  "  "
...
// Параметры:
//    СтрокаМетода - СтрокаТаблицыЗначений - 
Процедура ПодготовитьОписаниеРезультатаМетода(Знач СтрокаМетода, Знач ДляКонструктора = Ложь) Экспорт
	СыроеОписание = Неопределено;
	Если Истина
		И ЗначениеЗаполнено(СтрокаМетода.Описание) 
		И СтрокаМетода.ОписаниеРезультата = Неопределено
		И (Ложь
			Или ДляКонструктора
			Или СтрокаМетода.ТипЗначения <> Неопределено)
	Тогда
		СыроеОписание = СтрокаМетода.Описание;
	ИначеЕсли Лев(СтрокаМетода.ОписаниеРезультата, 2) = "//" Тогда
		СыроеОписание = СтрЗаменить(СтрокаМетода.ОписаниеРезультата, "//", "// Результат:");
	КонецЕсли; 
	Если СыроеОписание <> Неопределено Тогда
		...
		ВхожденияОписаний = ирОбщий.НайтиРегВыражениеЛкс(СыроеОписание, ШаблонОписанияРезультата, "ПредОписание, ТипЗначения, Описание", Ложь,, Ложь, Ложь);
		Если ВхожденияОписаний.Количество() > 0 Тогда
			ВхождениеОписания = ВхожденияОписаний[ВхожденияОписаний.Количество() - 1];
			ОписаниеРезультата = СокрЛП(ВхождениеОписания.Описание);
			СтрокаМетода.ОписаниеРезультата = ОчиститьТекстМодуляОтПрефиксаКомментария(ОписаниеРезультата);
			СтрокаМетода.ПозицияОписанияРезультата = ВхождениеОписания.ПозицияВхождения + СтрДлина(ВхождениеОписания.ПредОписание)  
				+ СтрДлина(ВхождениеОписания.Описание) - СтрДлина(ОписаниеРезультата)
				- (СтрокаМетода.ПозицияТела - СтрокаМетода.ПозицияСОписанием);
			Если СтрокаМетода.ТипЗначения <> Неопределено Тогда
				ТекстТипаЗначения = СокрЛП(СтрЗаменить(ВхождениеОписания.ТипЗначения, "//", "")); 
				Если ЗначениеЗаполнено(ТекстТипаЗначения) Тогда
					СтрокаМетода.ПозицияОписанияРезультата = СтрокаМетода.ПозицияОписанияРезультата - СтрДлина(ТекстТипаЗначения) - 1;
					КопияТекстТипаЗначения = ТекстТипаЗначения;
					Если УдалитьПрефиксИзТекстаТипов(КопияТекстТипаЗначения) Тогда
						СтрокаМетода.ТипЗначения = ТекстТипаЗначения;
					Иначе
						СтрокаМетода.ТипЗначения = СтрокаМетода.ТипЗначения + ", " + ТекстТипаЗначения; // TODO удаление дублей
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		Иначе
			СтрокаМетода.ОписаниеРезультата = "";
		КонецЕсли; 
	КонецЕсли;
КонецПроцедуры 
...
Процедура ПодготовитьОписаниеПеременнойМодуля(Знач СтрокаОпределения) Экспорт
	...
	Тогда
		...
		СтрокаОпределения.Описание = СокрЛП(Сред(СтрокаОпределения.Описание, СтрДлина(ТипЗначения) + 1));
...
Функция ПодготовитьТипЗначенияСловаМодуля(Знач ИмяПеременной, Знач ПолеТекстаМодуляКэш, Знач МетодМодуля, Знач ИмяИлиМодуль, Знач СтрокаОпределения, Знач ОписаниеТипа = "",
	...
	Иначе 
		...
		Если Ложь
			...
			Или Лев(СтрокаОпределения.Имя, 1) = "<"
		Тогда
			ТаблицаТипов = СтрокаОпределения.ТаблицаТипов;
		Иначе  
			...
			Попытка
				...
				Тогда
					//ПолеТекстаМодуляКэш.ЗагрузитьМетодМодуля(Неопределено); // Чтобы анализатор не видел тело метода. Но в методе позиция уже отрицательная, поэтому перенес в ПодготовитьТипЗначенияПеременнойМодуля.
					ТаблицаТиповИзКомментария = ПолеТекстаМодуляКэш.ТаблицаТиповИзТекста(СтрокаОпределения.ТипЗначения, ОписаниеТипа,, ДляСвойства,,,, ПозицияОписания, Истина,,, ЗапретитьРасчет);  
				...
				Если Истина
					...
					//И Найти(НРег(СтрокаОпределения.ТипЗначения), РазделительИз()) = 0 // Иначе часть после "из" будет потеряна https://turboconf.ru/Tasks/9668
				Тогда
					СтрокаОпределения.ТипЗначения = ?(ЗапретитьРасчет, "%, ", "") + ПредставлениеМассиваСтруктурТипов(ТаблицаТиповИзКомментария,,,, Истина);
				КонецЕсли;
				Если Ложь
...
// Параметры:
//    ИмяИлиМодуль - Строка, см. МодульМетаданных - 
...
Функция ПредставлениеМассиваСтруктурТипов(Знач ТаблицаТипов, Знач БезДублей = Ложь, Знач МаксКоличествоТипов = 10, Знач Подробное = Истина, Знач БезШаблонов = Ложь) Экспорт 
	...
	Если ТаблицаТипов <> Неопределено Тогда
		...
		ИначеЕсли ТаблицаТипов.Количество() = 1 Тогда
			ПредставлениеТипаКонтекста = ИмяТипаИзСтруктурыТипа(ТаблицаТипов[0], Подробное);
		Иначе
			СписокОбрезан = Неопределено;
			СписокИменТипов = ТаблицаТиповДляПользователя(ТаблицаТипов, БезДублей, МаксКоличествоТипов, СписокОбрезан, Подробное, БезШаблонов);
...
Функция ТаблицаТиповДляПользователя(Знач ТаблицаТипов, Знач БезДублей = Истина, Знач МаксКоличествоТипов = 0, выхСписокОбрезан = Ложь, Знач Подробное = Истина, Знач БезШаблонов = Ложь) Экспорт 
...
//   - 
//
Функция ПараметрыМетодаПлатформы(Знач СтрокаОписания, Знач ТолькоПервойСигнатуры = Ложь) Экспорт 
	...
	Если Не ЗначениеЗаполнено(СтрокаОписания.Описание) Тогда // Мультиметка443985642
		...
		Для Каждого ВариантМетода Из МассивВариантов Цикл
			...
			Если НовыйАдрес <> Неопределено Тогда
				...
				МаксНомерПараметра = СинтаксПомощник.ЗагрузитьОписаниеМетода(ВариантМетода.ТипКонтекста, ВариантМетода.Слово, ТекстДокумента, ВариантМетода.ТипСлова, ВариантМетода.ЯзыкПрограммы,,, ВариантМетода);
			Иначе
	...
	Если ТолькоПервойСигнатуры И СтрокиПараметров.Количество() Тогда
		ОтборПараметров.Вставить("ВариантСинтаксиса", СтрокиПараметров[0].ВариантСинтаксиса); /// Из-за этой добавки не попадаем в индекс, но теперь и полная таблица параметров обычно маленькая из-за динамической подгрузки
		СтрокиПараметров = ТаблицаПараметров.НайтиСтроки(ОтборПараметров);
	КонецЕсли;
	ИндексПараметра = 0;
	Пока Истина
		И МаксНомерПараметра > 0
		И МаксНомерПараметра < СтрокиПараметров.Количество() 
		И СтрокиПараметров[ИндексПараметра].ВариантСинтаксиса = СтрокиПараметров[МаксНомерПараметра].ВариантСинтаксиса
		И СтрокиПараметров[ИндексПараметра].Параметр = СтрокиПараметров[МаксНомерПараметра].Параметр
	Цикл
		// Удаляем дубли параметров виртуальных таблиц регистра бухгалтерии https://www.hostedredmine.com/issues/952173
		ТаблицаПараметров.Удалить(СтрокиПараметров[МаксНомерПараметра]);
		СтрокиПараметров.Удалить(МаксНомерПараметра); 
...
Функция НайтиСтрокуСвойстваПлатформы(Знач ТипКонтекста, Знач Слово, Знач ЯзыкПрограммы = 0) Экспорт
	Возврат Неопределено;
КонецФункции
...
Функция ТипыЭлементовКоллекции(Знач СтруктураТипаКоллекции, Знач ЯзыкПрограммы = 0, выхНужнаПроверкаТипов = Истина) Экспорт
	...
	Тогда
		...
		Иначе
			Результат.Добавить("Произвольный");
		КонецЕсли;
	...
	Тогда 
		...
		Если Истина
			И ТипЗнч(СтруктураТипаКоллекции.СтрокаОписания) = Тип("СтрокаТаблицыЗначений")
			И СтруктураТипаКоллекции.СтрокаОписания.Владелец().Колонки.Найти("Слово") <> Неопределено
			И СтруктураТипаКоллекции.СтрокаОписания.Слово = "ПодчиненныеЭлементы"
		Тогда
			Если ТипЗнч(СтруктураТипаКоллекции.Метаданные) = Тип("Структура") Тогда
				ИмяОбщегоТипаРодителя = ирОбщий.СвойствоСтруктурыЛкс(СтруктураТипаКоллекции.Метаданные, "ИмяТипаРасширения");
			ИначеЕсли мМассивТиповЭлементовУправляемойФормы.Найти(ТипЗнч(СтруктураТипаКоллекции.Метаданные)) <> Неопределено Тогда
				ИмяОбщегоТипаРодителя = СтруктураТипаИзЗначения(СтруктураТипаКоллекции.Метаданные).ИмяОбщегоТипа;
			КонецЕсли;
		КонецЕсли;
		Если ИмяОбщегоТипаРодителя = "РасширениеГруппыФормыДляГруппыКнопок" Тогда
			Результат.Добавить("КнопкаФормы");
		ИначеЕсли ИмяОбщегоТипаРодителя = "РасширениеГруппыФормыДляГруппыКолонок" Тогда
...
Функция ЛиИмяСистемногоПеречисления(Знач ИмяОбщегоТипа) Экспорт
	...
	Если ирОбщий.СтрНачинаетсяСЛкс(ИмяОбщегоТипа, "Перечисление") Тогда
		Возврат Истина;
	КонецЕсли;
	ЛиИмяСистемногоПеречисления = ОписаниеОбщегоТипаПоБазовому("Перечисление" + ИмяОбщегоТипа) <> Неопределено;
...
// Возвращаемое значение:
//   - 
Функция ДобавитьВТаблицуСлов(ТаблицаСлов, Знач Слово, Знач ТипСлова = "Свойство", Знач ТаблицаТипов = Неопределено, Знач ТипЗначения = "??", Знач Определение = "", Знач Вытеснять = Истина,
...
//   ВычислятьТипыМетодовМодулей - Булево, Неопределено - если Неопределено, то ВычислятьТипы И ЗначениеЗаполнено(СловоФильтр) 
//   ТаблицаСлов - см. НоваяТаблицаСлов - 
...
Функция НовыеПараметрыЗаполненияСлов(Знач ТипСловаФильтр = Неопределено, Знач БазовоеРасширениеКонфигурации = "", Знач ЯзыкПрограммы = 0, Знач Конфигурация = Неопределено,
		...
		Знач ЛиВместеСЛокальнымКонтекстом = Ложь, Знач ВиртуальнаяТаблица = Неопределено, Знач ПрименятьКонтракты = Ложь, Знач ДляОтображения = Ложь, Знач Вытеснять = Истина) Экспорт
	Результат = Новый Структура;
	...
	Результат.Вставить("ДляОтображения", ДляОтображения);
	Результат.Вставить("Вытеснять", Вытеснять);
...
Функция ВсеИменаСобытийЭлементовФормы(Знач _ЛиОбычнаяФорма = Ложь) Экспорт
	Если мВсеИменаСобытийЭлементовФормы = Неопределено Тогда
		...
		НайденыСобытияОбычнойФормы = Ложь;
		Для Каждого СтрокаСобытия Из ВсеИменаСобытий Цикл
			//ЛиСобытиеОбычныйФормы = Найти(СтрокаСобытия.ПутьКОписанию, "catalog56/");
			//НайденыСобытияОбычнойФормы = ЛиСобытиеОбычныйФормы Или НайденыСобытияОбычнойФормы;
			Если Ложь
				...
				//Или ЛиОбычнаяФорма И Не ЛиСобытиеОбычныйФормы
				//Или Не ЛиОбычнаяФорма И ЛиСобытиеОбычныйФормы
			Тогда
				Продолжить;
			КонецЕсли;
			мВсеИменаСобытийЭлементовФормы.Вставить(СтрокаСобытия.Слово, СтрокаСобытия);
		КонецЦикла;
		//Если Не НайденыСобытияОбычнойФормы Тогда
		//	ВызватьИсключение "Внутренняя ошибка описания платформы";
		//КонецЕсли;
	КонецЕсли;
	Возврат мВсеИменаСобытийЭлементовФормы;
КонецФункции

Функция ОписаниеОбщегоТипа(Знач ИмяОбщегоТипа, Знач ЯзыкПрограммы = 0, Знач ПоБазовомуТипу = Ложь) Экспорт
	ИмяОбщегоТипа = ирОбщий.ПервыйФрагментЛкс(ИмяОбщегоТипа, "["); // - для результата НайтиФайлы()
	Структура = Новый Структура("ЯзыкПрограммы", ЯзыкПрограммы);
	Если ПоБазовомуТипу Тогда
		Структура.Вставить("БазовыйТип", ИмяОбщегоТипа);
	Иначе
		Структура.Вставить("Слово", ИмяОбщегоТипа);
	КонецЕсли;
	СтрокиОбщегоТипа = ТаблицаОбщихТипов.НайтиСтроки(Структура);
...
Функция ОписаниеОбщегоТипаПоБазовому(Знач БазовыйТип, Знач ЯзыкПрограммы = 0) Экспорт
...
Функция ИмяТипаИзСтруктурыТипа(Знач СтруктураТипа, Знач Подробное = Истина) Экспорт
	...
	ИначеЕсли Истина
		...
		И ОбъектМД.Количество() > 0
		И ОбъектМД.Колонки.Найти(ИмяКолонкиФлагаТаблицыТипов()) <> Неопределено
	Тогда 
		КонкретныйТип = КонкретныйТип + РазделительИз() + ИмяТипаИзСтруктурыТипа(ОбъектМД[0]); // Нужно для описания результата метода https://turboconf.ru/Tasks/9668
	//ИначеЕсли Истина
	//	И (Ложь
...
Функция НоваяТаблицаТипов() Экспорт
	Если мТаблицаТипов = Неопределено Тогда
		...
		//мТаблицаТипов.Колонки.Добавить("СтрокаОписания"); // MemberInfo {TLI.TLIApplication}, см. ТаблицаКонтекстов[0], см. НоваяТаблицаМетодовМодуля()[0] - последний тип не вычисляется из-за цикла зависимости
		мТаблицаТипов.Колонки.Добавить("СтрокаОписания"); // СтрокаТаблицыЗначений, Структура, COMОбъект
		...
		мТаблицаТипов.Колонки.Добавить("ДержательМетаданных"); // В частности для типа "Строка" хранит координаты (возможно надо переделать на СтрокаОписания)
		...
		Если НужноВытеснять Тогда 
			...
			Иначе
				...
				Если СтрокаИсточника.Детальность > 0 Тогда 
					...
					КонецЦикла;
					Если ирОбщий.СтрНачинаетсяСЛкс(СтрокаИсточника.ИмяОбщегоТипа, "РасширениеФормы", Истина) Тогда  // Учитываем регистр для ускорения
	...
	Иначе
		...
		Тогда 
			Если Истина
				...
				И СтрокаПриемника.Метаданные = Неопределено
			Тогда
				СтрокаПриемника.Метаданные = СтрокаИсточника.Метаданные;
				СтрокаПриемника.ДержательМетаданных = СтрокаИсточника.ДержательМетаданных;
			КонецЕсли;
			Возврат Истина;
		КонецЕсли;
		ИмяКолонкиФлагаТаблицыТипов = ИмяКолонкиФлагаТаблицыТипов();
		...
		ИначеЕсли Истина
			И (Ложь
				Или ТипМетаданныхИсточника = Тип("Неопределено")
				Или ТипМетаданныхИсточника = Тип("Null"))
			И Не (Ложь
				Или ТипМетаданныхПриемника = Тип("Неопределено")
				Или ТипМетаданныхПриемника = Тип("Null"))
		Тогда
			СтрокаПриемника.Метаданные = СтрокаИсточника.Метаданные;
			Возврат Истина;
		ИначеЕсли Истина
		...
		Иначе
			...
			Если Истина
				И ТипМетаданныхПриемника = Тип("Структура")
				И (Ложь
					Или ТипМетаданныхИсточника = Тип("ТаблицаЗначений")
					Или ТипМетаданныхИсточника = Тип("ДеревоЗначений"))
			Тогда
			...
			ИначеЕсли Истина
				И ТипМетаданныхИсточника = Тип("Структура")
				И (Ложь
					Или ТипМетаданныхПриемника = Тип("ТаблицаЗначений")
					Или ТипМетаданныхПриемника = Тип("ДеревоЗначений"))
			Тогда 
				ТаблицаИсточник = СтрокаПриемника.Метаданные;
				СтруктураПриемник = СтрокаИсточника.Метаданные; // Структура
			КонецЕсли;
			Если Истина
...
Функция ОбновитьДетальностьСтруктурыТипа(СтруктураТипа) Экспорт 
	...
	ИначеЕсли Ложь 
		...
		Или ТипМетаданных = Тип("УправляемаяФорма") //@skip-check unknown-method-property
		#КонецЕсли
	Тогда
		Результат = 6;
...
Функция ДобавитьСтрокуДереваМД(СтрокаРодителяМД, ПолноеИмяТаблицы, Имя, Представление, ТипТаблицы = "", ИндексКартинки = 0, ЕстьТаблицаБД = Ложь, мТаблицаВсехТаблицБД = Неопределено,
	...
	Если ПолноеИмяТаблицы = Неопределено Тогда
		// перекрыта по имени, например ТЧ.Изменения->Изменения
		Возврат Неопределено;
	КонецЕсли;
...
Функция БазовыйФайлРедактораJSON() Экспорт 
	Если БазовыйФайлРедактораJSON = Неопределено Тогда
		...
		КаталогКомпоненты = "РедакторJSON";
		ДвоичныеДанные = ПолучитьМакет("РедакторJSON");
		...
		БазовыйФайлРедактораJSON = БазовыйКаталог + КаталогКомпоненты + мРазделительПутиКФайлу + "index.html";
		ПолучитьМакет("РедакторJSON_HTML").Записать(БазовыйФайлРедактораJSON);
	КонецЕсли; 
...
Функция БазовыйФайлРедактораКода() Экспорт 
	Если БазовыйФайлРедактораКода = Неопределено Тогда
		БазовыйКаталог = КаталогВременныхФайлов();
		КаталогКомпоненты = "РедакторMonaco";
		ДвоичныеДанные = ПолучитьМакет("РедакторMonaco");
		Чтение = Вычислить("Новый ЧтениеДанных(ДвоичныеДанные)"); // 8.3.9+
		Файл = Новый ЧтениеZipФайла(Чтение.ИсходныйПоток());
		Файл.ИзвлечьВсе(БазовыйКаталог + КаталогКомпоненты);
		...
		//ИмяФайла = БазовыйКаталог + КаталогКомпоненты + мРазделительПутиКФайлу + "decorations.css";
		//ТекстовыйДокумент = Новый ТекстовыйДокумент;
		...
		//|}");
		//ТекстовыйДокумент.Записать(ИмяФайла);
		БазовыйФайлРедактораКода = БазовыйКаталог + КаталогКомпоненты + мРазделительПутиКФайлу + "index.html";
	КонецЕсли; 
...
Функция СтрокаМетодаМодуляПоИмени(Знач МодульМетаданных, Знач ИмяМетода) Экспорт 
	Если Ложь
		...
		//Или ИмяМетода = ИмяМетодаИнициация() // Мешает переходам к определению в динамическом модуле
	Тогда
		Возврат Неопределено;
	КонецЕсли;
...
//     * Ключ - содержит имя реквизита
//     * Значение - ОписаниеТипов, см. НоваяТаблицаТипов - содержит описание типов реквизита
Функция ДочерниеСвойстваДанныхФормы(ДанныеФормы, Знач ЛиДляКоллекции = Неопределено, Знач Форма, Знач ИмяСвойства = "") Экспорт
	...
	Тогда
		Если Ложь
			Или ТипЗнч(ДанныеФормы) = Тип("ДанныеФормыКоллекция") 
			Или ТипЗнч(ДанныеФормы) = Тип("ДанныеФормыКоллекцияЭлементовДерева")
		Тогда
			ДанныеФормы.Очистить();
		ИначеЕсли ТипЗнч(ДанныеФормы) = Тип("ДанныеФормыДерево") Тогда
	...
	Для Каждого СтрокаЭлементаЦикла Из ДеревоВыбора.Строки Цикл
		...
		Тогда
			Если СтрокаДереваВыбора.ТипСлова = "Конструктор" Тогда
				...
				ТекущийВариант = "";
				Если СтрокаЭлементаЦикла.Строки.Количество() > 0 Тогда
					ТекущийВариант = СтрокаЭлементаЦикла.Строки[0].Ключ.Слово;
				КонецЕсли;
				НовыйТекущийВариант = ПодобратьВариантСинтаксисаМетода(ТаблицаВариантов, КоличествоФактПараметровМетода, ТекущийВариант, ЗначениеЗаполнено(ТекущийВариант));
...
Функция НайтиИмяМетодаСтрокиМодуля(Знач ИмяМодуляСжатое, Знач НомерСтроки, Знач СодержаниеСтроки = Неопределено, ТекстыМодулейКэш = Неопределено, СтароеИмяФайлаМодуляБезРасширения = "",
		Знач ЧислоСтрокДляПоискаИмениМетода = 2000, выхСодержаниеСтроки = "", выхСмещениеСтрокиМетода = 0, выхНомерПервойСтрокиМетода = 0) Экспорт
	Если Не ЗначениеЗаполнено(ИмяМодуляСжатое) Тогда
		// Динамический код
		Возврат "";
	КонецЕсли;
	ИмяМодуля = ирКлиент.ИмяМодуляИзСжатогоИмениЛкс(ИмяМодуляСжатое);
	ИмяМетода = НайтиИмяМетодаСтрокиФайлаМодуля(ИмяМодуля, НомерСтроки, СодержаниеСтроки, ТекстыМодулейКэш, СтароеИмяФайлаМодуляБезРасширения, ЧислоСтрокДляПоискаИмениМетода,,, выхСодержаниеСтроки,
		выхНомерПервойСтрокиМетода);
	Возврат ИмяМетода;
КонецФункции

Функция НайтиИмяМетодаСтрокиФайлаМодуля(Знач ИмяМодуля, Знач НомерСтроки, Знач СодержаниеСтроки = Неопределено, ТекстыМодулейКэш = Неопределено, СтароеИмяФайлаМодуляБезРасширения = "",
	Знач ЧислоСтрокДляПоискаИмениМетода = 2000, выхСмещениеСтрокиМетода = 0, Знач ПолеТекстаМодуля = Неопределено, выхСодержаниеСтроки = "", выхНомерПервойСтрокиМетода = 0) Экспорт
	...
	КонецЕсли;
	ЧислоСимволов = 200;
	выхСодержаниеСтроки = Лев(СокрЛП(ПолеТекстаМодуля.ПолеТекста.ПолучитьСтроку(НомерСтроки)), ЧислоСимволов);
	Если Ложь
		Или СодержаниеСтроки = Неопределено
		Или выхСодержаниеСтроки = Лев(СокрЛП(СодержаниеСтроки), ЧислоСимволов)
	Тогда   
		НужноеОписаниеМетода = Неопределено; // СтрокаТаблицыЗначений, Неопределено
		ИмяМетода = НайтиМетодПоНомеруСтроки(ПолеТекстаМодуля, НомерСтроки, ИмяМодуля, ТекстыМодулейКэш, ЧислоСтрокДляПоискаИмениМетода, НужноеОписаниеМетода);
		Если НужноеОписаниеМетода <> Неопределено Тогда 
			выхНомерПервойСтрокиМетода = НужноеОписаниеМетода.НачальнаяСтрока;
...
//   КэшиРазметкиМодуля - Соответствие, Неопределено - 
//   ЧислоСтрокДляПоискаИмениМетода - Число - 
//   КэшМодуля - Структура, Неопределено - 
//   НужноеОписаниеМетода - СтрокаТаблицыЗначений, Неопределено - 
// Возвращаемое значение:
//   Строка, Неопределено - 
Функция НайтиМетодПоНомеруСтроки(Знач ПолеТекстаМодуля, Знач НомерСтроки, Знач ИмяМодуля = "", КэшиРазметкиМодуля = Неопределено, Знач ЧислоСтрокДляПоискаИмениМетода = 2000, НужноеОписаниеМетода = Неопределено,
		Знач РазрешитьВнеМетодов = Ложь) Экспорт
	Если КэшиРазметкиМодуля = Неопределено Тогда
		КэшиРазметкиМодуля = Новый Соответствие;
	КонецЕсли;
	КэшМодуля = КэшиРазметкиМодуля[ИмяМодуля];
	Если КэшМодуля = Неопределено Тогда
		...
		КэшМодуля = Новый Структура("Методы", Методы);
		Если КэшиРазметкиМодуля <> Неопределено Тогда
			КэшиРазметкиМодуля[ИмяМодуля] = КэшМодуля;
		КонецЕсли;
		Методы = Неопределено;
	КонецЕсли;
	...
	Если НужноеОписаниеМетода = Неопределено Тогда
		...
		Если Вхождения.Количество() > 0 Тогда
			...
			Если ИмяМетода = Неопределено Тогда
				Если РазрешитьВнеМетодов Тогда
					ИмяМетода = ИмяМетодаВнеМетодов();
				Иначе
...
Функция ИмяМодуляИзИмениФайла(Знач ПолноеИмя) Экспорт
	//ИмяМодуля = ирОбщий.ТекстБезПоследнегоФрагментаЛкс(СтрЗаменить(ПолноеИмя, ПапкаКэшаМодулей.ПолноеИмя + "\", ""));
	ИмяМодуля = СтрЗаменить(ирОбщий.ПоследнийФрагментЛкс(ПолноеИмя, ПапкаКэшаМодулей.ПолноеИмя + "\"), "\", " ");
	ИмяМодуля = ирОбщий.ТекстБезПоследнегоФрагментаЛкс(ИмяМодуля);
	Возврат ИмяМодуля;
КонецФункции
...
Функция ПолучитьТаблицуШаблоновТекста(ИмяКласса, _мСообщенияЧерезПредупреждения = Ложь, ПринудительноОбновить = Ложь) Экспорт 
	...
	Пока Не Закончили Цикл 
		...
		ИначеЕсли Ответ = gpMsgReduction Тогда
			ТекущееСокращение = Парсер.CurrentReduction; // ОбработкаОбъект.ирТипGoldParserReduction 
...
Функция КонтекстНапарника() Экспорт
	Если мКонтекстНапарника <> Неопределено Тогда
		Возврат мКонтекстНапарника;
	КонецЕсли;
	КонтекстНапарника = Новый Структура("Сессия, ИдентификаторКомпьютера, СлужебныеПараметры");
	КонтекстНапарника.Вставить("Соединение"); // HTTPСоединение
	//КонтекстНапарника.Вставить("ФоновыйЗапросХттп"); // ОбработкаОбъект.ирФоновыйЗапросХттп - отказался от хранения тут из-за аварийных завершений при обращении к уничтоженному объекту
	КонтекстНапарника.Вставить("ОтправленныеОбъекты", Новый Соответствие);
	КонтекстНапарника.Вставить("ОбъектыДляОтправки", Новый Массив);
	КонтекстНапарника.ИдентификаторКомпьютера = ирОбщий.ХешЛкс(ИмяКомпьютера(), Истина);
	// Оригинальный алгоритм расчета ИдентификаторКомпьютера из com.e1c.edt.ai_1.0.0.v202509261242.jar:com/e1c/edt/ai/HardwareIdProvider.class
	//SystemInfo systemInfo = new SystemInfo();
	//byte[] bytes = (systemInfo.osName + '\n' + systemInfo.osVersion + '\n' + systemInfo.processorName + '\n' + systemInfo.availableProcessors + '\n' + systemInfo.totalPhysicalMemorySize).getBytes(StandardCharsets.UTF_8);
	//MessageDigest hash = (MessageDigest) this.messageDigestProvider.get();
	//hash.update(bytes);
	...
	//    }
	//}
	//id = this.hashTools.format(hash, false);
	мКонтекстНапарника = КонтекстНапарника;
	Возврат КонтекстНапарника;
КонецФункции
...
// Возвращаемое значение:
//  Форма -
Функция ПассивнаяФормаПоИмениКэш(Знач ИмяФормы, Знач ИмяРасширения = "") Экспорт
	Если Найти(ИмяФормы, " ") И Найти(ИмяФормы, "Ф::") = 0 Тогда
		Фрагменты = ирОбщий.СтрРазделитьЛкс(ИмяФормы, " ");
		Если Фрагменты.Количество() > 1 И ПустаяСтрока(ИмяРасширения) Тогда
			ИмяРасширения = Фрагменты[0];
			Фрагменты.Удалить(0);
		КонецЕсли;
		ИмяФормы = Фрагменты[0];
	КонецЕсли;
	КлючКэша = НРег(ИмяФормы);
	Результат = мПассивныеФормы[КлючКэша]; 
	Если Результат = Неопределено Тогда
		Попытка
			...
			Форма = ирКлиент.ПассивнаяФормаПоИмениЛкс(ИмяФормы); // Ошибка компиляции модуля не перехватывается
			ПолноеОписаниеОшибки = "Обработчик ПриСоздании формы вернул Отказ" + ТекстРекомендации;
			Если Найти(ИмяФормы, "Ф::") > 0 И Не ирКэш.ЛиСеансТолстогоКлиентаУПЛкс() Тогда
				ПолноеОписаниеОшибки = "Если форма управляемая, то ее невозможно создать из внешней обработки в обычном приложении.";
			КонецЕсли;
		Исключение
			Форма = Неопределено;
			ОписаниеОшибки = ОписаниеОшибки();
			ПолноеОписаниеОшибки = ОписаниеОшибки + ТекстРекомендации;
			Если Найти(ИмяФормы, "Ф::") > 0 И ирКэш.ЛиСеансТолстогоКлиентаУПЛкс() Тогда
				ПолноеОписаниеОшибки = ПолноеОписаниеОшибки + Символы.ПС
					+ "Если форма обычная, то ее невозможно создать из внешней обработки в управляемом приложении.";
			КонецЕсли;
			Если Найти(ОписаниеОшибки, "только для конкретного отчета") Тогда
				ФайлВнешнегоОтчетаОбщейФормы = ФайлМодуляИзИмениМодуля(ИмяФормы, "erf");
				Если Не ФайлВнешнегоОтчетаОбщейФормы.Существует() Тогда
					ШаблонВнешнегоОтчета = ШаблоныВнешнейОбработки["ВнешнийОтчет"];
					Если ШаблонВнешнегоОтчета = Неопределено Тогда
						ШаблонВнешнегоОтчета = НовыйШаблонВнешнейОбработки();
						ШаблонВнешнегоОтчета.Путь = РазвернутьНовыйШаблонВнешнейОбработки("",,,, "ШаблонВнешнегоОтчета") + ирОбщий.РазделительПутиКФайлуЛкс();
						ШаблонВнешнегоОтчета.ФайлСИменем = Новый Файл(ШаблонВнешнегоОтчета.Путь + "copyinfo.data.und");
						ШаблонВнешнегоОтчета.ТекстФайлаСИменем = ирОбщий.ПрочитатьТекстИзФайлаЛкс(ШаблонВнешнегоОтчета.ФайлСИменем.ПолноеИмя);
						ШаблоныВнешнейОбработки["ВнешнийОтчет"] = ШаблонВнешнегоОтчета;
					КонецЕсли;
					Текст = ШаблонВнешнегоОтчета.ТекстФайлаСИменем;
					Текст = СтрЗаменить(Текст, "%ИмяОбщейФормы%", ОбъектМД.Имя);
					ирОбщий.ЗаписатьТекстВФайлЛкс(Текст, ШаблонВнешнегоОтчета.ФайлСИменем.ПолноеИмя);
					УпаковатьВнешнююОбработку(ШаблонВнешнегоОтчета.Путь, ФайлВнешнегоОтчетаОбщейФормы.ПолноеИмя);
				КонецЕсли;
				Если ФайлВнешнегоОтчетаОбщейФормы.Существует() Тогда
					Попытка
						Форма = ирКлиент.ПассивнаяФормаПоИмениЛкс("Ф::" + ФайлВнешнегоОтчетаОбщейФормы.ПолноеИмя + "::ВнешнийОтчет1.Форма");
					Исключение
						ПолноеОписаниеОшибки = ОписаниеОшибки() + "." + Символы.ПС
							+ "Создание общей управляемой формы отчета поддерживается только в управляемом приложении и обычной формы - в обычном приложении";
					КонецПопытки;
				КонецЕсли;
			КонецЕсли;
		КонецПопытки;
		...
		Если Форма = Неопределено И ИмяФормы <> ИмяДинамическойФормы() Тогда
			Сообщение = "Анализатор кода не видит часть формы (" + ИмяФормы + ") из-за недоступности ее создания: " + ПолноеОписаниеОшибки;
			ИмяМетода = "ПриСозданииНаСервере";
		ИначеЕсли Истина
			И ИмитаторФормы = Неопределено 
			И ЗначениеЗаполнено(ИмяРасширения) 
			И ОбъектМД.РасширениеКонфигурации() = Неопределено
		Тогда
			Сообщение = "Анализатор кода не может создать кэш формы (" + ИмяФормы + ") из ее расширения. Перейти в основной модуль формы.";
		КонецЕсли;     
...
Функция ИменаРеквизитовФормы(Знач ФормаИлиДокументDOM, Знач ПутьКРодителю = Неопределено, Знач ЛиДляКоллекции = Ложь, выхИдентификаторыРеквизитов = Неопределено, ДокументДОМ = Неопределено,
	...
	Если Ложь
		Или ирОбщий.ЛиФормаИлиИмитаторЛкс(ФормаИлиДокументDOM, Ложь)
			...
			Если ТаблицаРеквизитов <> Неопределено Тогда
				...
				Тогда
					...
					Тогда
						...
						Тогда
							СтруктураТипаРегистра = СтруктураТипаИзКонкретногоТипа(Типы[0]);
							Если ЛиДляКоллекции Тогда
								СтруктураТипаРегистра = ТаблицаТиповЭлементовКоллекции(СтруктураТипаРегистра)[0];
							КонецЕсли;
							ТаблицаСлов = ТаблицаСловИзСтруктурыТипа(СтруктураТипаРегистра, НовыеПараметрыЗаполненияСлов("Свойство"));
							Для Каждого СтрокаСлова Из ТаблицаСлов Цикл
	...
	Иначе // Обычная форма
		...
		Если ДокументДОМ = Неопределено Тогда
			...
			КонецЕсли;
		КонецЕсли;
		РазыменовательПИ = Новый РазыменовательПространствИменDOM(Новый Соответствие);
		...
		КонецЕсли;
	КонецЕсли;
	Результат = НовыйИменаРеквизитовФормы(ИменаРеквизитов, ИмяОсновногоРеквизита);
...
// Нужна для анализатора кода для блокирования динамического вычисления.
// Параметры:
//   ИменаРеквизитов - Структура - 
//   ИмяОсновногоРеквизита -  - 
Функция НовыйИменаРеквизитовФормы(Знач ИменаРеквизитов, Знач ИмяОсновногоРеквизита) Экспорт
	Результат = Новый Структура;
...
Функция ТекстМодуляСгенерированнойВнешнейОбработки(ФайлВнешнейОбработки, Знач ЛиМодульОтФормы = Ложь) Экспорт 
	...
	Иначе
		...
		КонецЕсли;
		ФайлТекстаМодуляОбработки = Новый Файл(КаталогРаспаковки + мРазделительПутиКФайлу + СубПутьКФайлуМодуля);
...
Функция ВыбратьПроцессКонфигуратора(Знач ИдентификаторыОтладчиков = Неопределено) Экспорт 
	...
	ТаблицаКонфигураторов = ирОбщий.ТаблицаИзТекстаСРазделителемЛкс(РезультатСкрипта,,, Истина);
	//! ТаблицаКонфигураторов.Колонки.Добавить("Id");
	//! ТаблицаКонфигураторов.Колонки.Добавить("mainwindowtitle");
...
Процедура АктивизироватьОкноКонфигуратора(Знач ПередаваемыйТекст = "") Экспорт 
	...
	Если ЗначениеЗаполнено(ПередаваемыйТекст) Тогда
...
Процедура ПроверитьСтруктуруФайловогоКэша() Экспорт 
	...
	Для Каждого ЭлементПодкаталога Из СтруктураПодкаталоговФайловогоКэша Цикл
		ПолныйПутьКаталога = КаталогФайловогоКэша + мРазделительПутиКФайлу + ЭлементПодкаталога.Ключ;
		...
		Если Не ФайлКаталога.Существует() Тогда
			Попытка
				...
				ПробныйФайл.Записать(ФайлКаталога.ПолноеИмя + мРазделительПутиКФайлу + "1.txt");
			Исключение
				ОписаниеОшибки = ОписаниеОшибки();
		...
		Если ЭлементПодкаталога.Ключ = "a" Тогда
			...
			ФайлКэшаДвижка = Новый Файл(ПолныйПутьКаталога + мРазделительПутиКФайлу + "_EngineTimeStamp.mll");
			...
			Если ДатаКэшаДвижка <> ТекущаяДатаДвижка Тогда
				Попытка
					УдалитьФайлы(ПолныйПутьКаталога + мРазделительПутиКФайлу, "*.*");
				Исключение
					Неудача = Истина;
				КонецПопытки; 
...
// Проверяет актуальность кэша. Вызывается когда кэш в памяти уже точно не соответствует объекту БД.
...
Функция РазвернутьНовыйШаблонВнешнейОбработки(СуффиксМакета = "СМодулем", ГлобальныйКлюч = Неопределено, ФайлСИменем, ТекстФайлаСИменем = "", Знач БазовоеИмяМакета = "ШаблонВнешнейОбработки") Экспорт
	...
	ИмяМакета = БазовоеИмяМакета + СуффиксМакета;
	...
	Если БазовоеИмяМакета = "ШаблонВнешнегоОтчета" Тогда
		СубПутьКФайлуЗаголовка = СубПутьКФайлуЗаголовкаВнешнегоОтчета;
	Иначе
		СубПутьКФайлуЗаголовка = СубПутьКФайлуЗаголовкаВнешнейОбработки;
	КонецЕсли;
	ФайлСИменем = Новый Файл(лПутьКШаблонуВнешнейОбработки + ирОбщий.РазделительПутиКФайлуЛкс() + СубПутьКФайлуЗаголовка);
...
Процедура СоздатьКомандныйФайлДляУпаковкиФайлаВнешнейОбработки(Каталог, выхИмяКомандногоФайла = "") Экспорт
	...
	ТекстовыйДокумент.Записать(Каталог + мРазделительПутиКФайлу + выхИмяКомандногоФайла, КодировкаТекста.ANSI);
...
Процедура ПодготовитьПакер(Знач Каталог)
	...
	Если ирКэш.ЛиПлатформаWindowsЛкс() Тогда
		...
		ПолучитьМакет("Zlib1").Записать(Каталог + мРазделительПутиКФайлу + "Zlib1.dll");
	Иначе
		...
		КонецЕсли; 
	КонецЕсли;
	ПолноеИмяФайла = Каталог + ?(Прав(Каталог, 1) = мРазделительПутиКФайлу, "", мРазделительПутиКФайлу) + ИмяФайлаПакера;
...
Процедура РаспаковатьФайлВнешнейОбработки(ПолноеИмяФайла, ПутьРаспаковки) Экспорт 
	...
	Тогда
		...
		|");
		ПолноеИмяКомандногоФайла = ПутьРаспаковки + мРазделительПутиКФайлу + ИмяКомандногоФайла;
	...
	Если ЧитатьЛог Тогда
		ТекстовыйДокумент = Новый ТекстовыйДокумент;
		ТекстовыйДокумент.Прочитать(ПутьРаспаковки + мРазделительПутиКФайлу + ИмяФайлаЛога, ирОбщий.СистемнаяКодировкаТекстаОСЛкс());
	...
	Иначе
		...
		Если ТекстМакета <> Неопределено Тогда
			...
			Если ШаблонВнешнейОбработкиСМакетом = Неопределено Тогда
				ШаблонВнешнейОбработкиСМакетом = НовыйШаблонВнешнейОбработки();
		...
		Иначе
			ШаблонВнешнейОбработки = ШаблоныВнешнейОбработки[КлючШаблона];
			Если ШаблонВнешнейОбработки = Неопределено Тогда
				ШаблонВнешнейОбработки = НовыйШаблонВнешнейОбработки();
				Если ЛиМодульОтФормы Тогда
...
Функция НовыйШаблонВнешнейОбработки() Экспорт
	Возврат Новый Структура("Путь, ФайлСИменем, ФайлЗамок, ТекстФайлаСИменем, ТекстФайлаМодулем");
КонецФункции
...
Функция УпаковатьВнешнююОбработку(ПутьКШаблонуВнешнейОбработки, ИмяВыходногоФайла, СоздатьФайлыУпаковщика = Ложь) Экспорт
	...
	Если ирОбщий.СтрКончаетсяНаЛкс(ИмяВыходногоФайла, ".erf") Тогда
		СубПутьККонтрольномуФайлу = СубПутьККонтрольномуФайлуВнешнегоОтчета;
	Иначе
		СубПутьККонтрольномуФайлу = СубПутьККонтрольномуФайлуВнешнейОбработки;
	КонецЕсли;
	КонтрольныйФайл1 = Новый Файл(ПутьКШаблонуВнешнейОбработки + СубПутьККонтрольномуФайлу);
...
Функция ПрочитатьНастройкиКомпьютера(Знач ЛиДляБазы = Истина, Знач Обновить = Ложь) Экспорт 
	Если Не Обновить Тогда
		...
		КонецЕсли;
	КонецЕсли;
	СохранитьНастройкиКомпьютера = Ложь;
	...
	Если НастройкиКомпьютера = Неопределено Тогда
		...
		СохранитьНастройкиКомпьютера = Истина;
	КонецЕсли; 
	Если ЛиДляБазы Тогда
	...
	КонецЕсли;
	Если СохранитьНастройкиКомпьютера Тогда
		СохранитьНастройкиКомпьютера(ЛиДляБазы);
	КонецЕсли;
...
Процедура ПрочитатьНастройкиКомпьютераВсе(Знач Обновить = Ложь) Экспорт
	Если ЛиГлобальныеНастройкиВсехБаз Тогда
		ПрочитатьНастройкиКомпьютера(Ложь, Обновить);
	КонецЕсли;
	ПрочитатьНастройкиКомпьютера(Истина, Обновить);
КонецПроцедуры

Процедура СохранитьНастройкиКомпьютера(Знач ЛиДляБазы = Истина) Экспорт
	ИмяФайлаНастроекКомпьютера = ИмяФайлаНастроекКомпьютера(ЛиДляБазы);
	ирОбщий.СохранитьЗначениеВФайлЛкс(ПрочитатьНастройкиКомпьютера(ЛиДляБазы), ИмяФайлаНастроекКомпьютера,,, "Внутр", мЭтотОбъектГотов);
КонецПроцедуры
...
Функция СкриптКопированияВБуферОбменаCTRLКлик() Экспорт
	Если мСкриптКопированиеВБуферОбменаCTRLКлик = Неопределено Тогда
		мСкриптКопированиеВБуферОбменаCTRLКлик = ПолучитьМакет("СкриптКопированиеВБуферОбменаCTRLКлик").ПолучитьТекст();
	КонецЕсли;
	Возврат мСкриптКопированиеВБуферОбменаCTRLКлик;
КонецФункции

Функция СкриптРаскраскиКодаВстроенногоЯзыка() Экспорт
	Если мСкриптРаскраскиКодаВстроенногоЯзыка = Неопределено Тогда
		мСкриптРаскраскиКодаВстроенногоЯзыка = ПолучитьМакет("ПросмотрMarkdownHTML").ПолучитьТекст();
		мСкриптРаскраскиКодаВстроенногоЯзыка = СтрЗаменить(мСкриптРаскраскиКодаВстроенногоЯзыка, "%ТекстJS%", ""); 
	КонецЕсли;
	Возврат мСкриптРаскраскиКодаВстроенногоЯзыка;
КонецФункции
...
Функция ОбработатьВнешнееИсключениеАлгоритма(ДескрипторСервиса, ИнформацияОбОшибке, РежимВыполненияАлгоритма, Интерактивно = Истина) Экспорт 
	...
	Если ТипЗнч(ДескрипторСервиса) = Тип("СтрокаТаблицыЗначений") Тогда
		...
		Если лОбъект = Неопределено Тогда
			лОбъект = Новый ("СправочникОбъект.Сервисы2iS"); //@skip-check unknown-method-property
...
Функция ИменаСохраняемыхСвойств() Экспорт
	Возврат "АвторегистрацияComКомпонент, ВыделениеРезультатовПоиска, ИспользоватьЭмуляциюНажатияКлавиш, КлючСервисаИИ, КлючСервисаНапарник, СерверИИ, МодельИИ, ПараметрыМоделиИИ, ПредлагатьОчиститьМенеджерВременныхТаблицЧерезМинут";
КонецФункции
...
Функция КлючСервисаИИ(Знач Сервер = Неопределено) Экспорт 
	Если ЛиНапарникВРолиОбщегоИИ(Сервер) Тогда
		Возврат КлючСервисаНапарник();
	КонецЕсли;
...
Функция ЛиНапарникВРолиОбщегоИИ(Знач Сервер = Неопределено) Экспорт
	Возврат СерверИИ(, Сервер) = "https://code.1c.ai";
КонецФункции 
...
Функция МодельИИ(Знач РазрешитьПоУмолчанию = Истина, Знач Модель = Неопределено, Знач Сервер = "") Экспорт 
	...
	Если ПустаяСтрока(Модель) И РазрешитьПоУмолчанию Тогда
		...
		ИначеЕсли Сервер = "https://chat.qwen.ai/api" Тогда
...
Функция ПараметрыМоделиИИ() Экспорт
	ЛиНовыеПараметры = Ложь;
	Если ПараметрыМоделиИИ = Неопределено Тогда
		ПараметрыМоделиИИ = Новый СписокЗначений;
		ЛиНовыеПараметры = Истина;
	КонецЕсли;
	Если ирОбщий.НайтиЭлементКоллекцииЛкс(ПараметрыМоделиИИ, "Представление", "include_reasoning") = Неопределено Тогда
		ПараметрыМоделиИИ.Добавить(Ложь, "include_reasoning", ЛиНовыеПараметры); /// Блокируем передачу рассуждений. Не все модели поддерживают.
	КонецЕсли;
	Если ирОбщий.НайтиЭлементКоллекцииЛкс(ПараметрыМоделиИИ, "Представление", "reasoning") = Неопределено Тогда 
		ПараметрыМоделиИИ.Добавить(Ложь, "reasoning", Ложь); /// Блокируем рассуждения. Не все модели поддерживают.
	КонецЕсли;
	Возврат ПараметрыМоделиИИ;
КонецФункции
...
Функция НоваяТаблицаИндикаторов() Экспорт
	Результат = Новый ТаблицаЗначений;
	Результат.Колонки.Добавить("КоличествоПроходов", Новый ОписаниеТипов("Число"));
	Результат.Колонки.Добавить("ПредставлениеПроцесса", Новый ОписаниеТипов("Строка"));
	Результат.Колонки.Добавить("ЛиВыводитьВремя", Новый ОписаниеТипов("Булево"));
	Результат.Колонки.Добавить("РазрешитьПрерывание", Новый ОписаниеТипов("Булево"));
	Результат.Колонки.Добавить("Автозавершение", Новый ОписаниеТипов("Булево"));
	Результат.Колонки.Добавить("ДатаНачалаПроцесса", Новый ОписаниеТипов("Дата"));
	Результат.Колонки.Добавить("МинимальныйПериодОбновления", Новый ОписаниеТипов("Число"));
	Результат.Колонки.Добавить("СледующееОбновление", Новый ОписаниеТипов("Дата"));
	Результат.Колонки.Добавить("Шаг", Новый ОписаниеТипов("Число"));
	Результат.Колонки.Добавить("ТекстСостояния", Новый ОписаниеТипов("Строка"));
	Результат.Колонки.Добавить("СледующийСчетчик", Новый ОписаниеТипов("Число"));
	Результат.Колонки.Добавить("Счетчик", Новый ОписаниеТипов("Число"));
	Результат.Колонки.Добавить("СледующаяПроверкаПрерывания", Новый ОписаниеТипов("Дата"));
	Возврат Результат;
КонецФункции
...
#Если Сервер И Не Сервер Тогда
	...
	ИнициацияОписанияМетодовИСвойств();
	ПрочитатьСтатистикуВыбораАвтодополнения();
	НоваяТаблицаТипов();
	ТерминалыЯзыкаЗапросов();
#КонецЕсли
мРазделительПутиКФайлу = ирОбщий.РазделительПутиКФайлуЛкс();
мЭтотОбъектГотов = Ложь;
мРазрешеноСохранениеВХранилища = Истина;
ЛиГлобальныеНастройкиВсехБаз = Ложь;
...
шМетодНачало = "(?:Функция|Процедура|Function|Procedure)";
шМетодКонец = "(?:Конец|End)(?:Процедуры|Функции|Procedure|Function)";
шКонстанта = "(?:[\d]+|""[^""]*""|Неопределено|Undefined|Ложь|False|Истина|True)";
...
мМассивТиповСМетаданными.Добавить(Тип("НастройкаОтбора"));
...
мМассивТиповСМетаданными.Добавить(Тип("ВариантыНастроекКомпоновкиДанных"));
...
СубПутьКФайлуЗаголовкаВнешнегоОтчета = "06aa2a82-58f6-4c2f-9348-80293d030eef.data";
СубПутьККонтрольномуФайлуВнешнейОбработки = "1ad4dbd4-e136-4202-8121-02c33ad2af45.0.data";
СубПутьККонтрольномуФайлуВнешнегоОтчета = "06aa2a82-58f6-4c2f-9348-80293d030eef.data";
...
мИндикаторы = НоваяТаблицаИндикаторов();
...
мКэшФайловПоИменамМодулей = Новый Соответствие;
