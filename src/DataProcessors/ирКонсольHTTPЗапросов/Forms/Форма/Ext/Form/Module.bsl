...
Процедура ПриОткрытии()
	...
	//СписокВыбора = ЭлементыФормы.ТаймаутПодключения.СписокВыбора;
...
Процедура ОбновитьЗаменыВАдресеРесурса() Экспорт
	Для Каждого Вхождение Из ирОбщий.НайтиРегВыражениеЛкс(АдресРесурса, "(\{" + мПлатформа.шИмя + "\})") Цикл
		СтрокаПараметра = ПараметрыURL.Найти(Вхождение.Группа0, "Имя");
		Если СтрокаПараметра <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
		СтрокаПараметра = ПараметрыURL.Добавить();
		СтрокаПараметра.Имя = Вхождение.Группа0;
		СтрокаПараметра.Значение = Вхождение.Группа0;
	КонецЦикла;
КонецПроцедуры
...
Функция ПолучитьТокенOAuth2(Сервис, Ключ, Секрет, Scope, СоединениеHTTP)
	...
	ЗапросHTTP = ПолучитьЗапросHTTP(СтруктураURI.ПутьНаСервере, Заголовки, ирОбщий.ОбъектВТекстЖСОНЛкс(Данные));
	...
	СтруктураДанных = ирОбщий.ОбъектИзТекстаЖСОНЛкс(Данные);
...
Процедура КоманднаяПанельВыполнениеВыполнитьЗапрос(Кнопка = Неопределено)
	...
	Если СоединениеHTTP = Неопределено Тогда
		...
		Возврат;
	КонецЕсли; 
	//СоединениеHTTP.Таймаут = ТаймаутЗапроса;
...
Функция СформироватьТелоЗапросаИзДанныхФормы()
	...
	КонецЦикла;
	РазделительЧастейТела = "" + Новый УникальныйИдентификатор();
	УстановитьТелоЗапросаСтрока(ПолучитьТелоPOSTЗапросаFormData(СоответствияДанныеФормы, РазделительЧастейТела));
	ЭтаФорма.ТелоЗапросаДвоичныеДанные = Неопределено;
	Результат = УстановитьГенерируемыйЗаголовокЗапроса("Content-Type", "multipart/form-data; boundary=""" + РазделительЧастейТела + """");
	Возврат Результат;
...
Процедура ЗаголовкиЗапросаИмяНачалоВыбора(Элемент, СтандартнаяОбработка)
	...
	КонецЕсли; 
	ВыбраннаяСтрока = ирКлиент.ОткрытьТаблицуЗначенийЛкс(ДоступныеЗаголовкиЗапроса, НачальнаяСтрока, Истина, "Выберите заголовок запроса", Истина); // СтрокаТаблицыЗначений
...
Процедура НастроитьЭлементыФормы()
	...
	ЭлементыФормы.ПарольЗапроса.Доступность = ТипАвторизации <> "Bearer";
...
Процедура РегулярноеВыражениеНачалоВыбора(Элемент, СтандартнаяОбработка)
	ирКлиент.ПолеВводаРегулярногоВыраженияНачалоВыбораЛкс(Элемент, ТекстГдеИскать());
КонецПроцедуры
...
Процедура СформироватьЗапросOAuth2Нажатие(Элемент)
	...
	УстановитьТелоЗапросаСтрока(ирОбщий.ОбъектВТекстЖСОНЛкс(Данные));
...
Процедура ДействияФормыСгенерироватьПрограммныйКод(Кнопка)
	...
	Если ЗначениеЗаполнено(РегулярноеВыражение) Тогда
		...
		Текст = Текст + "
			...
			|мРегВыражение.Pattern = " + ирОбщий.ТекстВВыражениеВстроенногоЯзыкаЛкс(РегулярноеВыражение) + ";
			|Вхождения = мРегВыражение.Execute(ТекстГдеИскать);
...
Процедура ДействияФормыИмпортироватьИзPostman(Кнопка)
	...
	ОбъектИмпорта = ирОбщий.ОбъектИзТекстаЖСОНЛкс(Текст, Истина);
		...
		Если ТипОбъектаТелаЗапроса = "formdata" Тогда // Form-data
			...
			Для Каждого ЭлементТелаЗапроса Из ОбъектТелаЗапроса[ТипОбъектаТелаЗапроса] Цикл
				...
				Если ЭлементТелаЗапроса["type"] = "file" Тогда
					ЭлементТелаЗапросаИмпорта.Тип = ЭлементыФормы.ИсточникТелаЗапроса.СписокВыбора[1];
				...
				Иначе
					ЭлементТелаЗапросаИмпорта.Тип = ЭлементыФормы.ИсточникТелаЗапроса.СписокВыбора[0];
...
Процедура ДействияФормыИмпортироватьИзCURL(Кнопка)
	ТекстБуфераОбмена = СокрЛП(ирКлиент.ТекстИзБуфераОбменаОСЛкс());
	Если Не ирОбщий.СтрНачинаетсяСЛкс(ТекстБуфераОбмена, "curl") Тогда
		ирОбщий.СообщитьЛкс("В буфере обмена должен быть текст начинающийся с curl");
		Возврат;
	КонецЕсли;
	Ответ = Вопрос("Загрузить в новый запрос?", РежимДиалогаВопрос.ДаНет);
	Если Ответ = КодВозвратаДиалога.Да Тогда
		ЭлементыФормы.ДеревоЗапросов.ДобавитьСтроку();
		ЭлементыФормы.ДеревоЗапросов.ЗакончитьРедактированиеСтроки(Ложь);
	КонецЕсли;
	ЭтаФорма.ТелоЗапросаСтрока = "";
	ТекстБуфераОбмена = СтрЗаменить(ТекстБуфераОбмена, "curl", "");
	СтрокиТекста = ирОбщий.СтрРазделитьЛкс(ТекстБуфераОбмена, Символы.ПС);
	
	// Переменные для обработки многострочных данных
	ТекущиеДанные = "";
	ЛиВМногострочныхДанных = Ложь;
	НачалоМногострочныхДанных = Ложь;
	
	Для Индекс = 0 По СтрокиТекста.Количество() - 1 Цикл
		СтрокаТекста = СокрЛП(СтрокиТекста[Индекс]);
		...
		Если ЛиВМногострочныхДанных Тогда
			// Ищем конец многострочных данных (одинарную кавычку)
			ПозицияКавычки = Найти(СтрокаТекста, "'");
			Если ПозицияКавычки > 0 Тогда
				// Конец многострочных данных
				ТекущиеДанные = ТекущиеДанные + Символы.ПС + Лев(СтрокаТекста, ПозицияКавычки - 1);
				ЭтаФорма.ТелоЗапросаСтрока = ТекущиеДанные;
				ЭтаФорма.ИсточникТелаЗапроса = "ТелоЗапросаСтрока";
				ЛиВМногострочныхДанных = Ложь;
				ТекущиеДанные = "";
			Иначе
				// Продолжение многострочных данных
				ТекущиеДанные = ТекущиеДанные + Символы.ПС + СтрокаТекста;
			КонецЕсли;
			Продолжить;
		КонецЕсли;
		
		// Проверяем, начинаются ли многострочные данные в этой строке
		ПозицияНачалаДанных = Найти(СтрокаТекста, "-d '");
		Если ПозицияНачалаДанных > 0 Тогда
			// Начало многострочных данных
			ОстатокСтроки = Сред(СтрокаТекста, ПозицияНачалаДанных + 4);
			ПозицияКавычки = Найти(ОстатокСтроки, "'");
			
			Если ПозицияКавычки > 0 Тогда
				// Однострочные данные
				Данные = Лев(ОстатокСтроки, ПозицияКавычки - 1);
				ЭтаФорма.ТелоЗапросаСтрока = Данные;
				ЭтаФорма.ИсточникТелаЗапроса = "ТелоЗапросаСтрока";
			Иначе
				// Многострочные данные
				ЛиВМногострочныхДанных = Истина;
				ТекущиеДанные = ОстатокСтроки;
			КонецЕсли;
			
			// Обрабатываем остальную часть строки (если есть другие параметры)
			Если ПозицияНачалаДанных > 1 Тогда
				ЧастьДоДанных = Лев(СтрокаТекста, ПозицияНачалаДанных - 1);
				Если Не ПустаяСтрока(ЧастьДоДанных) Тогда
					Части = РазделитьСтрокуСЭкранированием(ЧастьДоДанных);
					ОбработатьЧасти(Части, ЛиВМногострочныхДанных, ТекущиеДанные);
				КонецЕсли;
			КонецЕсли;
		...
		Части = РазделитьСтрокуСЭкранированием(СтрокаТекста);
		ОбработатьЧасти(Части, ЛиВМногострочныхДанных, ТекущиеДанные);
		
	КонецЦикла;
	
	// Если после обработки всех строк остались незавершенные многострочные данные
	Если ЛиВМногострочныхДанных И Не ПустаяСтрока(ТекущиеДанные) Тогда
		ЭтаФорма.ТелоЗапросаСтрока = ТекущиеДанные;
		ЭтаФорма.ИсточникТелаЗапроса = "ТелоЗапросаСтрока";
	КонецЕсли;
...
Процедура ОбработатьЧасти(Части, ВМногострочныхДанных, ТекущиеДанные)
	Для j = 0 По Части.Количество() - 1 Цикл
		Часть = СокрЛП(Части[j]);
		Если ПустаяСтрока(Часть) Тогда
			Продолжить;
		КонецЕсли;
		Если Лев(Часть, 1) = "-" Тогда
			ОбработатьОпцию(Часть, Части, j, ВМногострочныхДанных, ТекущиеДанные);
		ИначеЕсли Найти(Часть, "://") > 0 Тогда
			ЗагрузитьКонечныйЗапрос(Часть);
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры

Процедура ОбработатьОпцию(Опция, Части, Индекс, ВМногострочныхДанных, ТекущиеДанные)
	Опция = ВРег(Опция);
	Если Опция = "-X" Тогда
		ЭтаФорма.МетодЗапроса = Части[Индекс + 1];
	ИначеЕсли Опция = "-H" ИЛИ Опция = "--HEADER" Тогда
		Если Индекс + 1 < Части.Количество() Тогда
			РазобратьЗаголовок(Части[Индекс + 1]);
		КонецЕсли;
	ИначеЕсли Ложь
		Или Опция = "-D" 
		ИЛИ Опция = "--DATA" 
		ИЛИ Опция = "--DATA-ASCII" 
		ИЛИ Опция = "--DATA-BINARY" 
		ИЛИ Опция = "--DATA-RAW" 
		ИЛИ Опция = "--DATA-URLENCODE" 
	Тогда
		Если Индекс + 1 < Части.Количество() Тогда
			ЭтаФорма.ТелоЗапросаСтрока = Части[Индекс + 1];
			ЭтаФорма.ИсточникТелаЗапроса = "ТелоЗапросаСтрока";
		КонецЕсли;
	ИначеЕсли Опция = "--CONNECT-TIMEOUT" Тогда
		Если Индекс + 1 < Части.Количество() Тогда
			//ЭтаФорма.ТаймаутПодключения = Число(Части[Индекс + 1]);
		КонецЕсли;
	ИначеЕсли Опция = "--MAX-TIME" Тогда
		Если Индекс + 1 < Части.Количество() Тогда
			ЭтаФорма.ТаймаутЗапроса = Число(Части[Индекс + 1]);
		КонецЕсли;
	ИначеЕсли Опция = "-V" ИЛИ Опция = "--VERBOSE" Тогда
		ПодробныйВывод = Истина;
	ИначеЕсли Опция = "-B" ИЛИ Опция = "--COOKIE" Тогда
		Если Индекс + 1 < Части.Количество() Тогда
			РазобратьКуки(Части[Индекс + 1]);
		КонецЕсли;
	ИначеЕсли Лев(Опция, 2) = "--" И Найти(Опция, "=") > 0 Тогда
		// Длинные опции со значениями через =
		РазобратьДлиннуюОпцию(Опция);
	КонецЕсли;
КонецПроцедуры

Процедура РазобратьДлиннуюОпцию(Опция)
	ПозицияРавенства = Найти(Опция, "=");
	Если ПозицияРавенства > 0 Тогда
		Ключ = Сред(Опция, 3, ПозицияРавенства - 3);
		Значение = Сред(Опция, ПозицияРавенства + 1);
		Если Ключ = "connect-timeout" Тогда
			//ЭтаФорма.ТаймаутПодключения = Число(Значение);
		ИначеЕсли Ключ = "max-time" Тогда
			ЭтаФорма.ТаймаутЗапроса = Число(Значение);
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры

Процедура РазобратьЗаголовок(ЗначениеЗаголовка)
	ЗначениеЗаголовка = СтрЗаменить(ЗначениеЗаголовка, "'", "");
	ЗначениеЗаголовка = СтрЗаменить(ЗначениеЗаголовка, """", "");
	Позиция = Найти(ЗначениеЗаголовка, ":");
	Если Позиция > 0 Тогда
		НовыйЗаголовок = ЗаголовкиЗапроса.Добавить();
		НовыйЗаголовок.Имя = СокрЛП(Лев(ЗначениеЗаголовка, Позиция - 1));
		НовыйЗаголовок.Значение = СокрЛП(Сред(ЗначениеЗаголовка, Позиция + 1));
		Если НовыйЗаголовок.Имя = "Authorization" Тогда
			Если ирОбщий.СтрНачинаетсяСЛкс(НовыйЗаголовок.Значение, "Basic ") Тогда
				ЭтаФорма.ПользовательЗапроса = ирОбщий.ТекстБезПервогоФрагментаЛкс(НовыйЗаголовок.Значение, " ");
			Иначе
				ЭтаФорма.ТипАвторизации = "";
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры

Функция РазделитьСтрокуСЭкранированием(Строка)
	Результат = Новый Массив;
	ТекущаяЧасть = "";
	ТипКавычек = "";
	ЛиВКавычках = Ложь;
	ЛиЭкранирование = Ложь;
	Для i = 1 По СтрДлина(Строка) Цикл
		Символ = Сред(Строка, i, 1);
		Если ЛиЭкранирование Тогда
			ТекущаяЧасть = ТекущаяЧасть + Символ;
			ЛиЭкранирование = Ложь;
		ИначеЕсли Символ = "\" Тогда
			ЛиЭкранирование = Истина;
		ИначеЕсли (Символ = """" ИЛИ Символ = "'") И Не ЛиВКавычках Тогда
			ЛиВКавычках = Истина;
			ТипКавычек = Символ;
		ИначеЕсли Символ = ТипКавычек И ЛиВКавычках Тогда
			ЛиВКавычках = Ложь;
			ТипКавычек = "";
		ИначеЕсли Символ = " " И Не ЛиВКавычках Тогда
			Если Не ПустаяСтрока(ТекущаяЧасть) Тогда
...
Процедура РазобратьКуки(ЗначениеКуки)
    ЗначениеКуки = СтрЗаменить(ЗначениеКуки, "'", "");
    ЗначениеКуки = СтрЗаменить(ЗначениеКуки, """", "");
    ПарыКуки = ирОбщий.СтрРазделитьЛкс(ЗначениеКуки, ";");
    Для Каждого ПараКуки Из ПарыКуки Цикл
        ПараКуки = СокрЛП(ПараКуки);
		Если ПустаяСтрока(ПараКуки) Тогда
			Продолжить;
		КонецЕсли;
	    ПозицияРавенства = Найти(ПараКуки, "=");
		Если ПозицияРавенства = 0 Тогда
			Продолжить;
		КонецЕсли;
        НовыйКуки = КукиЗапроса.Добавить();
		НовыйКуки.Имя = СокрЛП(Лев(ПараКуки, ПозицияРавенства - 1));
		НовыйКуки.Значение = СокрЛП(Сред(ПараКуки, ПозицияРавенства + 1));
    КонецЦикла;
КонецПроцедуры

Процедура ПользовательЗапросаНачалоВыбора(Элемент, СтандартнаяОбработка)
	ирКлиент.ПолеВводаРасширенногоЗначения_НачалоВыбораЛкс(Элемент, СтандартнаяОбработка);
КонецПроцедуры

Процедура ДействияФормыЭкспортВCURL(Кнопка)
	КомандаCURL = "curl";
	Если Не ирОбщий.СтрокиРавныЛкс(МетодЗапроса, "GET") Тогда
		КомандаCURL = КомандаCURL + " -X " + МетодЗапроса;
	КонецЕсли;
	КомандаCURL = КомандаCURL + " """ + КонечныйURL + """";
	Если ТипАвторизации = "Basic" И ЗначениеЗаполнено(ПользовательЗапроса) Тогда
		КомандаCURL = КомандаCURL + Символы.ПС + "  --user """ + ПользовательЗапроса + ":" + ПарольЗапроса + """";
	КонецЕсли;
	Если КукиЗапроса.Количество() > 0 Тогда
		СтрокаКуки = "";
		Для Каждого СтрокаКукиЗапроса Из КукиЗапроса Цикл
			Если ЗначениеЗаполнено(СтрокаКуки) Тогда
				СтрокаКуки = СтрокаКуки + "; ";
			КонецЕсли;
			СтрокаКуки = СтрокаКуки + СтрокаКукиЗапроса.Имя + "=" + СтрокаКукиЗапроса.Значение;
		КонецЦикла;
		КомандаCURL = КомандаCURL + Символы.ПС + "  --cookie """ + СтрокаКуки + """";
	КонецЕсли;
	Для Каждого ЗаголовокЗапроса Из ЗаголовкиЗапроса Цикл
		КомандаCURL = КомандаCURL + Символы.ПС + "  -H """ + ЗаголовокЗапроса.Имя + ": " + ЗаголовокЗапроса.Значение + """";
	КонецЦикла;
	Если ЭлементыФормы.ПанельТелоЗапроса.Доступность И ИсточникТелаЗапроса <> "ДвоичныеДанные" Тогда
		ТекстТелаЗапроса = ТелоЗапросаСтрока();
		Если ЗначениеЗаполнено(ТекстТелаЗапроса) Тогда
			КомандаCURL = КомандаCURL + Символы.ПС + "  --data '" + ТекстТелаЗапроса + "'";
		КонецЕсли;
	КонецЕсли;
	Если ТаймаутЗапроса > 0 И ТаймаутЗапроса <> 10 Тогда
		КомандаCURL = КомандаCURL + Символы.ПС + "  --max-time " + ТаймаутЗапроса;
	КонецЕсли;
	КомандаCURL = КомандаCURL + Символы.ПС + "  -w";
	ирКлиент.ОткрытьТекстЛкс(КомандаCURL,, "Обычный");
КонецПроцедуры
