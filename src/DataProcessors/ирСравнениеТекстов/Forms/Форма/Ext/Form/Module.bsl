...
Процедура ПриОткрытии()
	...
	Если Не РежимВыбора Тогда
		...
		ЭлементыФормы.СтруктураОтличий.Колонки.Принять.Видимость = Ложь;
	КонецЕсли;
...
Процедура РедакторHTMLДокументСформирован(Элемент)
	...
	Если РежимВыбора Тогда
		РедакторHTML.setOption("allowRevertBack", Истина); // https://github.com/salexdv/bsl_console/issues/375
	КонецЕсли;
...
Процедура РедакторHTMLonclickДинамический(ДанныеСобытия) Экспорт
	...
	Если Событие <> Неопределено Тогда
		Если Событие.event = "EVENT_CONTENT_CHANGED" Тогда 
			Если ПоБлокам Тогда
				...
				НовыйТекстБлока = РедакторСтороны2().getValue();
				мПолеТекстаПрограммы2.УстановитьГраницыВыделения(ТекущаяСтрока.Позиция2, ТекущаяСтрока.Позиция2 + СтрДлина(ТекущаяСтрока.Текст2));
				мПолеТекстаПрограммы2.ВыделенныйТекст(НовыйТекстБлока);
				ТекущаяСтрока.Текст2 = НовыйТекстБлока;
				ЭтаФорма.Текст2 = мПолеТекстаПрограммы2.ПолучитьТекст();
			Иначе
				//ЭтаФорма.Текст1 = РедакторСтороны1().getValue();
				ЭтаФорма.Текст2 = РедакторСтороны2().getValue();
			КонецЕсли;
		КонецЕсли;
		Если Ложь
			Или Событие.event = "EVENT_COMPARE_COMPLETE" // https://github.com/salexdv/bsl_console/issues/364
		Тогда 
	...
	ОбновитьТекущиеПути(); 
	мЛиАктивнаЛеваяСторона = РедакторHTML.getActiveEditor() = РедакторСтороны1();
КонецПроцедуры

Процедура ДействияФормыСледующийОтложенно() Экспорт
	ДействияФормыСледующий();
КонецПроцедуры
...
// До конца не отладил вычисление границ этих блоков.
Процедура _ОткатитьИзменениеБлока(Знач СтрокаОтличия)
	мПолеТекстаПрограммы1.УстановитьГраницыВыделения(СтрокаОтличия.НомерСтрокиНачало1, 1, ?(СтрокаОтличия.НомерСтрокиКонец1 = 0, СтрокаОтличия.НомерСтрокиНачало1, СтрокаОтличия.НомерСтрокиКонец1 + 1), 1);
	мПолеТекстаПрограммы2.УстановитьГраницыВыделения(СтрокаОтличия.НомерСтрокиНачало2, 1, ?(СтрокаОтличия.НомерСтрокиКонец2 = 0, СтрокаОтличия.НомерСтрокиНачало2, СтрокаОтличия.НомерСтрокиКонец2 + 1), 1);
	НовыйТекст = мПолеТекстаПрограммы1.ВыделенныйТекст();
	мПолеТекстаПрограммы2.ВыделенныйТекст(НовыйТекст);
	ЭтаФорма.Текст2 = мПолеТекстаПрограммы2.ПолеТекста.ПолучитьТекст();
...
Процедура ОбновитьСтруктуруОтличий() Экспорт
	...
	Если ЗначениеЗаполнено(ПараметрНомерСтроки2) Тогда
		...
		мЛиАктивнаЛеваяСторона = Ложь;
	КонецЕсли;
	Если мЛиАктивнаЛеваяСторона Тогда
	...
	Если мСтароеВыделение <> Неопределено Тогда
		...
		Если Не мЛиАктивнаЛеваяСторона Тогда
...
Функция ЗаполнитьНомераСтрокМетодов(Знач ПолеТекстаПрограммы) Экспорт
...
Функция ЗаполнитьНомераСтрокЗапросов(Знач ПолеТекстаПрограммы) Экспорт
...
Функция ПокрытиеОтличиемМетоды(Знач ПолеТекстаПрограммы, Знач НомерСтрокиНачало, Знач НомерСтрокКонец) Экспорт
	...
	КонецЕсли;
	Методы = ПолеТекстаПрограммы.мМодульМетаданных.Методы; // см. ЗаполнитьНомераСтрокМетодов
	...
	Иначе
		// Однострочный код использован для ускорения при разрешенной отладке. Выше расположен оригинал. Выполняйте изменения синхронно в обоих вариантах.
		Для Каждого МетодыЭлемент Из Методы Цикл   Если Не (Ложь   ИЛи МетодыЭлемент.НомерСтрокиНачало > НомерСтрокКонец   Или МетодыЭлемент.НомерСтрокиКонец < НомерСтрокиНачало)   Тогда   Результат.Добавить(МетодыЭлемент, МетодыЭлемент.Имя);   КонецЕсли;   КонецЦикла;
	КонецЕсли;
	Если Результат.Количество() = 0 Тогда
		Результат.Добавить(, мПлатформа.ИмяМетодаВнеМетодов());
	КонецЕсли;
	Возврат Результат;
КонецФункции
...
Функция ПокрытиеОтличиемЗапросы(Знач ПолеТекстаПрограммы, Знач НомерСтрокиНачало, Знач НомерСтрокКонец) Экспорт
	...
	Если НомерСтрокКонец = 0 Тогда
		НомерСтрокКонец = НомерСтрокиНачало;
	КонецЕсли;
	ПакетЗапросов = ПолеТекстаПрограммы.мПакетЗапросов; // см. ЗаполнитьНомераСтрокЗапросов 
	Результат = Новый СписокЗначений;
	Для ИндексЗапроса = 0 По ПакетЗапросов.Позиции0Запросов.ВГраница() Цикл
...
Процедура АктивироватьСторону2() Экспорт
	АктивироватьСторону(РедакторHTML().editor.getModifiedEditor());
	мЛиАктивнаЛеваяСторона = Ложь;
КонецПроцедуры 

Процедура АктивироватьСторону1() Экспорт
	АктивироватьСторону(РедакторHTML().editor.getOriginalEditor());
	мЛиАктивнаЛеваяСторона = Истина;
КонецПроцедуры 

Процедура АктивироватьСторону(Знач РедакторСтороны) Экспорт
	РедакторСтороны.focus();
	РедакторHTML().editor._activeEditor = РедакторСтороны;
КонецПроцедуры 
...
Процедура СтруктураОтличийПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки)
	...
	ОформлениеСтроки.Ячейки.Принять.ТолькоПросмотр = Не ДанныеСтроки.Принять;
...
Процедура ПоБлокамПриИзменении(Элемент = Неопределено)
	ИзменитьСвернутостьСравнениеБлоков(ПоБлокам);
	ИзменитьСвернутостьПанельСтруктуры(Не ПоБлокам);
	Если Не ПоБлокам Тогда
		Если ЭлементыФормы.СравнениеБлоков.ТекущаяСтрока <> Неопределено Тогда
			ТекущаяСтрока = ЭлементыФормы.СравнениеБлоков.ТекущаяСтрока;
			Если мЛиАктивнаЛеваяСторона И ЗначениеЗаполнено(ТекущаяСтрока.Имя1) Тогда
	...
	Если мЛиАктивнаЛеваяСторона И ЗначениеЗаполнено(ПутьВТексте1) Тогда
...
Процедура ЗаполнитьСравнениеБлоков() Экспорт
	...
	Если ВариантСинтаксиса = "ВстроенныйЯзык" Тогда
		...
		КэшПутиКомпиляции1 = Неопределено;
		КэшПутиГруппировки1 = Неопределено;
		КэшПутиКомпиляции2 = Неопределено;
		КэшПутиГруппировки2 = Неопределено;
		Для Каждого Методы1Элемент Из Методы1 Цикл
			...
			Если Методы2Элемент <> Неопределено Тогда
				...
				КонецЕсли;
				ТипОтличия = 0;
				Позиция2 = Методы2Элемент.ПозицияСОписанием;
				...
				ПутьГруппировки2 = мПолеТекстаПрограммы2.ПутьГруппировкиМетодаМодуля(Методы2Элемент,, КэшПутиГруппировки2);
				ПутьКомпиляции2 = мПолеТекстаПрограммы2.ПутьКомпиляцииМетодаМодуля(Методы2Элемент,, КэшПутиКомпиляции2);
			Иначе
				ТипОтличия = 1;  
				Позиция2 = Неопределено;
				НомерСтроки2 = Неопределено;
				Описание2 = Неопределено;
				ТекстБлока2 = Неопределено;
				ЛиЭкспорт2 = Неопределено;
				Аннотация2 = Неопределено;
				ПутьГруппировки2 = Неопределено;
				ПутьКомпиляции2 = Неопределено;
			КонецЕсли;
			Позиция1 = Методы1Элемент.ПозицияСОписанием;
			...
			ПутьГруппировки1 = мПолеТекстаПрограммы1.ПутьГруппировкиМетодаМодуля(Методы1Элемент,, КэшПутиГруппировки1);
			ПутьКомпиляции1 = мПолеТекстаПрограммы1.ПутьКомпиляцииМетодаМодуля(Методы1Элемент,, КэшПутиКомпиляции1);
			...
			СтрокаСравнения.Текст1 = ТекстБлока1;
			СтрокаСравнения.Текст2 = ТекстБлока2;
			СтрокаСравнения.Позиция1 = Позиция1;
			СтрокаСравнения.Позиция2 = Позиция2;
...
Процедура СтруктураОтличийПринятьПриИзменении(Элемент)
	_ОткатитьИзменениеБлока(ЭлементыФормы.СтруктураОтличий.ТекущиеДанные);
КонецПроцедуры

Процедура СтруктураОтличийПриИзмененииФлажка(Элемент, Колонка)
	ирКлиент.ТабличноеПолеПриИзмененииФлажкаЛкс(ЭтаФорма, Элемент, Колонка);
КонецПроцедуры

Процедура СкрытьРавноеПриИзменении(Элемент)
	Если СкрытьРавное Тогда
		РедакторHTML().hideUnchangedBlocks();
	Иначе
		РедакторHTML().showHiddenBlocks();
	КонецЕсли;
КонецПроцедуры
